...
void ConfigINI::loadConfigFile(){
	...
	fStream.open(p.c_str(), ios::in);
	if(!fStream){
		if(!autoCreate) log("load config, file [%s] not exist", iniFileName);
		else log("inifile [%s] not found, create a new file", iniFileName);
		return;
	}else mlog("file open OK\n");
	char line[4096];
	char ch;
	int i = 0;
	string index;
	...
	while(!fStream.eof()){
		...
		if(ch != '\n' || ch == '\r') line[i++] = ch;
			...
			if(line[0] == '[') index = str;
				...
				entry.index = str.substr(1, index.length() - 2); // index.substr(1, index.length() - 2);
...
void ConfigINI::setStringValueWithIndex(const char *index, const char *name, const char *value){
	...
	for(it = datas.begin(); it != datas.end(); it++){
		if(findIndex == false)
			if(strcmp(it->index.c_str(), index) == 0) findIndex = true;
		if(findIndex == true){
			if(strcmp(it->index.c_str(), index) != 0) break;
			else itInsertPos = it;
...
const char *ConfigINI::getStringValue(const char *index, const char *name){
	...
	for(unsigned int i = 0; i < datas.size(); i++){
		if(strcmp(datas[i].index.c_str(), index) == 0){
			...
			for(; i < datas.size(); i++)
				if(strcmp(datas[i].name.c_str(), name) == 0) return datas[i].value.c_str();
		}
	}
