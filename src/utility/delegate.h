...
template <std::size_t N>
struct generate_sequence : concat_invoke<generate_sequence_invoke<N / 2>,
                                         generate_sequence_invoke<N - N / 2>> {
};
...
template <typename T>
struct remove_first_type { typedef std::tuple<T> type; };
...
void delete_type_route(void *f) {
	if (static_cast<type_router<object_type> *>(f) !=
	    (type_router<object_type> *)0)
	    ...
	    , _data_type(
	          typeid(
	              typename std::remove_reference<
	                  typename std::remove_pointer<decltype(value)>::type>::type)
	              .name())
...
public:
		transmitter(std::string _method_name  // , std::string _return_type_name
		    ...
		    : _method_name(
		          _method_name)  // , _return_type_name(typeid(typename
...
private:
		...
		struct inner_impl {
			template <typename object_type_, typename return_type_, std::size_t... _I,
			          typename... _Arg_>
			inner_impl(
	...
	method(std::string _method_name, return_type (object_type::*f)(Arg...), return_type *r, typename static_if_shared<object_type, object_pointer_type>::type o  // shared_ptr<object_type> o // = (object_type *const)0 //nullptr
	    ...
	    , _transmitter(std::static_pointer_cast<void>(std::make_shared<transmitter<object_pointer_type, object_type, return_type, Arg...>>(_method_name  // , typeid(typename
	                                                                                                                                                     // std::remove_reference<return_type>::type).name()
	                                                                                                                                       ,
...
struct _interface<return_type(Arg...), _meta_info> : public _meta_info {
	...
	template <typename _object_type,
	          typename _object_pointer_type =
	              object_pointer_type>  // typename _object_pointer_type, typename
...
template <typename _meta_info>
struct record
// template <> struct record<meta_info<shared_ptr<void>>> : public
// meta_info<shared_ptr<void>>
{
	typedef _meta_info meta_info;
	typedef typename _meta_info::object_pointer_type object_pointer_type;
	...
	method_type &push_back(const std::string _method_name, return_type (object_type::*funct)(Arg...),
		...
		return *(method_field[_method_name] = new method_type(_method_name, funct, rt, static_cast<object_type *>(meta_info::_object),
		                                                      std::forward<Arg>(arg)...));  // fatal dangerous?
	}
	~record() {
...
typedef sd::_interface<sd::meta_info<boost::shared_ptr<void>>, void,
                       std::string const &>
