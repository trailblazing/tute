...
void AttachTableController::add_smart(QString attach_type) {
	...
	for (int i = 0; i < files.size(); ++i) {
		...
		// Если пользователь выбрал директорию (директорию выбирать нельзя, пока что
		// можно выбирать только файлы)
		...
		if (attachTableData->is_record_crypt())
			attach.encrypt();
		if (result) {
...
void AttachTableController::on_save_as_attach(void) {
	...
	if (selectedId.size() == 0) {
		show_message_box(
		    tr("Please select any attach(es) for save to your directory."));
	...
	if (selectedId.size() == 1) {
		...
		if (selectFiles.size() != 1) {
			show_message_box(
			    tr("For save sigle file you must set single result file name."));

			return;
		}
		// Указанное пользователем имя файла для сохранения аттача, взятое из формы
		// ввода
		...
		foreach (QString id, selectedId) {
			...
			QString fromFullFileName =
			    attachTableData->absolute_inner_file_name_by_id(id);
...
void AttachTableController::save_attach_to_user_place(
	...
	if (!result) {
		show_message_box(
		    tr("Can't save file %1. Any i/o problem.").arg(to_full_file_name));

		return;
	}
	// асшифровка файла, если он был зашифрован и данные хранились в базе (то
	// есть, это именно тип file, а не линк на файл)
...
void AttachTableController::on_open_attach(void) {
	...
	foreach (QString id, selectedId) {
		QString fullFileName = attachTableData->absolute_inner_file_name_by_id(id);
		// Если запись зашифрована и открывается файл (не линк), аттач надо
		// скопировать в директорию корзины и расшифровать
		if (attachTableData->is_record_crypt() &&
...
void AttachTableController::on_show_attach_info(void) {
	...
	messageBox.setDetailedText(
	    QString("<pre><p style='font-family:monospace'>") + "<b>Attach:</b> " +
	    attachTableData->file_name_by_id(id) + "\n" + "<b>Attach ID:</b> " + id +
	    "\n" + "<b>Attach type:</b> " +
	    attachTableData->attach(id).getField("type") + "\n" +
	    "<b>File size:</b> " +
	    QString::number(attachTableData->attach(id).getFileSize()) + " bytes\n" +
	    "<b>Full path:</b> " +
...
QList<QString> AttachTableController::selected_id(void) {
	...
	// Перебор выделенных элементов. Так как имеем несколько столбцов, то для
	// одной строки будет несколько QModelIndex
	int previousRow = -1;
	for (int i = 0; i < selectItems.size(); i++) {
		int row = selectItems.at(i).row();
		// Строка обратабывается только один раз (из-за того что для одной строки
		// несколько QModelIndex)
		if (row != previousRow) {
			previousRow = row;

			selectedId.append(
			    _model->data(selectItems.at(i), ATTACHTABLE_ROLE_ID).toString());
		}
	}
	return selectedId;
}
