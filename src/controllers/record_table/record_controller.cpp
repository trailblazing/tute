...
void rctrl_t::add_items_to_clipboard(ClipboardRecords* clipboardRecords, QModelIndexList items_copy)
{
    ...
    for (int i = 0; i < items_copy.size(); ++i) {
        ...
        boost::intrusive_ptr<i_t> record = _source_model->item(pos_source(((QModelIndex)index_).row()));
        if (record->is_lite())
            record->to_fat();
...
{ // int mode
    ...
    boost::fusion::at_key<home_type>(data) = web::Browser::_defaulthome;
...
void rctrl_t::on_sort_request(int logicalIndex, Qt::SortOrder order)
{
    ...
    if (order == Qt::AscendingOrder) {
        if (header_title == pin_field_description) {
            ...
            // int t = 0;
            for (auto v : v_list) {
		if (v->page()->host()->field<pin_type>() != char_from_check_state[Qt::Unchecked])
...
boost::intrusive_ptr<i_t> rctrl_t::synchronize(boost::intrusive_ptr<RecordIndex> record_index_)
{
    ...
    boost::intrusive_ptr<i_t> _found_item = _source_model->item([&](const id_value id) { return id == it->id(); });
    web::WebView* v = nullptr;
