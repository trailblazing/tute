...
void gl_para::init(const QString& app_name)
{
    ...
    if (QFile(_standard_paths.first).exists())
        _standard_path = _standard_paths.first;
    else if (QFile(_standard_paths.second).exists())
        _standard_path = _standard_paths.second;
    if ("" == _standard_path) { // (! QFile(_main_program_path + "/mode.ini").exists()){
        if (QDir::root().mkdir(_standard_paths.first))
            _standard_path = _standard_paths.first;
        else if (!QDir(QDir::homePath() + "/.config/").exists() || !QDir(QDir::homePath() + "/.config/" + _application_name).exists()) {
            if (QDir::root().mkdir(_standard_paths.second))
                _standard_path = _standard_paths.second;
        }
        //	if(""==_standard_path)
        else
        ...
        if ("" != _standard_path) {
            auto mode_file_location = _standard_path + "/" + gl_para::_mode_filename;
            if (!QFile(mode_file_location).exists())
                if (!QFile::copy(QString(":/resource/standardconfig/") + target_os() + "/" + _mode_filename, _standard_path + "/" + _mode_filename))
            ...
            if ((QFile::ReadUser | QFile::WriteUser) != (QFile::permissions(mode_file_location) & (QFile::ReadUser | QFile::WriteUser)))
                QFile::setPermissions(mode_file_location, QFile::ReadUser | QFile::WriteUser);
        }
    }
    ...
    QString temp_root = "";
    if (conf_root_dir != "./")
        temp_root = conf_root_dir;
    else if (_standard_path != "")
        temp_root = _standard_path;
...
bool gl_para::root_path(QString path_name)
{
    ...
    if (!QFile(_standard_path + "/" + _mode_filename).exists())
        if (!QFile::copy(QString(":/resource/standardconfig/") + globalparameters.target_os() + "/" + QString(_mode_filename), mode_file_full_name))
            critical_error("Can not copy " + QString(_mode_filename));
    if ((QFile::ReadUser | QFile::WriteUser) != (QFile::permissions(mode_file_full_name) & (QFile::ReadUser | QFile::WriteUser)))
        QFile::setPermissions(mode_file_full_name, QFile::ReadUser | QFile::WriteUser);
...
QString gl_para::root_path(void) const
{
    ...
    if (mode_ini.contains(root_name))
        ...
        if (QFile(mode_file_full_name).exists())
            QFile::remove(mode_file_full_name);
        if (!QFile::copy(QString(":/resource/standardconfig/") + globalparameters.target_os() + "/" + QString(_mode_filename), mode_file_full_name))
            critical_error("Can not copy " + QString(_mode_filename));
        if ((QFile::ReadUser | QFile::WriteUser) != (QFile::permissions(mode_file_full_name) & (QFile::ReadUser | QFile::WriteUser)))
            QFile::setPermissions(mode_file_full_name, QFile::ReadUser | QFile::WriteUser);
...
std::tuple<const bool, const QString> gl_para::coordinate_root(const QString& root_path_)
{ // bool standard_mode,
    ...
    //    QString		mode = setting.value("application_mode").toString();
    //    if(! is_ini_config_exist(full_current_path + "/conf.ini")){
    ...
    //        //	assert(succedded);
    //    }
    //    if((mode != "Standard") && is_ini_config_exist(full_current_path + "/conf.ini")){		// mode == portableItem ||
    ...
    //	}else{
    // Проверка ini-файла
    auto is_ini_config_exist = [&](const QString& config_filename) -> bool {
    ...
    auto recover_config = [&](const QString& root_path_local) {
        ...
        if (!QFile(config_path).exists())
            if (!QDir::root().mkpath(config_path))
                critical_error("Can not make path \"" + config_path + "\""); // throw std::runtime_error("Can not make path \"" + config_path.toStdString() + "\"");
        auto check_exists_and_permission = [&](const QString& name) {
            ...
            if (!QFile(config_location).exists())
                if (!QFile::copy(QString(":/resource/standardconfig/") + target_os() + "/" + name, config_location))
                    critical_error("Can not copy " + name); // throw std::runtime_error("Can not copy " + name.toStdString());
        ...
        if (!is_ini_config_exist(config_path + _conf_filename)) {
        ...
        } else if ((QFile::ReadUser | QFile::WriteUser) != (QFile::permissions(config_path + _conf_filename) & (QFile::ReadUser | QFile::WriteUser)))
            QFile::setPermissions(config_path + _conf_filename, QFile::ReadUser | QFile::WriteUser);
    ...
    auto figure_out_standard_work_path = [&] {
        ...
        if (root_dir_path.exists())
            ...
            if (root_dir_path.exists())
                ...
                if (!QDir::root().mkpath(standard_path_))
                    critical_error("auto figure_out_standard_work_path = [&] can not make path \"" + standard_path_ + "\"");
                else
                    recover_config(standard_path_);
            }
        }
    ...
    auto change_current = [&](const QString& root_path_) {
        ...
        if (non_conflicts(is_standard_path(std::get<1>(result_current))))
            std::get<0>(result_current) = true;
    ...
    auto check_and_recover_current_root = [&](const QString& root_cur) { // (bool is_standard_mode)
        ...
        if (!non_conflicts(current_path_is_standard)) {
            ...
            if (current_path_is_standard) {
                ...
                if (QDir::currentPath() != root_cur)
                    result_current = change_current(root_cur); // standardItem,
                else
                    std::get<0>(result_current) = true;
                // for data and trash folder	// _work_directory.length() == 0 // qDebug() << "Cant find standard work directory";
            } else {
                recover_config(root_path_); // if(_root_path == "") fix_root_dir_portable();
                if (QDir::currentPath() != root_cur)
                    result_current = change_current(root_cur); // portableItem,	// app_mode() != portableItem ||
                else
                    std::get<0>(result_current) = true;
                //		}
            }
    ...
    while (!std::get<0>(result = check_and_recover_current_root(std::get<1>(result)))) { // app_mode() == standardItem
        ...
        QString return_path = QInputDialog::getItem(tempWidget.get(), welcomeText, infoText, items, 0, false, &ok);
...
QString gl_para::application_name(void) const
{
    ...
    if (target_os() == "meego")
        app_name = QString("ru.webhamster") + "." + _application_name;
...
browser::DownloadManager* gl_para::request_download_manager()
{
    ...
    if (!found)
        _download_manager = new browser::DownloadManager(download_manager_singleton_name, _vtab_record);
...
void gl_para::edit_entry(Editentry* point) { _meta_editor = point; }

Editentry* gl_para::edit_entry() const { return _meta_editor; }
