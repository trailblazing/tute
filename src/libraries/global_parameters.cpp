...
bool gl_para::root_path(QString path_name){
	...
	if(!QFile(_standard_path + "/" + _mode_filename).exists())
		if(!QFile::copy(QString(":/resource/standardconfig/") + gl_paras.target_os() + "/" + QString(_mode_filename), mode_file_full_name)) critical_error("Can not copy " + QString(_mode_filename));
