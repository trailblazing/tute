...
void DiskHelper::backup(QString file_name_from)
{
    ...
    QString file_name_to_short = base_ + "_" + get_unical_id() + "." + extension_;
...
std::shared_ptr<QFileInfo> DiskHelper::copy_file_to_data_folder(const QString& file_name_to, const QString& file_name_from)
{
    ...
    // file_from.close();
    if (file_from_size_again > 0) { // if(fileInfoFrom.size() != 0){	// prevent to erase file target // file_from_size > 0 ||
...
QString DiskHelper::copy_file_to_trash_folder(QString file_name_from)
{
    ...
    if (QFile::exists(file_name_from)) {
        // Файл копируется в корзину
        if (QFile::copy(file_name_from, file_name_to))
            trashmonitoring.add_file(file_name_from_short); // file_name_to_short // Оповещение что в корзину добавлен файл
        else
            critical_error("Can not remove file\n" + file_name_from + "\nto reserve file\n" + file_name_to);
    }
    QFileInfo fileInfoTo(file_name_to);
...
QMap<QString, QString> DiskHelper::get_strings_from_directory(QString dirName, QString fileMask)
{
    ...
    if (directory.exists()) {
        ...
        qDebug() << "DiskHelper::get_strings_from_directory() : Can not find directory" << dirName;
    return result;
}
...
QMap<QString, QByteArray> DiskHelper::get_files_from_directory(QString dirName, QString fileMask)
{
    ...
    if (directory.exists()) {
        ...
        qDebug() << "DiskHelper::get_files_from_directory() : Can not find directory" << dirName;
    return result;
}
