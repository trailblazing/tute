...
void WalkHistory::add<WALK_HISTORY_GO_PREVIOUS>(const id_value &_record_id, int _cursor_position, int _scrollbar_position) {
	...
	if (_history_point == (history_id.length() - 1)) {
		...
		if (_record_id != history_id.at(_history_point)) {
			...
			QString message =
			    ...
			    "History max index " +
			    QString::number(history_id.length() - 1) +
			    ", History point " + QString::number(_history_point) +
			    ", History id " + history_id.at(_history_point) +
	...
	// }		// Закончилось условие что происходит запоминание с движением назад
	// по истории
}
...
void WalkHistory::add<WALK_HISTORY_GO_NEXT>(const id_value &_record_id, int _cursor_position, int _scrollbar_position) {
	...
	if (_history_point == (history_id.length() - 1)) {
		...
		if (_record_id != history_id.at(_history_point)) {
			...
			QString message =
			    ...
			    QString::number(history_id.length() - 1) + ", History point " +
	...
	// }		// Закончилось условие что происходит запоминание с движением вперед
	// по истории
}
