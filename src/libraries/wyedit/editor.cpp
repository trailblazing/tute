...
//    connect(_editor_context_menu, &EditorContextMenu::cut, this, &Editor::cut);
//    connect(_editor_context_menu, &EditorContextMenu::copy, this, &Editor::copy);
//    connect(_editor_context_menu, &EditorContextMenu::paste, this, &Editor::paste);
...
//	// Соединение сигнал-слот чтобы показать контекстное меню по долгому нажатию
//    connect(this, &Editor::tapandhold_gesture_finished, this, &Editor::on_custom_contextmenu_requested);
}
...
//    _bold->setAutoRaise(true);
	...
	_bold->setIcon(QIcon(":/resource/pic/edit_bold.svg"));  // style()->standardIcon(QStyle::SP_TitleBarShadeButton, 0, this) //
...
//    _dot_list->setAutoRaise(true);
	...
	_font_color = new FlatToolButton("", this); // fontColor->setShortcut(QKeySequence(tr("Ctrl+F")));
	...
	_settings->setIcon(QIcon(":/resource/pic/cogwheel.svg"));   //    settings->setIcon(QIcon(":/resource/pic/edit_settings.svg"));
...
void Editor::setTextCursor(const QTextCursor &cursor){_text_area->setTextCursor(cursor);}
void Editor::insertPlainText(const QString &text){_text_area->insertPlainText(text);}
void Editor::setPlainText(const QString &text){_text_area->setPlainText(text);}
...
void Editor::editor_save_callback(QObject *editor, const QString &saveText){
	...
	if(workWithCrypt == false){
		...
		if(! wfile.open(QIODevice::WriteOnly | QIODevice::Text)) critical_error("Editor::editor_save_callback() : Can\'t open text file " + fileName + " for write.");
		...
		QFile wfile(fileName);
		if(! wfile.open(QIODevice::WriteOnly)) critical_error("Editor::editor_save_callback() : Can\'t open binary file " + fileName + " for write.");
...
void Editor::on_italic_clicked(void){
	...
	if(_text_area->textCursor().hasSelection()){
		// Обычное форматирование
		if(! _text_area->fontItalic()) _text_area->setFontItalic(true);
		else _text_area->setFontItalic(false);
	}else{
		// Иначе надо выделить дополнительным курсором слово на
		// котором стоит курсор
		QTextCursor cursor = _text_area->textCursor();
...
void Editor::on_underline_clicked(void){
	...
	if(_text_area->textCursor().hasSelection()){
		// Обычное форматирование
		if(! _text_area->fontUnderline()) _text_area->setFontUnderline(true);
		else _text_area->setFontUnderline(false);
	}else{
		// Иначе надо выделить дополнительным курсором слово на
		// котором стоит курсор
		QTextCursor cursor = _text_area->textCursor();
...
void Editor::on_custom_contextmenu_requested(const QPoint &_position){
	...
	if(is_image_select() ||
	    is_cursor_on_image()) _context_menu->set_edit_image_properties(true);
	else _context_menu->set_edit_image_properties(false);
	// Контекстное меню запускается

	_context_menu->exec(_text_area->viewport()->mapToGlobal(_position)  // mapToGlobal(_position)
	...
	if(imageFormat.isValid()){
		...
		if(is_image_select()){
			...
			for(auto it = currentBlock.begin(); ! (it.atEnd()); ++ it){
				...
				// Если фрагмент содержит изображение
				if(fragment.isValid() &&
				    fragment.charFormat().isImageFormat()) break;// Переменная fragment содержит только картинку
			}
