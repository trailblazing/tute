...
Editor::Editor(QStackedWidget *main_stack, EditingWindow *blog_editor)
	: QWidget(main_stack)
	...
	// new QMenu(this)
	// , _hidetitlebar(new QWidget(this, Qt::FramelessWindowHint |
...
void Editor::init_config_file_name(QString config_full_name){
	...
	auto location =
	    globalparameters.root_path() + "/" + globalparameters.target_os();
	...
	if(! conf_file.exists())
		if(! QFile::copy(":/resource/standardconfig/" +
			...
			critical_error("void Editor::init_config_file_name(QString "
			    ...
			    config_full_name + "\"");
	conf_file.setPermissions(config_full_name
...
void Editor::setup_signals(void){
	...
	connect(_text_area, &EditorTextArea::cursorPositionChanged, this
	       , &Editor::on_cursor_position_changed);
	connect(_text_area, &EditorTextArea::selectionChanged, this
	       , &Editor::on_selection_changed);

	connect(_indent_slider, &IndentSlider::change_textindent_pos, this
	       , &Editor::on_indentline_change_textindent_pos);
	connect(_indent_slider, &IndentSlider::change_leftindent_pos, this
	       , &Editor::on_indentline_change_leftindent_pos);
	connect(_indent_slider, &IndentSlider::change_rightindent_pos, this
	       , &Editor::on_indentline_change_rightindent_pos);
	connect(_indent_slider, &IndentSlider::mouse_release, this
	       , &Editor::on_indentline_mouse_release);

	connect(this, &Editor::send_set_textindent_pos, _indent_slider
	       , &IndentSlider::set_textindent_pos);
	connect(this, &Editor::send_set_leftindent_pos, _indent_slider
	       , &IndentSlider::set_leftindent_pos);
	connect(this, &Editor::send_set_rightindent_pos, _indent_slider
	       , &IndentSlider::set_rightindent_pos);

	// Соединение сигнал-слот чтобы показать контекстное меню по правому клику в
	// редакторе
	connect(_text_area, &EditorTextArea::customContextMenuRequested, this
	       , &Editor::on_custom_contextmenu_requested);

	// Соединение сигнал-слот чтобы показать контекстное меню по долгому нажатию
	connect(_text_area, &EditorTextArea::tap_and_hold_gesture_finished, this
	       , &Editor::on_custom_contextmenu_requested);
	...
	connect(_find_dialog, &EditorFindDialog::find_text, this
	       , &Editor::on_findtext_signal_detect);

	connect(_text_area, &EditorTextArea::update_indentline_geometry_signal, this
	       , &Editor::on_update_indentline_geometry_slot);
	...
	// ветке
	connect(this, &Editor::customContextMenuRequested, this
	       , &Editor::on_custom_contextmenu_requested);
...
void Editor::setup_buttons(void){
	...
	// Кнопка Bold
	_bold
	    = add_action(_tools_line_0, this, tr((editor_prefix + "bold").c_str())
			, tr("Bold (Ctrl+B)"), tr("Bold (Ctrl+B)")
			, QIcon(":/resource/pic/edit_bold.svg")
			, [&](bool){on_bold_clicked();}
			, QKeySequence(QKeySequence::Bold));
	...
	// Кнопка Underline
	_underline
	    = add_action(_tools_line_0, this, tr((editor_prefix + "underline").c_str())
			, tr("Underline (Ctrl+U)"), tr("Underline (Ctrl+U)")
			, QIcon(":/resource/pic/edit_underline.svg")
			, [&](bool){on_underline_clicked();}
			, QKeySequence(QKeySequence::Underline));

//	action_Bold
//	    = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "Bold").c_str())
//			, tr("Bold"), tr("Make the selected text bold")
//			, QIcon(QPixmap(mini_bold_xpm))
//			, [&](bool){_blog_editor->makeBold();});
//	action_Italic
//	    = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "Italic").c_str())
//			, tr("Italic"), tr("Italicise the selected text")
//			, QIcon(QPixmap(mini_ital_xpm))
//			, [&](bool){_blog_editor->makeItalic();});
//	action_Underline
//	    = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "Underline").c_str())
...
//			, [&](bool){_blog_editor->makeUnderline();});
	...
	// Кнопка Monospace
	_monospace
	    = add_action(_tools_line_0, this, tr((editor_prefix + "monospace").c_str())
	...
	// Кнопка Code
	_code
	    = add_action(_tools_line_0, this, tr((editor_prefix + "code").c_str())
			, tr("Code (Ctrl+M). Select a whole paragraphs to format text as code.")
			, tr("Code (Ctrl+M). Select a whole paragraphs to format text as code.")
			, QIcon(":/resource/pic/edit_code.svg"), [&](bool){on_code_clicked();}
			, QKeySequence(tr("Ctrl+M")));
//	action_Blockquote
//	    = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "Blockquote").c_str())
//			, tr("Blockquote")
...
//			, [&](bool){_blog_editor->makeBlockquote();});
	...
	// Кнопка Clear
	_clear
	    = add_action(_tools_line_0, this, tr((editor_prefix + "clear").c_str())
			...
			, tr("Reset format (Ctrl+K). When selected whole paragraph both "
			     "text and paragraph format is reset to default or just "
			     "text format in other case.")
			, QIcon(":/resource/pic/edit_clear.svg")
			, [&](bool){on_clear_clicked();}, QKeySequence(tr("Ctrl+K")));


	// Кнопка нумерованного списка
	_numeric_list
	    = add_action(_tools_line_0, this
			, tr((editor_prefix + "numericlist").c_str())
			, tr("Numeric list"), tr("Numeric list")
			, QIcon(":/resource/pic/edit_listnumeric.svg")
			, [&](bool){on_numericlist_clicked();});

	// Кнопка списка с точками
	_dot_list
	    = add_action(_tools_line_0, this, tr((editor_prefix + "dotlist").c_str())
			, tr("Marked list"), tr("Marked list")
			, QIcon(":/resource/pic/edit_listdot.svg")
			, [&](bool){on_dotlist_clicked();});


	// Кнопка увеличения отступа
	_indent_plus
	    = add_action(_tools_line_0, this
			, tr((editor_prefix + "indentplus").c_str())
			, tr("Increase indent"), tr("Increase indent")
			, QIcon(":/resource/pic/edit_indentplus.svg")
			, [&](bool){on_indentplus_clicked();});


	// Кнопка уменьшения отступа
	_indent_minus
	    = add_action(_tools_line_0, this
			, tr((editor_prefix + "indentminus").c_str())
			, tr("Decrease indent"), tr("Decrease indent")
			, QIcon(":/resource/pic/edit_indentminus.svg")
			, [&](bool){on_indentminus_clicked();});

	// Кнопка выравнивания по левому краю
	_align_left
	    = add_action(_tools_line_0, this, tr((editor_prefix + "alignleft").c_str())
	...
	_align_center
	    = add_action(_tools_line_0, this, tr((editor_prefix + "aligncenter").c_str())
	...
	_align_right
	    = add_action(_tools_line_0, this, tr((editor_prefix + "alignright").c_str())
	...
	_align_width
	    = add_action(_tools_line_0, this, tr((editor_prefix + "alignwidth").c_str())
	...
	_action_open
	    = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "Open ...").c_str())
			, tr("Open ..."), tr("Load a saved entry from the disk")
			, QIcon(":/resource/pic/fileopen.svg")
			, [&](bool){_blog_editor->choose();});
	_action_save
	    = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "Save").c_str())
			, tr("Save")
			, tr("Save this entry to disk for editing or publication later")
			, QIcon(":/resource/pic/save.svg")
			, [&](bool){_blog_editor->save();});


	_action_link
	    = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "Link").c_str())
			, tr("Link")
	...
	_action_image
	    = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "Image").c_str())
			, tr("Image")
			, tr(std::string(std::string("Insert an image into the text, the location, alignment and alternate \n") + "text to be specified now").c_str())
			, QIcon(":/resource/pic/flip.svg")//QPixmap(imgIcon_xpm)
			, [&](bool){_blog_editor->insertImage();});
	_action_more
	    = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "More").c_str())
			, tr("More")
			, tr((std::string("Set this as the end of the main entry (which appears on the index) \n") + "and the beginning of the extended entry (which appears " + "when you click More, \n" + "Extended entry or view the entry on its own page).").c_str())
			, QIcon(":/resource/pic/more.svg")//QPixmap(more_xpm)
			, [&](bool){_blog_editor->insertMore();});
	_action_preview
	    = add_action(_tools_line_0, this
			, tr(std::string(editor_prefix + "Entry in preview").c_str())
			, tr("Entry in preview"), tr("Preview the entry in this window")
	...
	_action_blog_this
	    = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "Blog this!").c_str())
			, tr("Blog this!"), tr("Post this entry to the blog")
			, QIcon(":/resource/pic/pentalpha.svg")// pentalpha.svg//QPixmap(blogThisIcon_xpm)
...
//	_tools_line_0->adjustSize();     // without this, without display
	...
	_expand_tools_lines
	    = add_action(_tools_line_0, this, tr((editor_prefix + "expand_tools_lines").c_str())
	...
	_to_attach
	    = add_action(_tools_line_0, this, tr((editor_prefix + "attach").c_str())
			, tr("Show attach files"), tr("Show attach files"), _icon_attach_not_exists
			, [&](bool){on_to_attach_clicked();});
	...
	_expand_edit_area
	    = add_action(_tools_line_0, this, tr((editor_prefix + "expand_edit_area").c_str())
	...
	_save
	    = add_action(_tools_line_0, this, tr((editor_prefix + "save").c_str())
			, tr("Save (Ctrl+S)"), tr("Save (Ctrl+S)")
			, QIcon(":/resource/pic/edit_save.svg")
			, [&](bool){on_save_clicked();}, QKeySequence(tr("Ctrl+S")));


	// Кнопка "назад", используется в мобильном интерфейсе
	// Button "back", used in the mobile interface
	_back
	    = add_action(_tools_line_0, this, tr((editor_prefix + "back").c_str())
			, tr("Back"), tr("Back")
			, QIcon(":/resource/pic/mobile_back.svg")
	...
	_find_in_base
	    = add_action(_tools_line_0, this
			, tr((editor_prefix + "find_in_base").c_str())
			, tr("Find in base"), tr("Find in base")
			, QIcon(":/resource/pic/find_in_base.svg")
			, [&](bool){on_find_in_base_clicked();});
	// Кнопка вызова виджета поиска текста
	_find_text
	    = add_action(_tools_line_0, this, tr((editor_prefix + "findtext").c_str())
	...
	_settings
	    = add_action(_tools_line_0, this, tr((editor_prefix + "settings").c_str())
			, tr("Editor settings"), tr("Editor settings")
			, QIcon(":/resource/pic/cogwheel.svg")
			, [&](bool){on_settings_clicked();});


	// Кнопка просмотра HTML кода
	_show_html
	    = add_action(_tools_line_0, this, tr((editor_prefix + "showhtml").c_str())
			, tr("Edit HTML code"), tr("Edit HTML code")
			, QIcon(":/resource/pic/edit_showhtml.svg")
			, [&](bool){on_showhtml_clicked();});


	// Кнопка включения отображения символов фарматирования
	_show_formatting
	    = add_action(_tools_line_0, this, tr((editor_prefix + "showformatting").c_str())
	...
	_show_text
	    = add_action(_tools_line_0, this, tr((editor_prefix + "show_text").c_str())
	...
	// Выбор шрифта
	_font_select
	    = add_action(_tools_line_1, this, tr((editor_prefix + "fontselect").c_str())
	...
	_font_size =
	    add_action(_tools_line_1, this, tr((editor_prefix + "fontsize").c_str())
		      , tr("Select font size"), tr("Select font size")
		      , qobject_cast<sapp_t *>(qApp)->defaultIcon()
		      , (FlatComboBox *) (nullptr), this, &Editor::on_fontsize_changed);
	...
	_font_color
	    = add_action(_tools_line_1, this, tr((editor_prefix + "fontcolor").c_str())
			, tr("Text color"), tr("Text color")
			, QIcon(":/resource/pic/edit_fontcolor.svg")
			, [&](bool){on_fontcolor_clicked();});
	...
	_create_table
	    = add_action(_tools_line_1, this
			, tr((editor_prefix + "createtable").c_str())
			, tr("Create a new table"), tr("Create a new table")
			, QIcon(":/resource/pic/edit_createtable.svg")
			, [&](bool){on_createtable_clicked();});


	_table_remove_row
	    = add_action(_tools_line_1, this, tr((editor_prefix + "table_remove_row").c_str())
	...
	_table_remove_col
	    = add_action(_tools_line_1, this, tr((editor_prefix + "table_remove_col").c_str())
	...
	_table_add_row
	    = add_action(_tools_line_1, this
			, tr((editor_prefix + "table_add_row").c_str())
			, tr("Add row(s)"), tr("Add row(s)")
			, QIcon(":/resource/pic/edit_table_add_row.svg")
			, [&](bool){on_table_add_row_clicked();});


	_table_add_col
	    = add_action(_tools_line_1, this
			, tr((editor_prefix + "table_add_col").c_str())
			, tr("Add column(s)"), tr("Add column(s)")
			, QIcon(":/resource/pic/edit_table_add_col.svg")
			, [&](bool){on_table_add_col_clicked();});

	_table_merge_cells
	    = add_action(_tools_line_1, this, tr((editor_prefix + "table_merge_cells").c_str())
			, tr("Merge cells"), tr("Merge cells")
			, QIcon(":/resource/pic/edit_table_merge_cells.svg")
			, [&](bool){on_table_merge_cells_clicked();});

	_table_split_cell
	    = add_action(_tools_line_1, this, tr((editor_prefix + "table_split_cell").c_str())
...
void Editor::add_action_by_name(QString toolName, QToolBar *line){
	...
	if(toolName == "separator") line->addSeparator();
		...
		}else critical_error("WyEdit: Can not find editor tool with name '" + toolName +
			    "'. Please check editor *.ini file");
	}
}
#endif
...
void Editor::assembly(void){
	...
	full_layout->addLayout(_textformat_buttons_layout);

	// Добавляется область редактирования
	full_layout->addWidget(_text_area); // reconfiged by MetaEditor::assembly
...
QTextCursor Editor::textCursor() const {return _text_area->textCursor();}
...
QString Editor::toPlainText() const {return _text_area->toPlainText();}
void Editor::ensureCursorVisible() const {_text_area->ensureCursorVisible();}

QTextDocument *Editor::document(void){return _text_area->document();}
...
QString Editor::work_directory(void){return _work_directory;}
...
QString Editor::file_name(void){return _work_file_name;}
...
void Editor::editor_save_callback(QObject *editor, const QString &save_text){
	...
	if(work_with_crypt == false){
		...
		if(! wfile.open(QIODevice::WriteOnly | QIODevice::Text)) critical_error("Editor::editor_save_callback() : Can\'t open text file " +
			    file_name + " for write.");
...
void Editor::save_textarea(void){
	...
	if(work_directory().length() != 0 && file_name().length() != 0 &&
...
bool Editor::load_textarea(){
	...
	if(load_callback_func == nullptr){
		...
		if(! f.open(QIODevice::ReadOnly)){
			critical_error("WyEdit: File " + fileName +
			    " not readable. Check permission.");
...
//			_text_area->insertPlainText(QString("<strong>%1</strong>").arg(_text_area->textCursor().selectedText()));
		...
		if(cursor.charFormat().fontWeight() != QFont::Bold) format.setFontWeight(QFont::Bold);
		else format.setFontWeight(0);
...
//			_text_area->insertPlainText(QString("<em>%1</em>").arg(_text_area->textCursor().selectedText()));
		...
		QTextCharFormat format;
		if(! cursor.charFormat().fontItalic()) format.setFontItalic(true);
...
//			_text_area->insertPlainText(QString("<u>%1</u>").arg(_text_area->textCursor().selectedText()));
		...
		QTextCharFormat format;
		if(! cursor.charFormat().fontUnderline()) format.setFontUnderline(true);
...
void Editor::on_save_clicked(void){save_textarea();}
...
void Editor::on_clear_clicked(void){
	...
	qDebug() << "Cursor position: " << textArea->textCursor().position() <<
...
void Editor::on_alignleft_clicked(void){align_text(Qt::AlignLeft);}

// Форматирование по центру
void Editor::on_aligncenter_clicked(void){align_text(Qt::AlignHCenter);}

// Форматирование по правому краю
void Editor::on_alignright_clicked(void){align_text(Qt::AlignRight);}

// Форматирование по ширине
void Editor::on_alignwidth_clicked(void){align_text(Qt::AlignJustify);}
...
void Editor::set_fontselect_on_display(QString font_name){
	...
	if(font_name.size() > 0) _font_select->setCurrentIndex(_font_select->findText(font_name));
...
void Editor::on_selection_changed(void){
	...
	if(differentBoldFlag == 1) set_outline_button_higlight(BT_BOLD, false);
	else if(startBold == true) set_outline_button_higlight(BT_BOLD, true);
	// Кнопка Italic
	if(differentItalicFlag == 1) set_outline_button_higlight(BT_ITALIC, false);
	else if(startItalic == true) set_outline_button_higlight(BT_ITALIC, true);
	// Кнопка Underline
	if(differentUnderlineFlag == 1) set_outline_button_higlight(BT_UNDERLINE, false);
	else if(startUnderline == true) set_outline_button_higlight(BT_UNDERLINE, true);
	// Кнопки выравнивания
...
bool Editor::is_key_for_tool_line_update(QKeyEvent *event){
	if(event->modifiers().testFlag(Qt::ControlModifier) ||
	    ...
	    event->key() == Qt::Key_F11 || event->key() == Qt::Key_F12) return true;
	else return false;
}
...
void Editor::on_indentline_change_textindent_pos(int i){
	...
	_text_area->set_indentedge_pos(_indent_slider->get_current_indentedge_pos() -
	    _text_area->lineWidth() - 1);
}
...
void Editor::on_table_add_row_clicked(void){
	...
	if(table){
		...
		bool	ok		= false;
		int		addNum	= QInputDialog::getInt(this, tr("Append rows to table")
						      , tr("Append rows:"), 1, 1, 100, 1, &ok);
...
void Editor::on_table_add_col_clicked(void){
	...
	if(table){
		...
		bool	ok		= false;
		int		addNum	= QInputDialog::getInt(this, tr("Append columns to table")
						      , tr("Append columns:"), 1, 1, 100, 1, &ok);
...
void Editor::on_table_split_cell_clicked(void){
	...
	if(table){
		...
		cursor.setPosition(table->lastCursorPosition().position() + 1
				  , QTextCursor::KeepAnchor);

		// Выделенный фрагмент выделяется в отдельный объект
		QTextDocumentFragment fragment(cursor);

		EditorAbsTable current_table(table->columns(), table->rows()
					    , fragment.toHtml());
		...
		if(dialog.exec() == QDialog::Accepted){
			...
			if(hSplit > 1){
				if(hCurrMerge == 1) current_table.split_single_cell_by_horisontal(cellColCursor
												     , cellRowCursor, hSplit);
				else current_table.split_merged_cell_by_horisontal(cellColCursor
										      , cellRowCursor);
			}
			// Разбивка по вертикали
			if(vSplit > 1){
				if(vCurrMerge == 1) current_table.split_single_cell_by_vertical(
						cellColCursor, cellColCursor + hSplit - 1, cellRowCursor, vSplit);
				else current_table.split_merged_cell_by_vertical(
...
void Editor::on_insert_image_from_file_clicked(void){
	...
	if(is_image_select() || is_cursor_on_image()){
		...
		for(int i = 0; i < files.size(); ++ i){
			...
			_text_area->document()->addResource(QTextDocument::ImageResource
							   , QUrl(imageName), image);
...
void Editor::edit_image_properties(void){
	...
	dialog.set_info(tr("Real image size ") + QString::number(realImageWidth) +
	    " x " + QString::number(realImageHeight) + tr(" pixels"));
	...
	if(imageFormat.isValid()){
		...
		if(is_cursor_on_image()){
			...
			if(_text_area->textCursor().atBlockStart()) helper.movePosition(QTextCursor::NextCharacter
											   , QTextCursor::KeepAnchor);
			else helper.movePosition(QTextCursor::PreviousCharacter
						    , QTextCursor::KeepAnchor);
			helper.setCharFormat(imageFormat);
		}
	}
...
void Editor::on_to_attach_clicked(void){attach_callback_func();}
...
void Editor::on_show_text_clicked(void){
	...
	if(misc_field("title").length() > 0) showText->setWindowTitle(misc_field("title"));
	QTextDocument *cloneDocument = _text_area->document()->clone(
		showText); // Для метода clone указан parent, который уничтожится при
	// закрытии окна, и за собой уничтожит этот временный документ
...
void Editor::back_callback(void (*func)(void)){back_callback_func = func;}
...
void Editor::clear_all_misc_field(void){_misc_fields.clear();}
...
void Editor::dir_file_empty_reaction(int mode){
	...
	else critical_error("Editor::setDirFileEmptyReaction() : Unsupport mode " +
		    QString::number(mode));
}

int Editor::dir_file_empty_reaction(void){return _dir_file_empty_reaction;}
