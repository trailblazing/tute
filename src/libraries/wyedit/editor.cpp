...
void Editor::setup_buttons(void)
{
    ...
    // Кнопка Underline
    _underline = add_action(_tools_line_0, this, tr((editor_prefix + "underline").c_str()), tr("Underline (Ctrl+U)"), tr("Underline (Ctrl+U)"), QIcon(":/resource/pic/edit_underline.svg"), [&](bool) { on_underline_clicked(); }, QKeySequence(QKeySequence::Underline));
    ...
    // Кнопка Clear
    _clear = add_action(_tools_line_0, this, tr((editor_prefix + "clear").c_str()), tr("Reset format (Ctrl+K). When selected whole paragraph both "
    ...
    _dot_list = add_action(_tools_line_0, this, tr((editor_prefix + "dotlist").c_str()), tr("Marked list"), tr("Marked list"), QIcon(":/resource/pic/edit_listdot.svg"), [&](bool) { on_dotlist_clicked(); });
    ...
    _action_save = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "Save").c_str()), tr("Save"), tr("Save this entry to disk for editing or publication later"), QIcon(":/resource/pic/save.svg"), [&](bool) { _blogger->save_text_context(); });
    ...
    _action_more = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "More").c_str()), tr("More"), tr((std::string("Set this as the end of the main entry (which "
    ...
    _action_preview = add_action(_tools_line_0, this, tr(std::string(editor_prefix + "Entry in preview").c_str()), tr("Entry in preview"), tr("Preview the entry in this window"), QIcon(":/resource/pic/preview.svg") // QPixmap(previewIcon_xpm)
        ,
    ...
    _action_blog_this = add_action(
        _tools_line_0, this, tr(std::string(editor_prefix + "Blog this!").c_str()), tr("Blog this!"), tr("Post this entry to the blog"), QIcon(":/resource/pic/pull.svg") // pentalpha.svg//QPixmap(blogThisIcon_xpm)
        ,
    ...
    _save = add_action(_tools_line_0, this, tr((editor_prefix + "save").c_str()), tr("Save (Ctrl+S)"), tr("Save (Ctrl+S)"), QIcon(":/resource/pic/edit_save.svg"), [&](bool) { on_save_clicked(); }, QKeySequence(tr("Ctrl+S")));
    ...
    _settings = add_action(_tools_line_0, this, tr((editor_prefix + "settings").c_str()), tr("Editor settings"), tr("Editor settings"), QIcon(":/resource/pic/cogwheel.svg"), [&](bool) { on_settings_clicked(); });

    // Кнопка просмотра HTML кода
    _show_html = add_action(_tools_line_0, this, tr((editor_prefix + "showhtml").c_str()), tr("Edit HTML code"), tr("Edit HTML code"), QIcon(":/resource/pic/edit_showhtml.svg"), [&](bool) { on_showhtml_clicked(); });
    ...
    _font_color = add_action(_tools_line_1, this, tr((editor_prefix + "fontcolor").c_str()), tr("Text color"), tr("Text color"), QIcon(":/resource/pic/edit_fontcolor.svg"), [&](bool) { on_fontcolor_clicked(); });
    ...
    auto file_name = [&](QString file_name_) {
        ...
        if (file_name_.size() > 0) {
            ...
            if (fileInfo.absoluteDir().exists() && f.exists())
                _work_file_name = file_name_;
        } else
            _work_file_name = "";
    };

    file_name(file_name_);
    if (original_dir != dir_name)
        load_textarea();
    return result;
}
...
void Editor::on_code_clicked(void)
{
    ...
    if (_editor_config->get_code_indent_size_apply() == true && enableIndent == true) {
