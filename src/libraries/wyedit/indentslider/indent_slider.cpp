...
#include "../editor.h"
#include "indent_slider.h"

#if QT_VERSION == 0x050600
W_OBJECT_IMPL(IndentSlider)
#endif

IndentSlider::IndentSlider(int width, int height, QWidget *parent)
	: QWidget(parent){
...
void IndentSlider::paintEvent(QPaintEvent *event){
	...
	// исуется прямоугольник
	painter.drawRect(indentline_left_pos, widget_height / 2 - indentline_height / 2, indentline_right_pos - indentline_left_pos, indentline_height);

	// исуются движки отступов
...
int IndentSlider::get_current_indentedge_pos(void){
	if(current_sel_indent == SEL_NONE) return 0;
	if(current_sel_indent == SEL_TEXT_INDENT) return get_geometry_textindent_pos();
	if(current_sel_indent == SEL_LEFT_INDENT) return get_geometry_leftindent_pos();
	if(current_sel_indent == SEL_RIGHT_INDENT) return get_geometry_rightindent_pos();
	return 0;
}
...
void IndentSlider::mousePressEvent(QMouseEvent *event){
	...
	current_sel_indent = SEL_NONE;
	if(y < (widget_height / 2) && x > (indentline_left_pos + leftindent_pos + textindent_pos - indenttriangle_width / 2) && x < (indentline_left_pos + leftindent_pos + textindent_pos + indenttriangle_width / 2)) current_sel_indent = SEL_TEXT_INDENT;
	else if(y > widget_height / 2 && x > indentline_left_pos + leftindent_pos - indenttriangle_width / 2 && x < indentline_left_pos + leftindent_pos + indenttriangle_width / 2) current_sel_indent = SEL_LEFT_INDENT;
	else if(y > widget_height / 2 && x > indentline_right_pos - (rightindent_pos + indenttriangle_width / 2) && x < indentline_right_pos - (rightindent_pos - indenttriangle_width / 2)) current_sel_indent = SEL_RIGHT_INDENT;
...
void IndentSlider::mouseMoveEvent(QMouseEvent *event){
	...
	if(current_sel_indent == SEL_LEFT_INDENT){
		...
		if(leftindent_pos > indentline_right_pos - indentline_left_pos) leftindent_pos = indentline_right_pos - indentline_left_pos;
	...
	if(current_sel_indent == SEL_RIGHT_INDENT){
		...
		if(rightindent_pos > indentline_right_pos - indentline_left_pos) rightindent_pos = indentline_right_pos - indentline_left_pos;
