...
void IndentSlider::resizeEvent(QResizeEvent *event) {
	Q_UNUSED(event);
	qDebug() << "Textarea geometry X Y " << this->geometry().x() << " "
	         << this->geometry().y();
	qDebug() << "Textarea geometry Width Height " << this->geometry().width()
	         << " " << this->geometry().height();
	qDebug() << "Textarea frame geometry Width Height "
...
void IndentSlider::paintEvent(QPaintEvent *event) {
	...
	painter.drawRect(
	    indentline_left_pos, widget_height / 2 - indentline_height / 2,
...
void IndentSlider::draw_textindent(QPainter *painter) {
	QPointF points[5];
	points[0] =
	    QPointF((float)(indentline_left_pos + leftindent_pos + textindent_pos), (float)(widget_height / 2));
...
void IndentSlider::draw_leftindent(QPainter *painter) {
	...
	points[1] = QPointF(
	    (float)(indentline_left_pos + leftindent_pos - indenttriangle_width / 2),
	    (float)(widget_height * 3 / 4));
	points[2] = QPointF(
	    (float)(indentline_left_pos + leftindent_pos - indenttriangle_width / 2),
	    (float)(widget_height));
	points[3] = QPointF(
...
void IndentSlider::mousePressEvent(QMouseEvent *event) {
	...
	if (y < (widget_height / 2) &&
	    x > (indentline_left_pos + leftindent_pos + textindent_pos -
	         indenttriangle_width / 2) &&
	    x < (indentline_left_pos + leftindent_pos + textindent_pos +
	         indenttriangle_width / 2))
		current_sel_indent = SEL_TEXT_INDENT;
	else if (y > widget_height / 2 && x > indentline_left_pos + leftindent_pos - indenttriangle_width / 2 && x < indentline_left_pos + leftindent_pos + indenttriangle_width / 2)
		current_sel_indent = SEL_LEFT_INDENT;
	else if (y > widget_height / 2 && x > indentline_right_pos - (rightindent_pos + indenttriangle_width / 2) && x < indentline_right_pos - (rightindent_pos - indenttriangle_width / 2))
...
void IndentSlider::mouseMoveEvent(QMouseEvent *event) {
	...
	if (current_sel_indent == SEL_TEXT_INDENT) {
		if (x < indentline_left_pos)
			x = indentline_left_pos;
		if (x > indentline_right_pos)
			x = indentline_right_pos;
	...
	if (current_sel_indent == SEL_LEFT_INDENT) {
		...
		if (indentline_left_pos + leftindent_pos + textindent_pos <
		    indentline_left_pos) {
			textindent_pos = -leftindent_pos;
			emit change_textindent_pos(textindent_pos);
		}
		if (indentline_left_pos + leftindent_pos + textindent_pos >
		    indentline_right_pos) {
			textindent_pos =
			    indentline_right_pos - indentline_left_pos - leftindent_pos;
			emit change_textindent_pos(textindent_pos);
		}
	}
