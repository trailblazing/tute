...
#include "editor_abs_table_cell.h"

#include "../../main.h"

// Конструктор пустой таблицы
EditorAbsTable::EditorAbsTable(int x, int y, QObject *parent)
	: QObject(parent){
	columns = x;
	rows = y;
...
// Конструктор таблицы
EditorAbsTable::EditorAbsTable(int x, int y, QString tableText, QObject *parent)
	: QObject(parent){
	columns = x;
	rows = y;
...
void EditorAbsTable::clear_table(void){
	for(int i = 0; i < columns; i++)
		for(int j = 0; j < rows; j++) cells[i][j].clear();
}
...
void EditorAbsTable::clear_supercell_size_is_modify(void){
	for(int i = 0; i < columns; i++)
		for(int j = 0; j < rows; j++)
			if(cells[i][j].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL) cells[i][j].set_supercell_size_is_modify(false);
}
...
void EditorAbsTable::convert_table_to_internal(QString tableText){
	...
	while(!xmlReader.atEnd()){
		...
		if(xmlReader.tokenType() == QXmlStreamReader::EndElement){
			...
			if(xmlReader.name().toString() == "table") tagTableBalance--;
			// Если обнаружен конец строки
			if(xmlReader.name().toString() == "tr") tagTrBalance--;
...
QString EditorAbsTable::get_table(void){
	...
	for(int y = 0; y < rows; y++){
		...
		for(int x = 0; x < columns; x++){
			if(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_NORMAL_CELL || cells[x][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL){
				table = table + "<td ";
...
void EditorAbsTable::print_internal_table(void){
	for(int y = 0; y < rows; y++){
		for(int x = 0; x < columns; x++) cells[x][y].print_cell();
		printf("\n");
	}
}
...
int EditorAbsTable::get_cell_col_or_row_span(int x, int y, QString propName){
	if(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_NULL_CELL) critical_error("Detect bad table structure");
	if(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_NORMAL_CELL) return 1;
	if(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL) return cells[x][y].get_html_property(propName).toInt();
...
void EditorAbsTable::split_single_cell_by_horisontal(int x, int y, int div){
	...
	if(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_NORMAL_CELL) splitEnable = true;
	else if(cells[x][y].get_html_property("colspan").toInt() == 0 || cells[x][y].get_html_property("colspan").toInt() == 1) splitEnable = true;
	if(splitEnable == false) critical_error("Cell (" + QString::number(x) + "," + QString::number(y) + ") can not split by horisontal to " + QString::number(div) + " parts. Method split_single_cell_by_horisontal().");
			...
			if(i != y){
				switch(cells[x][i].get_cell_type()){
					...
					case EditorAbsTableCell::IS_SUPER_CELL: {
						// асширить суперячейку можно только один раз
				...
				if(!(cells[x][i].get_cell_type() == EditorAbsTableCell::IS_NORMAL_CELL || cells[x][i].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL)) critical_error("Detect splitting target null or sub cell while split by horisontal cell " + QString::number(x) + " " + QString::number(i));
				// Проверяем суперячейку на colspan<2
				if(cells[x][i].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL){
					int colspan = cells[x][i].get_html_property("colspan").toInt();
					if(colspan >= 2) critical_error("Detect splitting target super cell with colspan " + QString::number(colspan) + " while split by horisontal cell " + QString::number(x) + " " + QString::number(i));
				}
				// Если ячейка обычная
...
void EditorAbsTable::split_merged_cell_by_horisontal(int x, int y){
	...
	print_internal_table();
	// Проверка, можно ли разбивать ячейку
	if(!(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL && cells[x][y].get_html_property("colspan").toInt() >= 2)) critical_error("Cell (" + QString::number(x) + "," + QString::number(y) + ") can not split by horisontal, this is not supercell with colspan>2");
	...
	if(rowSpan < 2){
		...
		for(int i = x; i <= (x + div - 1); i++){
			...
			for(int j = (y + 1); j <= (y + rowSpan - 1); j++) cells[i][j].set_ref_super_cell_xy(i, y);
		}
...
void EditorAbsTable::split_single_cell_by_vertical(int x1, int x2, int y, int div){
	...
	// азбиваемые ячейки должны быть неразбиты по вертикали
	// то есть иметь rowspan равный 0 или 1
	for(int x = x1; x <= x2; x++){
		bool splitEnable = false;
		if(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_NORMAL_CELL) splitEnable = true;
		else if(cells[x][y].get_html_property("rowspan").toInt() == 0 || cells[x][y].get_html_property("rowspan").toInt() == 1) splitEnable = true;
		if(splitEnable == false) critical_error("Cell (" + QString::number(x) + "," + QString::number(y) + ") can not split by vertical to " + QString::number(div) + " parts. Method split_single_cell_by_vertical().");
	}
	// Перебирается счетчик разбиений
	for(int k = 1; k < div; k++){
		clear_supercell_size_is_modify();
		...
		for(int i = 0; i < columns; i++){
			...
			if(i < x1 || i > x2){
				...
				if(!(cells[i][y].get_cell_type() == EditorAbsTableCell::IS_NORMAL_CELL || cells[i][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL)) critical_error("Detect splitting target null or sub cell while split by vertical cell " + QString::number(i) + " " + QString::number(y));
				// Проверяем суперячейку на rowspan<2
				if(cells[i][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL){
					int rowSpan = cells[i][y].get_html_property("rowspan").toInt();
					if(rowSpan >= 2) critical_error("Detect splitting target super cell with rowspan " + QString::number(rowSpan) + " while split by vertical cell " + QString::number(i) + " " + QString::number(y));
				}
				// Если ячейка обычная
...
void EditorAbsTable::split_merged_cell_by_vertical(int x1, int x2, int y){
	...
	// Проверка, можно ли разбивать ячейки
	for(int x = x1; x <= x2; x++)
		if(!(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL && cells[x][y].get_html_property("rowspan").toInt() >= 2)) critical_error("Cell (" + QString::number(x) + "," + QString::number(y) + ") can not split by vertical, this is not supercell with rowspan>2");
	for(int x = x1; x <= x2; x++)
		if(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL){
			...
			if(colSpan < 2){
				...
				for(int i = y; i <= (y + div - 1); i++){
					...
					// и им только корректируются ссылки на суперячейку
					for(int j = (x + 1); j <= (x + colSpan - 1); j++) cells[j][i].set_ref_super_cell_xy(x, i);
				}
			} // Закрылось условие объединяла ли суперячейка только горизонтальные ячейки
