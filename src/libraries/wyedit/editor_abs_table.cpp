		...
		if (xmlReader.tokenType() == QXmlStreamReader::StartElement) {
			...
			if (xmlReader.name().toString() == "td") {
				...
				if (tagTdBalance == 1) {
					...
					while (cells[x][y].get_cell_type() !=
					       EditorAbsTableCell::IS_NULL_CELL)
						x++;
					...
					if (rowSpan == 0)
						rowSpan = 1;
					// Если ячейка растянута на несколько ячеек по горизонтали или
					// вертикали
					if (colSpan > 1 || rowSpan > 1) {
						qDebug() << "Is super cell, Colspan" << colSpan << "Rowspan"
						         << rowSpan;
						...
						for (int i = 0; i < colSpan; i++)
							for (int j = 0; j < rowSpan; j++)
								if (!(i == 0 && j == 0)) {
									...
									cells[x + i][y + j].set_cell_type(
									    EditorAbsTableCell::IS_SUB_CELL);

									// Эта подчиненная ячейка запоминает координаты своей
									// супер-ячейки
		...
		if (xmlReader.tokenType() == QXmlStreamReader::EndElement) {
			...
			if (xmlReader.name().toString() == "td") {
				...
				if (tagTdBalance == 0) {
					...
					QString cell_text = tableText.mid(
					    cellStartPosition, cellEndPosition - cellStartPosition);
...
void EditorAbsTable::split_single_cell_by_horisontal(int x, int y, int div) {
	...
	if (cells[x][y].get_cell_type() == EditorAbsTableCell::IS_NORMAL_CELL)
		splitEnable = true;
	else if (cells[x][y].get_html_property("colspan").toInt() == 0 || cells[x][y].get_html_property("colspan").toInt() == 1)
		splitEnable = true;
	...
	for (int k = 1; k < div; k++) {
		...
		for (int i = 0; i < rows; i++) {
			...
			if (i != y) {
				switch (cells[x][i].get_cell_type()) {
					...
					case EditorAbsTableCell::IS_SUB_CELL: {
						...
						if (!(super_x == x && super_y == y)) {
							...
							// асширить суперячейку можно только один раз
							if (cells[super_x][super_y].get_supercell_size_is_modify() ==
							    false) {
								// В самой суперячейке увеличивается colspan, так как она
								// стала содержать на единицу больше ячеек
								int temp_colspan =
								    cells[super_x][super_y].get_html_property("colspan").toInt();
								cells[super_x][super_y].set_html_property(
								    "colspan", QString::number(temp_colspan + 1));
							...
							// Поэтому координаты ссылки на суперячейку (у соседней с права
							// ячейки)
				...
				if (!(cells[x][i].get_cell_type() ==
				          EditorAbsTableCell::IS_NORMAL_CELL ||
				      cells[x][i].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL))
					critical_error(
					    "Detect splitting target null or sub cell while split "
					    "by horisontal cell " +
					    QString::number(x) + " " + QString::number(i));
...
void EditorAbsTable::split_merged_cell_by_horisontal(int x, int y) {
	...
	if (!(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL &&
		...
		critical_error(
		    "Cell (" + QString::number(x) + "," + QString::number(y) +
		    ") can not split by horisontal, this is not supercell with colspan>2");
...
void EditorAbsTable::split_single_cell_by_vertical(int x1, int x2, int y, int div) {
	qDebug() << "Internal table before split cells (" << x1 << "," << y << ")-("
	         << x2 << "," << y << ") to" << div << "by vertical";
	print_internal_table();
	...
	for (int x = x1; x <= x2; x++) {
		...
		if (cells[x][y].get_cell_type() == EditorAbsTableCell::IS_NORMAL_CELL)
			splitEnable = true;
		else if (cells[x][y].get_html_property("rowspan").toInt() == 0 || cells[x][y].get_html_property("rowspan").toInt() == 1)
			splitEnable = true;
		if (splitEnable == false)
	...
	for (int k = 1; k < div; k++) {
		...
		for (int i = 0; i < columns; i++) {
			...
			if (i < x1 || i > x2) {
					...
					case EditorAbsTableCell::IS_SUB_CELL: {
						...
						if (!(superX >= x1 && superX <= x2 && superY == y)) {
							...
							if (cells[superX][superY].get_supercell_size_is_modify() == false) {
								...
								// стала содержать на единицу больше ячеек
								int tempRowSpan =
								    cells[superX][superY].get_html_property("rowspan").toInt();
								cells[superX][superY].set_html_property(
								    "rowspan", QString::number(tempRowSpan + 1));
							...
							// Иначе координаты указывают на разбиваемую ячейку
							// Поэтому координаты ссылки на суперячейку (у соседней снизу
							// ячейки)
				...
				if (!(cells[i][y].get_cell_type() ==
				          EditorAbsTableCell::IS_NORMAL_CELL ||
				      cells[i][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL))
					critical_error(
					    "Detect splitting target null or sub cell while split "
					    "by vertical cell " +
					    QString::number(i) + " " + QString::number(y));
...
void EditorAbsTable::split_merged_cell_by_vertical(int x1, int x2, int y) {
	qDebug() << "Split merged cell (" << x1 << "," << y << ")-(" << x2 << "," << y
	         << ") by vertical";
	print_internal_table();
	// Проверка, можно ли разбивать ячейки
	for (int x = x1; x <= x2; x++)
		if (!(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL &&
			...
			critical_error(
			    "Cell (" + QString::number(x) + "," + QString::number(y) +
			    ") can not split by vertical, this is not supercell with rowspan>2");
	for (int x = x1; x <= x2; x++)
		if (cells[x][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL) {
			...
			}  // Закрылось условие объединяла ли суперячейка только горизонтальные
			   // ячейки
	...
	for (int i = insX + 2; i < columns; i++)
		for (int j = 0; j < rows; j++)
			if (cells[i][j].get_cell_type() == EditorAbsTableCell::IS_SUB_CELL) {
				...
				// qDebug()<<"For cell ("<<i<<","<<j<<"correct X ref to supercell
				// from"<<super_x;
				if (superX > insX)
					cells[i][j].set_ref_super_cell_x(superX + 1);
			}
}
