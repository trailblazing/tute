    ...
    for (int k = 1; k < div; k++) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            if (i != y) {
                ...
                if (!(cells[x][i].get_cell_type() == EditorAbsTableCell::IS_NORMAL_CELL || cells[x][i].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL))
...
void EditorAbsTable::split_merged_cell_by_horisontal(int x, int y)
{
    ...
    print_internal_table();
    // Проверка, можно ли разбивать ячейку
    if (!(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL && cells[x][y].get_html_property("colspan").toInt() >= 2))
    ...
    for (int k = 1; k < div; k++) {
        ...
        for (int i = 0; i < columns; i++) {
            ...
            if (i < x1 || i > x2) {
                ...
                if (!(cells[i][y].get_cell_type() == EditorAbsTableCell::IS_NORMAL_CELL || cells[i][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL))
...
void EditorAbsTable::split_merged_cell_by_vertical(int x1, int x2, int y)
{
    ...
    // Проверка, можно ли разбивать ячейки
    for (int x = x1; x <= x2; x++)
        if (!(cells[x][y].get_cell_type() == EditorAbsTableCell::IS_SUPER_CELL && cells[x][y].get_html_property("rowspan").toInt() >= 2))
