...
void EditorAbsTableCell::print_cell(void)
{
    ...
    printf("%c[%d,%d](%d,%d)%s ", type, htmlProperty["colspan"].toInt(), htmlProperty["rowspan"].toInt(), refSuperCellX, refSuperCellY, val.toUtf8().data()); // анее было toAscii, но в Qt 5.2 toAscii уже не поддерживается
}
