...
EditorImageProperties::EditorImageProperties(QWidget *parent)
	: QDialog(parent){
	imageRealWidth	= 0;
	imageRealHeight = 0;
...
void EditorImageProperties::setup_signals(){
	...
	connect(heightSpin, _valueChanged, this, &EditorImageProperties::on_changed_height);
...
// асчет размеров картинки в процентах на основании реальных размеров
...
void EditorImageProperties::update_percent(void){
	...
	// асчет процента далается на основании более длинной стороны рисунка
	if(imageRealWidth > imageRealHeight) calculatePercent = (widthSpin->value() * 100) / imageRealWidth;
	else calculatePercent = (heightSpin->value() * 100) / imageRealHeight;
	isRelateSizeSetted = true;
