...
EditorConfigMisc::EditorConfigMisc(std::shared_ptr<EditorConfig> editor_config_, QWidget* parent)
    : ConfigPage(parent), _editor_config(editor_config_)
{
	...
	indentStep->setValue(editor_config_->get_indent_step());
...
int EditorConfigMisc::apply_changes(void)
{
	...
	if (_editor_config->get_indent_step() != indentStep->value())
		_editor_config->set_indent_step(indentStep->value());
