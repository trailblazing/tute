...
EditorTextArea::EditorTextArea(QWidget *parent)
    : QTextEdit(parent) {
	...
	// this->setStyleSheet("QTextEdit::table {border:1px solid maroon;
...
bool EditorTextArea::event(QEvent *event) {
	if (event->type() == QEvent::Gesture) {
		// qDebug() << "In gesture event(): " << event << " Event type: " <<
		// event->type();
...
void EditorTextArea::tap_and_hold_gesture_triggered(
...
void EditorTextArea::paintEvent(QPaintEvent *event) {
	...
	while (true) {
		// Новая проверяемая позиция курсора устанавливается в середину области
		// поиска
		analysePoint = (documentStartFrameSearch + documentEndFrameSearch) / 2;
		cur.setPosition(analysePoint);

		// qDebug() << "Start " << document_start_frame_search << " End " <<
		// document_end_frame_search << " Analyse position " << analyse_point;
	...
	// qDebug() << "Found vision area at position " << cur.position() << "
	// Iterations " << start_frame_iteration_count;
	...
	do {
		...
		char c_ansi = c.toLatin1();  // анее было toAscii(), но этот метод теперь не
		                             // поддерживается в Qt 5.2
		if (c_ansi == ' ') {         // Пробел
			QRect rect = cursorRect(cur);
			int y = (rect.top() + rect.bottom()) / 2;
			// Если координаты курсора вышли за размер области обзора
			if (y > viewport()->height())
				break;  // Дальше рисовать ненужно

			// исуется точка на месте пробела
			...
			// исуется стрелка на месте табуляции
		...
		} else if (c.unicode() == QChar::ParagraphSeparator) {  // Перевод строки (в
			                                                    // виде границы между
			                                                    // абзацами)
			QRect rect = cursorRect(cur);
			int y1 = rect.top() + 3;
			...
			// исуется пи на месте перевода строки
			...
			// исуется изогнутая стрелка на месте перевода строки
...
int EditorTextArea::get_indent_started_right(void) {
	return this->lineWidth() + this->viewport()->width() + 1 -
