...
bool EditorTextArea::eventFilter(QObject *o, QEvent *e){
	...
	if(o == this->verticalScrollBar())
		if(e->type() == QEvent::Hide || e->type() == QEvent::Show) emit update_indentline_geometry_signal();
...
void EditorTextArea::paintEvent(QPaintEvent *event){
	...
	while(true){
		...
		if(dopCursorRectangle.top() < 0 && cursorRectangle.top() >= 0) break;
		...
		if(cursorRectangle.top() > 0) documentEndFrameSearch = analysePoint; // Нижняя граница двигается вверх
		else documentStartFrameSearch = analysePoint; // Верхняя граница двигается вниз
		// Если границы сомкнулись
		if((documentStartFrameSearch + 1) == documentEndFrameSearch || documentStartFrameSearch == documentEndFrameSearch){
	...
	do {
		...
		char c_ansi = c.toLatin1(); // анее было toAscii(), но этот метод теперь не поддерживается в Qt 5.2
		if(c_ansi == ' '){  // Пробел
