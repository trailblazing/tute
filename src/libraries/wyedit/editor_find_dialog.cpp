...
void EditorFindDialog::find_clicked(void){
	...
	if(_math_case->isChecked()) flags |= QTextDocument::FindCaseSensitively;
	if(_whole_words->isChecked()) flags |= QTextDocument::FindWholeWords;
	if(_search_backward->isChecked()) flags |= QTextDocument::FindBackward;
...
void EditorFindDialog::hideEvent(QHideEvent *event){
	...
	if(this->width() > 0 && this->height() > 0 && !(this->x() <= 0 && this->y() <= 0)){
		...
		// EditorConfig *_editor_config = qobject_cast<Editor *>(parent())->_editor_config;

		// Запоминается геометрия
		QRect g = this->frameGeometry();
		qDebug() << "Frame geometry X " << g.x() << " Y " << g.y() << " W " << g.width() << " H" << g.height();
		QString gs = QString::number(g.x()) + "," + QString::number(g.y()) + "," + QString::number(g.width()) + "," + QString::number(g.height());
