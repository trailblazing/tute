    ...
    : QWidget(parent)
{
    ...
    if (!conf_fileinfo->exists()) {
        ...
        if (!editor_conf.exists()) // if(editor_conf.exists() && editor_conf.isFile()) QFile::remove(editor_conf_file);
            if (!QFile::copy(QString(":/resource/standardconfig/") + globalparameters.target_os() + "/editorconf.ini", editor_conf_file))
    ...
    if (conf_fileinfo->isWritable() == false || conf_fileinfo->isReadable() == false)
...
QStringList EditorConfig::get_parameter_table_3(bool withEndSignature)
{
    ...
    table << "tools_line_1"
          << "QString"
          << "freeze_browser_view,clear,separator,bold,italic,underline,separator,monospace,code,alignleft,aligncenter,alignright,alignwidth,fontselect,fontsize";
    table << "tools_line_2"
          << "QString"
          << " ";
    if (withEndSignature)
...
void EditorConfig::update_version(int versionFrom, int versionTo, QStringList baseTable, QStringList finalTable)
{
    ...
    while (i.hasNext()) {
        ...
        if (beforeParamFlag == 1) {
            ...
            toTable[toName]["value"] = update_version_change_value(versionFrom, versionTo, toName, fromValue, toValue);
        ...
        if (beforeParamFlag == 2) {
            ...
            toTable[toName]["value"] = update_version_allowcollision(versionFrom, versionTo, toName, fromType, fromValue, toType, toValue);
