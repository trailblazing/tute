...
//                                    else
	...
	if(file_data) result = DiskHelper::copy_file_to_data_folder(file_name_fullpath, globalparameters.root_path() + "/" + QDir(appconfig.trash_dir()).dirName() + "/" + file_data->_name);
//        result = target_file;
//    }
    }
    return result;
...
//    QFile			currentFile(_path + "/" + _file_name);
    auto _file_size = filesize(QString(_path + "/" + _file_name).toStdString().c_str());;// currentFile.size();
