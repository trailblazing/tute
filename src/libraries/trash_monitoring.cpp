...
TrashMonitoring::recover_from_trash(std::shared_ptr<QFileInfo> target_file, bool force_remove)
{
    ...
    QFileInfo file(target_file_full_name);
    bool file_exist = file.exists();
    auto file_size = file_exist ? file.size() : -1;
    if (file_exist) {
        if (0 >= file_size) {
    ...
    if (need || force_remove) {
        ...
        auto recover_from_source = [&] {
            ...
            if (!QDir(gl_paras->root_path() + "/" + QDir(appconfig->trash_dir()).dirName())
                ...
                QDir(gl_paras->root_path())
                    .mkdir(QDir(appconfig->trash_dir()).dirName());
            if (!DiskHelper::file_cover(QResource(QString(":/resource/standarddata/") + _file_name), trash_file_full_name))
                throw std::runtime_error("Can not copy index.xml");
            else
                QFile::setPermissions(gl_paras->root_path() + "/" + QDir(appconfig->trash_dir()).dirName() + "/" + _file_name, QFile::ReadUser | QFile::WriteUser);
            // bool succedded =
        ...
        if (file_data)
            result = DiskHelper::file_cover(s, target_file_full_name);
        // result = target_file;
        // }
    }
    return result;
...
=======
    ...
    for (int i = 0; i < fileInfoList.size(); i++) {
        ...
        auto _file_size = // filesize_non_qt(_file_name.toStdString().c_str()); //
...
void TrashMonitoring::remove_oldest_file(QString file_name_)
{
    if (_files_table.size() > 10) {
        ...
        QString _file_found = _path + "/" + found->_name; // got ? _path + "/" + found->_name : _path
