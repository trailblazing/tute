...
std::shared_ptr<QFileInfo> TrashMonitoring::recover_from_trash(std::shared_ptr<QFileInfo> target_file, bool force_remove){
    ...
    auto						need				= false;
    auto						_file_name			= target_file->fileName();	// "mytetra.xml";
...
//                                    else
        ...
        result = DiskHelper::copy_file_to_data_folder(appconfig.trash_dir() + '/' + file_data->_name);
//        result = target_file;
//    }
    }
    return result;
...
//    QString		full_current_path = main_program_file_info.absolutePath();
    ...
    if(! _dir.exists()){
        DiskHelper::create_directory(globalparameters.work_directory()	// full_current_path
                                    , "trash");
...
//		unsigned int	_file_time	= fileInfoList.at(i).created().toTime_t();
        unsigned int _file_size	= filesize(_file_name.toStdString().c_str());// fileInfoList.at(i).size();
...
//    QFile			currentFile(_path + "/" + _file_name);
    unsigned int _file_size = filesize(QString(_path + "/" + _file_name).toStdString().c_str());;// currentFile.size();
