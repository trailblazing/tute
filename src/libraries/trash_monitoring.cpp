...
TrashMonitoring::recover_from_trash(std::shared_ptr<QFileInfo> target_file, bool force_remove) {
	std::shared_ptr<QFileInfo> result(nullptr);  // = false;
	auto need = false;
	auto _file_name = target_file->fileName();  // "index.xml";
	auto target_file_full_name =
...
void TrashMonitoring::init(QString _trash_path) {
	// auto		_main_program_file =
	// globalparameters.main_program_file();
	// QFileInfo	main_program_file_info(_main_program_file);
	// QString		full_current_path =
	// main_program_file_info.absolutePath();
...
void TrashMonitoring::add_file(QString _file_name) {
	...
	auto _file_size =
	    filesize(QString(_path + "/" + _file_name).toStdString().c_str());
	;  // currentFile.size();
...
void TrashMonitoring::update(void) {
	...
	while (_files_table.size() > appconfig->trash_max_file_count() ||
		...
		if (_files_table.size() <=
		    10)  // Оставляется последний файл, какого бы размера он не был
			break;
		else
			remove_oldest_file();
	}
...
void TrashMonitoring::remove_file_fullpath(QString file_name_) {
	// QString _file_name = got ? _path + "/" + _files_table.last()._file_name :
	// _path + "/" + found._file_name;
...
void TrashMonitoring::remove_oldest_file(QString file_name_) {
	if (_files_table.size() > 10) {
		...
		QString _file_found =
		    _path + "/" + found->_name;  // got ? _path + "/" + found->_name : _path
		                                 // + "/" + _files_table.last()->_name;

		qDebug() << "Remove file " << _file_found << " from trash";
		//		if(0 == found->size()){
		//			if(QFile::exists(_file_found)){
		//				if(QFile::remove(_file_found)){  // Файл физически
		//удаляется
		//					if(_files_table.removeOne(found)) _dir_size = _dir_size -
		//found->size();
		//				}else{
		//					critical_error("In remove_oldest_file. Can not delete file \"" +
		//_file_found + "\" which file size is zero");
		////				exit(0);
		//				}
		//			}
		//		}else
		if (QFile::exists(_file_found)) {
			if (QFile::remove(_file_found)) {  // Файл физически удаляется
				...
				//				//
				//				QMutableListIterator<std::shared_ptr<FileData>
				//> iter(_files_table);
				//				while(iter.hasNext()){
				//					auto it = iter.next();
...
TrashMonitoring::FileData::FileData(TrashMonitoring *tm, const QString &name_)
    ...
    , _time([&] {
	    ...
	    if (_name != "") {
		    ...
		    _file_time = std::chrono::system_clock::to_time_t(
...
std::ifstream::pos_type TrashMonitoring::FileData::size() const {
	...
	return const_cast<FileData *>(this)->_size = filesize(
...
bool operator==(const TrashMonitoring::FileData &fd0, const TrashMonitoring::FileData &fd1) {
	return fd0._name == fd1._name && fd0._time == fd1._time &&
	    fd0._size == fd1._size && fd0._trashmonitoring == fd1._trashmonitoring;
}
...
const TrashMonitoring::FileData &TrashMonitoring::FileData::
operator=(const TrashMonitoring::FileData &fd) {
	if (&fd != this) {
		_name = fd._name;
		_time = fd._time;
		_size = fd.size();
