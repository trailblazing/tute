...
std::shared_ptr<QFileInfo> TrashMonitoring::recover_from_trash(std::shared_ptr<QFileInfo> target_file, bool force_remove){
	...
	if(need || force_remove){
		...
		if(_files_table.size() == 0){
			...
			if(!QFile::copy(QString(":/resource/standarddata/") + _file_name, gl_paras->root_path() + "/" + QDir(appconfig->trash_dir()).dirName() + "/" + _file_name)) throw std::runtime_error("Can not copy index.xml");
			else QFile::setPermissions(gl_paras->root_path() + "/" + QDir(appconfig->trash_dir()).dirName() + "/" + _file_name, QFile::ReadUser | QFile::WriteUser);
		...
		if(file_data) result = DiskHelper::copy_file_to_data_folder(file_name_fullpath, gl_paras->root_path() + "/" + QDir(appconfig->trash_dir()).dirName() + "/" + file_data->_name);
		// result = target_file;
		// }
	}
	return result;
...
TrashMonitoring::FileData::FileData(TrashMonitoring *tm, const QString &n)
	: _trashmonitoring(tm)
	  ...
	  , _time([&] {
			  ...
			  }else _file_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
	                  return _file_time;
		  }
	          ())
	  , _size([&] {
