...
void AccountsDialog::changeListIndex(int index)
{
    ...
    if (doingNewAccount) {
        if (dirty)
            lwAccountList->item((lwAccountList->count()) - 1)->setText(leName->text().isEmpty() ? tr("(No name)") : leName->text());
        else {
...
void AccountsDialog::acceptAccount()
{
    ...
    if (doingNewAccount) {
        ...
        lwAccountList->item(currentRow)->setText(leName->text().isEmpty() ? tr("(No name)") : leName->text());
    doingNewAccount = false;
    setClean();
}
