...
AccountsDialog::AccountsDialog(QList<AccountsDialog::Account> &acctList, int acctIndex, QWidget *parent)
	: QDialog(parent){
	QString a;
	...
	for(int i = 0; i < accountList.count(); i++){
		...
		if(a.isEmpty()) lwAccountList->addItem(tr("(No name)"));
		else lwAccountList->addItem(a);
	}
	...
	Q_FOREACH(QWidget *w, accountWidgets) w->setEnabled(false);
	...
	connect(addAccount, SIGNAL(triggered(bool)), this, SLOT(doNewAccount()));
	connect(removeAccount, SIGNAL(triggered(bool)), this, SLOT(removeThisAccount()));
	connect(lwAccountList, SIGNAL(currentRowChanged(int)), this, SLOT(changeListIndex(int)));
...
void AccountsDialog::changeListIndex(int index){
	leBlogURI->clear();
	if(doingNewAccount){
		if(dirty) lwAccountList->item((lwAccountList->count()) - 1)->setText(leName->text().isEmpty() ? tr("(No name)") : leName->text());
		else{
	...
	if(accountList[currentRow].hostedBlogType != -1) cbHostedBlogType->setCurrentIndex(accountList[currentRow].hostedBlogType);
	...
	chUseWordpressAPI->setCheckState(accountList[currentRow].useWordpressAPI
	                                 ? Qt::Checked
					 : Qt::Unchecked);
	Q_FOREACH(QWidget *w, accountWidgets) w->setEnabled(true);
}
...
void AccountsDialog::doNewAccount(){
	...
	lwAccountList->disconnect(SIGNAL(currentRowChanged(int)), this, SLOT(changeListIndex(int)));
	currentRow = -1;
	...
	connect(qApp, SIGNAL(focusChanged(QWidget *,QWidget *)), this, SLOT(assignSlug()));
...
void AccountsDialog::setDirty(){
	dirty = true;
	Q_FOREACH(QWidget *w, accountWidgets) disconnect(w, 0, this, SLOT(setDirty()));
}
...
void AccountsDialog::setClean(){
	...
	Q_FOREACH(QWidget *w, accountWidgets) disconnect(w, 0, this, SLOT(setDirty()));
	Q_FOREACH(QWidget *v, accountWidgets)
		if(qobject_cast<QLineEdit *>(v)) connect(v, SIGNAL(textChanged(const QString&)), this, SLOT(setDirty()));
}
...
void AccountsDialog::acceptAccount(){
	...
	if(doingNewAccount){
		...
		if(lwAccountList->count() == 1) connect(lwAccountList, SIGNAL(currentRowChanged(int)), this, SLOT(changeListIndex(int)));
	}else	if(lwAccountList->count()) lwAccountList->item(currentRow)->setText(leName->text().isEmpty() ? tr("(No name)") : leName->text());
	doingNewAccount = false;
	setClean();
}
...
void AccountsDialog::assignSlug(){
	if(accountList.count() != 0){
		currentAccountId = QString("%1_%2").arg(entryDateTime.toString(Qt::ISODate)).arg(leName->text().toLower().replace(QRegExp("\\s"), "_"));
...
void AccountsDialog::on_pbOK_clicked(){
	if(doingNewAccount && !dirty) accountList.removeAt(currentRow);
	accept();
}
...
void AccountsDialog::on_leBlogURI_returnPressed(){
	...
	if(!uri.isValid()){
		QMessageBox::information(0, tr((program_title_string + ": URI not valid").c_str()), tr("That web location is not valid."), QMessageBox::Cancel);
		return;
	}
	// bool found = false;
	// QStringList hostedAccountStrings, hostedAccountServers, hostedAccountLocations;
	QStringList wpmuHosts;
	wpmuHosts	<< "wordpress.com"
	                << "blogsome.com"
	                << "blogs.ie"
	                << "hadithuna.com"
	                << "edublogs.com"
	...
	if(cbHostedBlogType->currentIndex() >= 5 && cbHostedBlogType->currentIndex() <= 7){
		QString endpoint, ep;
		if(cbHostedBlogType->currentIndex() == 7)                                                                                                                // i.e. Textpattern
			endpoint = "rpc/index.php";
		else endpoint = "xmlrpc.php";
		ep = uris.section("//", 1, 1).section("/", 0, 0);

		urisLocation = uri.path();
		if(re.exactMatch(urisLocation)) urisLocation = urisLocation.section("/", 0, -2).append(endpoint);
		else urisLocation.append(urisLocation.endsWith('/') ? endpoint
	...
	if(re.exactMatch(loc)) loc = loc.section("/", -2, 0, QString::SectionIncludeTrailingSep)
		                     .append("rsd.xml");
	else loc.append(loc.endsWith('/') ? "rsd.xml" : "/rsd.xml");
	 ...
	 */
	if(qApp->overrideCursor() == 0) qApp->setOverrideCursor(QCursor(Qt::BusyCursor));
	networkBiz = FindingRsdXml;

	connect(netmgr, SIGNAL(finished(QNetworkReply *)), this, SLOT(handleHttpDone(QNetworkReply *)));
}
...
void AccountsDialog::handleHttpDone(QNetworkReply *reply){
	...
	int i;
	QUrl url;
	QString failure = tr((program_title_string + " failed to auto-configure access to your account. "
	...
	if(reply->error() == QNetworkReply::NoError){
		switch(networkBiz){
			case FindingRsdXml:
				...
				if(rsdXml.documentElement().tagName() == "rsd"){
					...
					QMessageBox::information(this, tr((program_title_string + ": Failure").c_str()), failure, QMessageBox::Cancel);
				netmgr->disconnect();
				reply->disconnect();
				reply->deleteLater();
				currentReply = NULL;
		...
		QMessageBox::information(this, tr((program_title_string + " - Network failure (%1)").c_str()).arg((int) reply->error()), tr((program_title_string + " could not contact the site.  Please consult the documentation "
	...
	if(obj == leBlogURI){
		switch(event->type()){
			case QEvent::FocusOut:
				if(!leBlogURI->text().isEmpty()) on_leBlogURI_returnPressed();
			default:
