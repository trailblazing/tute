...
void AccountsDialog::setClean(){
	...
	Q_FOREACH(QWidget *w, accountWidgets) disconnect(w, 0, this, SLOT(setDirty()));
	Q_FOREACH(QWidget *v, accountWidgets){
		if(qobject_cast<QLineEdit *>(v)) connect(v, SIGNAL(textChanged(const QString&))
							    , this, SLOT(setDirty()));
	}
}
...
void AccountsDialog::acceptAccount(){
	...
	if(doingNewAccount){
		...
		if(lwAccountList->count() == 1) connect(lwAccountList, SIGNAL(currentRowChanged(int))
							   , this, SLOT(changeListIndex(int)));
	}else{
...
void AccountsDialog::on_leBlogURI_returnPressed(){
	...
	if(! uri.isValid()){
		QMessageBox::information(0, tr((program_title_string + ": URI not valid").c_str())
...
void AccountsDialog::handleHttpDone(QNetworkReply *reply){
	...
	if(reply->error() == QNetworkReply::NoError){
		...
		QMessageBox::information(this
					, tr((program_title_string + " - Network failure (%1)").c_str()).arg((int) reply->error())
					, tr((program_title_string + " could not contact the site.  Please consult the documentation "
								     ...
								     "server and location manually.").c_str())
					, QMessageBox::Cancel);
		netmgr->disconnect();
		reply->deleteLater();
		currentReply = NULL;
