    ...
    : QDialog(parent)
{
    ...
    // Set up internal account lists; one for the current contents of the
    // accounts,
    // one for backup
    ...
    accountWidgets << leName << cbHostedBlogType << leEndpoint << leLogin
                   << lePassword << chCategoriesEnabled << chPostDateTime
                   << chAllowComments << chAllowTB << chUseWordpressAPI;
...
void AccountsDialog::changeListIndex(int index)
{
    ...
    if (doingNewAccount) {
        if (dirty)
            lwAccountList->item((lwAccountList->count()) - 1)
    ...
    chCategoriesEnabled->setCheckState(
        accountList[currentRow].categoriesEnabled ? Qt::Checked : Qt::Unchecked);
    chPostDateTime->setCheckState(
        accountList[currentRow].postDateTime ? Qt::Checked : Qt::Unchecked);
    chAllowComments->setCheckState(
        accountList[currentRow].allowComments ? Qt::Checked : Qt::Unchecked);
    chAllowTB->setCheckState(accountList[currentRow].allowTB ? Qt::Checked : Qt::Unchecked);
    chUseWordpressAPI->setCheckState(
        accountList[currentRow].useWordpressAPI ? Qt::Checked : Qt::Unchecked);
    Q_FOREACH (QWidget* w, accountWidgets)
        w->setEnabled(true);
}
...
void AccountsDialog::doNewAccount()
{
    ...
    connect(sapp_t::instance(), SIGNAL(focusChanged(QWidget*, QWidget*)), this, SLOT(assignSlug()));
...
void AccountsDialog::acceptAccount()
{
    ...
    if (doingNewAccount) {
        ...
        if (lwAccountList->count() == 1)
            connect(lwAccountList, SIGNAL(currentRowChanged(int)), this, SLOT(changeListIndex(int)));
    } else if (lwAccountList->count())
        lwAccountList->item(currentRow)
            ->setText(leName->text().isEmpty() ? tr("(No name)") : leName->text());
    doingNewAccount = false;
    setClean();
}
...
void AccountsDialog::assignSlug()
{
    if (accountList.count() != 0) {
        currentAccountId = QString("%1_%2")
                               .arg(entryDateTime.toString(Qt::ISODate))
                               .arg(leName->text().toLower().replace(QRegExp("\\s"), "_"));
...
void AccountsDialog::on_leBlogURI_returnPressed()
{
    ...
    // bool found = false;
    // QStringList hostedAccountStrings, hostedAccountServers,
    // hostedAccountLocations;
    ...
    if (uris.contains("squarespace.com")) {
        leEndpoint->setText(
            "http://www.squarespace.com/do/process/external/PostInterceptor");
    ...
    if (cbHostedBlogType->currentIndex() >= 5 && cbHostedBlogType->currentIndex() <= 7) {
        ...
        if (re.exactMatch(urisLocation))
            ...
            urisLocation.append(
                urisLocation.endsWith('/') ? endpoint : QString("/%1").arg(endpoint));
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        case FindingRsdXml:
            ...
            if (rsdXml.documentElement().tagName() == "rsd") {
                ...
                QMessageBox::information(
                    this, tr((program_title_string + ": Failure").c_str()), failure,
                    QMessageBox::Cancel);
            netmgr->disconnect();
            reply->disconnect();
            reply->deleteLater();
        ...
        case FindingXmlrpcPhp:
            /*
                                              // If it finds xmlrpc.php, it
         returns a short string with a successful (200) status code
                                              if( responseHeader.statusCode() ==
         200 ) {
                                              leServer->setText( currentHost );
                                              currentHost = QString();
                                              leLocation->setText( QUrl(
         currentReq.path() ).path() );
                                              http->close();
                                              currentReq = QHttpRequestHeader();
                                              networkBiz = NoBusiness;
                                              disconnect( http, SIGNAL(
         requestFinished() ), this, 0 );
                                              disconnect( http, SIGNAL( done(
         bool ) ), this, 0 );
                                              break;
                                              }
                                              else {
                                              http->close();
                                              http->setHost( QUrl(
         currentReq.path() ).host() );
                                              http->get( QUrl( currentReq.path()
         ).path().replace( "xmlrpc.php", "rsd.xml" ) );
                                              currentReq = QHttpRequestHeader();
                                              networkBiz = FindingRsdXml;
                                              }  */
            break;
        ...
        QMessageBox::information(
            this, tr((program_title_string + " - Network failure (%1)").c_str()).arg(static_cast<int>(reply->error())),
            tr((program_title_string + " could not contact the site.  Please consult the documentation "
...
bool AccountsDialog::eventFilter(QObject* obj, QEvent* event)
{
    ...
    // When the blog URI is typed in and Tab is pressed, the same must happen as
    // when
    ...
    // The text in the location field must start with /; otherwise, the web server
    // will
    // return an error.
