...
void QijSearchWidget::setTextEdit(QWidget *te){
	...
	if(pted){
	    ...
	    _textEdit		= 0;
	    if(! leFindText->text().isEmpty()) findInTextEdit(leFindText->text(), Stay);
	}else qDebug("This widget is not a QTextEdit or a QPlainTextEdit");
    }
}
...
void QijSearchWidget::setExpertEnabled(bool a){
    ...
    if(! a) chRegExp->setChecked(false);
#endif
}
...
void QijSearchWidget::find(){
	// qDebug( "find" );
    if(! leFindText->text().length()) leFindText->selectAll();
    show();
...
void QijSearchWidget::findAgain(){
    show();
    if(leFindText->text().isEmpty()) leFindText->setFocus(Qt::MenuBarFocusReason);
    else findInTextEdit(leFindText->text(), dir);
}
...
void QijSearchWidget::findInTextEdit(const QString &text, direction currentDirection){
    ...
    if(! text.isEmpty()){
	if(currentDirection == Backward) options |= QTextDocument::FindBackward;
	if(chMatchCase->isChecked()) options |= QTextDocument::FindCaseSensitively;
	if(chWholeWords->isChecked()) options |= QTextDocument::FindWholeWords;
#if QT_VERSION >= 0x040200
	if(chRegExp->isChecked()) newCursor = doc->find(QRegExp(text, chMatchCase->isChecked() ? Qt::CaseSensitive : Qt::CaseInsensitive), c);
	else
#endif
	...
	if(newCursor.isNull()){
...
#if QT_VERSION >= 0x040200
	    if(chRegExp->isChecked()) newCursor = doc->find(QRegExp(text), ac, options);
	    else
#endif
		...
		newCursor = c;
	    }else emit searchMessage(tr("Wrapped the search"));
	}
    }
    if(! isVisible()) show();
#if QT_VERSION < 0x040400
...
#else
    if(_textEdit) _textEdit->setTextCursor(newCursor);
    else _plainTextEdit->setTextCursor(newCursor);
#endif
...
void QijSearchWidget::on_leFindText_returnPressed(){
    if(! leFindText->text().isEmpty()) findInTextEdit(leFindText->text(), dir);
}
