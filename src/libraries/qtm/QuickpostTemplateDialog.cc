...
QuickpostTemplateDialog::QuickpostTemplateDialog(QStringList &titles, QStringList &templates, QList<int> &pubStates, QList<bool> &ctStates, QList<QStringList> &ahLists, bool status, QWidget *parent)
    ...
    connect(addTemplate, SIGNAL(triggered(bool))
	   , this, SLOT(doNewTemplate()));
    connect(removeTemplate, SIGNAL(triggered(bool))
	   , this, SLOT(removeThisTemplate()));
    connect(lwTemplates, SIGNAL(currentRowChanged(int))
	   , this, SLOT(changeListIndex(int)));
    connect(insertTabIntoTemplate, SIGNAL(triggered(bool))
	   , this, SLOT(doInsertTabIntoTemplate()));
}
...
void QuickpostTemplateDialog::acceptTemplate(){
	...
	lwTemplates->item(lwTemplates->count() - 1)->setText(leName->text().isEmpty()
	    ? tr("(No name)")
	    : leName->text());
	if(lwTemplates->count() == 1) connect(lwTemplates, SIGNAL(currentRowChanged(int))
					     , this, SLOT(changeListIndex(int)));
	...
	if(lwTemplates->count()){
	    lwTemplates->item(currentRow)->setText(leName->text().isEmpty()
...
void QuickpostTemplateDialog::on_pbOK_clicked(){
	...
	for(i = 0; i < _assocHostLists.count(); i ++){
		...
		if(_assocHostLists[i].at(j).isEmpty() ||
		    emptyString.exactMatch(_assocHostLists[i].at(j))) _assocHostLists[i].removeAt(j);
	    }
	}
    }
    accept();
...
void QuickpostTemplateDialog::on_leName_textEdited(const QString &newName){
    if(currentRow != - 1){
	// qDebug( "title edited" );
	lwTemplates->item(currentRow)->setText(newName.isEmpty()
	    ? tr("(No name)") : newName);
