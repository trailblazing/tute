...
#define _METAWEBLOG_NEWMEDIAOBJECT \
	Blogger::HttpBusinessType::_metaWeblog_newMediaObject
...
bool XmlRpcHandler::startElement(const QString &, const QString &, const QString &qName, const QXmlAttributes &attr) {
	...
	if ((qName == "struct")) {
		insideStruct = true;
		if (((_request_type == _BLOGGER_GETUSERSBLOGS) ||
		     (_request_type == _MT_GETCATEGORYLIST))) {
			...
			// qDebug() << "Creating super element: " <<
			// superElementName.toAscii().data();
...
bool XmlRpcHandler::characters(const QString &str) {
	...
	if (str.startsWith("http://", Qt::CaseInsensitive) &&
	    _request_type != _METAWEBLOG_NEWMEDIAOBJECT)
...
bool XmlRpcHandler::endElement(const QString &, const QString &, const QString &qName) {
	...
	if (qName == "struct") {
		if (!returnElement.isNull() && !currentSuperElement.isNull()) {
			// qDebug() << "Appending current super-element: " <<
			// qName.toAscii().data();
...
bool XmlRpcHandler::fatalError(const QXmlParseException &exc) {
	...
	_faultString.append(QString("Line %1, column %2: %3").arg(exc.lineNumber()).arg(exc.columnNumber()).arg(exc.message()));
	methodResponseFinished = true;
	return false;
}
/*
