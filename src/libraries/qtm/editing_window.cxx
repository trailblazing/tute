...
EditingWindow::EditingWindow(
    ...
    , _local_storage_directory([&]() -> QString {
        QString path = QString(_app->isSandbox() ? "%1/qtm-sandbox" : "%1/qtm-blog").arg(globalparameters.root_path() + "/" + globalparameters.target_os());
        if (!QDir(path).exists())
...
#endif
{
    ...
    QFile accountsXmlFile(
        PROPERSEPS(QString("%1/qtmaccounts2.xml").arg(_local_storage_directory)));
...
// qDebug() << "Can't read the XML";
#endif
            accountsXmlFile.close();
            accountsElement = _accounts_dom.createElement(
                program_title_qstring + "Accounts");
            ...
            if (!server.isEmpty()) {
                ...
                nameElem.appendChild(_accounts_dom.createTextNode(tr("Default account")));
                serverElem = _accounts_dom.createElement("server");
                serverElem.appendChild(
                    _accounts_dom.createTextNode(server));
                ...
                Q_FOREACH (QString s, attribs) {
                    if (*(accountAttributes[s])) {
                        attribElem = _accounts_dom.createElement(
                            "attribute");
...
EditingWindow::~EditingWindow()
{
    if (_editentry->_editing_list.contains(this))
        _editentry->_editing_list.removeOne(this);
    deleteLater();
}
...
//    superMenu->setMainWindow(this);
    ...
    userAgentString = QString(program_title_qstring + "/%1").arg(app_version); // QTM_VERSION
...
/*ui.actionPrint->setEnabled( false );    // implemented
    ...
    addTBPingAction = add_action<QListWidget, QAction, SideTabWidget>(
        _control_tab->lwTBPings, _control_tab, tr("Add trackback ping"), tr("Add trackback ping"), tr(""), di, [&](bool) { addTBPing(); });
    addClipPingAction = add_action<QListWidget, QAction, SideTabWidget>(
        _control_tab->lwTBPings, _control_tab, tr("Add ping from clipboard"), tr("Add ping from clipboard"), tr(""), di, [&](bool) { addClipTBPing(); });
    removeTBPingAction = add_action<QListWidget, QAction, SideTabWidget>(
        _control_tab->lwTBPings, _control_tab, tr("Remove trackback ping"), tr("Remove trackback ping"), tr(""), di, [&](bool) { removeTBPing(); });
    ...
    addKeywordTag_forList = add_action<QListWidget, QAction, QWidget>(
        _control_tab->lwAvailKeywordTags, nullptr, tr("Add this tag"), tr("Add this tag"), tr(""), di, [&](bool) { addKeywordTagFromAvailTags(); });
    removeKeywordTag_forList = add_action<QListWidget, QAction, QWidget>(
        _control_tab->lwKeywordTags, nullptr, tr("Remove this tag"), tr("Remove this tag"), tr(""), di, [&](bool) { removeKeywordTag(); });
    _refreshKeywordTags = add_action<QListWidget, QAction, QWidget>(
        _control_tab->lwAvailKeywordTags, nullptr, tr("Refresh"), tr("Refresh"), tr(""), di, [&](bool) { refreshKeywordTags(); });
    ...
    _child_category_here_action = add_action<QListWidget, QAction, QListWidget>(
        _control_tab->lwOtherCats, _control_tab->lwOtherCats, tr("Add child category"), tr("Add child category"), tr(""), di, [&](bool) { newChildCategory(); });
...
//
    ...
    //          if((sizes[1] > width() - sizes[0]) || (sizes[1] <= 0))
    // sizes[1]
    //= width() - sizes[0];
    //          assert(sizes[1] >= 0);
...
void EditingWindow::positionWidget(QWidget* w, QWidget* refWidget)
{
    ...
    if (qobject_cast<QWidget*>(refWidget)) {
        ...
        if ((g.right() + 30) >= (r.right() + 30) && (g.bottom() + 30) >= (r.bottom() + 30))
            w->move(r.left(), r.top());
        else
...
void EditingWindow::showEvent(QShowEvent* event)
{
    ...
    if (editor_object->document()->isEmpty() && !dirtyIndicator->isVisible() && !event->spontaneous() && !entryEverSaved)
        _control_tab->leTitle->setFocus(Qt::ActiveWindowFocusReason);
...
void EditingWindow::setTextFonts()
{
    ...
    if (enableHighlighting) {
        // setDirtySignals( false );
        if (_super_menu)
            _super_menu->setHighlightingChecked(true);
...
void EditingWindow::setInitialAccount()
{
    ...
    if (accountsList.count() == 1 && accountsList.at(0).firstChildElement("details").isNull()) {
    ...
    for (i = 0; i < accountsList.count(); i++) {
        ...
        if (!thisTitleElem.isNull())
            ...
            _control_tab->cbAccountSelector->addItem(
                tr("Unnamed account"), accountsList.at(i).toElement().attribute("id").isEmpty()
    ...
    for (i = 0; i < accountsList.count(); i++) {
        ...
        if (i == accountsList.count() - 1) {
            ...
            currentAccountElement = _accounts_dom.documentElement().firstChildElement(
                "account");
            extractAccountDetails();
            populateBlogList();
            setNetworkActionsEnabled(true);
...
void EditingWindow::readSettings()
{
    ...
    // #ifdef Q_OS_WIN32
    //    QString _local_storage_directory = QString(qtm->isSandbox() ? "%1\\QTMSandbox"
    ...
    move(qtm_settings.value("position", QPoint(20, 20)).toPoint());
    {
        ...
        line_list = (qtm_settings.value("splitter_sizelist", "100,100"))
                        .toString()
                        .split(",");
        for (int i = 0; i < line_list.size(); ++i)
            list.append(line_list.at(i).toInt());
    ...
    //    QString("%1/sandbox").arg(qtm_settings.value("localStorageDirectory",
    //    _local_storage_directory).toString()) :
    //    qtm_settings.value("localStorageDirectory",
    //    _local_storage_directory).toString();
    ...
    _local_storage_file_extension = qtm_settings.value("localStorageFileExtn", "cqt").toString();
...
void EditingWindow::handleEnableCategories()
{
    ...
    if (_super_menu)
        _super_menu->setCategoriesEnabled(categoriesEnabled);
}
...
#else
        if (_current_http_business != _mt_getCategoryList)
...
QString EditingWindow::processWithMarkdown(const QString& text)
{
    ...
    // Now wait until the process finishes; use a loop and process events every 10th of
    // a second
    for (int i = 0; i <= 300; ++i) {
    ...
    if (conversionStringB.length() < conversionString.length() || proc.exitStatus() != QProcess::NormalExit) {
...
void EditingWindow::getAccounts(const QString& title)
{
    ...
    for (i = 0; i < accountsList.count(); i++) {
        ...
        if (thisAccountsAttribStrings.contains("allowComments") || thisAccountsAttribStrings.contains("comments"))
            acct.allowComments = true;
        if (thisAccountsAttribStrings.contains("allowTB") || thisAccountsAttribStrings.contains("trackback"))
            acct.allowTB = true;
    ...
    if (acctsDialog.exec()) {
        ...
        for (i = 0; i < returnedAccountsList.count(); i++) {
            ...
            hbtElement.appendChild(
                newAccountsDom.createTextNode(QString::number(returnedAccountsList.at(i).hostedBlogType)));
            ...
            serverElement.appendChild(
                newAccountsDom.createTextNode(thisUrl.host()));
            ...
            if (returnedAccountsList.at(i).categoriesEnabled || returnedAccountsList.at(i).postDateTime || returnedAccountsList.at(i).allowComments || returnedAccountsList.at(i).allowTB)
...
void EditingWindow::getPreferences(const QString& title)
{
    ...
    if (_local_storage_directory.isEmpty()) {
        ...
        prefs_dialog.leLocalDir->setText(_local_storage_directory);
    ...
    prefs_dialog.chSuppressReadMore->setCheckState(suppressReadMore ? Qt::Checked : Qt::Unchecked);
    prefs_dialog.chUseUtf8->setCheckState(useUtf8 ? Qt::Checked : Qt::Unchecked);

    prefs_dialog.chAllowRegexSearch->setCheckState(allowRegexSearch ? Qt::Checked : Qt::Unchecked);
    ...
    prefs_dialog.resize(QSize(prefs_dialog.width(), prefs_dialog.minimumHeight()));
    if (!title.isEmpty())
        prefs_dialog.setWindowTitle(title);
...
#endif
        _local_storage_directory = PROPERSEPS(prefs_dialog.leLocalDir->text());
...
// qDebug( "setting double click function" );
        ...
        if (_local_storage_directory.isEmpty()) {
            ...
            localStorageDirectory = QString(qtm->isSandbox() ? "%1\\QTMsandbox" : "%1\\QTM blog")
                                        .arg(globalparameters.root_path() + "/" + globalparameters.target_os()); // QDir::homePath()
#else
            _local_storage_directory = QString(_app->isSandbox() ? "%1/qtm-sandbox" : "%1/qtm-blog")
                                           .arg(globalparameters.root_path() + "/" + globalparameters.target_os()); // QDir::homePath()
#endif
        }
        ...
        if (!qd.exists()) {
            addToConsole(
                tr("Making directory %1").arg(_local_storage_directory));
            if (!qd.mkpath(_local_storage_directory))
                _status_widget->showMessage(
                    tr("Could not create QTM directory."), 2000);
            if (!noAutoSave)
                saveAccountsDom();
        }
        /*setDirtySignals( false );
        ...
        qtm_settings.setValue("application/version", app_version); // QTM_VERSION
        qtm_settings.beginGroup("account");
        qtm_settings.setValue(
            "localStorageDirectory", _local_storage_directory.replace("~", QDir::homePath()));
        qtm_settings.setValue("localStorageFileExtn", _local_storage_file_extension);
...
void EditingWindow::setHighlighting(bool hl)
{
    ...
    // connect( editor_object->document(), SIGNAL( contentsChanged() ), this, SLOT(
    // dirtify() ) );
}
...
void EditingWindow::saveAccountsDom()
{
    ...
    if (domOut.open(QIODevice::WriteOnly)) {
        ...
        if (!dir.exists()) {
            if (QMessageBox::question(
                    0, tr("Cannot find storage directory"), tr((program_title_string + " cannot find the directory you specified to "
                ...
                if (dir.mkpath(_local_storage_directory)) {
                    ...
                    if (domOut.open(QIODevice::WriteOnly)) {
                        ...
                        _status_widget->showMessage(
                            tr("Could not write to accounts "
                               "file."),
                            2000);
                } else
                    _status_widget->showMessage(
                        tr("Could not create the directory."), 2000);
            }
        }
    }
}
...
void EditingWindow::populateAccountList()
{ // slot
    int i;
    QDomElement ct, detail;
    QString cid, cname;

    QDomNodeList accountNodeList = _accounts_dom.documentElement().elementsByTagName("account");
    ...
    if (a) {
        ...
        for (i = 0; i < a; i++) {
            ...
            if (!detail.isNull()) {
                ...
                _control_tab->cbAccountSelector->addItem(
                    decodeXmlEntities(cname), cid);
            }
        }
    }
}

void EditingWindow::populateBlogList()
{ // slot
    ...
    if (a) {
        ...
        for (i = 0; i < a; i++) {
            ...
            _control_tab->cbBlogSelector->addItem(
                decodeXmlEntities(blogNodeList.at(i).firstChildElement("blogName").text()),
        ...
        if (!catsElement.isNull()) {
            ...
            if (b) {
                ...
                if (uncategorized)
                    _control_tab->cbMainCat->setCurrentIndex(
                        uncategorized);
            } else {
...
void EditingWindow::populateTagList()
{ // slot
    if (useWordpressAPI) {
...
void EditingWindow::refreshBlogList()
{ // slot
    QDomDocument doc;
    QDomElement methodCall = doc.createElement("methodCall");
...
void EditingWindow::handleSslErrors(const QList<QSslError>& errorList)
{
    ...
    if (i)
        _status_widget->showMessage(tr("%1 SSL errors").arg(i), 2000);
}
#endif

void EditingWindow::changeCurrentBlog(int b)
{ // slot
    currentBlog = b;
}

void EditingWindow::changeAccount(int a)
{ // slot
    ...
    if (currentAccount != a) {
        ...
        if (!blogsElement.isNull()) {
            ...
            if (b) {
                ...
                changeBlog(0, true);
                currentBlogid = _control_tab->cbBlogSelector->itemData(0)
                                    .toString();
...
void EditingWindow::extractAccountDetails()
{ // slot
...
void EditingWindow::extractAccountAttributes()
{
    ...
    if (_super_menu)
        _super_menu->addCatAction->setEnabled(useWordpressAPI);
}

void EditingWindow::changeBlog(int b, bool fromChangeAccount)
{ // slot
                ...
                for (i = 0; i < c; i++) {
                    currentCategoryText = decodeXmlEntities(
                        catsList.at(i)
                            .firstChildElement("categoryName")
                            .text());
                    currentCatId = catsList.at(i)
                                       .firstChildElement("categoryId")
                                       .text();
                    _control_tab->cbMainCat->addItem(
                        currentCategoryText, QVariant(currentCatId));
...
{
    editor_object->insertPlainText(
        QString("<em>%1</em>").arg(editor_object->textCursor().selectedText()));
}

void EditingWindow::makeUnderline()
{
    editor_object->insertPlainText(
        QString("<u>%1</u>").arg(editor_object->textCursor().selectedText()));
}

void EditingWindow::insertMore()
{
    if (!editor_object->toPlainText().contains("<!--more-->"))
        editor_object->insertPlainText("<!--more-->");
    else
...
{
    editor_object->insertPlainText(
        QString("<blockquote>%1</blockquote>")
            .arg(editor_object->textCursor().selectedText()));
}
...
{
    editor_object->insertPlainText(
        QString("<p>%1</p>").arg(editor_object->textCursor().selectedText()));
}
...
void EditingWindow::insertSelfLink()
{
    ...
    if (QUrl(linkString, QUrl::StrictMode).isValid())
        editor_object->insertPlainText(
            QString("<a href=\"%1\">%1</a>").arg(linkString));
    else
...
void EditingWindow::insertImage()
{
    ...
    if (image_entry.exec()) {
        ...
        if (ieui.sbMarginLeft->value() != 0)
            styleString += QString("margin-left: %1px; ")
                               .arg(ieui.sbMarginLeft->value());
        if (ieui.sbMarginRight->value() != 0)
            styleString += QString("margin-right: %1px; ")
                               .arg(ieui.sbMarginRight->value());
        if (ieui.sbMarginTop->value() != 0)
            styleString += QString("margin-top: %1px; ")
                               .arg(ieui.sbMarginTop->value());
        if (ieui.sbMarginBottom->value() != 0)
            styleString += QString("margin-bottom: %1px")
                               .arg(ieui.sbMarginBottom->value());
        // Insert style string into main insertion string
        if (!styleString.isEmpty())
            insertionString += QString(" style=\"%1\"").arg(styleString.trimmed());
        insertionString += " />";
...
void EditingWindow::makeUnorderedList()
{
    ...
    if (!listString.isEmpty())
        editor_object->insertPlainText(getHTMLList(QString("ul"), listString));
}
...
void EditingWindow::makeOrderedList()
{
    QString listString = editor_object->textCursor().selection().toPlainText();
    if (!listString.isEmpty())
        editor_object->insertPlainText(getHTMLList(QString("ol"), listString));
}
...
void EditingWindow::doPreview(bool isChecked, bool markdownFailed)
{
    ...
    if (isChecked) {
        if (useMarkdown && !markdownFailed) {
            ...
            QRegExp re(
                "^(<table|thead|tfoot|caption|tbody|tr|td|th|div|dl|dd|dt|"
                "ul|ol|li|select|form|blockquote|address|math|p|h[1-6])>");
            do {
                line = a.readLine();
                if (!line.isEmpty()) {
...
{
    if (_super_menu)
        _super_menu->setEditActionsEnabled(state);
        ...
        if (!entryBlogged) {
            ...
            if (doMarkdownWhenPosting) {
                ...
                if (!convertedString.isNull())
                    ...
                    _status_widget->showMessage(
                        tr("Markdown conversion failed; posting "
                           "main entry as is."),
                        2000);
                if (!extEntry.isEmpty()) {
                    ...
                    if (!convertedString.isNull())
                        ...
                        _status_widget->showMessage(
                            tr("Markdown conversion failed; "
                               "posting extension as is."),
                            2000);
                }
            }
            ...
            rpcstruct.appendChild(XmlMember(doc, "title", "string", _control_tab->leTitle->text()));
            ...
            rpcstruct.appendChild(XmlMember(doc, "mt_allow_pings", "boolean", takeTB ? "1" : "0"));
            rpcstruct.appendChild(
                XmlMember(doc, "mt_text_more", "string", extEntry));
            if (_control_tab->teExcerpt->toPlainText().length())
                ...
                rpcstruct.appendChild(
                    XmlMember(doc, "mt_excerpt", "string", ""));
            if (!_control_tab->lwKeywordTags->count())
                rpcstruct.appendChild(
                    XmlMember(doc, "mt_keywords", "string", ""));
            else {
                for (count = 0;
                     count < _control_tab->lwKeywordTags->count();
                     count++) {
                    keywordTagList.append(
                        _control_tab->lwKeywordTags->item(count)
                            ->text());
                    if (count != _control_tab->lwKeywordTags->count() - 1)
                        keywordTagList.append(", ");
                }
                rpcstruct.appendChild(XmlMember(doc, "mt_keywords", "string", keywordTagList));
            }
            if (_control_tab->lwTBPings->count()) {
                for (count = 0; count < _control_tab->lwTBPings->count();
                     count++)
                    tblist.append(_control_tab->lwTBPings->item(count)->text());
                rpcstruct.appendChild(
                    XmlRpcArray(doc, "mt_tb_ping_urls", tblist));
            }
            ...
            if (location.contains("mt-xmlrpc.cgi") && _control_tab->cbStatus->currentIndex() == 1)
                _control_tab->progressBar->setMaximum(
                    (categoriesEnabled && !_control_tab->chNoCats->isChecked())
                        ? 6
                        : 4);
            else
                _control_tab->progressBar->setMaximum(
            ...
            if ((useWordpressAPI || location.contains("mt-xmlrpc.cgi")) && cw->cbStatus->currentIndex() == 1)
    ...
    if (!_current_http_business) {
        ...
        if (doMarkdownWhenPosting) {
            ...
            if (!convertedString.isNull())
                ...
                _status_widget->showMessage(
                    tr("Markdown conversion failed; posting main entry "
                       "as is."),
                    2000);
        }
        dateTime = QDateTime::currentDateTime().toString(Qt::ISODate);
        ...
        if (_control_tab->chAllowComments->isChecked()) {
            ...
            for (int a = 0; a < _control_tab->lwOtherCats->count(); a++) {
                if (_control_tab->lwOtherCats->isItemSelected(
                    ...
                    catList << _control_tab->cbMainCat->itemData(a)
                                   .toString();
                }
            }
        }
        ...
        if (_control_tab->lwKeywordTags->count())
            for (count = 0; count < _control_tab->lwKeywordTags->count();
                 count++)
                tagList << _control_tab->lwKeywordTags->item(count)->text();
        ...
        rpcstruct.appendChild(XmlMember(doc, "post_title", "string", _control_tab->leTitle->text()));
        ...
        if (!_control_tab->lePassword->text().isEmpty())
        ...
        if (catList.count()) {
            ...
            taxStruct.appendChild(
                XmlRpcArray(doc, "category", "string", catList));
        ...
        if (tagList.count())
            rpcstruct.appendChild(
                XmlRpcArray(doc, "tags_input", "string", tagList));
        if (tblist.length())
    ...
    if (doMarkdownWhenPosting) {
        ...
        if (!extEntry.isEmpty()) {
            ...
            if (!convertedString.isNull())
                ...
                _status_widget->showMessage(
                    tr("Markdown conversion failed; posting extension "
                       "as is."),
                    2000);
        }
    }
...
// qDebug() << "posted prim cat";
                ...
                arrayStruct.appendChild(
                    XmlMember(doc, "isPrimary", "boolean", "1"));
                arrayValue.appendChild(arrayStruct);
                _data.appendChild(arrayValue);
                for (int a = 0; a < _control_tab->lwOtherCats->count();
                     a++) {
                    if (_control_tab->lwOtherCats->isItemSelected(
                            _control_tab->lwOtherCats->item(a))) {
                        cat = currentBlogElement
                                  .firstChildElement(
                                      "categories")
                                  .childNodes()
                                  .at(a)
                        ...
                        secCatId = _control_tab->cbMainCat->itemData(a)
                                       .toString();
                        secCatName = _control_tab->cbMainCat->itemText(
                            a);
                        arrayValue = doc.createElement("value");
                        arrayStruct = doc.createElement("struct");
                        arrayStruct.appendChild(
            ...
            if (_control_tab->progressBar->isVisible())
                _control_tab->progressBar->setValue(
                    _control_tab->progressBar->value() + 1);
#ifdef WIN_QTV
...
void EditingWindow::publishPost()
{ // slot
    QDomDocument doc;
    if (!_current_http_business) {
        QDomElement methodCall = doc.createElement("methodCall");
...
void EditingWindow::saveAs(bool exp)
{
    ...
    if (_control_tab->leTitle->text().isEmpty())
        suggestedFilename = _local_storage_directory;
    else {
    ...
    if (!fn.isEmpty()) {
        if (!_local_storage_file_extension.isEmpty())
            if (!fn.endsWith(QString(".%1").arg(_local_storage_file_extension)))
                fn.append(
                    QString(".%1").arg(_local_storage_file_extension));
        filename = fn;
        /*
...
void EditingWindow::save(const QString& fname, bool exp)
{
    ...
    if (_control_tab->lePassword->text().length())
    ...
    if (exp) {
        ...
        out << QString("AcctBlog:%1@%2 (%3)\n") // Include the blog name so it can
                   // be relayed to the user later
                   .arg(currentBlogid)
    ...
    if (!_control_tab->chNoCats->isChecked()) {
        ...
        out << "PrimaryID:none\n";
    if (_control_tab->teExcerpt->toPlainText().length() > 0)
...
void EditingWindow::choose(QString fname)
{
    ...
    if (fname.isEmpty())
        fn = QFileDialog::getOpenFileName(this, tr("Choose a file to open"), _local_storage_directory, extn);
    else
        fn = fname;
    if (!fn.isEmpty()) {
        if (!useNewWindows) {
            if (saveCheck(true)) {
                if (!load(fn))
                    _status_widget->showMessage(
                        tr("File could not be loaded."), 2000);
                else {
                    _app->add_recent_file(_control_tab->leTitle->text(), fn);
            ...
            EditingWindow* e = new EditingWindow(
                _tree_screen, _entrance, _profile, _find_screen, _editentry, Qt::Widget, _style_source);
            if (e->load(fn, true)) {
#ifdef USE_SYSTRAYICON
                e->setSTI(sti);
#endif
...
bool EditingWindow::load(const QString& fname, bool fromSTI)
{
    ...
    if (fromSTI) {
        ...
        testAccountsElement = _accounts_dom.firstChildElement(program_title_qstring + "Accounts");
        if (testAccountsElement.isNull())
            accountsElement = _accounts_dom.createElement(
                program_title_qstring + "Accounts");
        else
            accountsElement = _accounts_dom.firstChildElement(
                program_title_qstring + "Accounts");
        currentAccountElement = _accounts_dom.createElement("account");
        accountsElement.appendChild(currentAccountElement);
        if (testAccountsElement.isNull())
    ...
    if (!t.readLine().startsWith(program_title_qstring + " saved blog entry")) {
        ...
        _status_widget->showMessage(
            tr(("This is not a " + program_title_string + " blog entry.")
                    .c_str()),
            2000);
        return false;
    }
    ...
    if (emap.contains("Publish"))
        _control_tab->cbStatus->setCurrentIndex(emap.value("Publish") == "1" ? 1 : 0);
    if (emap.contains("EntryNumber")) {
        entryNumber = emap.value("EntryNumber");
        entryBlogged = true;
    }
    if (emap.contains("Comments"))
        _control_tab->chAllowComments->setChecked(
            emap.value("Comments") == "1" ? true : false);
    if (emap.contains("TB"))
        _control_tab->chAllowTB->setChecked(emap.value("TB") == "1" ? true : false);
    ...
    if (emap.contains("Tags")) {
        ...
        if (d)
            for (c = 0; c <= d; c++)
                _control_tab->lwTags->addItem(
                    tags.section(QChar(';'), c, c));
        else
    ...
    if (emap.contains("Keywords")) {
        ...
        if (d)
            for (c = 0; c <= d; c++)
                _control_tab->lwKeywordTags->addItem(
    ...
    if (emap.contains("Excerpt"))
        _control_tab->teExcerpt->setPlainText(
    ...
    if (!loadedAccountId.isNull()) {
        ...
        for (g = 0; g <= accts.count(); g++) {
            if (g == accts.count()) {
                ...
                // finding the account
                QMessageBox::information(
                    0, tr((program_title_string + " - No such account")
                               .c_str()),
                    tr((program_title_string + " could not find this account (perhaps it was "
                                               "deleted).\n\n"
                ...
                newDefaultAccount.setAttribute(
                    "id", QString("newAccount_%1")
                ...
                newNameElement.appendChild(
                    QDomText(_accounts_dom.createTextNode(tr("New blank element"))));
                newDetailElement.appendChild(newNameElement);
                newDefaultAccount.appendChild(newDetailElement);
                _accounts_dom.firstChildElement("accounts")
                    .appendChild(newDefaultAccount);
            ...
            if (accts.at(g).toElement().attribute("id") == loadedAccountId) {
                ...
                QString st;
                for (h = 0; h < _control_tab->cbAccountSelector->count();
                     h++) {
                    st = _control_tab->cbAccountSelector->itemData(h)
                             .toString();
                ...
                _control_tab->cbBlogSelector->disconnect(
                    SIGNAL(activated(int)), this, 0);
                // qDebug() << "connecting changeBlog";
                connect(_control_tab->cbBlogSelector, SIGNAL(activated(int)), this, SLOT(changeBlog(int)));
                if (!isOK) {
                    QMessageBox::information(
                        0, tr((program_title_string + " - Invalid blog")
                                   .c_str()),
                        tr("Could not get a valid blog number.  "
                           "Please set it again."),
                        QMessageBox::Ok);
                    return true;
                }
                for (hh = 0; hh < _control_tab->cbBlogSelector->count();
                     hh++) {
                ...
                if (!catsElement.isNull()) {
                    ...
                    if (b) {
                        ...
                        for (int i = 0; i < catNodeList.size();
                             i++) {
                            QString cc = catNodeList.at(i)
                                             .firstChildElement(
                                                 "categoryI"
                                                 "d")
                                             .text();
                        ...
                        _control_tab->chNoCats->setEnabled(false);
                        _control_tab->cbMainCat->setEnabled(false);
                        _control_tab->lwOtherCats->setEnabled(
                            false);
                    }
                }
                filename = fname;
    ...
    for (int e = 0; e <= accts.count(); e++) {
        ...
        // ).text();
        if (details.firstChildElement("server").text() == server && details.firstChildElement("location").text() == location && details.firstChildElement("login").text() == login) {
            // qDebug() << "match found";
...
void EditingWindow::setLoadedPostCategories()
{ // slot
    int a, b, j, z;
    int i = 0;
    QString c, cc, d;
    ...
    QDomNodeList blogNodes = currentAccountElement.firstChildElement("blogs").elementsByTagName("blog");
    if (!_control_tab->cbBlogSelector->count()) {
        j = blogNodes.size();
        for (z = 0; z < j; z++)
            _control_tab->cbBlogSelector->addItem(
                decodeXmlEntities(blogNodes.at(z).firstChildElement("blogName").text()),
...
void EditingWindow::uploadFile()
{
    ...
    if (!_current_http_business) {
        QString uploadFilename = QFileDialog::getOpenFileName(
            this, tr("Select file to upload"), QDir::homePath());
                ...
                if (inFile.open(QIODevice::ReadOnly)) {
                    ...
                    if (conversionBuffer.isEmpty())
                        ...
                        if (!fileInBase64.isEmpty()) {
                            QDomElement methodCall = doc.createElement(
                                "methodCall");
                            methodCall.appendChild(
                                XmlMethodName(doc, "metaWeblog."
                                                   "newMediaObje"
                                                   "ct"));
                            QDomElement params = doc.createElement("params");

                            QString blogid = currentBlogElement
                                                 .firstChildElement(
                                                     "blogid")
                                                 .text();
                            /*              QRegExp
                            ...
                            responseData = "";
                            placeNetworkRequest(
                                _metaWeblog_newMediaObject, requestArray);
                            if (QApplication::
                                    overrideCursor()
                                == 0)
                                QApplication::setOverrideCursor(QCursor(Qt::BusyCursor));
                            ...
                            _status_widget->showMessage(
                                tr("Upload was cancelled."), 2000);
                    }
                    inFile.close();
                } else
                    _status_widget->showMessage(
                        tr("Could not open the file."), 2000);
            } else
                _status_widget->showMessage(
                    tr("That file could not be found."), 2000);
        }
    } else
...
void EditingWindow::handleSideWidgetPageSwitch(int index)
{
    ...
    int uncategorized = 0;
    if (index == 1) {
        if (_control_tab->cbMainCat->count() == 0 && networkActionsEnabled && categoriesEnabled) {
            ...
            if (cl.count() == 0) {
                _status_widget->showMessage(
                    tr("Getting categories, please wait"), 2000);
                refreshCategories();
            } else {
                _control_tab->cbMainCat->clear();
                _control_tab->lwOtherCats->clear();
            ...
            if (newCategoryDialog.exec()) {
                if (!ncui.leName->text().isEmpty()) {
                    if (!_current_http_business) {
                        ...
                        params = doc.createElement("params");
                        params.appendChild(
                            XmlValue(doc, "string", blogid));
                        params.appendChild(
                            XmlValue(doc, "string", login));
                        params.appendChild(
                            XmlValue(doc, "string", password));
                        ...
                        rpcstruct.appendChild(XmlMember(doc, "parent_id", "int", ncui.cbParent->itemData(ncui.cbParent->currentIndex()).toString()));
                        rpcstruct.appendChild(XmlMember(doc, "description", "string", ncui.teDescription->toPlainText().simplified()));
                        ...
                        _status_widget->showMessage(
                            tr("All HTTP requests are "
                               "blocked."),
                            2000);
                } else
                    _status_widget->showMessage(
                        tr("Your category must have a name."), 2000);
            }
        }
    }
}
...
void EditingWindow::addKeywordTagFromAvailTags()
{
    if (_control_tab->lwAvailKeywordTags->count() && _control_tab->lwAvailKeywordTags->currentRow() != -1) {
        ...
        if (!matches.count())
            _control_tab->lwKeywordTags->addItem(currentItem->text());
    }
}
...
void EditingWindow::refreshKeywordTags()
{
    if (!_current_http_business) {
        if (useWordpressAPI) {
            ...
            params.appendChild(
                XmlValue(doc, "string", currentAccountElement.firstChildElement("details").firstChildElement("login").text()));
            params.appendChild(
                XmlValue(doc, "string", currentAccountElement.firstChildElement("details").firstChildElement("password").text()));
            ...
            _status_widget->showMessage(
                tr("This feature only works on Wordpress."), 2000);
    } else {
#ifdef QTM_DEBUG
        statusWidget->showMessage(
            tr("refreshCategories: All HTTP requests are blocked"), 2000);
...
{
    int l;
    QRegExpValidator tagFormat(QRegExp("^http:\\/\\/(www\\.)?technorati\\.com\\/tag\\/"
                                       "([a-zA-Z0-9\\.%]+[\\+ ])*[a-zA-Z0-9\\.%]+$"),
        this);
...
bool EditingWindow::saveCheck()
{
    ...
    activateWindow();
    int a = QMessageBox::warning(this, tr("Not saved"), tr("You have not saved this file.\n"
                                                           "Save first?"),
...
void EditingWindow::handleFind()
{
    if (_search_widget->isVisible()) {
        if (_super_menu)
            _super_menu->findAction->setText(tr("&Find ..."));
        _search_widget->close();
    } else {
        if (_super_menu)
            _super_menu->findAction->setText(tr("Exit &find"));
        _search_widget->find();
    }
}
    ...
    while (aldIterator.hasNext()) {
        ...
        if (autoLinkTitleDictionary.contains(aldIterator.key())) {
            ...
            titleText = doc.createTextNode(
                autoLinkTitleDictionary.value(aldIterator.key()));
        ...
        if (autoLinkTargetDictionary.contains(aldIterator.key())) {
            ...
            targetText = doc.createTextNode(QString::number(autoLinkTargetDictionary.value(aldIterator.key())));
            ...
            for (int i = 0; i < entryList.count(); i++) {
                ...
                if (currentKeyElement.isNull() || currentURLElement.isNull())
                    ...
                    if (!currentTitleElement.isNull())
                        autoLinkTitleDictionary.insert(
                            currentKey, currentTitleElement.text());
                    if (!currentTargetElement.isNull()) {
                        currentTarget = currentTargetElement.text().toInt(
                            &ok);
                        if (ok)
                            autoLinkTargetDictionary.insert(
                                currentKey, currentTarget);
                    }
                }
            }
        }
...
{
    if (val == _control_tab->progressBar->maximum())
