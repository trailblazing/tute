...
QuickpostTemplateDialog::QuickpostTemplateDialog(QStringList &titles, QStringList &templates, QList<int> &pubStates, QList<bool> &ctStates, QList<QStringList> &ahLists, bool status, QWidget *parent)
	...
	Q_FOREACH(QString a, _templateTitles){
		if(a.isEmpty()) lwTemplates->addItem(tr("(No name)"));
		else lwTemplates->addItem(a);
	}
...
void QuickpostTemplateDialog::changeListIndex(int index){
	if(doingNewTemplate){
		if(dirty) lwTemplates->item((lwTemplates->count()) - 1)->setText(leName->text().isEmpty() ? tr("(No name)") : leName->text());
		else{
...
void QuickpostTemplateDialog::acceptTemplate(){
	if(doingNewTemplate){
		...
		lwTemplates->item(lwTemplates->count() - 1)->setText(leName->text().isEmpty() ? tr("(No name)") : leName->text());
		if(lwTemplates->count() == 1) connect(lwTemplates, SIGNAL(currentRowChanged(int)), this, SLOT(changeListIndex(int)));
		...
		if(lwTemplates->count()){
			lwTemplates->item(currentRow)->setText(leName->text().isEmpty() ? tr("(No name)") : leName->text());
	...
	if(_assocHostLists.count()){
		for(i = 0; i < _assocHostLists.count(); i++){
			for(j = 0; j < _assocHostLists[i].count(); j++)
				if(_assocHostLists[i].at(j).isEmpty() || emptyString.exactMatch(_assocHostLists[i].at(j))) _assocHostLists[i].removeAt(j);
		}
	}
	accept();
}
...
void QuickpostTemplateDialog::on_leName_textEdited(const QString &newName){
	if(currentRow != -1){
		// qDebug( "title edited" );
		lwTemplates->item(currentRow)->setText(newName.isEmpty() ? tr("(No name)") : newName);
		_templateTitles[currentRow] = newName;
	}
}

void QuickpostTemplateDialog::on_teTemplateText_textChanged(){
	if(currentRow != -1) _templateStrings[currentRow] = teTemplateText->toPlainText();
}

void QuickpostTemplateDialog::on_teAssocHosts_textChanged(){
	if(currentRow != -1) _assocHostStrings[currentRow] = teAssocHosts->toPlainText();
}

void QuickpostTemplateDialog::on_cbDefaultPublishStatus_currentIndexChanged(int newState){
	if(currentRow != -1) _defaultPublishStates[currentRow] = newState;
}

void QuickpostTemplateDialog::on_chCopyTitle_stateChanged(int){
	if(currentRow != -1) _copyTitleStates[currentRow] = chCopyTitle->isChecked();
}
...
QList<QStringList> QuickpostTemplateDialog::compileAssocHostLists(QStringList &input){
	QList<QStringList> rv;
	Q_FOREACH(QString a, input) rv.append(a.split("\n"));
	return rv;
}

QStringList QuickpostTemplateDialog::compileAssocHostStrings(QList<QStringList> &input){
	QStringList rv;
	Q_FOREACH(QStringList a, input) rv.append(a.join("\n"));
	return rv;
}
