...
void PrefsDialog::on_pbBrowse_clicked() {
	QString dir =
	    QFileDialog::getExistingDirectory(0, tr("Choose a directory"), "/home");
	if (!dir.isEmpty())
...
void PrefsDialog::on_tbEditorFgColor_clicked() {  // slot
	QPalette palette = lEditorExampleText->palette();
	QColor color =
	    QColorDialog::getColor(palette.color(QPalette::WindowText), this);
	if (color.isValid()) {
...
void PrefsDialog::setLabelText(QLabel *label, bool isBold, bool isItalic) {
	label->setText(QString("%1%2Sample text%3%4").arg(isBold ? "<strong>" : "").arg(isItalic ? "<em>" : "").arg(isItalic ? "</em>" : "").arg(isBold ? "</strong>" : ""));
}
