      ...
      _topic_editor_config(
          [&]() -> std::shared_ptr<QSettings> {
              if (!QDir(_current_topic_full_folder_name).exists())
                  if (!QDir::root().mkpath(_current_topic_full_folder_name))
                      critical_error("Can not create directory: \"" + _current_topic_full_folder_name + "\"");
              ...
              //			  }
              if ((QFile::ReadUser | QFile::WriteUser) != (QFile::permissions(_current_topic_full_config_name) & (QFile::ReadUser | QFile::WriteUser)))
                  QFile::setPermissions(_current_topic_full_config_name, QFile::ReadUser | QFile::WriteUser);
...
// qDebug() << "Can't read the XML";
    ...
    ///
    {
        ...
        if (edh == 0)
            edh = _editor_docker->sizeHint().height();
        auto edw = _editor_docker->geometry().width();
        if (edw == 0)
            edw = _editor_docker->sizeHint().width();
        auto ch = _central_widget->geometry().height();
        if (ch == 0)
            ch = _central_widget->sizeHint().height();
        auto cw = _central_widget->geometry().width();
        if (cw == 0)
            cw = _central_widget->sizeHint().width();
...
Blogger::~Blogger()
{
    ...
    //	deleteLater();
    if (_record_screen)
        connect(this, &Blogger::close_request, _record_screen, &rs_t::on_blogger_close);
    if (_browser) {
...
void Blogger::showEvent(QShowEvent* event)
{
    ...
    if (_editor->document()->isEmpty() && !_dirty_indicator->isVisible() && !event->spontaneous() && !_entry_ever_saved)
...
void Blogger::readSettings()
{
    ...
    perlPath = _topic_editor_config->value("perlPath", defaultPerlLocation).toString();
    markdownPath = _topic_editor_config->value("markdownPath", defaultMarkdownPath).toString();
    doMarkdownWhenPosting = _topic_editor_config->value("doMarkdownWhenPosting", useMarkdown).toBool();
    ...
    categoriesEnabled = _topic_editor_config->value("categoriesEnabled", true).toBool();
    ...
    enableHighlighting = _topic_editor_config->value("enableHighlighting", true).toBool();
    suppressReadMore = _topic_editor_config->value("suppressReadMore", true).toBool();
    useUtf8 = _topic_editor_config->value("useUtf8", false).toBool();
#ifdef USE_SYSTRAYICON
    copyTitle = _topic_editor_config->value("copyTitle", true).toBool();
#endif
    allowRegexSearch = _topic_editor_config->value("allowRegexSearch", false).toBool();
    _topic_editor_config->endGroup();
    _topic_editor_config->beginGroup("fonts");
    editorFontString = _topic_editor_config->value("editorFontString", "").toString();
    previewFontString = _topic_editor_config->value("previewFontString", "").toString();
    consoleFontString = _topic_editor_config->value("consoleFontString", "").toString();
    editorBgColor = QColor(_topic_editor_config->value("editorBgColor", "#FFFFFF").toString());
    editorFgColor = QColor(_topic_editor_config->value("editorFgColor", "#000000").toString());
    previewBgColor = QColor(_topic_editor_config->value("previewBgColor", "#FFFFFF").toString());
    previewFgColor = QColor(_topic_editor_config->value("previewFgColor", "#000000").toString());
    consoleBgColor = QColor(_topic_editor_config->value("consoleBgColor", "#FFFFFF").toString());
    consoleFgColor = QColor(_topic_editor_config->value("consoleFgColor", "#000000").toString());
    _topic_editor_config->endGroup();

    _topic_editor_config->beginGroup("highlighting");
    tagBgColor = QColor(_topic_editor_config->value("tagBgColor", "#FFFFFF").toString());
    tagFgColor = QColor(_topic_editor_config->value("tagFgColor", "#008000").toString());
    entityBgColor = QColor(_topic_editor_config->value("entityBgColor", "#FFFFFF").toString());
    entityFgColor = QColor(_topic_editor_config->value("entityFgColor", "#000080").toString());
    commentBgColor = QColor(_topic_editor_config->value("commentBgColor", "#FFFFFF").toString());
    commentFgColor = QColor(_topic_editor_config->value("commentFgColor", "#008080").toString());
    linkBgColor = QColor(_topic_editor_config->value("linkBgColor", "#ffffff").toString());
    linkFgColor = QColor(_topic_editor_config->value("linkFgColor", "#0000ff").toString());
    ...
    STI2ClickFunction = _topic_editor_config->value("doubleClickFunction", 0).toInt();
    _topic_editor_config->endGroup();
#endif
...
void Blogger::getAccounts(const QString& title)
{
    ...
    for (i = 0; i < accounts_list.count(); i++) {
        ...
        for (j = 0; j < accounts_attributes.count(); j++)
    ...
    if (!title.isEmpty())
        accounts_dialog.setWindowTitle(title);
...
// qDebug( "setting double click function" );
        ...
        if (!qd.exists()) {
            ...
            if (!qd.mkpath(gl_paras->editors_shared_full_path_name()))
                _status_widget->showMessage(tr("Could not create QTM directory."), 2000);
            if (!_no_auto_save)
                saveAccountsDom();
        }
        /*setDirtySignals( false );
    ...
    if (currentAccount != a) {
        ...
        if (!blogsElement.isNull()) {
            ...
            if (b) {
                ...
                _control_tab->cbBlogSelector->clear();
                for (int i = 0; i < b; i++)
                    _control_tab->cbBlogSelector->addItem(decodeXmlEntities(blogsList.at(i).firstChildElement("blogName").text()), blogsList.at(i).firstChildElement("blogid").text());
...
void Blogger::handleConsole(bool isChecked)
{
    ...
    if (_super_menu) {
        ...
        menuPreviewAction = _super_menu->getPreviewAction(); // qobject_cast<QAction *>(_super_menu->getPreviewAction());
...
void Blogger::insertAutoLink()
{
    ...
    if (autoLinkDictionary.contains(selectedTextLC)) {
        ...
        if (autoLinkTargetDictionary.contains(selectedTextLC))
            targetString = QString(" target=\"%1\"").arg(targets.value(autoLinkTargetDictionary.value(selectedTextLC)));
...
void Blogger::insertImage()
{
    ...
    if (image_entry.exec()) {
        ...
        if (ieui.leTitle->text().size())
            insertionString += QString(" title=\"%1\"").arg(ieui.leTitle->text().replace("\"", "&quot;"));
        if (ieui.leAltText->text().size())
            insertionString += QString(" alt=\"%1\"").arg(ieui.leAltText->text().replace("\"", "&quot;"));
        // Alignment
        ...
        if (ieui.cbBorderStyle->currentIndex() != 0 && ieui.sbBorderWidth->value() != 0)
            styleString += QString("border: %1px %2; ").arg(ieui.sbBorderWidth->value()).arg(borderStyles[ieui.cbBorderStyle->currentIndex() - 1]);
        // Margins
...
void Blogger::makeUnorderedList()
{
    ...
    if (!listString.isEmpty())
        _editor->insertPlainText(getHTMLList(QString("ul"), listString));
}
...
void Blogger::makeOrderedList()
{
    QString listString = _editor->textCursor().selection().toPlainText();
    if (!listString.isEmpty())
        _editor->insertPlainText(getHTMLList(QString("ol"), listString));
}
        ...
        if (!entryBlogged) {
            ...
            if (doMarkdownWhenPosting) {
                ...
                if (!convertedString.isNull())
                    ...
                    _status_widget->showMessage(tr("Markdown conversion failed; posting main entry as is."), 2000);
                if (!extEntry.isEmpty()) {
                    ...
                    if (!convertedString.isNull())
                        ...
                        _status_widget->showMessage(tr("Markdown conversion failed; posting extension as is."), 2000);
                }
            }
            ...
            if (_control_tab->lwTags->count()) {
                ...
                for (count = 0; count < tags; count++)
                    techTagString.append(QString("<a href=\"http://technorati.com/tag/%1\" rel=\"tag\">%2</a>%3").arg(_control_tab->lwTags->item(count)->text().replace(' ', '+')).arg(_control_tab->lwTags->item(count)->text().replace("+", " ")).arg((count == tags - 1) ? "</p>\n\n" : ", "));
            ...
            if (location.contains("mt-xmlrpc.cgi") && _control_tab->cbStatus->currentIndex() == 1)
                _control_tab->progressBar->setMaximum((categoriesEnabled && !_control_tab->chNoCats->isChecked()) ? 6 : 4);
            else
                _control_tab->progressBar->setMaximum((categoriesEnabled && !_control_tab->chNoCats->isChecked()) ? 4 : 2);
...
// qDebug() << "posted prim cat";
                ...
                for (int a = 0; a < _control_tab->lwOtherCats->count();
                    ...
                    if (_control_tab->lwOtherCats->isItemSelected(
                        ...
                        arrayStruct.appendChild(XmlMember(doc, "categoryName", "string", secCatName));
...
{
    if (topic_ != _control_tab->topic())
        _control_tab->topic(topic_);
}
...
{
    if (title_ != _control_tab->title())
        _control_tab->title(title_);
}
            ...
            _topic_editor_config
                ...
                if (!QDir(_current_topic_full_folder_name).exists())
                    if (!QDir::root().mkpath(_current_topic_full_folder_name))
                        critical_error("Can not create directory: \"" + _current_topic_full_folder_name + "\"");
                if (!QFile(_current_topic_full_config_name).exists())
                    if (!QFile::copy(QString(":/resource/standardconfig/") + gl_paras->target_os() + "/" + ::gl_para::_editor_conf_filename, _current_topic_full_config_name))
                        critical_error(QString("Can not copy \"") + ::gl_para::_editor_conf_filename + "\""); // throw std::runtime_error("Can not copy document.ini");
                if ((QFile::ReadUser | QFile::WriteUser) != (QFile::permissions(_current_topic_full_config_name) & (QFile::ReadUser | QFile::WriteUser)))
                    QFile::setPermissions(_current_topic_full_config_name, QFile::ReadUser | QFile::WriteUser);
                return std::make_unique<QSettings>(_current_topic_full_config_name, QSettings::IniFormat);
            }();
            _local_storage_file_extension = _topic_editor_config->value("local_storage_file_ext", "cqt").toString();
            if (new_topic != _control_tab->topic())
                _control_tab->topic(new_topic);
                        ...
                        if (!QFile(_current_topic_config_name).exists()) {
                            if (QFile(original_config_name).exists()) {
                                ...
                                if (!QFile::copy(QString(":/resource/standardconfig/") + gl_paras->target_os() + "/" + ::gl_para::_editor_conf_filename, _current_topic_config_name))
                                    critical_error(QString("Can not copy default \"") + ::gl_para::_editor_conf_filename + "\""); // throw std::runtime_error("Can not copy document.ini");
                            }
                        }
                        if ((QFile::ReadUser | QFile::WriteUser) != (QFile::permissions(_current_topic_config_name) & (QFile::ReadUser | QFile::WriteUser)))
                            QFile::setPermissions(_current_topic_config_name, QFile::ReadUser | QFile::WriteUser);
        ...
        if (original_topic_folder != dest_topic_folder) {
            if (original_topic_folder == gl_paras->editors_shared_full_path_name() + "/undefined") {
                ...
                if (QDir(original_topic_folder).exists())
                    if (!QDir(original_topic_folder).removeRecursively())
                        critical_error("Can\'t remove folder \"" + original_topic_folder + "\"");
            } else {
                QDir dir;
            ...
            if (_record_screen)
                _record_screen->topic(new_topic);
        }
    }
}
    ...
    if (!_entry_ever_saved) {
        ...
        if ("" != file_name_with_full_path) {
            ...
            if (QFile::exists(file_name_with_full_path)) {
                ...
                if (backup.exists())
                    if (!QFile::remove(backup_file_name))
                        critical_error("Can\'t remove file \"" + backup_file_name + "\"");
                if (!QFile::rename(file_name_with_full_path, backup_file_name))
                    critical_error("Rename file \"" + file_name_with_full_path + "\" to \"" + backup_file_name + "\" failed");
        ...
        if (tags) {
            out << "Tags:";
            for (count = 0; count < tags; count++)
                out << QString(count ? ";%1" : "%1").arg(_control_tab->lwTags->item(count)->text().replace(' ', '+'));
            out << "\n";
        }
        ...
        if (tags) {
            out << "Keywords:";
            for (count = 0; count < tags; count++)
                out << QString(count ? ",%1" : "%1").arg(_control_tab->lwKeywordTags->item(count)->text());
            out << "\n";
        }
        if (!_control_tab->chNoCats->isChecked()) {
    ...
    if (!fn.isEmpty()) {
        if (!_local_storage_file_extension.isEmpty())
            if (!fn.endsWith(QString(".%1").arg(_local_storage_file_extension)))
                fn.append(QString(".%1").arg(_local_storage_file_extension));
        _filename = fn;
...
bool Blogger::load(const QString& fname, bool fromSTI)
{
    ...
    if (fromSTI) {
        ...
        if (testAccountsElement.isNull())
            accountsElement = _accounts_dom.createElement(program_title_qstring + "Accounts");
        else
            accountsElement = _accounts_dom.firstChildElement(program_title_qstring + "Accounts");
        currentAccountElement = _accounts_dom.createElement("account");
        accountsElement.appendChild(currentAccountElement);
        if (testAccountsElement.isNull())
            _accounts_dom.appendChild(accountsElement);
        if (testPI.isNull())
            _accounts_dom.insertBefore(_accounts_dom.createProcessingInstruction("xml", "version=\"1.0\""), _accounts_dom.firstChild());
    }
    ...
    if (emap.contains("Comments"))
        _control_tab->chAllowComments->setChecked(emap.value("Comments") == "1" ? true : false);
    if (emap.contains("TB"))
    ...
    if (emap.contains("Tags")) {
        ...
        if (d)
            for (c = 0; c <= d; c++)
                _control_tab->lwTags->addItem(tags.section(QChar(';'), c, c));
        else
    ...
    if (emap.contains("Keywords")) {
        ...
        if (d)
            for (c = 0; c <= d; c++)
                _control_tab->lwKeywordTags->addItem(keywords.section(QChar(','), c, c));
        else
    ...
    if (!loadedAccountId.isNull()) {
        ...
        for (g = 0; g <= accts.count(); g++) {
            if (g == accts.count()) {
                ...
                _control_tab->cbAccountSelector->addItem(tr("New blank element"), newDefaultAccount.attribute("id"));
            ...
            if (accts.at(g).toElement().attribute("id") == loadedAccountId) {
                ...
                for (h = 0; h < _control_tab->cbAccountSelector->count();
                    ...
                    if (st == loadedAccountId)
                        _control_tab->cbAccountSelector->setCurrentIndex(h);
                }
                ...
                connect(_control_tab->cbBlogSelector, SIGNAL(activated(int)), this, SLOT(changeBlog(int)));
                if (!isOK) {
                    QMessageBox::information(0, tr((program_title_string + " - Invalid blog").c_str()), tr("Could not get a valid blog number. Please set it again."), QMessageBox::Ok);
                    return true;
                }
                        ...
                        for (int i = 0; i < catNodeList.size(); i++) {
                            ...
                            if (cc == QString::number(primaryCat)) {
                                ...
                                _control_tab->lwOtherCats
                                    ->setItemSelected(_control_tab->lwOtherCats->item(i), true);
                        }
                    } else {
                        _control_tab->chNoCats->setEnabled(false);
                        _control_tab->cbMainCat->setEnabled(false);
            ...
            if (currentBlog > blogs.count()) {
                ...
                if (blogs.at(currentBlog)
                    ...
                    for (hh = 0; hh < blogs.count(); hh++)
                        _control_tab->cbBlogSelector->addItem(decodeXmlEntities(blogs.at(hh).firstChildElement("blogName").text()), QVariant(blogs.at(hh).firstChildElement("blogid").text()));
    ...
    newTitle.appendChild(QDomText(_accounts_dom.createTextNode(tr("New un-named account"))));
...
{ // slot
    ...
    if (!_control_tab->cbBlogSelector->count()) {
        j = blogNodes.size();
        for (z = 0; z < j; z++)
            _control_tab->cbBlogSelector->addItem(decodeXmlEntities(blogNodes.at(z).firstChildElement("blogName").text()), QVariant(blogNodes.at(z).firstChildElement("blogName").text()));
    ...
    if (b) {
        ...
        _control_tab->lwOtherCats->clear();
        for (j = 0; j < b; j++) {
            _control_tab->cbMainCat->addItem(catNodeList.at(j).firstChildElement("categoryName").text(), QVariant(catNodeList.at(j).firstChildElement("categoryId").text()));
    ...
    if (_no_alpha_cats) {
        ...
        do {
            ...
            a = c.toInt(&isOK);
            if (isOK)
                _control_tab->lwOtherCats->setItemSelected(_control_tab->lwOtherCats->item(a), true);
            else
                break;
            i++;
        } while (c != f);
        ...
        for (i = 0; i < catNodes.size(); i++) {
            ...
            if (cc == QString::number(primaryCat)) {
                ...
                _control_tab->lwOtherCats->setItemSelected(_control_tab->lwOtherCats->item(i), true);
        }
        initialChangeBlog = false;
    }
}
    ...
    if (useWordpressAPI) {
        ...
        if (!catsElement.isNull()) {
            ...
            if (b) {
                ...
                for (j = 0; j < b; j++)
                    ncui.cbParent->addItem(catNodeList.at(j).firstChildElement("categoryName").text(), QVariant(catNodeList.at(j).firstChildElement("categoryId").text()));
            ...
            if (newCategoryDialog.exec()) {
                if (!ncui.leName->text().isEmpty()) {
                    if (!_current_http_business) {
                        ...
                        rpcstruct.appendChild(XmlMember(doc, "name", "string", ncui.leName->text()));
...
void Blogger::addKeywordTagFromAvailTags()
{
    if (_control_tab->lwAvailKeywordTags->count() && _control_tab->lwAvailKeywordTags->currentRow() != -1) {
        ...
        QList<QListWidgetItem*> matches = _control_tab->lwKeywordTags->findItems(currentItem->text(), Qt::MatchFixedString | Qt::MatchCaseSensitive);
        if (!matches.count())
...
void EditingWindow::author(const QString& author_) { _editor->author(author_); }

void EditingWindow::home(const QString& home_) { _editor->home(home_); }

void EditingWindow::url(const QString& url_) { _editor->url(url_); }

void EditingWindow::tags(const QString& tags_) { _editor->tags(tags_); }
...
void Blogger::go_walk_history(void)
{
    ...
    if (static_cast<QString>(record_id).length() > 0) {
        ...
        if (item && item != know_model_board()->root_item()) {
            ...
            // walkhistory.set_drop(false);
            TreeIndex::activate([&] { return _tree_view->source_model(); }, _tree_view->current_item(), item->field<url_type>(), std::bind(&tv_t::move, _tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), url_target.toStdString()) || url_equal(it_->field<url_type>().toStdString(), url_target.toStdString()); });
        }
        // else {
        // walkhistory.set_drop(false);
        // }
