...
SuperMenu::SuperMenu(QWidget *parent, SysTrayIcon *sti)
	...
	/*editingMenus << editMenu << insMenu << viewMenu << categoryMenu << servicesMenu
	  << helpMenu; */
    Q_FOREACH(QAction *a, editingWindowActions) a->setEnabled(false);
...
void SuperMenu::newEntry(){
    ...
    if(currentWindow) EditingWindow::positionWidget(c, currentWindow);
	// setNoStatusBar( c );
    c->show();
...
void SuperMenu::setRecentFiles(const QList<sapp_t::recentFile> &rfs){
    recentFiles = rfs;
    updateRecentFileMenu();
}
...
void SuperMenu::setMainWindow(EditingWindow *ew){
    ...
    Q_FOREACH(QAction *a, editingWindowActions) disconnect(a, SIGNAL(triggered()), 0, 0);
    if(! currentWindow)
		Q_FOREACH(QAction *b, editingWindowActions) b->setEnabled(false);
    else{
	/*Q_FOREACH( QMenu *d, editingMenus ) {
	  d->setEnabled( true );
	} */
	Q_FOREACH(QAction *c, editingWindowActions) c->setEnabled(true);
...
void SuperMenu::handleConsole(bool isChecked){
    if(currentWindow) currentWindow->handleConsole(isChecked);
}
