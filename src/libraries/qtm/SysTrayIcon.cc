...
// #if QT_VERSION <= 0x050000   // using Qt 5 on a Mac
    ...
    connect(qtm, SIGNAL(recentFilesUpdated(QList<sapp_t::recentFile> ))
	   , this, SLOT(setRecentFiles(QList<sapp_t::recentFile> )));
...
void SysTrayIcon::setRecentFiles(const QList<sapp_t::recentFile> &rfs){
    recentFiles = rfs;
    updateRecentFileMenu();
}
...
void SysTrayIcon::newDoc(){
    ...
    if(activeWidget) EditingWindow::positionWidget(c, activeWidget);
