...
//    _sti	= sti;
	...
	connect(_app, SIGNAL(recent_files_updated(QList<sapp_t::RecentFile> ))
	       , this, SLOT(set_recent_files(QList<sapp_t::RecentFile> )));
...
void SuperMenu::choose(QString fname){
	...
	if(! fn.isEmpty()){
		...
		if(! e->load(fn, true)){
			QMessageBox::warning(0, program_title_qstring
...
*/
		...
		if(localStorageDirectory.isEmpty()) localStorageDirectory = QString("%1/qtm-blog").arg(QDir::homePath());
...
void SuperMenu::setConsoleActionTitle(bool consoleShown){
	...
	if(showConsoleAction->isChecked() != consoleShown) showConsoleAction->setChecked(consoleShown);
}
...
void SuperMenu::setPreviewActionTitle(bool previewShown){
	...
	if(previewAction->isChecked() != previewShown) previewAction->setChecked(previewShown);
}
