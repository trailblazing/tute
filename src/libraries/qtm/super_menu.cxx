...
SuperMenu::SuperMenu(FindScreen* _find_screen, Editentry* editentry, QWidget* parent, SysTrayIcon* sti)
    : super(parent)
    , _sti(sti)
    ...
    , _editentry(editentry)
{
    ...
    _editing_win_actions << cutAction << copyAction << pasteAction << tidyPasteAction
...
{
    EditingWindow* c = new EditingWindow(_tree_screen, _entrance, _profile, _find_screen, _editentry, Qt::Widget, _style_source);
    c->setSTI(_sti);
    c->setWindowTitle(QObject::tr((program_title_string + " - new entry [*]").c_str()));
    if (_editing_win)
        EditingWindow::positionWidget(c, _editing_win);
    // setNoStatusBar( c );
    c->show();
...
void SuperMenu::choose(QString fname)
{
    ...
    QString extn(QString("%1 (*.%2)").arg(tr("Blog entries")).arg(localStorageFileExtn));
    if (fname.isEmpty()) {
...
void SuperMenu::about()
{
    ...
    QString about_string = abui.label->text().replace("(VERSION)", app_version) // QTM_VERSION
...
void SuperMenu::getPreferences()
{
    ...
    bool categoriesEnabled, useNewWindows, savePassword, postAsSave, allowComments, allowTB, copyTitle, allowRegexSearch, useMarkdown, doMarkdownWhenPosting, stripParaTags, suppressReadMore, useUtf8;
...
*/
        ...
        settings.beginGroup("account");
        settings.setValue("localStorageDirectory", localStorageDirectory.replace("~", QDir::homePath()));
...
void SuperMenu::editing_win(EditingWindow* editing_win_)
{
    _editing_win = 0;
    ...
    if (!_editing_win)
        Q_FOREACH (QAction* b, _editing_win_actions)
            b->setEnabled(false);
    else {
        /*Q_FOREACH( QMenu *d, editingMenus ) {
		  d->setEnabled( true );
		} */
        Q_FOREACH (QAction* c, _editing_win_actions)
            c->setEnabled(true);
        ...
        connect(closeAction, &QAction::triggered, [&](bool) { globalparameters.edit_entry()->editor_switch(); });
        // SLOT(close()), temporary polic, current I only have one currentWindow

        // Assign edit menu actions
        connect(cutAction, SIGNAL(triggered(bool)), _editing_win, SLOT(cut()));
        connect(copyAction, SIGNAL(triggered(bool)), _editing_win, SLOT(copy()));
        CON_TRIG(copyULAction, SLOT(copyURL()));
        connect(pasteAction, SIGNAL(triggered(bool)), _editing_win, SLOT(paste()));
        connect(tidyPasteAction, SIGNAL(triggered(bool)), _editing_win, SLOT(tidyPaste()));

        //

        // Сигналы контекстного меню
        connect(this, &EditorContextMenu::undo, static_cast<Editor*>(_editing_win->editor()), &Editor::on_undo);
        connect(this, &EditorContextMenu::redo, _editing_win->editor(), &Editor::on_redo);
        connect(this, &EditorContextMenu::cut, _editing_win->editor(), &Editor::cut);
        connect(this, &EditorContextMenu::copy, _editing_win->editor(), &Editor::copy);
        connect(this, &EditorContextMenu::paste, _editing_win->editor(), &Editor::paste);
        connect(this, &EditorContextMenu::selectAll, _editing_win->editor(), &Editor::on_selectAll);
        connect(this, &EditorContextMenu::contextMenuEditImageProperties, _editing_win->editor(), &Editor::on_context_menu_edit_image_properties);

        //

        connect(undoAction, SIGNAL(triggered(bool)), _editing_win, SLOT(undo()));
        connect(redoAction, SIGNAL(triggered(bool)), _editing_win, SLOT(redo()));
...
{
    if (_editing_win)
        _editing_win->handleConsole(isChecked);
}
