...
SysTrayIcon::SysTrayIcon(ts_t *tree_screen
	...
	setIcon(QIcon(":/resource/pic/logo.svg"));// (QIcon(QPixmap(sysTrayIcon_xpm)));
#endif
	...
	if(isUnityThere && !args.contains("--no-new-window")) newWindow = true;
	menu = new QMenu;
	...
	if(!isUnityThere) menu->addAction(newWindowAtStartup);
#endif
...
void SysTrayIcon::updateRecentFileMenu(){
	...
	while(i.hasNext())
		if(!QFile::exists(i.next().filename)) i.remove();
	recentFilesMenu->clear();
	for(j = 0; j < 10; ++j){
		if(j < recentFiles.count()){
			t = recentFiles.value(j).title.section(' ', 0, 5);
			if(t != recentFiles.value(j).title) t.append(tr(" ..."));
#ifdef Q_OS_MAC
			...
			if(!recentFiles.count()) recentFilesMenu->addAction(noRecentFilesAction);
		}
	}
}
...
void SysTrayIcon::choose(QString fname){
	...
	settings.beginGroup("account");
	local_storage_file_ext	= settings.value("local_storage_file_ext", "cqt").toString();
	localStorageDirectory	= settings.value("editors_shared_directory", "").toString();
	settings.endGroup();

	QString extn(QString("%1 (*.%2)").arg(tr("Blog entries")).arg(local_storage_file_ext));
...
#ifdef Q_WS_X11
	if(ar == QSystemTrayIcon::MiddleClick) quickpost(QClipboard::Selection);
#endif
#endif
}
						...
						for(i = 0; i < assocHostLists.count(); ++i){
							if(assocHostLists.at(i).count()){
								for(j = 0; j < assocHostLists.at(i).count(); j++){
									if((!assocHostLists.at(i).at(j).isEmpty()) && cbtext.contains(QRegExp(QString("[/\\.]%1[/\\.]").arg(assocHostLists.at(i).at(j))))){
										qpt = true;
										quickpostFromTemplate(i, quickpostTemplateActions.value(i)->postTemplate(), cbtext);
										break;
									}
								}
							}
...
void SysTrayIcon::quickpostFromTemplate(int id, QString templateString, QString t){
	(void) templateString;
	activeTemplate = id;
	if(t.isNull()) cbtext = QApplication::clipboard()->text(QClipboard::Clipboard);
	else cbtext = t;
	if(cbtext.startsWith("https://")){
#ifndef Q_OS_MAC
		showMessage(tr("Error"), tr("This version of QTM does not support HTTPS."));
	...
	if(!QUrl(cbtext, QUrl::StrictMode).isValid()){
#ifdef Q_OS_MAC
		QMessageBox::warning(0, tr("No selection!"), tr("The selection is not a web location, or is invalid."), QMessageBox::Cancel);
#else
...
void SysTrayIcon::quickpostFromDBus(QString &url, QString &content){
	...
	for(i = 0; i < assocHostLists.count(); ++i){
		if(assocHostLists.at(i).count()){
			for(j = 0; j < assocHostLists.at(i).count(); j++){
				if((!assocHostLists.at(i).at(j).isEmpty()) && url.contains(QRegExp(QString("[/\\.]%1[/\\.]").arg(assocHostLists.at(i).at(j))))){
					activeTemplate = i;
					break;
				}
			}
...
void SysTrayIcon::chooseQuickpostTemplate(){
	...
	connect(ldui.lwFileTemplateList, SIGNAL(itemDoubleClicked(QListWidgetItem *)), &chooser, SLOT(reject()));
	if(chooser.exec())
		if(quickpostTemplateActions.at(ldui.lwFileTemplateList->currentRow()) != 0) quickpostTemplateActions[ldui.lwFileTemplateList->currentRow()]->trigger();
#endif
}
...
void SysTrayIcon::setupQuickpostTemplates(){
	...
	QSettings settings;
	settings.beginGroup("account");
	templateFile = QString("%1/qptemplates.xml").arg(settings.value("editors_shared_directory", "").toString());
	settings.endGroup();
			...
			if(tnc){
				for(int i = 0; i < templateNodes.count(); i++){
					...
					if(!currentAssocHostListsElement.isNull()){
						...
						for(j = 0; j < assocHostsInTemplate.count(); j++) assocHostLists[i].append(assocHostsInTemplate.at(j).toElement().text());
					}
					currentCopyStatusElement = templateNodes.at(i).firstChildElement("copyTitleStatus");
					if(currentCopyStatusElement.isNull()) copyTitleStatusList.append(false);
					else{
						currentCopyStatusElementText = currentCopyStatusElement.text();
						if(currentCopyStatusElementText.startsWith("1")) copyTitleStatusList.append(true);
						else copyTitleStatusList.append(false);
					}
				}
			}
			...
			if(titles.size()){
				...
				for(t = 0; t < numTemplates; t++){
					...
					currentDefaultPublishStatus = useDefaultPublishStatus
								      ? defaultPublishStates.item(t).toElement().text().toInt(&ok)
								      : 2;
					...
					}else defaultPublishStatusList.append(2);
...
void SysTrayIcon::configureQuickpostTemplates(QWidget *parent){
	...
	QuickpostTemplateDialog templateDialog(templateTitleList, templateList, defaultPublishStatusList, copyTitleStatusList, assocHostLists, _copyTitle, parent);
	...
	if(templateDialog.exec()){
		...
		templateDocument.insertBefore(templateDocument.createProcessingInstruction("xml", "version=\"1.0\""), templateDocument.firstChild());
		settings.beginGroup("account");
#if defined Q_OS_WIN32
		QString templateFileDir = QDir::toNativeSeparators(settings.value("editors_shared_directory", QString("%1/QTM blog").arg(QDir::homePath())).toString());
#else
		QString templateFileDir = settings.value("editors_shared_directory", QString("%1/qtm-blog").arg(QDir::homePath())).toString();
#endif
		QDir tfd(templateFileDir);
		if(!tfd.exists()) tfd.mkpath(templateFileDir);
	...
	}else configureTemplates->setEnabled(true);
}
