...
SysTrayIcon::SysTrayIcon(ts_t* tree_screen, browser::BrowserDock* browser_dock, HidableTabWidget* vtab_record, FindScreen* find_screen, EditorDock* editor_dock, wn_t* main_window, browser::Profile* profile, QString style_source, bool noWindow, Qt::WindowFlags flags, QObject* parent)
...
void SysTrayIcon::doQuit()
{
    ...
    for (a = 0; a < tlw.size(); a++)
        if ((QString(tlw[a]->metaObject()->className()) == "EditingWindow") && tlw[a]->isVisible())
            edwins++;
    if (!edwins)
