...
SysTrayIcon::SysTrayIcon(ts_t* _tree_screen, browser::Entrance* _entrance, wn_t* _main_window, browser::Profile* _profile, Qt::WindowFlags flags, QString style_source, FindScreen* find_screen, Editentry* editentry, bool noWindow, QObject* parent)
...
// setNoStatusBar( c );
    ...
    if (editing_win_)
        EditingWindow::positionWidget(c, editing_win_);
...
void SysTrayIcon::doQP(QString receivedText)
{
    ...
    EditingWindow* c = new EditingWindow(_tree_screen, _entrance, _profile, _find_screen, _editentry, Qt::Widget, _style_source, newPost);
    c->setSTI(this);
    c->setPostClean();
    if (activeTemplate >= 0) {
...
void SysTrayIcon::setupQuickpostTemplates()
{
    ...
    QSettings settings;
    settings.beginGroup("account");
    templateFile = QString("%1/qptemplates.xml").arg(settings.value("localStorageDirectory", "").toString());
    settings.endGroup();
...
void SysTrayIcon::configureQuickpostTemplates(QWidget* parent)
{
    ...
    if (templateDialog.exec()) {
        ...
        QString templateFileDir = QDir::toNativeSeparators(settings.value("localStorageDirectory", QString("%1/QTM blog").arg(QDir::homePath())).toString());
#else
        QString templateFileDir = settings.value("localStorageDirectory", QString("%1/qtm-blog").arg(QDir::homePath())).toString();
#endif
