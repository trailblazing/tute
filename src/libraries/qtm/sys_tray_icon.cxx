...
SysTrayIcon::SysTrayIcon(ts_t *_tree_screen
	...
	QCoreApplication::setApplicationVersion(version.toStdString().c_str());
	userAgentString = (program_title_string + "/").c_str();
	userAgentString.append(version.toStdString().c_str());
...
void SysTrayIcon::choose(QString fname){
	...
	if(fname.isEmpty()) fn = QFileDialog::getOpenFileName(0, tr("Choose a file to open")
	...
	if(! fn.isEmpty()){
		...
		if(! e->load(fn, true)){
#ifdef Q_OS_MAC
			QMessageBox::warning(0, program_title_string.c_str()
