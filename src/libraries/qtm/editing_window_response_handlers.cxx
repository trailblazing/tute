...
void Blogger::blogger_getUsersBlogs(QByteArray response){
	...
	if(!fstring.isEmpty()){
		...
		if(!i) statusBar()->showMessage(tr("No blogs found"), 2000);
			...
			for(int a = 0; a < i; a++) _control_tab->cbBlogSelector->addItem(blogNodeList.at(a).firstChildElement("blogName").text(), QVariant(blogNodeList.at(a).firstChildElement("blogid").text()));
			...
			if(loadedEntryBlog != 999) connect(this, SIGNAL(httpBusinessFinished()), this, SLOT(doInitialChangeBlog()));
		}
	}
...
// qDebug() << "posted the piece";
	...
	if(response.contains("<fault>")){
		...
		entryNumber = parsedData.section("<string>", 1, 1)
		              .section("</string>", -2, -2);
		if(!useWordpressAPI)
			if(!_control_tab->chNoCats->isChecked()) connect(this, SIGNAL(httpBusinessFinished()), this, SLOT(setPostCategories()));
...
void Blogger::metaWeblog_editPost(QByteArray response){
	...
	if(response.contains("<fault>")){
		...
		if(!useWordpressAPI) connect(this, SIGNAL(httpBusinessFinished()), this, SLOT(setPostCategories()));
		if(!_entry_ever_saved){
			if(postAsSave && _clean_save){
				setWindowModified(false);
				_dirty_indicator->hide();
...
// qDebug() << "Handling RPC response";
	...
	if(!response.contains("<fault>")){
		if(response.contains("<name>url</name>")){
			...
			if(_super_menu) _super_menu->copyURLAction->setVisible(true);
...
void Blogger::mt_setPostCategories(QByteArray response){
	...
	if(rdata.contains("<fault>")){
		...
		if(_doing_new_post){
			if((location.contains("mt-xmlrpc.cgi") || useWordpressAPI) && _control_tab->cbStatus->currentIndex() == 1) connect(this, SIGNAL(httpBusinessFinished()), this, SLOT(publishPost()));
