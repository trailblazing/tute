...
void EditingWindow::blogger_getUsersBlogs(QByteArray response){
	...
	if(! i) statusBar()->showMessage(tr("No blogs found"), 2000);
	else{
		...
		if(domOut.open(QIODevice::WriteOnly)){
		    ...
		    domOut.close();
			// qDebug() << "saved";
		}else statusBar()->showMessage(tr("Could not write to accounts file (error %1)").arg((int) domOut.error()), 2000);
	    }
	    for(int a = 0; a < i; a ++){
		_control_tab->cbBlogSelector->addItem(blogNodeList.at(a).firstChildElement("blogName").text()
	    ...
	    if(loadedEntryBlog != 999) connect(this, SIGNAL(httpBusinessFinished())
...
void EditingWindow::metaWeblog_newPost(QByteArray response){
	...
	entryNumber = parsedData.section("<string>", 1, 1)
	    .section("</string>", - 2, - 2);
	if(! useWordpressAPI){
	    if(! _control_tab->chNoCats->isChecked()) connect(this, SIGNAL(httpBusinessFinished())
...
void EditingWindow::metaWeblog_editPost(QByteArray response){
	...
	if(! useWordpressAPI) connect(this, SIGNAL(httpBusinessFinished())
				     , this, SLOT(setPostCategories()));
	if(! entryEverSaved){
	    if(postAsSave && cleanSave){
		setWindowModified(false);
...
//	    cw->cbPageSelector->setCurrentIndex(0);
	    ...
	    if(_super_menu) _super_menu->copyULAction->setVisible(true);
	    statusBar()->showMessage(tr("Your file is here: %1").arg(remoteFileLocation), 2000);
	}else statusBar()->showMessage(tr("The upload returned a fault."), 2000);
    }else statusBar()->showMessage(tr("The upload returned a fault."), 2000);
#ifndef NO_DEBUG_OUTPUT
...
void EditingWindow::mt_publishPost(QByteArray response){
    ...
    else statusBar()->showMessage(tr("The post was published successfully."), 2000);
    if(QApplication::overrideCursor() != 0) QApplication::restoreOverrideCursor();
}
...
void EditingWindow::mt_getCategoryList(QByteArray response){
    ...
    xis.setData(response);
    reader.parse(&xis);
    while(! handler.isMethodResponseFinished()) QCoreApplication::processEvents();
    ...
    if(! noAlphaCats) qSort(catList.begin(), catList.end(), EditingWindow::caseInsensitiveLessThan);
	...
	if(! i){
		...
		_control_tab->cbMainCat->addItem(it->section(" ##ID:", 0, 0)
				      , QVariant(ccID));
		_control_tab->lwOtherCats->addItem(it->section(" ##ID:", 0, 0));
		if(ccID == "1") uncategorized = itval;
	    ...
	    if(currentBlogElement.firstChildElement("categories").isNull()) currentBlogElement.appendChild(newCategoriesElement);
	    else currentBlogElement.replaceChild(newCategoriesElement, currentBlogElement.firstChildElement("categories"));
...
void EditingWindow::mt_setPostCategories(QByteArray response){
	...
	if(_doing_new_post){
	    if((location.contains("mt-xmlrpc.cgi") || useWordpressAPI) && _control_tab->cbStatus->currentIndex() == 1) connect(this, SIGNAL(httpBusinessFinished())
...
void EditingWindow::wp_getTags(QByteArray response){
	...
	addToConsole(accountsDom.toString(2));
    }else statusBar()->showMessage(tr("This blog has no tags."), 2000);
    addToConsole(QString(response));
    if(QApplication::overrideCursor() != 0) QApplication::restoreOverrideCursor();
}
...
void EditingWindow::wp_newCategory(QByteArray response){
    ...
    }else statusBar()->showMessage(tr("The request caused a fault; see console."), 2000);
