...
void EditingWindow::metaWeblog_newPost(QByteArray response){
	...
	entryNumber = parsedData.section("<string>", 1, 1)
	    .section("</string>", - 2, - 2);
	if(! useWordpressAPI){
	    if(! cw.chNoCats->isChecked()) connect(this, SIGNAL(httpBusinessFinished())
...
void EditingWindow::metaWeblog_newMediaObject(QByteArray response){
	...
	if(response.contains("<name>url</name>")){
	    ...
	    if(superMenu) superMenu->copyULAction->setVisible(true);
...
void EditingWindow::mt_setPostCategories(QByteArray response){
	...
	if(doingNewPost){
	    if((location.contains("mt-xmlrpc.cgi") || useWordpressAPI) && cw.cbStatus->currentIndex() == 1) connect(this, SIGNAL(httpBusinessFinished())
