...
SideTabWidget::SideTabWidget(ts_t* _tree_screen, FindScreen* _find_screen, Editentry* _editentry, browser::Entrance* _entrance, QMainWindow* _main_window, browser::Profile* _profile, QString style_source_, QSplitter* splitter, QWidget* parent)
    : HidableTabWidget(_tree_screen, _find_screen, _editentry, _entrance, _main_window, _profile, style_source_, splitter, std::make_shared<QSettings>(globalparameters.root_path() + "/" + globalparameters.target_os() + "/" + gl_para::_editor_conf_filename, QSettings::IniFormat), "geometry", "splitter_sizelist", "collapsed", parent)
{
