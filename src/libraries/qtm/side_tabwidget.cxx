...
SideTabWidget::SideTabWidget(ts_t* _tree_screen, FindScreen* _find_screen, browser::BrowserDock* _browser_dock, EditingWindow* editing_window, browser::Profile* _profile, QString style_source_, QSplitter* splitter, QWidget* parent)
    : HidableTabWidget(_tree_screen, _find_screen, _browser_dock, editing_window, _profile, style_source_, splitter, std::make_shared<QSettings>(gl_paras->root_path() + "/" + gl_paras->target_os() + "/" + gl_para::_editor_conf_filename, QSettings::IniFormat), "geometry", "splitter_sizelist", "collapsed", parent)
