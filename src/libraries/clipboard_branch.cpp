...
QList<boost::intrusive_ptr<i_t> > ClipboardBranch::records_by_parent_id(QString id) const
{
    ...
    for (auto dom : dom_list) {
        boost::intrusive_ptr<i_t> it(i_t::dangle_instance()); // new TreeItem(nullptr)
    ...
    QList<boost::intrusive_ptr<i_t> > current_items)
{
    ...
    if (current_items.size() > 0) {
	    ...
	    if (_current_parent->field<crypt_type>() == crypt_value(true)) // boost::mpl::c_str < crypt_type > ::value //_current_item_absolute_path
		    ...
		    item->child_direct(i)->field<crypt_type>() == crypt_value(true)) // boost::mpl::c_str < crypt_type > ::value
                    encrypt_presence = true;
                else
                    nocrypt_presence = true;
            }
...
void ClipboardBranch::branch_push(const QString& _clip_parent_id, boost::intrusive_ptr<i_t> current_item)
{ // QStringList path
    // Добавление ветки
    ...
    // Добавление конечной записи
    auto record_duplicate_to_parent_id = [&](const QString& _clip_parent_id, boost::intrusive_ptr<i_t> record) -> void {
        // todo: Сделать проверку, есть ли ветка с указанным id
