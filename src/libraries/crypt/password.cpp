...
bool Password:: retrievePassword(){
	...
	if(databaseconfig.get_crypt_mode() == 0){
	    ...
	    int			i = enterPwd.exec();
	    if(i == QDialog::Rejected)return false;	// Была нажата отмена, ничего ненужно делать
		...
		if(checkMiddleHash() == true){
		    ...
		    return true;
		}else appconfig.password_middle_hash("");	// хранимый пароль сбрасывается
	    }
		// Запрос пароля у пользователя
	    bool passwordEnterResult = enterExistsPassword();
	    if(passwordEnterResult == true)return true;
	    else return false;
...
bool Password:: replacePassword(void){
    ...
    if(databaseconfig.get_crypt_mode() == 0)return false;
	// Запрашивается пароль
...
bool Password:: checkMiddleHash(void){
    ...
    if(line == SAVED_PASSWORD_CHECKING_LINE)return true;
    else{
...
void Password:: saveCheckPasswordKey(QString password){
    QByteArray	hash;
    QByteArray	salt;
    for(int i = 0; i < 32; i ++)salt.append(rand() % 0xFF);
    Pbkdf2Qt hashAlgorythm;
...
bool Password:: enterExistsPassword(void){
    ...
    if(appconfig.auto_close_password_enable())enterPwd.setCancelDelay(appconfig.auto_close_password_delay());
    int i = enterPwd.exec();
    if(i == QDialog::Rejected)return false;	// Была нажата отмена, ничего ненужно делать
	...
	// еще не запомнен
	if(appconfig.password_save_flag() &&
	    appconfig.password_middle_hash().length() == 0) smartSaveMiddleHash(password);
	return true;
    }else{
...
void Password:: resetPassword(void){
    ...
    if(databaseconfig.get_crypt_mode() == 0)return;
