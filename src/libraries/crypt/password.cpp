...
// Если пароль в системе уже был установлен, проверяется, задан ли правильный
// пароль
...
bool Password::replacePassword(void) {
	...
	tkm_t *dataModel = static_cast<
	    tkm_t *>(  // find_object<TreeKnowView>(knowtreeview_singleton_name)
...
void Password::setCryptKeyToMemory(QString password) {
	...
	// Ключ в виде MD5
	QByteArray key =
	    QCryptographicHash::hash(middleHash, QCryptographicHash::Md5);
	// qDebug() << "Password::setCryptKeyToMemory() : Set crypt key to:" <<
	// key.toHex();
...
void Password::setCryptKeyToMemoryFromMiddleHash(void) {
	...
	QByteArray middleHash =
	    QByteArray::fromBase64(appconfig->password_middle_hash().toLatin1());
...
bool Password::checkMiddleHash(void) {
	...
	QByteArray middleHash =
	    QByteArray::fromBase64(appconfig->password_middle_hash().toLatin1());
	// qDebug() << "Password::checkMiddleHash() : middleHash :" <<
	// middleHash.toHex();
	...
	QByteArray checkData = QByteArray::fromBase64(
	    databaseconfig->get_middle_hash_check_data().toLatin1());
...
QByteArray Password::calculateMiddleHash(QString password) {
	...
	middleHash = hashAlgorythm.Pbkdf2(password.toUtf8(), salt, CRYPT_CHECK_ROUNDS, CRYPT_CHECK_HASH_LEN);

	// qDebug() << "Password::calculateMiddleHash() : middleHash " <<
	// middleHash.toHex();
...
void Password::smartSaveMiddleHash(QString password) {
	...
	if (appconfig->password_save_flag() &&
	    appconfig->password_middle_hash().length() == 0) {
		QByteArray middleHash = calculateMiddleHash(password);
		// qDebug() << "Password::smartSaveMiddleHash() : middleHash " <<
		// middleHash.toHex();
...
void Password::saveCheckPasswordKey(QString password) {
	...
	Pbkdf2Qt hashAlgorythm;
	hash = hashAlgorythm.Pbkdf2(password.toUtf8(), salt, CRYPT_CHECK_ROUNDS, CRYPT_CHECK_HASH_LEN);
...
void Password::resetPassword(void) {
	...
	messageBox.setText(tr(
	    "Your items tree no more of encrypted data. Would you "
	    "like to remove the encrypt password?"));
	messageBox.addButton(tr("Cancel"), QMessageBox::RejectRole);
	QAbstractButton *resetButton =
	    messageBox.addButton(tr("Remove password"), QMessageBox::AcceptRole);
	messageBox.exec();
...
bool Password::checkPasswordWithExists(QString password) {
	...
	QByteArray hash =
	    QByteArray::fromBase64(databaseconfig->get_crypt_check_hash().toLatin1());
	QByteArray salt =
	    QByteArray::fromBase64(databaseconfig->get_crypt_check_salt().toLatin1());

	// Вычисляется хеш пароля с использованием считанной соли
	QByteArray passwordHash;
	Pbkdf2Qt hashAlgorythm;
	passwordHash = hashAlgorythm.Pbkdf2(password.toUtf8(), salt, CRYPT_CHECK_ROUNDS, CRYPT_CHECK_HASH_LEN);
