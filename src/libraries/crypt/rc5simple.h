...
#define RC5_ROTL(x, y) \
	(((x) << (y & (RC5_W - 1))) | ((x) >> (RC5_W - (y & (RC5_W - 1)))))
#define RC5_ROTR(x, y) \
	(((x) >> (y & (RC5_W - 1))) | ((x) << (RC5_W - (y & (RC5_W - 1)))))

#define RC5_GET_BYTE_FROM_WORD(w, n) ((unsigned char)((w) >> ((n)*8) & 0xff))
#define RC5_GET_WORD_FROM_BYTE(b0, b1, b2, b3) \
	((RC5_TWORD)(b0 + (b1 << 8) + (b2 << 16) + (b3 << 24)))
...
class RC5Simple {
	...
	inline unsigned int RC5_GetIntFromByte(unsigned char b0, unsigned char b1, unsigned char b2, unsigned char b3);
