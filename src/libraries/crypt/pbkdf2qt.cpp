...
QByteArray Pbkdf2Qt::HmacSha1(QByteArray text, QByteArray key){
	...
	for(int i = 0; i < PBKDF2QT_SHA1_BLOCK_LENGTH; i++) k_pad[i] = k_pad[i] ^ 0x36;
	ctx.reset();
	...
	k_pad.fill(0);
	k_pad.replace(0, key.length(), key);
	for(int i = 0; i < PBKDF2QT_SHA1_BLOCK_LENGTH; i++) k_pad[i] = k_pad[i] ^ 0x5c;
	ctx.reset();
	ctx.addData(k_pad, PBKDF2QT_SHA1_BLOCK_LENGTH);
...
int Pbkdf2Qt::Pbkdf2_calculate(const char *pass, unsigned int pass_len, const char *salt, unsigned int salt_len, char *key, unsigned int key_len, unsigned int rounds){
	char *asalt, obuf[PBKDF2QT_SHA1_DIGEST_LENGTH];
	unsigned int i, j;
	unsigned int count;
	unsigned int r;
	if(rounds < 1 || key_len == 0) return -1;
	if(salt_len == 0 || salt_len > PBKDF2QT_SALT_SIZE_MAX - 1) return -1;
	if((asalt = (char *) malloc(salt_len + 4)) == nullptr) return -1;
	...
	for(count = 1; key_len > 0; count++){
		...
		for(i = 1; i < rounds; i++){
			...
			for(j = 0; j < sizeof(obuf); j++) obuf[j] ^= d1[j];
		}
...
QByteArray Pbkdf2Qt::Pbkdf2(QByteArray pass, QByteArray salt, unsigned int rounds, unsigned int key_len){
	char *key;
	key = (char *) malloc(key_len);
	if(key == nullptr) return QByteArray();
	int calculate_result = Pbkdf2_calculate(pass, pass.length(), salt, salt.length(), key, key_len, rounds);
