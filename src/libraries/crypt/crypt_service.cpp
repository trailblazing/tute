...
void CryptService::convertVectorToByteArray(
    const std::vector<unsigned char> &vec, QByteArray &qba) {
...
// Шифрация строки
// езультирующая зашифрованная строка - это набор шифрованных байт,
...
QString CryptService::decryptString(QByteArray key, QString line) {
	...
	/*
         ...
         qDebug() << "Decrypt source HEX" << QByteArray::fromBase64(
     line.toAscii()
         ).toHex();
...
// Шифрование файла на диске, вместо незашифрованного файла появляется
// зашифрованный
void CryptService::encryptFile(QByteArray key, QString fileName) {
	encDecFileSmart(key, fileName, 0);
}

// асшифровка файла на диске, вместо зашифрованного файла появляется
// нешифрованный
...
void CryptService::encDecFileSmart(QByteArray key, QString fileName, int mode) {
	...
	if (!file.open(QIODevice::ReadOnly))
		critical_error("encDecFileSmart() : Can\'t open binary file " + fileName + " for reading.");
	vector<unsigned char> vectorKey;
	convertByteArrayToVector(key, vectorKey);
	...
	if (!file.open(QIODevice::WriteOnly))
		critical_error("encryptFile() : Can\'t open binary file " + fileName + " for write.");
	file.write(result);
