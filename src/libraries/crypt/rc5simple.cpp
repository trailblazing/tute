...
void RC5Simple::RC5_Setup(unsigned char *key){
	...
	for(int i = 0; i < RC5_B; i++) RC5_LOG(("%.2X", key[i]));
	RC5_LOG(("\n"));
	...
	for(i = RC5_B - 1, l[RC5_C - 1] = 0; i != -1; i--) l[i / u] = (l[i / u] << 8) + key[i];
	RC5_LOG(("RC5_Setup, l[]: "));
	for(int i = 0; i < RC5_C; i++) RC5_LOG(("%.2X", l[i]));
	RC5_LOG(("\n"));
	for(rc5_s[0] = rc5_p, i = 1; i < RC5_T; i++) rc5_s[i] = rc5_s[i - 1] + rc5_q;
	RC5_LOG(("RC5_Setup, rc5_s[]: "));
	for(int i = 0; i < RC5_T; i++) RC5_LOG(("%.2X", rc5_s[i]));
	RC5_LOG(("\n"));
	// 3*t > 3*c
...
void RC5Simple::RC5_SetKey(vector<unsigned char> &key){
	...
	for(int i = 0; i < RC5_B; i++) rc5_key[i] = key[i];
}
...
void RC5Simple::RC5_Encrypt(vector<unsigned char> &in, vector<unsigned char> &out){
	...
	for(int i = 0; i < RC5_BLOCK_LEN; i++) iv[i] = (unsigned char) RC5_Rand(0, 0xFF);
	...
	for(int i = 0; i < RC5_BLOCK_LEN; i++) in[i] = data_size[i];
	// Отвлекли: почисть ребенку зубы
	// Add firsted random data
	unsigned int firsRandomDataBlocks = 0;
	if(rc5_formatVersion == RC5_FORMAT_VERSION_2) firsRandomDataBlocks = 1; // At start at format version 2, in begin data add one block with random data
	if(rc5_formatVersion == RC5_FORMAT_VERSION_3) firsRandomDataBlocks = 2; // At start at format version 3, in begin data add two blocks with random data (full width is 128 bit)
	if(firsRandomDataBlocks > 0)
		for(unsigned int n = 1; n <= firsRandomDataBlocks; n++){
			in.insert(in.begin(), RC5_BLOCK_LEN, 0);
			for(int i = 0; i < RC5_BLOCK_LEN; i++) in[i] = (unsigned char) RC5_Rand(0, 0xFF);
		}
	...
	for(int i = 0; i < in.size(); i++) RC5_LOG(("%.2X ", in[i]));
	RC5_LOG(("\n"));
#endif
	...
	if(rc5_formatVersion == RC5_FORMAT_VERSION_1){
		...
		for(int i = 0; i < RC5_BLOCK_LEN; i++) out[i] = iv[i];
	...
	if(rc5_formatVersion >= RC5_FORMAT_VERSION_2){
		...
		for(int i = 0; i < (RC5_BLOCK_LEN - 1); i++) out[i] = signature[i];
		// Format version byte in header
		if(rc5_isSetFormatVersionForce) out[RC5_BLOCK_LEN - 1] = rc5_formatVersion;
		else out[RC5_BLOCK_LEN - 1] = RC5_FORMAT_VERSION_CURRENT;
		// Save start IV to second block in output data
		for(int i = 0; i < RC5_BLOCK_LEN; i++) out[i + RC5_BLOCK_LEN] = iv[i];
	...
	while((RC5_BLOCK_LEN * (block + 1)) <= in.size()){
		...
		for(int i = 0; i < RC5_BLOCK_LEN; i++) temp_in[i] = in[shift + i] ^ iv[i];
		...
		for(int i = 0; i < RC5_BLOCK_LEN; i++) iv[i] = out[notCryptDataSize + shift + i];
		block++;
	}
	// Cleaning IV
	for(int i = 0; i < RC5_BLOCK_LEN; i++) iv[i] = 0;
	// -----------------------------------
	...
	if(firsRandomDataBlocks > 0)
		for(unsigned int n = 1; n <= firsRandomDataBlocks; n++) in.erase(in.begin(), in.begin() + RC5_BLOCK_LEN);
	...
	if(in.size() > clean_data_size) in.erase(in.begin() + clean_data_size, in.end());
}
...
void RC5Simple::RC5_Decrypt(vector<unsigned char> &in, vector<unsigned char> &out){
	...
	if(rc5_isSetFormatVersionForce == true) formatVersion = rc5_formatVersion; // If format set force, format not autodetected
	else{
		// Signature bytes
		const char *signature = RC5_SIMPLE_SIGNATURE;
		...
		for(int i = 0; i < (RC5_BLOCK_LEN - 1); i++)
			if(in[i] != signature[i]) isSignatureCorrect = false;
		// If not detect signature
		if(isSignatureCorrect == false) formatVersion = RC5_FORMAT_VERSION_1; // In first version can't signature
		else{
			// Get format version
			unsigned char readFormatVersion = in[RC5_BLOCK_LEN - 1];
			// If format version correct
			if(readFormatVersion >= RC5_FORMAT_VERSION_2 && readFormatVersion <= RC5_FORMAT_VERSION_CURRENT) formatVersion = readFormatVersion;
			else formatVersion = RC5_FORMAT_VERSION_1; // If version not correct, may be it format 1 ( probability 0.[hrendesyatih]1 )
		}
	}
	...
	// Get IV
	unsigned char iv[RC5_BLOCK_LEN];
	for(int i = 0; i < RC5_BLOCK_LEN; i++) iv[i] = in[i + ivShift];
	...
	while((RC5_BLOCK_LEN * (block + 1)) <= in.size()){
		...
		// Un XOR
		for(int i = 0; i < RC5_BLOCK_LEN; i++) ct_part[i] ^= iv[i];
...
#endif
		// Generate next IV for Cipher Block Chaining (CBC)
		for(int i = 0; i < RC5_BLOCK_LEN; i++) iv[i] = in[shift + i];
		// Save decrypt data
		for(int i = 0; i < RC5_BLOCK_LEN; i++){
	...
	if(out.size() > data_size) out.erase(out.begin() + data_size, out.end());
}
...
void RC5Simple::RC5_EncDecFile(unsigned char *in_name, unsigned char *out_name, int mode){
	...
	fclose(in_file);
	if(mode == RC5_MODE_ENCODE) RC5_Encrypt(in, out); // Encrypt data vector
	else RC5_Decrypt(in, out); // Decrypt data vector
	// Create output file
...
inline unsigned int RC5Simple::RC5_GetIntFromByte(unsigned char b0, unsigned char b1, unsigned char b2, unsigned char b3){
