...
void RC5Simple::RC5_Setup(unsigned char *key) {
	...
	RC5_LOG(("\n"));
	// 3*t > 3*c
	for (a = b = i = j = k = 0; k < 3 * RC5_T;
	     k++, i = (i + 1) % RC5_T, j = (j + 1) % RC5_C) {
...
void RC5Simple::RC5_Encrypt(vector<unsigned char> &in, vector<unsigned char> &out) {
	...
	if (rc5_formatVersion == RC5_FORMAT_VERSION_2)
		firsRandomDataBlocks = 1;  // At start at format version 2, in begin data add
	                               // one block with random data
	if (rc5_formatVersion == RC5_FORMAT_VERSION_3)
		firsRandomDataBlocks = 2;  // At start at format version 3, in begin data add
	                               // two blocks with random data (full width is 128
	                               // bit)
	...
	while ((RC5_BLOCK_LEN * (block + 1)) <= in.size()) {
		...
		temp_word_1 =
		    RC5_GET_WORD_FROM_BYTE(temp_in[0], temp_in[1], temp_in[2], temp_in[3]);

		temp_word_2 = RC5_GET_WORD_FROM_BYTE(
		    temp_in[RC5_WORD_LEN + 0], temp_in[RC5_WORD_LEN + 1],
		    temp_in[RC5_WORD_LEN + 2], temp_in[RC5_WORD_LEN + 3]);
		...
		for (int i = 0; i < RC5_WORD_LEN; i++) {
			...
			out[notCryptDataSize + shift + RC5_WORD_LEN + i] =
			    RC5_GET_BYTE_FROM_WORD(ct[1], i);
		}
...
void RC5Simple::RC5_Decrypt(vector<unsigned char> &in, vector<unsigned char> &out) {
	...
	if (rc5_isSetFormatVersionForce == true)
		formatVersion =
		    rc5_formatVersion;  // If format set force, format not autodetected
	else {
		// Signature bytes
		const char *signature = RC5_SIMPLE_SIGNATURE;
		...
		if (isSignatureCorrect == false)
			...
			if (readFormatVersion >= RC5_FORMAT_VERSION_2 &&
			    readFormatVersion <= RC5_FORMAT_VERSION_CURRENT)
				formatVersion = readFormatVersion;
			else
				formatVersion = RC5_FORMAT_VERSION_1;  // If version not correct, may be
			                                           // it format 1 ( probability
			                                           // 0.[hrendesyatih]1 )
		}
	}
	...
	if (formatVersion == RC5_FORMAT_VERSION_1) {
		ivShift = 0;
		firstDataBlock =
		    1;  // Start decode from block with index 1 (from block 0 read IV)
		removeBlocksFromOutput = 1;
		blockWithDataSize = 1;
	}
	if (formatVersion == RC5_FORMAT_VERSION_2) {
		ivShift = RC5_BLOCK_LEN;
		firstDataBlock = 2;          // Start decode from block with index 2 (0 - signature
		                             // and version, 1 - IV)
	...
	while ((RC5_BLOCK_LEN * (block + 1)) <= in.size()) {
		...
		temp_word_2 = RC5_GET_WORD_FROM_BYTE(
		    in[shift + RC5_WORD_LEN], in[shift + RC5_WORD_LEN + 1],
		...
		RC5_LOG(
		    ("Block data. Word 1: %.2X, Word 2: %.2X\n", temp_word_1, temp_word_2));
		...
		if (block == blockWithDataSize) {
			data_size =
			    RC5_GetIntFromByte(ct_part[0], ct_part[1], ct_part[2], ct_part[3]);

			RC5_LOG(("Decrypt data size: %d\n", data_size));
			// Uncorrect decrypt data size
			if ((unsigned int)data_size > (unsigned int)in.size()) {
				// RC5_LOG(( "Incorrect data size. Decrypt data size: %d, estimate data
				// size: ~%d\n", data_size,  in.size() ));
	...
	// Remove from output a blocks with technical data (random blocks, size,
	// etc...)
