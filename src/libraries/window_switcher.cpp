...
WindowSwitcher::WindowSwitcher(QString object_name, Editentry* meta_editor, QObject* parent)
    : QObject(parent)
{
    ...
    meta_editor->back_callback(&WindowSwitcher::record_to_recordtable); // Устанавливается функция обратного вызова при клике на кнопку Back
}
...
void WindowSwitcher::record_to_recordtable(void)
{
    ...
    if (!globalparameters.window_switcher()->status())
        return;
...
void WindowSwitcher::recordtable_ro_record_editor(void)
{
    ...
    // Скрываются все прочие области
    globalparameters.tree_screen()->hide();
    for (auto& i : globalparameters.record_screens())
        i->hide();
    // globalParameters.getFindScreen()->hide();
