...
// const QIcon default_icon = QIcon(":/resource/pic/trace.svg");//
// QIcon(QLatin1String(":/resource/pic/trace.svg"));

MtComboBox::MtComboBox(QWidget *parent)
    : QComboBox(parent) {
	Q_UNUSED(parent);
...
void FlatComboBox::flat(bool flat) {
	flat_ = flat;
}
...
FlatFontComboBox::~FlatFontComboBox() {
	delete itemDelegate;
}
...
void FlatFontComboBox::flat(bool flat) {
	flat_ = flat;
}
