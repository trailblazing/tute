...
ts_t::ts_t(QString object_name, const AppConfig& _appconfig, QMenu* _filemenu, QMenu* _toolsmenu, wn_t* _main_window)
...
void ts_t::setup_actions(void)
{
    ...
    connect(ac, &QAction::triggered, this, [&](bool checked = false) mutable -> void {
        ...
        _tree_view->delete_permanent([&] { return _tree_view->source_model(); }, [&] {QList<boost::intrusive_ptr<TreeItem> > r;for(auto ix : _selectitems) r << _tree_view->source_model()->child(ix);return r; }(), &tkm_t::delete_permanent, "cut", false);
...
void ts_t::edit_field_context(QModelIndex index_current)
{
    ...
    edit_field(edit_record_dialog.getField("pin"), edit_record_dialog.getField("name"), edit_record_dialog.getField("author"), edit_record_dialog.getField("home"), edit_record_dialog.getField("url"), edit_record_dialog.getField("tags"));
}

// Функция сохранения отредактированных полей записи в таблицу конечных записей
void ts_t::edit_field(QString pin, QString name, QString author, QString home, QString url, QString tags)
{
...
void ts_t::item_move_up_dn_branch(int (TreeItem::*_move)())
{ // int direction
...
void ts_t::on_custom_contextmenu_requested(const QPoint& _position)
{
    ...
    if (index.isValid()) {
        ...
        if (cryptFlag != "1" || (cryptFlag == "1" && globalparameters.crypt_key().length() > 0)) {
