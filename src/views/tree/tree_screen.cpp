...
// know_root_holder::know_root_holder(const AppConfig &appconfig, TreeScreen *_this)
// : _know_root(new KnowModel(_this))
// {
//// Загрузка данных
//// auto xml =
// _know_root->init_from_xml(appconfig->tetra_dir() + "/" + globalparameters.main_program_file() + ".xml");
      ...
      _treescreenlayout(new QVBoxLayout())
{
    ...
    ////    _selected_branch->init_from_xml(appconfig->datadir() + "/candidate.xml");
    ...
    ////    _shadow_branch->init_from_xml(appconfig->datadir() + "/default_page.xml");
...
void ts_t::setup_actions(void)
{
    ...
    connect(ac, &QAction::triggered, this, [&]() mutable -> void { item_move_up_dn_branch(&i_t::move_up); }); // &TreeScreen::item_move_up_branch
    ...
    connect(ac, &QAction::triggered, this, [&]() mutable -> void { item_move_up_dn_branch(&i_t::move_dn); }); // &TreeScreen::item_move_dn_branch
    ...
    connect(ac, &QAction::triggered, [&]() mutable -> void {
        _tree_view->new_item(TreeIndex::require_treeindex([&]() -> tkm_t* { return _tree_view->source_model(); }, _tree_view->current_item()), std::bind(&tv_t::new_child, _tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3), [&](boost::intrusive_ptr<TreeIndex> _tree_index, QString _id, QString _name) -> boost::intrusive_ptr<i_t> { return _tree_index->current_model()()->new_child(_tree_index, _id, _name); });
    });
    ...
    connect(ac, &QAction::triggered, [&]() mutable {
        _tree_view->new_item(TreeIndex::require_treeindex([&]() -> tkm_t* { return _tree_view->source_model(); }, _tree_view->current_item()) // tree_index
            ,
            std::bind(&tv_t::new_child, _tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3), [&](boost::intrusive_ptr<TreeIndex> _tree_index, QString _id, QString _name) -> boost::intrusive_ptr<i_t> {
    ...
    connect(ac, &QAction::triggered, this, [&](bool checked = false) mutable -> void {
        ...
        _tree_view->delete_permanent([&] { return _tree_view->source_model(); }, [&] {QList<boost::intrusive_ptr<i_t> > r; for(auto ix : _selectitems) r << _tree_view->source_model()->child(ix); return r; }(), &tkm_t::delete_permanent, "cut", false);
...
////    // Загрузка данных
////    _treemodelknow->init_from_xml(appconfig->datadir() + "/" + index_xml_file_name);
// _tree_view->reset();
    ...
    , _layout(new QStackedLayout(this))
{
    ...
    if (widget_right->objectName() == record_screen_multi_instance_name)
        _vtab_record->addTab(widget_right, QIcon(":/resource/pic/three_leaves_clover.svg"), QString("Browser")); // QString("Browser ") + QString::number(vtab_record->count())
...
void tsv_t::widget_right(QWidget* rs)
{
    ...
    if (_widget_right->objectName() == record_screen_multi_instance_name)
        _vtab_record->addTab(_widget_right, QIcon(":/resource/pic/three_leaves_clover.svg"), QString("Browser")); // QString("Browser ") + QString::number(vtab_record->count())
