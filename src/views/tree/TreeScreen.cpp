...
// auto _host_index = _modelindex->parent_index();
		...
		if(_alternative_items.size() > 0){	// && _item_has_no_child_first
		    ...
		    result = it.next();
		    while(it.hasNext()){
			result = _tree_view->merge(TreeIndex::merge_instance(TreeIndex::instance(_current_model, result, result->parent()), it.next()));// TreeIndex::instance(_current_model, result, result->parent()), it.next());
		    }
			// children_transfer(_new_item, _current_model);
		}else{
			// Вставка новых данных в модель дерева записей
		    result = _current_model()->new_child(TreeIndex::instance([&]() -> tkm_t * {return _tree_view->source_model();}, _tree_view->current_item()->parent())	// _tree_index
							, _id, _name);
		}
		return result;
	    });
...
void ts_t:: setup_signals(void){
    ...
    connect(_tree_view, &tv_t::tapAndHoldGestureFinished, this, &ts_t::on_custom_contextmenu_requested);
