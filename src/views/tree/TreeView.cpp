...
bool tv_t::gestureEvent(QGestureEvent *event){
    ...
    if(QGesture *gesture = event->gesture(Qt::TapAndHoldGesture))tapAndHoldGestureTriggered(static_cast<QTapAndHoldGesture *>(gesture));
    return true;
}
// Обработчик жеста TapAndHoldGesture
// Вызывается из обработчика жестов
void tv_t::tapAndHoldGestureTriggered(QTapAndHoldGesture *gesture){
...
void tv_t::dropEvent(QDropEvent *event){
	...
	for(int i = 0; i < clipboard_records->size(); i ++){
	    ...
	    auto item_on_tree = _know_root->child([&](boost::intrusive_ptr<const TreeItem> it){
		...
		_know_root->move_child(TreeIndex::instance([&](){return _know_root;}, tree_item_drop, tree_item_drop->parent()), tree_item_drag);																																// tree_item_drop->child_insert(0, record, ADD_NEW_RECORD_TO_END);
...
boost::intrusive_ptr<TreeItem> tv_t::session_root_auto(){
    auto find_root = [&]() -> boost::intrusive_ptr<TreeItem> {
		...
		if(final_common_parent != it && ! final_common_parent->is_ancestor_of([&](boost::intrusive_ptr<const TreeItem> it_){return it_ == it;})){
			...
			while(near_root->parent()){
			    ...
			    if(near_root->is_ancestor_of([&](boost::intrusive_ptr<const TreeItem> it_){return it_ == it;}) && near_root->is_ancestor_of([&](boost::intrusive_ptr<const TreeItem> it_){return it_ == final_common_parent;})){
				break;
			    }
			}
		    }
    ...
    auto board_session_id_defined = [&](){
	    return _know_model_board->session_id() != "" && _know_model_board->child([=](boost::intrusive_ptr<const TreeItem> t){
...
boost::intrusive_ptr<TreeItem> tv_t::current_item(){
	...
	if(! index_result.isValid()){
		...
		if(_know_root->root_item()->count_direct() == 0){
		    ...
		    it = TreeIndex::instance([&]() -> tkm_t * {return _know_root;}, new TreeItem(it), it)->item_bind(nullptr
...
QItemSelection tv_t::LocalLizeInitializer::operator()(){
    ...
    auto index_localize = [&](const QModelIndexList &_origin_index_list) -> QModelIndexList {
		...
		if(! _origin_index_list.contains(_tree_view->current_index())){											// ?
			...
			found = static_cast<tkm_t *>(_source_model())->merge(TreeIndex::merge_instance(TreeIndex::instance(_source_model, found, found->parent()), duplicated_item)
...
boost::intrusive_ptr<TreeItem> tv_t::new_child(boost::intrusive_ptr<TreeIndex> _modelindex	// std::function<KnowModel *()> _current_model, QModelIndex _current_index
	...
	while(it.hasNext()){
		...
		result = merge(TreeIndex::merge_instance(TreeIndex::instance(_current_model, result, result->parent()), _it));	// TreeIndex::instance(_current_model, result, result->parent()), _it);
	    }
	}
...
void tv_t::tree_empty_controll(void){
...
//	    );
	TreeIndex::instance([&]() -> tkm_t * {return _know_root;}, new TreeItem(it), it)->item_bind(nullptr
												  , QUrl(browser::Browser::_defaulthome)
												  , std::bind(&tv_t::paste_child, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3)
												  , [&](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(),  browser::Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(),  browser::Browser::_defaulthome);});
    }
...
boost::intrusive_ptr<TreeItem> tv_t::new_item(boost::intrusive_ptr<TreeIndex> _modelindex	// std::function<KnowModel *()> _current_model, QModelIndex _current_index
	...
	if(ok && ! _name.isEmpty()){											// return nullptr;    // Если была нажата отмена
		...
		while(it.hasNext()){
		    ...
		    if(_it->id() == result->id()){																													// prevent error merge
			// boost::intrusive_ptr<TreeIndex> tree_index = [&] {boost::intrusive_ptr<TreeIndex> tree_index; try {tree_index = new TreeIndex(_current_model, result); } catch(std::exception &e) {throw e; } return tree_index; } ();
			result = merge(TreeIndex::merge_instance(TreeIndex::instance(_current_model, result, result->parent()), _it));
		    }
		}
	    }else{
...
boost::intrusive_ptr<TreeItem> tv_t::paste_children_from_children(boost::intrusive_ptr<TreeIndex> _parent_modelindex	// std::function<KnowModel *()> _current_model, QModelIndex _current_index
	...
	merge(TreeIndex::merge_instance(_parent_modelindex, _blank_header));											// TreeIndex(_current_model, _current_parent)
	...
	if(static_cast<QModelIndex>(host_index).isValid()){
	    ...
	    _blank_header->traverse_direct([&](boost::intrusive_ptr<Linker> il){																	// != reserved
			...
			if(found_item){																	// candidate->id()
			    ...
			    found_item = merge(TreeIndex::merge_instance(TreeIndex::instance(_current_model, found_item, found_item->parent()), candidate));
				// reserved++;
			}else{
...
void tv_t::paste_children_from_clipboard(boost::intrusive_ptr<TreeIndex> _sibling_modelindex){	// std::function<QString(const QModelIndex &, ClipboardBranch *)>  _model_paste_from_clipboard
    ...
    auto host_index = _sibling_modelindex->host_index();
    boost::intrusive_ptr<TreeItem> _result(nullptr);
	...
	if(buffer.size() > 0){											// == blank_header->count_direct()
		...
		paste_child(_sibling_modelindex, it_on_tree, [&](boost::intrusive_ptr<const Linker> il){return il->host()->field<name_type>() == it_on_tree->field<name_type>();});																																										// it->parent(blank_header);
	    }
	    _result = _current_model()->item(host_index);
	}else if(blank_header){
	    _result = paste_children_from_children(_sibling_modelindex, blank_header, [&](boost::intrusive_ptr<const Linker> target, boost::intrusive_ptr<const Linker> source) -> bool {
			return target->host()->field<url_type>() == source->host()->field<url_type>() && target->host()->field<name_type>() == source->host()->field<name_type>();
		    });
	}
	if(_result){
	    ...
	    if(host_parent_index.isValid())_current_model()->update_index(host_parent_index);
...
boost::intrusive_ptr<TreeItem> tv_t::paste_child(boost::intrusive_ptr<TreeIndex>       _treeindex	// std::function<KnowModel *()> _current_model, QModelIndex _current_index
    ...
    auto paste_sibling = [&](boost::intrusive_ptr<TreeIndex>       _modelindex
		...
		if(_alternative_items.size() > 0){																												// && same->is_empty()
			...
			result = merge(TreeIndex::merge_instance(TreeIndex::instance(current_model, result, result->parent()), it.next()));
			assert(result);
			assert(current_model()->item([=](boost::intrusive_ptr<const TreeItem> t){return t->id() == result->id();}));
		    }
			// boost::intrusive_ptr<TreeIndex> tree_index = [&] {boost::intrusive_ptr<TreeIndex> tree_index; try{tree_index = new TreeIndex(current_model, result); } catch(std::exception &e) {throw e; } return tree_index; } ();
		    result = merge(TreeIndex::merge_instance(TreeIndex::instance(current_model, result, result->parent()), _source_item));
		    assert(result);
		    if(result){
			assert(current_model()->item([=](boost::intrusive_ptr<const TreeItem> t){return t->id() == result->id();}));
		    }
			...
			// Вставка новых данных в модель дерева записей
		    result = static_cast<tkm_t *>(current_model())->move_child(TreeIndex::instance(current_model, current_parent, current_parent->parent()), _source_item);																// TreeIndex(current_model, current_parent, _modelindex.sibling_order())
...
//		    QString id = get_unical_id();
		...
		// Вставка новых данных в модель дерева записей
		result = static_cast<tkm_t *>(current_model())->move_child(TreeIndex::instance(current_model, host, host->parent()), _source_item);
		assert(result);
		assert(result != _know_model_board->root_item());
		// Установка курсора на только что созданную позицию
		// QModelIndex
	...
	if(static_cast<QModelIndex>(_know_root->index(_item_common_parent)).isValid()					// _item_common_parent->field("name") != clipboard_items_root
		...
		if(enable_delete){
			...
			if(it->id() == static_cast<tkm_t *>(_current_model())->session_id()){
			    static_cast<tkm_t *>(_current_model())->session_id(TreeIndex::instance(_current_model, it->parent()));
			}
...
boost::intrusive_ptr<TreeItem> tv_t::merge(boost::intrusive_ptr<TreeMerge> _tree_merge){
    ...
    boost::intrusive_ptr<TreeItem> source = _tree_merge->to_be_merged();
...
//	}
	if(result){
	    if(result != source){											// && target_on_tree
		if(source->id() == static_cast<tkm_t *>(_current_model())->session_id()){
		    static_cast<tkm_t *>(_current_model())->session_id(TreeIndex::instance(_current_model, result, result->parent()));
		}
		globalparameters.mainwindow()->setDisabled(true);
		result = static_cast<tkm_t *>(_current_model())->merge(TreeIndex::merge_instance(tree_index, source)	// target_on_tree  // keep// remove   // std::bind(_know_model_board, this)
	...
	auto merge_by_url = [&](){
		...
		if(duplicated_items_list_on_board_by_url.size() > 1){
			...
			if(duplicated_item != result_item){																																// (duplicated_item->id() != result_item->id()) ||
			    ...
			    result_item = merge(TreeIndex::merge_instance(TreeIndex::instance(_source_model, result_item, result_item->parent()), duplicated_item));
			}
		    }
		}
	    };
