...
boost::intrusive_ptr<TreeItem> KnowView::current_item()
{
        ...
        if(index_result.isValid())
                item_result = _know_root->item(index_result);                                                  // selectionModel()->currentIndex()
        else {
    ...
    )
{
        ...
        if(_blank_header->count_direct() > 0) {
                ...
                if(host_parent_index.isValid()) {
                        ...
                        if(current_count > 0)
                                result = host_parent;                                                                                                                                 // _current_model()->item(_current_parent_index);    // _blank_header;
    ...
    )
{
        ...
        auto host_parent = _treeindex->host_parent();
        ...
        if(host_parent && _source_item) {                          // current_index.isValid()
                ...
                if(!host_parent->contains_direct(std::forward<const boost::intrusive_ptr<const Linker> >(_source_item->linker()))) {                                                                 // current_item->contains_direct(_source_item->linker()) ||
                        if(host_parent->count_direct() > 0) {
                                auto _child = host_parent->item_direct(_treeindex->sibling_order());                                                                                                                                                 // current_item->count_direct() - 1
                                ...
                                // Вставка новых данных в модель дерева записей
                                result = static_cast<KnowModel *>(current_model())->model_move_as_child(TreeIndex::instance(current_model, host_parent->parent(), host_parent), _source_item);
                                assert(result);
                                assert(result != _know_model_board->root_item());
                                // Установка курсора на только что созданную позицию
                                //        QModelIndex
