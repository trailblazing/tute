    ...
    , _delegate(new HtmlDelegate(this))
{
	...
	if (appconfig->interface_mode() == "desktop") {
		connect(this, &tv_t::clicked, this, [&](const QModelIndex& index) {
			...
			if (_item)
				index_invoke(
				    TreeIndex::require_treeindex([&] { return _know_root; }, _item));
...
{ // Q_DECL_OVERRIDE
	...
	if (mouse_button == Qt::RightButton) {
		...
		if (next_index.isValid()) {
			...
			select_as_current(
			    TreeIndex::require_treeindex([&] { return _know_root; }, it));
	...
	if (is_dragable(event)) {
		...
		for (int i = 0; i < clipboard_records->size(); i++) {
			...
			rctrl_t* _record_controller =
			    tree_item_drag->page()
				->record_controller(); // find_object<RecordController>("table_screen_controller");
...
// find_object<MetaEditor>(meta_editor_singleton_name)
					...
					_record_controller->editing_window()->clear_all(); // Нужно очистить
											   // поле
											   // редактирования
											   // чтобы не видно
											   // было текста
				...
				TreeLevel::instance(TreeIndex::require_treeindex([&]() { return _know_root; }, tree_item_drop),
				    tree_item_drag)
...
boost::intrusive_ptr<i_t> tv_t::session_root_auto()
{
	...
	auto fix_root = [&] {
		...
		_current_model()->session_id(
		    TreeIndex::require_treeindex(_current_model, find_root()));
		know_model_save();
...
boost::intrusive_ptr<i_t> tv_t::current_item()
{
	...
	if (index_result.isValid())
		...
		if (!index_result.isValid()) {
			...
			// browser->record_screen()->record_controller()->source_model();},
			// nullptr, it);
...
QModelIndex tv_t::current_index(void) const
{
	...
	//////            } else {
	//////                RecordModel::ModelIndex record_index([&] {return
	///browser->record_screen()->record_controller()->source_model();}, nullptr,
	///it);
	//////                it = browser->item_bind(record_index);
	//////            }
...
// }
// _item->binder() ? [&] {auto c =
// _item->binder()->page()->record_controller();auto v =
// _item->binder()->page()->record_controller()->view();c->update_record_view(_item);return
...
QModelIndex tv_t::select_as_current(boost::intrusive_ptr<TreeIndex> _tree_index, select_strategy _select_strategy, current_strategy _current_strategy)
{
	...
	QModelIndex _host_index =
	    _tree_index->host_index(); // index(_modelindex->host());
	...
	// _item->binder()->page()->record_controller() ? [&] {
...
{ // web::WebView *view, const index_tree &_index
	...
	web::WebView* activated_view =
	    browser_
		->currentTab(); // globalparameters.main_window()->vtab_record()->find([&](boost::intrusive_ptr<const
	...
	if (static_cast<QModelIndex>(_index)
			...
			if (duplicated_items_list_on_board_by_url.size() > 1) {
				for (auto _i : duplicated_items_list_on_board_by_url) {
					...
					if (duplicated_item != result_item)
						result_item = TreeLevel::instance(TreeIndex::require_treeindex(_source_model, result_item),
								  duplicated_item)
		...
		if (duplicated_items_list_on_board_by_id.size() > 1) {
			for (auto _i : duplicated_items_list_on_board_by_id) {
				...
				if (duplicated_item != result_item)
					result_item =
					    TreeLevel::instance(
						TreeIndex::require_treeindex(_source_model, result_item),
						duplicated_item)
		...
		if (!gl_paras->main_window()->find(
			[&](boost::intrusive_ptr<const ::Binder>
				b) { // !result_item->is_activated()
				bool result = false;
			...
			//
			setExpanded(static_cast<QModelIndex>(_index).parent(),
			    true);                             // (isExpanded(_index)) ? setExpanded(_index, false) :
    ...
    : _tree_view(_tree_view)
{
	...
	if (-1 ==
	    _tree_view->selectionModel()->selectedIndexes().indexOf(
			...
			if (!_origin_index_list.contains(_tree_view->current_index())) { // ?
				...
				if (found && duplicated_item && found != duplicated_item) {
					found =
						...
						->merge(
						    TreeLevel::instance(
							TreeIndex::require_treeindex(_source_model, found),
							duplicated_item),
	...
	if (_alternative_items.size() > 0) {
		...
		while (it.hasNext()) {
			...
			if (_it->id() == result->id()) { // prevent error merge
				...
				// e; } return tree_index; } ();
				result = TreeLevel::instance(TreeIndex::require_treeindex(_current_model, result), _it)->merge(); // TreeIndex::instance(_current_model, result,
																  // result->parent()), _it);
			}
		}
...
boost::intrusive_ptr<i_t> tv_t::tree_empty_controll(void)
{
	...
	if (_know_model_board->rowCount() == 0) {
		...
		// );
		result =
		    TreeIndex::require_treeindex([&]() -> tkm_t* { return _know_root; }, new i_t(it))
			->bind(web::Browser::_defaulthome, std::bind(&tv_t::move, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool {
				return url_equal(url_value(detail::to_qstring(it_->field<home_key>())), web::Browser::_defaulthome) || url_equal(it_->field<url_key>(), web::Browser::_defaulthome);
			});
	} else
...
{ // bool insert_sibling_branch
	...
	if (static_cast<QModelIndex>(_target_index).isValid()) {
		...
		if (ok &&
			...
			if (_alternative_items.size() > 0) {
				...
				while (it.hasNext()) {
					...
					if (_it->id() == result->id()) { // prevent error merge
						...
						result =
						    TreeLevel::instance(
							TreeIndex::require_treeindex(_current_model, result), _it)
							->merge();
					}
				}
			} else {
			...
			// })); } catch(std::exception &e) {throw e; } return tree_index; } ();
			select_as_current(
			    TreeIndex::require_treeindex([&] { return _know_root; }, result));
			    ...
			    if (_alternative_items.size() > 0) { // && same->is_empty()
				    ...
				    while (it.hasNext()) {
					    ...
					    result = TreeLevel::instance(
							 TreeIndex::require_treeindex(current_model, result),
							 it.next())
							 ->merge();
					    assert(result);
					    assert(
				    ...
				    // Вставка новых данных в модель дерева записей
				    result = TreeLevel::instance(TreeIndex::require_treeindex(current_model, current_parent),
						 _source_item)
		...
		if (!host->contains_direct(
			...
			if (host->count_direct() > 0) {
				...
				result = paste_sibling(
				    TreeIndex::require_treeindex(current_model, _child), _source_item,
				...
				result =
				    TreeLevel::instance(
					TreeIndex::require_treeindex(current_model, host), _source_item)
					->move();
				assert(result);
				assert(result != _know_model_board->root_item());
				// Установка курсора на только что созданную позицию
				...
				// })); } catch(std::exception &e) {throw e; } return tree_index; } ();
				select_as_current(
				    TreeIndex::require_treeindex(current_model, result)); // setto
				// selectionModel()->select(setto, current_tree_selection_mode);
				// selectionModel()->setCurrentIndex(setto,
				// current_tree_current_index_mode);   // ClearAndSelect
				// А можно было установить курсор на нужную позицию и так
			...
			for (auto it : _items)
				...
				if (it->parent() == _item_common_parent) { // if(index.isValid() &&
					...
					if (it->id() == _current_model()->session_id())
						_current_model()->session_id(TreeIndex::require_treeindex(_current_model, _item_common_parent)); ////        keep =
																		 ///source; remove =
																		 ///target;
					// boost::intrusive_ptr<TreeIndex> tree_index = [&]
					// {boost::intrusive_ptr<TreeIndex> tree_index; try{tree_index = new
				...
				if (enable_delete) {
					...
					for (auto it :
						...
						if (it->id() ==
							...
							static_cast<tkm_t*>(_current_model())
							    ->session_id(TreeIndex::require_treeindex(_current_model, it->parent()));
					...
					if (result_items.size() > 0) {
						...
						if (gl_paras->main_window()->record_screens().size() > 0) {
							...
							auto tabmanager =
							    rs->browser()->tabmanager(); // current_browser->tabmanager();
...
{ // const
	...
	if (indexs_valid) { // _index_current.isValid()
		...
		_sub_branch->branch_push(
		    TreeIndex::require_treeindex(_source_model, current_item()),
	...
	if (_to_be_operated) {
		...
		if (result) {
			if (result != _to_be_operated) { // && target_on_tree
				...
				result =
				    static_cast<tkm_t*>(_current_model())
					->merge(TreeLevel::instance(tree_index,
						    _to_be_operated) // target_on_tree  // keep// remove
								     // // std::bind(_know_model_board,
								     // this)
					    ,
					    std::bind(&tv_t::delete_permanent, this, std::placeholders::_1, std::placeholders::_2, &tkm_t::delete_permanent, "cut",
						    false) // , std::bind(&tv_t::delete_permanent,
				...
				assert(_current_model()->item([=](boost::intrusive_ptr<const i_t> t) {
					return t->id() == result->id();
				}));
				...
				if (_ro_be_operated_is_current_of_rocord_view)
					if (r_ctrl)
						r_ctrl->select_as_current(
						    r_ctrl->index<pos_proxy>(_to_be_operated));
				know_model_save();
...
{ // _new_session_root_item
	...
	auto cursor_follow_up_impl = [&](void) -> std::pair<
		...
		if (current_item_) {
			...
			if (parent_ && parent_ != current_root_ &&
				...
				select_as_current(
				    TreeIndex::require_treeindex([&] { return _know_root; }, current));
	...
	if (!to_be_contained)
		...
		if (current_item_) {
			...
			select_as_current(
			    TreeIndex::require_treeindex(source_model_, current_item_));
		}
		_tree_screen->enable_up_action();
