...
//    setStyleSheet("QTreeView::branch {  border-image: url(none.png); }");
	...
	connect(this, &tv_t::clicked, this, [&](const QModelIndex &index){auto _item = _know_root->child(index);if(_item)index_invoke(TreeIndex::instance([&] {return _know_root;}, _item));});		// globalparameters.main_window()->vtab_record()->activated_browser()->tabmanager()->currentWebView(),
...
void tv_t::mousePressEvent(QMouseEvent *event){	// Q_DECL_OVERRIDE
	...
	if(next_index.isValid()){
	    ...
	    select_as_current(TreeIndex::instance([&] {return _know_root;}, it));
...
void tv_t::dropEvent(QDropEvent *event){
	...
	for(int i = 0; i < clipboard_records->size(); i ++){
		...
		TreeLevel::instance(TreeIndex::instance([&](){return _know_root;}, tree_item_drop), tree_item_drag)->move();	// _know_root// tree_item_drop->child_insert(0, record, ADD_NEW_RECORD_TO_END);
...
boost::intrusive_ptr<TreeItem> tv_t::session_root_auto(){
	...
	_current_model()->session_id(TreeIndex::instance(_current_model, find_root()));
	know_model_save();
...
void tv_t::session_root_manual(bool checked){
    ...
    _source_model()->session_id(TreeIndex::instance(_source_model, it));
}
...
//	}
	if(! item_result && selectionModel()->selectedIndexes().size() > 0){
		...
		// if(tree_index)
	    index_result = select_as_current(TreeIndex::instance([&] {return _know_root;}, item_result));
	}
	if(! index_result.isValid()){
		...
		if(_know_root->root_item()->count_direct() == 0){
		    ...
		    item_result = TreeIndex::instance([&]() -> tkm_t * {return _know_root;}, new TreeItem(item_result))
...
//    const index_tree &_index = _know_root->index(_item);
	...
	auto merge_by_url = [&](){
		...
		if(duplicated_items_list_on_board_by_url.size() > 1){
			...
			if(duplicated_item != result_item)result_item = TreeLevel::instance(TreeIndex::instance(_source_model, result_item), duplicated_item)->merge();																																// (duplicated_item->id() != result_item->id()) ||
...
QItemSelection tv_t::LocalLizeInitializer::operator ()(){
    ...
    auto index_localize = [&](const QModelIndexList &_origin_index_list) -> QModelIndexList {
		...
		if(! _origin_index_list.contains(_tree_view->current_index())){											// ?
			...
			found = static_cast<tkm_t *>(_source_model())->merge(TreeLevel::instance(TreeIndex::instance(_source_model, found), duplicated_item)
...
boost::intrusive_ptr<TreeItem> tv_t::new_child(boost::intrusive_ptr<TreeIndex> _modelindex	// std::function<KnowModel *()> _current_model, QModelIndex _current_index
	...
	while(it.hasNext()){
		...
		result = TreeLevel::instance(TreeIndex::instance(_current_model, result), _it)->merge();	// TreeIndex::instance(_current_model, result, result->parent()), _it);
	    }
	}
	...
	// })); } catch(std::exception &e) {throw e; } return tree_index; } ();
    select_as_current(TreeIndex::instance([&] {return _know_root;}, result));
...
//	    );
	result = TreeIndex::instance([&]() -> tkm_t * {return _know_root;}, new TreeItem(it))
	    ->bind(QUrl(browser::Browser::_defaulthome)
		  , std::bind(&tv_t::move, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4)
		  , [&](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(), browser::Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(), browser::Browser::_defaulthome);});
...
boost::intrusive_ptr<TreeItem> tv_t::new_item(boost::intrusive_ptr<TreeIndex> _modelindex// std::function<KnowModel *()> _current_model, QModelIndex _current_index
	...
	if(ok && ! _name.isEmpty()){											// return nullptr;    // Если была нажата отмена
		...
		while(it.hasNext()){
		    ...
		    if(_it->id() == result->id()){																													// prevent error merge
			// boost::intrusive_ptr<TreeIndex> tree_index = [&] {boost::intrusive_ptr<TreeIndex> tree_index; try {tree_index = new TreeIndex(_current_model, result); } catch(std::exception &e) {throw e; } return tree_index; } ();
			result = TreeLevel::instance(TreeIndex::instance(_current_model, result), _it)->merge();
		    }
		}
	    }else{
...
//	    if(current_count > 0)result = host_parent;																					// _current_model()->item(_current_parent_index);    // _blank_header;
		...
		// return il == result->linker() && il->host() == result && result->parent() == il->host_parent();
		// })); } catch(std::exception &e) {throw e; } return tree_index; } ();
		select_as_current(TreeIndex::instance([&] {return _know_root;}, result[0]));																							// setto
		// selectionModel()->select(setto, current_tree_selection_mode);
		// selectionModel()->setCurrentIndex(setto, current_tree_current_index_mode);   // ClearAndSelect
...
boost::intrusive_ptr<TreeItem> tv_t::move(boost::intrusive_ptr<TreeIndex>       _treeindex	// std::function<KnowModel *()> _current_model, QModelIndex _current_index
    ...
    auto paste_sibling_impl = [&](boost::intrusive_ptr<TreeIndex>       _modelindex
		...
		if(_alternative_items.size() > 0){																													// && same->is_empty()
			...
			result = TreeLevel::instance(TreeIndex::instance(current_model, result), it.next())->merge();
			assert(result);
			assert(current_model()->item([=](boost::intrusive_ptr<const TreeItem> t){return t->id() == result->id();}));
		    }
			// boost::intrusive_ptr<TreeIndex> tree_index = [&] {boost::intrusive_ptr<TreeIndex> tree_index; try{tree_index = new TreeIndex(current_model, result); } catch(std::exception &e) {throw e; } return tree_index; } ();
		    result = TreeLevel::instance(TreeIndex::instance(current_model, result), _source_item)->merge();
		    assert(result);
		    if(result){
			assert(current_model()->item([=](boost::intrusive_ptr<const TreeItem> t){return t->id() == result->id();}));
		    }
			...
			// Вставка новых данных в модель дерева записей
		    result = TreeLevel::instance(TreeIndex::instance(current_model, current_parent), _source_item)->move();	// static_cast<tkm_t *>(current_model())// TreeIndex(current_model, current_parent, _modelindex.sibling_order())
...
//		know_model_save();
		...
		select_as_current(TreeIndex::instance(current_model, result));
		assert(
		    (_source_item == result) ||
...
//    assert(host_index.isValid() ? current_model()->item(host_index) != nullptr : true);
	...
	if(! host->contains_direct(_source_item->linker())){											// current_item->contains_direct(_source_item->linker()) ||
		...
		result = paste_sibling(TreeIndex::instance(current_model, _child), _source_item, _substitute_condition);									// _current_model, _current_model()->index(current_item->item_direct(0))
		assert(result);
	    }else{
		// Получение ссылки на узел, который соответствует выделенной строке
		// auto current_item = _current_model->item(_current_index);
...
//		    QString id = get_unical_id();
		...
		// Вставка новых данных в модель дерева записей
		result = TreeLevel::instance(TreeIndex::instance(current_model, host), _source_item)->move();
		assert(result);
		assert(result != _know_model_board->root_item());
		// Установка курсора на только что созданную позицию
		// QModelIndex
		...
		// return il == result->linker() && il->host() == result && result->parent() == il->host_parent();
		// })); } catch(std::exception &e) {throw e; } return tree_index; } ();
		select_as_current(TreeIndex::instance(current_model, result));																						// setto
		// selectionModel()->select(setto, current_tree_selection_mode);
		// selectionModel()->setCurrentIndex(setto, current_tree_current_index_mode);   // ClearAndSelect
		// А можно было установить курсор на нужную позицию и так
		// knowTreeView->selectionModel()->setCurrentIndex(kntrmodel->index(item->childCount()-1,0,index),
	...
	if(static_cast<QModelIndex>(_know_root->index(_item_common_parent)).isValid()){					// _item_common_parent->field("name") != clipboard_items_root
	    ...
	    for(auto it : _items)
			...
			if(it->parent() == _item_common_parent){																							// if(index.isValid() && index.parent() == _index_common_parent) {
			    if(it->id() == _current_model()->session_id())_current_model()->session_id(TreeIndex::instance(_current_model, _item_common_parent));////        keep = source; remove = target;
		...
		if(enable_delete){
			...
			if(it->id() == static_cast<tkm_t *>(_current_model())->session_id())static_cast<tkm_t *>(_current_model())->session_id(TreeIndex::instance(_current_model, it->parent()));
...
//                                    select_as_current(TreeIndex::instance(_current_model, left_sibling_item->parent(), left_sibling_item));
			...
			select_as_current(TreeIndex::instance(_current_model, left_sibling_item));
...
QModelIndexList tv_t::copy(void){	// const
	...
	_sub_branch->branch_push(TreeIndex::instance(_source_model, current_item()), items);											// , _clipboard_root_parent_id
...
//	}
	if(result){
		...
		if(_to_be_operated->id() == static_cast<tkm_t *>(_current_model())->session_id())static_cast<tkm_t *>(_current_model())->session_id(TreeIndex::instance(_current_model, result));
		globalparameters.main_window()->setDisabled(true);
