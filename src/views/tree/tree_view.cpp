...
browser::WebView* tv_t::index_invoke(boost::intrusive_ptr<TreeIndex> _tree_index)
{ // browser::WebView *view, const index_tree &_index
    ...
    if (static_cast<QModelIndex>(_index).isValid()) { // && index_current() != _index
            ...
            if (duplicated_items_list_on_board_by_url.size() > 1) {
                for (auto _i : duplicated_items_list_on_board_by_url) {
                    ...
                    if (duplicated_item != result_item)
                        result_item = TreeLevel::instance(TreeIndex::create_treeindex_from_item(_source_model, result_item), duplicated_item)->merge(); // (duplicated_item->id() != result_item->id()) ||
        ...
        if (!gl_paras->main_window()->find([&](boost::intrusive_ptr<const ::Binder> b) { // !result_item->is_activated()
            ...
            if (rs)
                rs->editing_window()->save_text_context();
            //
            ...
            if (_parent && !(_to_be_operated->parent() == _parent && _parent->contains_direct(_to_be_operated->linker()))) {
                ...
                if (!_linker) {
                    ...
                    if (_linker->host() != _to_be_operated)
                        _linker->host(std::forward<boost::intrusive_ptr<TreeItem> >(_to_be_operated)); // std::move(boost::intrusive_ptr<TreeItem>(this))
            ...
            for (auto it : _items)
                ...
                if (it->parent() == _item_common_parent) { // if(index.isValid() && index.parent() == _index_common_parent) {
                    if (it->id() == _current_model()->session_id())
                        _current_model()->session_id(TreeIndex::create_treeindex_from_item(_current_model, _item_common_parent)); ////        keep = source; remove = target;
                ...
                if (enable_delete) {
                    ...
                    for (auto it : items_candidate) { // for(int i = 0; i < _result.size(); ++i) {
                        if (it->id() == static_cast<tkm_t*>(_current_model())->session_id())
                            static_cast<tkm_t*>(_current_model())->session_id(TreeIndex::create_treeindex_from_item(_current_model, it->parent()));
        ...
        if (result) {
            if (result != _to_be_operated) { // && target_on_tree
                ...
                if (session_id_changed)
                    static_cast<tkm_t*>(_current_model())->session_id(TreeIndex::create_treeindex_from_item(_current_model, result));
...
std::pair<boost::intrusive_ptr<TreeItem>, boost::intrusive_ptr<TreeItem> > tv_t::cursor_focus(boost::intrusive_ptr<TreeItem> to_be_contained)
...
boost::intrusive_ptr<TreeItem> tv_t::intercept(boost::intrusive_ptr<TreeItem> result)
