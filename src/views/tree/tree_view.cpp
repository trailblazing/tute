...
tv_t::tv_t(QString name, ts_t *tree_screen)
	...
	if(appconfig->interface_mode() == "desktop") connect(selectionModel(), &QItemSelectionModel::currentRowChanged, this, &tv_t::on_current_row_changed); // &TreeScreen::step_into_sub_branch
	// connect(this, &KnowView::pressed, this, &KnowView::on_pressed);
	if(appconfig->interface_mode() == "mobile") connect(this, &tv_t::clicked, this, [&](const QModelIndex &index){cursor_focus(source_model()->item(index));}); // (index_tree(index));
	if(appconfig->interface_mode() == "desktop"){
		connect(this, &tv_t::clicked, this, [&](const QModelIndex &index){auto _item = _know_root->child(index); if(_item) index_invoke(TreeIndex::require_treeindex([&] {return _know_root;}, _item)); }); // globalparameters.main_window()->vtab_record()->activated_browser()->tabmanager()->currentWebView(),
		...
		connect(this, &tv_t::doubleClicked, this, [&](const QModelIndex &index){cursor_focus(source_model()->item(index));}); // (index_tree(index));	// index_invoke_in_sub_branch
	}
}
...
// boost::intrusive_ptr<TreeItem> branch_item = TreeItem::dangle_instance(data);	// boost::intrusive_ptr<TreeItem>(new TreeItem(nullptr, data));							// _know_root->root_item()

// bool modified = false;
// for(auto &browser : _entrance->browsers()){
// auto tabmanager = browser->tabmanager();																						// record_controller()->source_model();  // ->record_table();
// for(int i = 0; i < tabmanager->count(); i ++){
// auto page_item = tabmanager->webView(i)->page()->host();	// should never occur in current implement, 2016-07-10
...
//// if(tree_index) {
// paste_children_from_children(TreeIndex::instance([&]() -> tkm_t * {return _know_root;}, tree_view->session_root_auto(), tree_view->session_root_auto()->parent())// tree_index
// , branch_item
...
bool tv_t::gestureEvent(QGestureEvent *event){
	...
	if(QGesture *gesture = event->gesture(Qt::TapAndHoldGesture)) tapandhold_gesture_triggered(static_cast<QTapAndHoldGesture *>(gesture));
	return true;
}
...
void tv_t::tapandhold_gesture_triggered(QTapAndHoldGesture *gesture){
	...
	if(gesture->state() == Qt::GestureFinished)
		if(gl_paras->target_os() == "android") emit tapandhold_gesture_finished(mapFromGlobal(gesture->position().toPoint()));
}

void tv_t::mousePressEvent(QMouseEvent *event){ // Q_DECL_OVERRIDE
	...
	if(mouse_button == Qt::RightButton){
		...
		if(next_index.isValid()){
			...
			select_as_current(TreeIndex::require_treeindex([&] {return _know_root;}, it));
	...
	if(is_dragable(event)){
		...
		auto check_crypt = [&](boost::intrusive_ptr<TreeItem> tree_item_drag) -> bool {
					   ...
					   if(tree_item_drag != tree_item_drop){
						   ...
						   if(tree_item_drag->field<crypt_type>() != "1" && tree_item_drop->field<crypt_type>() == "1" && gl_paras->crypt_key().length() == 0){
		...
		for(int i = 0; i < clipboard_records->size(); i++){
			...
			auto item_on_tree = _know_root->child([&](boost::intrusive_ptr<const TreeItem> it){return it->id() == tree_item_drag->id();});
			assert(item_on_tree);
			if(item_on_tree) tree_item_drag = item_on_tree;
			rctrl_t *_record_controller = tree_item_drag->page()->record_controller(); // find_object<RecordController>("table_screen_controller"); // Указатель на контроллер таблицы конечных записей

			web::TabWidget *_tabmanager = tree_item_drag->page()->view()->tabmanager();
			if(check_crypt(tree_item_drag)){
				...
				if(tree_item_drag->binder()) _tabmanager->closeTab(_tabmanager->indexOf(tree_item_drag->page()->view())); // _record_controller->remove_child(record->field("id"));
...
// find_object<MetaEditor>(meta_editor_singleton_name)
				...
				TreeLevel::instance(TreeIndex::require_treeindex([&](){return _know_root;}, tree_item_drop), tree_item_drag)->move(); // _know_root// tree_item_drop->child_insert(0, record, ADD_NEW_RECORD_TO_END);
...
boost::intrusive_ptr<TreeItem> tv_t::session_root_auto(){
	...
	auto find_root = [&]() -> boost::intrusive_ptr<TreeItem> {
				 ...
				 if(index_list.size() == 0){
					 ...
					 while(_know_root->root_item()->count_direct() == 0)
						 cursor_focus();
					 if(_know_root->root_item()->count_direct() == 0) it = tree_empty_controll();
					 else it = _know_root->root_item()->child_direct(0);
					 items << it;
				 }else
					 for(auto &_i : index_list) items << _know_root->child(_i);
				 assert(items.size() > 0);
				 ...
				 for(auto it : items){
					 if(final_common_parent != it && !TreeIndex::is_ancestor_of(final_common_parent, it)){
						 if(TreeIndex::is_ancestor_of(it, final_common_parent)) final_common_parent = it;
						 else{
							 ...
							 while(near_root->parent()){
								 ...
								 if(TreeIndex::is_ancestor_of(near_root, it) && TreeIndex::is_ancestor_of(near_root, final_common_parent)) break;
							 }
						 }
					 }
				 }
	...
	auto view_session_id_defined = [&](){return _current_model()->item([=](boost::intrusive_ptr<const TreeItem> t){return t->id() == _current_model()->session_id();});};

	auto board_session_id_defined = [&](){return _know_model_board->session_id() != "" && _know_model_board->child([=](boost::intrusive_ptr<const TreeItem> t){return t->id() == _know_model_board->session_id();});};
	...
	auto fix_root = [&] {
		...
		_current_model()->session_id(TreeIndex::require_treeindex(_current_model, find_root()));
		know_model_save();
		assert(view_session_id_defined());
	};
	if(!board_session_id_defined())	fix_root(); // if manual setted, cancel this
	if(!view_session_id_defined()) fix_root(); // while(!view_session_id_defined() && _current_model()->root_item() != know_model_board()->root_item()) {
...
void tv_t::session_root_manual(bool checked){
	...
	_source_model()->session_id(TreeIndex::require_treeindex(_source_model, it));
}

boost::intrusive_ptr<TreeItem> tv_t::current_item(){
	...
	if(index_result.isValid()) item_result = _know_root->child(index_result); // selectionModel()->currentIndex()
		...
		if(!item_result && selectionModel()->selectedIndexes().size() > 0){
			...
			// if(tree_index)
			index_result = select_as_current(TreeIndex::require_treeindex([&] {return _know_root;}, item_result));
		}
		if(!index_result.isValid()){
			...
			if(item_result == _know_model_board->root_item()){  // !result.isValid()
				...
				if(_know_root->root_item()->count_direct() == 0){
					...
					item_result = TreeIndex::require_treeindex([&]() -> tkm_t * {return _know_root;}, new TreeItem(item_result))
						      ->bind(QUrl(web::Browser::_defaulthome), std::bind(&tv_t::move, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(), web::Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(), web::Browser::_defaulthome);});
...
QModelIndex tv_t::current_index(void) const {
	...
	// if(view_){
	// item_result = view_->page()->host();
	// if(item_result) result = _know_root->index(item_result);																																																																																																																					// select_as_current(TreeIndex::instance([&](){return _know_root;}, item_result));
	// }
	// }
...
// }else _item = _know_root->root_item()->child(_know_root->root_item()->current_count() - 1);
// assert(_item);
...
// if(appconfig->interface_mode() == "mobile") emit this->clicked(_result);	// QModelIndex selIdx = recordSourceModel->index(pos, 0);
...
//// In response to the mobile version of the record is no choice (not processed signal line change to the selection model)
//// Therefore, the recording must be made a virtual click to fill the final table of records
// if(appconfig->getInterfaceMode() == "mobile") emit this->clicked(_result);	// QModelIndex selIdx=recordSourceModel->index(pos, 0);
...
QModelIndex tv_t::select_as_current(boost::intrusive_ptr<TreeIndex> _tree_index, select_strategy _select_strategy, current_strategy _current_strategy){
	...
	//// _record_controller->cursor_to_index(_record_controller->index<PosProxy>(_item));                                                                                                                                                         // IdType(it->field("id"))
	//// }
	// return record_controller->synchronize_record_view(_item);
	//// return c->cursor_to_index(c->index<PosProxy>(_item));
	// } () : nullptr : nullptr : nullptr;
	if(_host_index.isValid() && _host_index != selectionModel()->currentIndex()){  // _index_current //
		// int pos = index.row();
...
////    assert(_result == selectionModel()->currentIndex());
		...
		// In response to the mobile version of the record is no choice (not processed signal line change to the selection model)
		// Therefore, the recording must be made a virtual click to fill the final table of records
		if(appconfig->interface_mode() == "mobile") emit this->clicked(_host_index); // QModelIndex selIdx = recordSourceModel->index(pos, 0);

#endif

		// emit this->clicked(index);
	}else if(!_host_index.isValid()) _result = selectionModel()->currentIndex();
...
web::WebView *tv_t::index_invoke(boost::intrusive_ptr<TreeIndex> _tree_index){ // web::WebView *view, const index_tree &_index
	...
	//
//	_host->activate(std::bind(&wn_t::find, gl_paras->main_window(), std::placeholders::_1));
//	v = _page->view();

	auto browser_ = _page ? _page->browser() : gl_paras->main_window()->browser(_host, true); //_host
	web::WebView *activated_view = browser_->currentTab(); // globalparameters.main_window()->vtab_record()->find([&](boost::intrusive_ptr<const ::Binder> b) -> bool {return url_equal((b->host()->field<home_type>()).toStdString(), _host->field<home_type>().toStdString()) || url_equal((b->host()->field<url_type>()).toStdString(), _host->field<url_type>().toStdString());});;
	...
	if(static_cast<QModelIndex>(_index).isValid()){  // && index_current() != _index
		...
		auto merge_by_url
			= [&](){
				  auto duplicated_items_list_on_board_by_url = _source_model()->children([&](boost::intrusive_ptr<const TreeItem> it){return (url_equal(it->field<home_type>().toStdString(), result_item->field<home_type>().toStdString())) && (it->field<home_type>() != web::Browser::_defaulthome);}); // source_model()
				  if(duplicated_items_list_on_board_by_url.size() > 1){
					  for(auto _i : duplicated_items_list_on_board_by_url){
						  auto duplicated_item = _i;
						  if(duplicated_item != result_item) result_item = TreeLevel::instance(TreeIndex::require_treeindex(_source_model, result_item), duplicated_item)->merge(); // (duplicated_item->id() != result_item->id()) ||
		...
		auto duplicated_items_list_on_board_by_id = _source_model()->children([&](boost::intrusive_ptr<const TreeItem> it){return it->id() == result_item->id();});
		...
		if(!gl_paras->main_window()->find([&](boost::intrusive_ptr<const ::Binder> b){  // !result_item->is_activated()
			...
			if(result_item->count_direct() > 0){  // && (result_item->field("url") != "" || result_item->field("url") == "" )
				...
				if(result_item->field<crypt_type>() == "1"){
			...
			// else {      //        if(result_item->field("url") != web::Browser::_defaulthome)
			////        if(result_item->field("url") != "") globalparameters.entrance()->activate(result_item);
			//// Переключаются окна (используется для мобильного интерфейса)
			////        globalparameters.window_switcher()->switchFromTreeToRecordtable();
			// }
			if(result_item->field<home_type>() == "" && result_item->field<url_type>() != "") result_item->field<home_type>(result_item->field<url_type>());
			if(result_item->field<home_type>() != ""){
				...
				//				auto browser_nearest = gl_paras->main_window()->activated_browser();
				...
				if(record_previous_item != result_item){
					boost::intrusive_ptr<RecordIndex> record_index = RecordIndex::instance([&] {return browser_->tabmanager()->record_controller()->source_model();}, result_item); // record_previous_item,

					auto browser_bind_activate
						= [&](boost::intrusive_ptr<RecordIndex> _record_index) -> web::WebView * {
							  return browser_->bind(_record_index)->activate(std::bind(&wn_t::find, gl_paras->main_window(), std::placeholders::_1)); // item_bind_();
						  };
...
void tv_t::update_selected_indexes(void){
	...
	for(int i = 0; i < selectitems.size(); ++i) update(selectitems.at(i));
}
...
tv_t::LocalLizeInitializer::LocalLizeInitializer(tv_t *_tree_view)
	...
	if(-1 == _tree_view->selectionModel()->selectedIndexes().indexOf(_tree_view->current_index())) throw std::runtime_error(formatter() << "! locallize invalid index");
...
QItemSelection tv_t::LocalLizeInitializer::operator ()(){
				      ...
				      if(_origin_index_list.size() > 0){
					      ...
					      if(!_origin_index_list.contains(_tree_view->current_index())){  // ?
						      ...
						      if(found && duplicated_item && found != duplicated_item){
							      found = static_cast<tkm_t *>(_source_model())->merge(TreeLevel::instance(TreeIndex::require_treeindex(_source_model, found), duplicated_item), std::bind(&tv_t::delete_permanent, _tree_view, std::placeholders::_1, std::placeholders::_2, &tkm_t::delete_permanent, "cut", false) // , std::bind(&tv_t::delete_permanent, _tree_view, std::placeholders::_1, std::placeholders::_2, &tkm_t::delete_permanent_recursive, "cut", false)
														  );
						      }
					      }
					      std::set<QModelIndex> indexes;
					      for(auto idx : _origin_index_list)
						      if(indexes.find(idx) == indexes.end()) indexes.insert(idx);
					      QModelIndexList new_list;
					      for(auto i : indexes) new_list << i;
					      // preparations
	...
	auto remove_multi_columns = [&](const QModelIndexList _origin_index_list) -> QModelIndexList {
					    QModelIndexList _index_list;
					    std::set<int> row_list;
					    for(auto idx : _origin_index_list)
						    if(row_list.find(idx.row()) == row_list.end()){
							    row_list.insert(idx.row());
							    if(idx.column() == 0) _index_list << idx;
						    }
					    return _index_list;
				    };
...
void tv_t::expand_selected(bool expand){
	...
	QModelIndexList selectitems = selectionModel()->selectedIndexes();
	for(int i = 0; i < selectitems.size(); ++i) expand_or_collapse_recurse(selectitems.at(i), expand);
}

// template<>
boost::intrusive_ptr<TreeItem> tv_t::new_child(boost::intrusive_ptr<TreeIndex> _modelindex // std::function<KnowModel *()> _current_model, QModelIndex _current_index
	...
	if(_alternative_items.size() > 0){
		...
		while(it.hasNext()){
			...
			if(_it->id() == result->id()){  // prevent error merge
				...
				result = TreeLevel::instance(TreeIndex::require_treeindex(_current_model, result), _it)->merge(); // TreeIndex::instance(_current_model, result, result->parent()), _it);
			}
		}
	...
	// })); } catch(std::exception &e) {throw e; } return tree_index; } ();
	select_as_current(TreeIndex::require_treeindex([&] {return _know_root;}, result));
...
boost::intrusive_ptr<TreeItem> tv_t::tree_empty_controll(void){
	...
	if(_know_model_board->rowCount() == 0){
		...
		setModel(_know_root);
		cursor_focus(it);
		// new_child(TreeIndex::instance([&]() -> km_t * {return source_model();}, new TreeItem(it), it), tr("Rename me")																				// TreeIndex([&]()->KnowModel* { return source_model();}, source_model()->root_item())
		// , std::bind(&km_t::new_child, _know_root, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3)						// std::placeholders::_1    // std::placeholders::_3

		// );
		result = TreeIndex::require_treeindex([&]() -> tkm_t * {return _know_root;}, new TreeItem(it))
			 ->bind(QUrl(web::Browser::_defaulthome), std::bind(&tv_t::move, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(), web::Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(), web::Browser::_defaulthome);});
...
boost::intrusive_ptr<TreeItem> tv_t::new_item(boost::intrusive_ptr<TreeIndex> _modelindex // std::function<KnowModel *()> _current_model, QModelIndex _current_index
					     , const tv_t::add_new_delegate &_branch_add_new, const tv_t::add_new &_branch_add_new_impl
	...
	if(static_cast<QModelIndex>(_target_index).isValid()){
		...
		if(ok && !_name.isEmpty()){  // return nullptr;    // Если была нажата отмена
			auto target_item = _modelindex->host(); // _current_model()->item(_current_index);
			auto target_parent = target_item->parent();
			QList<boost::intrusive_ptr<TreeItem> > _alternative_items;
			if(target_parent) _alternative_items = target_parent->children_direct([&](boost::intrusive_ptr<const Linker> il){return il->host()->name() == _name;});
			// auto _name_same_no_child_first = [&] {boost::intrusive_ptr<TreeItem> result; for(auto i : _items_with_same_name) {if(i->count_direct() == 0) {result = i; break;}} return result;}();
			if(_alternative_items.size() > 0){
				// result = _name_same_no_child_first ? _name_same_no_child_first : _items_with_same_name[0];
				...
				while(it.hasNext()){
					...
					if(_it->id() == result->id()){  // prevent error merge
						// boost::intrusive_ptr<TreeIndex> tree_index = [&] {boost::intrusive_ptr<TreeIndex> tree_index; try {tree_index = new TreeIndex(_current_model, result); } catch(std::exception &e) {throw e; } return tree_index; } ();
						result = TreeLevel::instance(TreeIndex::require_treeindex(_current_model, result), _it)->merge();
					}
				}
			}else{
				...
				// KnowModel *(KnowView::*_source_model_func)() = &KnowView::source_model;
				// auto _source_model = [&]() {return source_model();}; // std::bind(_source_model_func, _tree_view);
				result = _branch_add_new(_modelindex, _name // source_model()->item(_current_index)->name()
							,
...
QList<boost::intrusive_ptr<TreeItem> > tv_t::move_children(boost::intrusive_ptr<TreeIndex> _parent_modelindex // std::function<KnowModel *()> _current_model, QModelIndex _current_index
							  , boost::intrusive_ptr<TreeItem> _blank_header, const tv_t::substitute_condition_double &_substitute_condition){
	auto _current_model = _parent_modelindex->current_model();
	auto host_index = _parent_modelindex->host_index();
	// auto					host_parent	= _parent_modelindex->host()->parent();
	...
	auto view = static_cast<tv_t *>(static_cast<QObject *>(_current_model())->parent());
	assert(view);
	if(static_cast<QModelIndex>(_current_model()->index(_blank_header)).isValid() && _current_model()->index(_blank_header) != _current_model()->index(host) && _substitute_condition(_blank_header->linker(), host->linker())) TreeLevel::instance(_parent_modelindex, _blank_header)->merge(); // && _know_root->index(_blank_header).isValid()   // _blank_header->field("name") != clipboard_items_root
	...
	if(_blank_header->count_direct() > 0){
		...
		if(static_cast<QModelIndex>(host_index).isValid()){
			...
			_blank_header->traverse_direct([&](boost::intrusive_ptr<Linker> il){  // != reserved
							       ...
							       if(candidate){
								       // {// time consuming
								       // auto found_item = _current_model()->item([=](boost::intrusive_ptr<const TreeItem> t) -> bool {return t->name() == candidate->name() && t->id() == candidate->id();});
								       //// Подветка
								       // if(found_item)found_item = TreeLevel::instance(TreeIndex::instance(_current_model, found_item), candidate)->merge();	// candidate->id()
								       ...
								       // auto r = move(_parent_modelindex, found_item, std::bind(_substitute_condition, std::placeholders::_1, found_item->linker()));
								       // }
								       auto r = move(_parent_modelindex, candidate, std::bind(_substitute_condition, std::placeholders::_1, candidate->linker())); //
			...
			// if(current_count > 0)result = host_parent;																					// _current_model()->item(_current_parent_index);    // _blank_header;
			if(result.size() > 0){  // && _blank_header->field("name") != clipboard_items_root
				// _current_model->endInsertRows();

				// Установка курсора на только что созданную позицию
				// QModelIndex
				setto = _current_model()->index(std::bind(_substitute_condition, std::placeholders::_1, result[0]->linker())); // [&](boost::intrusive_ptr<const TreeItem> it)->bool{return it->id() == begin_item->id();}
				...
				// return il == result->linker() && il->host() == result && result->parent() == il->host_parent();
				// })); } catch(std::exception &e) {throw e; } return tree_index; } ();
				select_as_current(TreeIndex::require_treeindex([&] {return _know_root;}, result[0])); // setto
				// selectionModel()->select(setto, current_tree_selection_mode);
				// selectionModel()->setCurrentIndex(setto, current_tree_current_index_mode);   // ClearAndSelect
				...
				gl_paras->main_window()->setEnabled(true);
				// return
				auto result_check = _current_model()->item([=](boost::intrusive_ptr<const TreeItem> it){return it->id() == result[0]->id();}); // setto
...
void tv_t::paste_clipboard(boost::intrusive_ptr<TreeIndex> _sibling_tree_index){ // std::function<QString(const QModelIndex &, ClipboardBranch *)>  _model_paste_from_clipboard
	...
	auto paste_children_from_clipboard = [](boost::intrusive_ptr<TreeItem> _blank_header, ClipboardBranch *_sub_branch // , int _sibling_order
						     ...
						     auto move_as_child_from_clipboard_object = [](boost::intrusive_ptr<TreeItem> _parent, boost::intrusive_ptr<TreeItem> _to_be_operated, int _pos, int _mode = add_new_record_after) -> boost::intrusive_ptr<TreeItem> {
													...
													if(_parent && !(_to_be_operated->parent() == _parent && _parent->contains_direct(_to_be_operated->linker()))){
														...
														assert(!_parent->path_list().contains(_to_be_operated->field<id_type>()));
														...
														if(!_linker){
															...
															auto link_result = _linker->parent(_parent, _pos, _mode); // _linker->host_parent()->release(this->linker());
															assert(link_result == _linker);
															assert(_linker->integrity_external(_to_be_operated, _parent));
														}
						     ...
						     child_assembly_recursive
							     = [&](boost::intrusive_ptr<TreeItem> _current_clip_parent_parent, boost::intrusive_ptr<TreeItem> _current_clip_parent // , const QString &_current_clip_records_parent_id
								  ,
								   ...
								   // , const int                       _sibling_order
								       ...
								       if(_current_clip_parent_parent) contain_current_parent = _current_clip_parent_parent->contains_direct(_current_clip_parent->linker());
								       else contain_current_parent = _current_clip_parent;
						     ...
						     _blank_header->field<id_type>(_sub_branch->clip_blank_header_id()); // "id",
						     _result = child_assembly_recursive(nullptr, _blank_header, _sub_branch); // , _clip_blank_header_id // *id_iterator
	...
	if(!(mime_data->hasFormat(QString(gl_para::_program_instance_name) + "/branch"))) return;
	////    // Получение списка индексов QModelIndex выделенных элементов
	////    QModelIndexList selectitems = selectionModel()->selectedIndexes();
	// QModelIndex _current_index;
	////    // Если выбрано более одной ветки
	...
	if(static_cast<QModelIndex>(host_index).isValid()){
		...
		boost::intrusive_ptr<TreeItem> blank_header(TreeItem::dangle_instance(data)); // new TreeItem(nullptr, data)
		...
		blank_header->traverse_direct([&](boost::intrusive_ptr<Linker> il_in_clipboard){
						      auto index = static_cast<tkm_t *>(_current_model())->index([&](boost::intrusive_ptr<const Linker> link_in_tree){return link_in_tree->host()->id() == il_in_clipboard->host()->id();});
						      if(static_cast<QModelIndex>(index).isValid()){
							      ...
							      if(static_cast<QModelIndex>(index).parent().isValid()){
								      ...
								      if(  (static_cast<tkm_t *>(_current_model())->child([&](boost::intrusive_ptr<const TreeItem> t){return t->id() == parent_item->id();})) // if(it_on_tree->up_linker()->host_parent()->field("name") == clipboard_items_root)it_on_tree->parent(parent_item);
		...
		if(_result){
			...
			if(host_parent_index.isValid())	_current_model()->update_index(index_tree(host_parent_index));
			...
			// find_object<MainWindow>("mainwindow")
			gl_paras->main_window()->set_tree_position(source_model()->root_item()->id(), pasted_branch_path);
...
boost::intrusive_ptr<TreeItem> tv_t::move(boost::intrusive_ptr<TreeIndex> _treeindex // std::function<KnowModel *()> _current_model, QModelIndex _current_index
					 , boost::intrusive_ptr<TreeItem> _source_item, const tv_t::substitute_condition &_substitute_condition, bool save_immediately){
	//
	auto paste_sibling_impl = [&](boost::intrusive_ptr<TreeIndex> _modelindex, boost::intrusive_ptr<TreeItem> _source_item, const tv_t::substitute_condition &substitute_condition_) -> boost::intrusive_ptr<TreeItem> { // ->boost::intrusive_ptr<TreeItem>
					  boost::intrusive_ptr<TreeItem> result(nullptr);
					  ...
					  if(current_parent && current_parent->id() != ""){
						  ...
						  // return il == host->linker() && il->host() == host && host->parent() == il->host_parent();
						  // }));
						  // } catch(std::exception &e) {throw e; }
						  ...
						  gl_paras->main_window()->setDisabled(true);
						  // Получение уникального идентификатора
						  if(((QString) _source_item->id()).length() == 0) _source_item->field<id_type>(_source_item->field<dir_type>().length() > 0 ? _source_item->field<dir_type>() : get_unical_id());
						  // QModelIndex setto;
						  ...
						  if(_alternative_items.size() > 0){  // && same->is_empty()
							  ...
							  while(it.hasNext()){
								  ...
								  result = TreeLevel::instance(TreeIndex::require_treeindex(current_model, result), it.next())->merge();
								  assert(result);
								  assert(current_model()->item([=](boost::intrusive_ptr<const TreeItem> t){return t->id() == result->id();}));
							  }
							  // boost::intrusive_ptr<TreeIndex> tree_index = [&] {boost::intrusive_ptr<TreeIndex> tree_index; try{tree_index = new TreeIndex(current_model, result); } catch(std::exception &e) {throw e; } return tree_index; } ();
							  result = TreeLevel::instance(TreeIndex::require_treeindex(current_model, result), _source_item)->merge();
							  assert(result);
							  if(result){
								  assert(current_model()->item([=](boost::intrusive_ptr<const TreeItem> t){return t->id() == result->id();}));
							  }
							  ...
							  // Вставка новых данных в модель дерева записей
							  result = TreeLevel::instance(TreeIndex::require_treeindex(current_model, current_parent), _source_item)->move(); // static_cast<tkm_t *>(current_model())// TreeIndex(current_model, current_parent, _modelindex.sibling_order())
						  ...
						  select_as_current(TreeIndex::require_treeindex(current_model, result));
						  assert(
							  (_source_item == result) || substitute_condition_(result->linker()) // (_source_item->name() == result->name())
							);
	...
	auto paste_sibling = [&](boost::intrusive_ptr<TreeIndex> _modelindex, boost::intrusive_ptr<TreeItem> _source_item, const tv_t::substitute_condition &substitute_condition_) -> boost::intrusive_ptr<TreeItem> {
				     auto host = _modelindex->host(); // _current_model()->item(_current_index);
	...
	if(host && _source_item){  // current_index.isValid()
		...
		if(!host->contains_direct(_source_item->linker())){  // current_item->contains_direct(_source_item->linker()) ||
			if(host->count_direct() > 0){
				auto _child = host->child_direct(0); // _treeindex->sibling_order()   // current_item->count_direct() - 1
				...
				result = paste_sibling(TreeIndex::require_treeindex(current_model, _child), _source_item, _substitute_condition); // _current_model, _current_model()->index(current_item->item_direct(0))
				assert(result);
			}else{
				// Получение ссылки на узел, который соответствует выделенной строке
				// auto current_item = _current_model->item(_current_index);
				...
				// find_object<MainWindow>("mainwindow")
				gl_paras->main_window()->setDisabled(true);
				if(((QString) _source_item->id()).length() == 0) _source_item->field<id_type>(_source_item->field<dir_type>().length() > 0 ? _source_item->field<dir_type>() : get_unical_id()); // Получение уникального идентификатора
				// QString id = get_unical_id();
				...
				// Вставка новых данных в модель дерева записей
				result = TreeLevel::instance(TreeIndex::require_treeindex(current_model, host), _source_item)->move();
				assert(result);
				assert(result != _know_model_board->root_item());
				// Установка курсора на только что созданную позицию
				// QModelIndex
				...
				// return il == result->linker() && il->host() == result && result->parent() == il->host_parent();
				// })); } catch(std::exception &e) {throw e; } return tree_index; } ();
				select_as_current(TreeIndex::require_treeindex(current_model, result)); // setto
				// selectionModel()->select(setto, current_tree_selection_mode);
				// selectionModel()->setCurrentIndex(setto, current_tree_current_index_mode);   // ClearAndSelect
				// А можно было установить курсор на нужную позицию и так
				// knowTreeView->selectionModel()->setCurrentIndex(kntrmodel->index(item->childCount()-1,0,index),
...
QList<boost::intrusive_ptr<TreeItem> > tv_t::delete_permanent(const std::function<tkm_t *()> &_current_model, QList<boost::intrusive_ptr<TreeItem> > _items, delete_strategy _delete_strategy, const QString &_mode, const bool _cut_branch_confirm){
	QList<boost::intrusive_ptr<TreeItem> > items_candidate;
	QList<boost::intrusive_ptr<TreeItem> > result_items;
...
//		auto current_browser = page_ ? page_->browser() : gl_paras->main_window()->browser(_items[0]);
		...
		has_encrypt
			= [&](QList<boost::intrusive_ptr<TreeItem> > _result){
				  ...
				  for(auto it : _result){  // for(int i = 0; i < _index_list.size(); ++i) {
					  ...
					  QList<QStringList> cryptFlagsList = it->path_children_all(boost::mpl::c_str<crypt_type>::value);
		...
		std::sort(_items.begin(), _items.end(), [&](boost::intrusive_ptr<TreeItem> t0, boost::intrusive_ptr<TreeItem> t1){return static_cast<QModelIndex>(_current_model()->index(t0)).row() < static_cast<QModelIndex>(_current_model()->index(t1)).row();});
		...
		if(static_cast<QModelIndex>(_know_root->index(_item_common_parent)).isValid()){  // _item_common_parent->field("name") != clipboard_items_root
			auto deleted_position_first = _item_common_parent->sibling_order([&](boost::intrusive_ptr<const Linker> il){return il->host()->id() == _item_to_be_deleted_first->id() && _item_to_be_deleted_first->linker() == il && _item_to_be_deleted_first->parent() == il->host_parent();}); // items_update_selected();
			// auto deleted_position_last = _source_model->item(index_to_be_delete_last)->sibling_order();
			...
			for(auto it : _items)
				...
				if(it->parent() == _item_common_parent){  // if(index.isValid() && index.parent() == _index_common_parent) {
					if(it->id() == _current_model()->session_id()) _current_model()->session_id(TreeIndex::require_treeindex(_current_model, _item_common_parent)); ////        keep = source; remove = target;
			...
			if(!((gl_paras->crypt_key().size() == 0) && has_encrypt(items_candidate))){
				...
				if(enable_question){
					...
					message_box.exec();
					if(message_box.clickedButton() == delete_button) enable_delete = true;
					else enable_delete = false;
				}
				...
				if(enable_delete){
					...
					for(int i = 0; i < items_candidate.size(); ++i)	qDebug() << items_candidate.at(i)->path_list(); // (_current_model()->item(  //))
					// auto _source_model = source_model();
					...
					for(auto it : items_candidate){  // for(int i = 0; i < _result.size(); ++i) {
						if(it->id() == static_cast<tkm_t *>(_current_model())->session_id()) static_cast<tkm_t *>(_current_model())->session_id(TreeIndex::require_treeindex(_current_model, it->parent()));
						if(it->linker()->host() && it->linker()->host_parent()){
							auto result = (static_cast<tkm_t *>(_current_model())->*_delete_strategy)(it->linker());
							// just for counting, result.get() == 0 is fine
							result_items.append(result); // model_delete_one( // just for counting, 0 is fine
					...
					if(result_items.size() > 0){
						...
						auto get_left_sibling_from_tree
							= [&]() -> boost::intrusive_ptr<TreeItem> {
								  boost::intrusive_ptr<TreeItem> left_sibling_(nullptr);
								  ...
								  if(_item_common_parent && static_cast<QModelIndex>(_know_root->index(_item_common_parent)).isValid()){  // _item_common_parent->field("name") != clipboard_items_root
									  ...
									  if(new_count > 0){
										  int new_position = 0;
										  if((deleted_position_first - 1) >= 0)	new_position = deleted_position_first - 1;
										  else new_position = deleted_position_first;
										  setto = _current_model()->index(new_position, 0, static_cast<QModelIndex>(index_to_be_delete_first).parent());
										  left_sibling_ = _item_common_parent->child_direct(new_position);
										  ...
										  while(_item_common_parent && _item_common_parent != _know_model_board->root_item()){
											  if(_item_common_parent != _current_model()->root_item() && static_cast<QModelIndex>(_current_model()->index(_item_common_parent)).isValid()){
												  ...
												  if(_item_common_parent){
													  ...
													  tree_empty_controll();
													  left_sibling_ = source_model()->root_item()->child_direct(0);
													  setto = _current_model()->index(left_sibling_);
												  }
											  }
										  }
						...
						if(gl_paras->main_window()->record_screens().size() > 0){
							//							web::Browser *current_browser = gl_paras->main_window()->activated_browser();
							auto w	= gl_paras->vtab_record()->currentWidget();
							auto rs = static_cast<rs_t *>(w);
							auto tabmanager = rs->browser()->tabmanager();//current_browser->tabmanager();
							...
							if(good_list.size() > 0){
								...
								assert(setto.isValid());
							}else left_sibling_item = get_left_sibling_from_tree();
						}else left_sibling_item = get_left_sibling_from_tree();
						select_as_current(TreeIndex::require_treeindex(_current_model, left_sibling_item));
...
void tv_t::cut(bool _cut_branch_confirm){
	...
	if(index_list.size() > 0){  // copy_result
		delete_permanent([&](){return source_model();}, [&](){QList<boost::intrusive_ptr<TreeItem> > r; for(auto ix : index_list) r << source_model()->child(ix); return r;} (), &tkm_t::delete_permanent_recursive, "cut", _cut_branch_confirm);
...
QModelIndexList tv_t::copy(void){ // const
				  // bool copy_result = false;
				  // auto _tree_screen = static_cast<TreeScreen *>(parent());
	...
	if(_selectitems.size() >= 1){
		...
		if(indexs_valid) _index_current = static_cast<tkm_t *>(_source_model())->index(items[0]); // _selectitems[0];
	}
	...
	if(indexs_valid){  // _index_current.isValid()
		...
		// if(_know_model_board->item(_index_current)->field(boost::mpl::c_str < crypt_type > ::value) == "1")   //_current_item_absolute_path
		...
		// item->item_direct(i)->field(boost::mpl::c_str < crypt_type > ::value) == "1") {
		// encrypt_presence = true;
		// } else {
		// nocrypt_presence = true;
		// }
		...
		_sub_branch->branch_push(TreeIndex::require_treeindex(_source_model, current_item()), items); // , _clipboard_root_parent_id
...
void tv_t::modify(void){
	...
	if(_current_index.isValid()){
		...
		if(item->field<crypt_type>() == "1" && gl_paras->crypt_key().length() == 0) return;
		// Получение имени ветки
		QString name = item->field<name_type>();

		// Создается окно ввода данных
		bool ok;
		QString newname = QInputDialog::getText(this, tr("Edit item name"), tr("Item name:"), QLineEdit::Normal, name, &ok);
...
boost::intrusive_ptr<TreeItem> tv_t::merge(boost::intrusive_ptr<TreeLevel> _tree_merge){
	boost::intrusive_ptr<TreeIndex> tree_index = _tree_merge->tree_index(); // const std::function<KnowModel *()> &_current_model, boost::intrusive_ptr<TreeItem> target
	boost::intrusive_ptr<TreeItem> _to_be_operated = _tree_merge->to_be_operated();
	boost::intrusive_ptr<TreeItem> result = tree_index->host();
	if(_to_be_operated){
		auto _current_model = tree_index->current_model();
		//// auto target = modelindex->host();   // host_parent();
		////    auto source = source;	// _current_model()->item([=](boost::intrusive_ptr<const TreeItem> t){return t->id() == source->id();});
		// if(source->is_ancestor_of([&](boost::intrusive_ptr<const TreeItem> it_){return it_ == result;})){
		// auto temp = result;
		// result = source;	// paste_sibling(TreeIndex::instance(_current_model, source, source->parent()), result, [&](boost::intrusive_ptr<const Linker> il){return il->host()->id() == result->id();});
		// source = temp;
		...
		if(result){
			if(result != _to_be_operated){  // && target_on_tree
				...
				bool _ro_be_operated_is_current_of_rocord_view = false;
				if(_to_be_operated->id() == static_cast<tkm_t *>(_current_model())->session_id()) session_id_changed = true;
				...
				if(_to_be_operated->binder()){
					...
					if(p){
						auto v = p->view();
						auto r_ctrl = p->record_controller();
						if(v && r_ctrl){
							auto rv = r_ctrl->view();
							if(rv)
								if(rv->current_item() == _to_be_operated) _ro_be_operated_is_current_of_rocord_view = true;
						}
					}
				}
				gl_paras->main_window()->setDisabled(true);
				...
				assert(_current_model()->item([=](boost::intrusive_ptr<const TreeItem> t){return t->id() == result->id();}));
				if(session_id_changed) static_cast<tkm_t *>(_current_model())->session_id(TreeIndex::require_treeindex(_current_model, result));
				if(_ro_be_operated_is_current_of_rocord_view)
					if(r_ctrl) r_ctrl->select_as_current(r_ctrl->index<pos_proxy>(_to_be_operated));
				know_model_save();
...
void tv_t::on_current_row_changed(const QModelIndex &current, const QModelIndex &previous){
	...
	// }
	if(previous.isValid() && _previous_index != previous) _previous_index = previous;
	   ...
	   !globalparameters.main_window()->vtab_record()->find([&](boost::intrusive_ptr<const ::Binder> b) -> bool {
									...
									if(ref_it->binder()){
										if(url_equal(b->host()->field<home_type>().toStdString(), ref_it->field<home_type>().toStdString())){
											if(ref_it->binder()->page())
												if(ref_it->binder()->page()->view()->load_finished()) result = true;
										}
									}
									return result;
								})){
		...
		if(selected_item->count_direct() > 0){  // && (result_item->field("url") != "" || result_item->field("url") == "" )
			...
			if(selected_item->field<crypt_type>() == "1"){
				...
				if(globalparameters.crypt_key().length() == 0){
					...
					if(password.retrievePassword() == false){
						...
						auto parent = TreeItem::dangle_instance(QMap<QString, QString>()); // boost::intrusive_ptr<TreeItem>(new TreeItem(nullptr, QMap<QString, QString>()));
						auto host = boost::intrusive_ptr<TreeItem>(new TreeItem(parent, QMap<QString, QString>()));
						intercept(host); // TreeIndex::instance(_soure_model, host, parent)// _know_branch->clear();  // root_item(boost::intrusive_ptr<TreeItem>(new TreeItem(QMap<QString, QString>(), nullptr)));
			...
			if(_soure_model()->root_item() != selected_item){
				...
				intercept(selected_item); // TreeIndex::instance(_soure_model, selected_item, selected_item->parent())
...
std::pair<boost::intrusive_ptr<TreeItem>, boost::intrusive_ptr<TreeItem> > tv_t::cursor_focus(boost::intrusive_ptr<TreeItem> to_be_contained){ // _new_session_root_item
	...
	if(!current_root_ && absolute_root_) intercept(absolute_root_); // be merged?
	...
	//
	auto cursor_follow_up_impl
		= [&](void) -> std::pair<boost::intrusive_ptr<TreeItem>, boost::intrusive_ptr<TreeItem> > { // std::pair<boost::intrusive_ptr<TreeItem>, boost::intrusive_ptr<TreeItem> > tv_t::cursor_follow_up(void){
			  ...
			  _check_crypt_conflict
				  = [&](boost::intrusive_ptr<const TreeItem> current_root){
					    bool _result = false;
					    // Вначале все инструменты работы с веткой включаются
					    QMapIterator<QString, QAction *> i(_tree_screen->_actionlist);
					    while(i.hasNext()){
			  ...
			  auto root_up_input = [&](boost::intrusive_ptr<TreeItem> &root_item){
						       assert(root_item);
						       boost::intrusive_ptr<TreeItem> result(root_item);
						       ...
						       if(root_item && !_check_crypt_conflict(root_item)){  // && _root_item->parent() != _know_model_board->root_item()
							       ...
							       if(!TreeIndex::is_ancestor_of(current_item_->parent(), root_item)) current_item_ = current_item_->parent(); // if(! current_item_->parent()->path_list().contains(root_item->field<id_type>()))
							       select_as_current(TreeIndex::require_treeindex([&] {return _know_root;}, current_item_));
			  ...
			  if(current_item_){
				  ...
				  if(parent_ && parent_ != current_root_ && !TreeIndex::is_ancestor_of(parent_, current_root_)){  // current_item_->parent()->path_list().size() > root_to_update->path_list().size()
					  current = parent_;

					  select_as_current(TreeIndex::require_treeindex([&] {return _know_root;}, current));
				  }else if(parent_ == current_root_ && current_root_) current_root_ = root_up_input(current_root_);
			  }else if(current_root_->count_direct() == current_root_->count_children_all() && current_root_) current_root_ = root_up_input(current_root_);
	...
	if(!to_be_contained) result = cursor_follow_up_impl();
		...
		while(current_root_ && !TreeIndex::is_ancestor_of(current_root_, to_be_contained)){  // while(TreeIndex::is_ancestor_of([&] {return _know_root;}, to_be_contained, current_item_)){		// current_root// && _new_session_root_item->id() != _current_item->id()		// current_root->id()
			// reset();
		...
		if(current_item_){
			...
			select_as_current(TreeIndex::require_treeindex(source_model_, current_item_));
		}
		_tree_screen->enable_up_action();
...
boost::intrusive_ptr<TreeItem> tv_t::intercept(boost::intrusive_ptr<TreeItem> result){ // boost::intrusive_ptr<TreeIndex> modelindex){   // QString id
										       ...
										       // auto result = modelindex->host();					// _item;    // _know_model_board->item([&](boost::intrusive_ptr<const TreeItem> it) {return it->id() == id;});     // _know_model_board->index(KnowModel::delegater(id))
	...
	reset();
	setModel(_know_root);
	if(result != _know_model_board->root_item()) result = cursor_focus(result).first;
	return result;
}

void tv_t::synchronized(bool _synchronized){_know_model_board->synchronized(_synchronized);}

bool tv_t::synchronized(){return _know_model_board->synchronized();}
...
// setDropIndicatorShown(true);
// if(guiConfigGroup.readEntry("selected_delegate", "compact") == QLatin1String("compact"))setItemDelegate(d->compactDelegate);
...
// void ContactListWidget:: requestFileTransferChannels(const Tp::AccountPtr &account, const Tp::ContactPtr &contact, const QStringList &filenames, const QDateTime &userActionTime){
...
// if(groupsConfig.readEntry(index.data(AccountsModel::IdRole).toString(), false))expand(index);
// }
// }
// }
...
// if(! index.data(AccountsModel::ItemRole).canConvert<ContactModelItem *>())return;
...
// if((event->pos() - d->dragStartPosition).manhattanLength() < QApplication::startDragDistance())return;
...
// else event->ignore();
// }else if(event->mimeData()->hasFormat("application/vnd.telepathy.contact"))event->acceptProposedAction();
// else event->ignore();
// }
