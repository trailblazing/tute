...
HtmlDelegate::HtmlDelegate(tv_t *_tree_view) : _tree_view(_tree_view){}
...
void tv_t::tapandhold_gesture_triggered(QTapAndHoldGesture *gesture){
    ...
    if(gesture->state() == Qt::GestureFinished)
            if(globalparameters.target_os() == "android") emit tapandhold_gesture_finished(mapFromGlobal(gesture->position().toPoint()));
}
    ...
    if(is_dragable(event)){
        ...
        auto check_crypt = [&](boost::intrusive_ptr<TreeItem> tree_item_drag) -> bool {
                ...
                if(tree_item_drag != tree_item_drop){
                    ...
                    if(tree_item_drag->field<crypt_type>() != "1" &&
                        tree_item_drop->field<crypt_type>() == "1" &&
...
boost::intrusive_ptr<TreeItem> tv_t::session_root_auto(){
    ...
    auto	find_root		= [&]() -> boost::intrusive_ptr<TreeItem> {
            ...
            if(index_list.size() == 0){
                ...
                while(_know_root->root_item()->count_direct() == 0) cursor_focus();
                if(_know_root->root_item()->count_direct() == 0) it = tree_empty_controll();else it = _know_root->root_item()->child_direct(0);
                items << it;
            }else for(auto &_i : index_list) items << _know_root->child(_i);assert(items.size() > 0);
            ...
            auto find_shortest_list = [&]() -> QStringList {
                    ...
                    for(auto &list : items_path_list) if(list.size() < result.size()) result = list;
                    return result;
                };
            ...
            if(item_result == _know_model_board->root_item()){																		// !result.isValid()
                ...
                if(_know_root->root_item()->count_direct() == 0){
                    ...
                    item_result = TreeIndex::create_treeindex_from_item([&]() -> tkm_t * {return _know_root;}, new TreeItem(item_result))
                        ->bind(QUrl(browser::Browser::_defaulthome)
...
QItemSelection tv_t::LocalLizeInitializer::operator ()(){
                ...
                if(! _origin_index_list.contains(_tree_view->current_index())){											// ?
                    ...
                    if(found && duplicated_item && found != duplicated_item){
                        found = static_cast<tkm_t *>(_source_model())->merge(TreeLevel::instance(TreeIndex::create_treeindex_from_item(_source_model, found), duplicated_item)
                                                                            , std::bind(&tv_t::delete_permanent, _tree_view, std::placeholders::_1, std::placeholders::_2, &tkm_t::delete_permanent, "cut", false)	// , std::bind(&tv_t::delete_permanent, _tree_view, std::placeholders::_1, std::placeholders::_2, &tkm_t::delete_permanent_recursive, "cut", false)
                                );
                    }
                }
    ...
    auto remove_multi_columns = [&](const QModelIndexList _origin_index_list) -> QModelIndexList {
            ...
            for(auto idx : _origin_index_list)
                    if(row_list.find(idx.row()) == row_list.end()){row_list.insert(idx.row());if(idx.column() == 0) _index_list << idx;}
            return _index_list;
        };
...
//			result = _source_item->parent(_parent, _pos, _mode)->host();
    ...
    if(! (mime_data->hasFormat("mytetra/branch"))) return;
	////    // Получение списка индексов QModelIndex выделенных элементов
	////    QModelIndexList selectitems = selectionModel()->selectedIndexes();
	// QModelIndex _current_index;
	////    // Если выбрано более одной ветки
...
QList<boost::intrusive_ptr<TreeItem> > tv_t::delete_permanent(const std::function<tkm_t *()>       &_current_model
...
void tv_t::cut(bool _cut_branch_confirm){
    ...
    if(index_list.size() > 0){						// copy_result
        delete_permanent([&](){return source_model();}
                        , [&](){QList<boost::intrusive_ptr<TreeItem> > r;for(auto ix : index_list) r << source_model()->child(ix);return r;} ()
