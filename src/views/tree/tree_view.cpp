...
void HtmlDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    if (0 == index_item_->count_direct() && !(optionV4.state & QStyle::State_Selected))
    ...
    , _delegate(new HtmlDelegate(this))
{
    ...
    if (appconfig->interface_mode() == "desktop")
        connect(selectionModel(), &QItemSelectionModel::currentRowChanged, this, &tv_t::on_current_row_changed); // &TreeScreen::step_into_sub_branch
    // connect(this, &KnowView::pressed, this, &KnowView::on_pressed);
    if (appconfig->interface_mode() == "mobile")
        connect(this, &tv_t::clicked, this, [&](const QModelIndex& index) { cursor_focus(source_model()->item(index)); }); // (index_tree(index));
    if (appconfig->interface_mode() == "desktop") {
...
void tv_t::tapandhold_gesture_triggered(QTapAndHoldGesture* gesture)
{
    ...
    if (gesture->state() == Qt::GestureFinished)
        if (gl_paras->target_os() == "android")
            emit tapandhold_gesture_finished(mapFromGlobal(gesture->position().toPoint()));
}
...
void tv_t::dropEvent(QDropEvent* event)
{
    ...
    if (is_dragable(event)) {
        ...
        for (int i = 0; i < clipboard_records->size(); i++) {
            ...
            auto item_on_tree = _know_root->child([&](boost::intrusive_ptr<const i_t> it) { return it->id() == tree_item_drag->id(); });
            assert(item_on_tree);
            if (item_on_tree)
...
boost::intrusive_ptr<i_t> tv_t::session_root_auto()
{
            ...
            if (final_common_parent != it && !TreeIndex::is_ancestor_of(final_common_parent, it)) {
                if (TreeIndex::is_ancestor_of(it, final_common_parent))
                    ...
                    while (near_root->parent()) {
                        ...
                        if (TreeIndex::is_ancestor_of(near_root, it) && TreeIndex::is_ancestor_of(near_root, final_common_parent))
                            break;
                    }
                }
            }
    ...
    auto view_session_id_defined = [&]() { return _current_model()->item([=](boost::intrusive_ptr<const i_t> t) { return t->id() == _current_model()->session_id(); }); };

    auto board_session_id_defined = [&]() { return _know_model_board->session_id() != "" && _know_model_board->child([=](boost::intrusive_ptr<const i_t> t) { return t->id() == _know_model_board->session_id(); }); };
    ...
    if (!view_session_id_defined())
        fix_root(); // while(!view_session_id_defined() && _current_model()->root_item() != know_model_board()->root_item()) {
...
boost::intrusive_ptr<i_t> tv_t::current_item()
{
    ...
    if (index_result.isValid())
        item_result = _know_root->child(index_result); // selectionModel()->currentIndex()
    else {
        ...
        if (!index_result.isValid()) {
            ...
            if (item_result == _know_model_board->root_item()) { // !result.isValid()
                ...
                if (_know_root->root_item()->count_direct() == 0) {
                    ...
                    item_result = TreeIndex::require_treeindex([&]() -> tkm_t* { return _know_root; }, new i_t(item_result))
                                      ->bind(QUrl(web::Browser::_defaulthome), std::bind(&tv_t::move, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), web::Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(), web::Browser::_defaulthome); });
...
// }
    ...
    if (_item) {
        ...
        if (v) {
            auto p = v->page();
            if (p) {
                if (p->tabmanager()->current_item() != _item)
                    p->tabmanager()->select_as_current(v);
...
{ // web::WebView *view, const index_tree &_index
    ...
    if (static_cast<QModelIndex>(_index).isValid()) { // && index_current() != _index
        ...
        auto merge_by_url
            = [&]() {
                  auto duplicated_items_list_on_board_by_url = _source_model()->children([&](boost::intrusive_ptr<const i_t> it) { return (url_equal(it->field<home_type>().toStdString(), result_item->field<home_type>().toStdString())) && (it->field<home_type>() != web::Browser::_defaulthome); }); // source_model()
                  if (duplicated_items_list_on_board_by_url.size() > 1) {
                      for (auto _i : duplicated_items_list_on_board_by_url) {
                          ...
                          if (duplicated_item != result_item)
                              result_item = TreeLevel::instance(TreeIndex::require_treeindex(_source_model, result_item), duplicated_item)->merge(); // (duplicated_item->id() != result_item->id()) ||
        ...
        auto duplicated_items_list_on_board_by_id = _source_model()->children([&](boost::intrusive_ptr<const i_t> it) { return it->id() == result_item->id(); });
        ...
        if (!gl_paras->main_window()->find([&](boost::intrusive_ptr<const ::Binder> b) { // !result_item->is_activated()
            ...
            // }
            if (result_item->field<home_type>() == "" && result_item->field<url_type>() != "")
                result_item->field<home_type>(result_item->field<url_type>());
            ...
            if (v) {
                ...
                if (index != tabmanager->currentIndex())
                    tabmanager->setCurrentIndex(index); // c->index<PosSource>(c->source_model()->index(_binder->item()))
            }
        }
        if (v)
...
boost::intrusive_ptr<i_t> tv_t::tree_empty_controll(void)
{
    ...
    if (_know_model_board->rowCount() == 0) {
        ...
        // );
        result = TreeIndex::require_treeindex([&]() -> tkm_t* { return _know_root; }, new i_t(it))
                     ->bind(QUrl(web::Browser::_defaulthome), std::bind(&tv_t::move, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), web::Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(), web::Browser::_defaulthome); });
    } else
...
boost::intrusive_ptr<i_t> tv_t::new_item(boost::intrusive_ptr<TreeIndex> _modelindex // std::function<KnowModel *()> _current_model, QModelIndex _current_index
    ,
...
{ // bool insert_sibling_branch
    ...
    if (static_cast<QModelIndex>(_target_index).isValid()) {
        ...
        if (ok && !_name.isEmpty()) { // return nullptr;    // Если была нажата отмена
            ...
            QList<boost::intrusive_ptr<i_t> > _alternative_items;
            if (target_parent)
                _alternative_items = target_parent->children_direct([&](boost::intrusive_ptr<const Linker> il) { return il->host()->name() == _name; });
            // auto _name_same_no_child_first = [&] {boost::intrusive_ptr<TreeItem> result; for(auto i : _items_with_same_name) {if(i->count_direct() == 0) {result = i; break;}} return result;}();
            if (_alternative_items.size() > 0) {
                // result = _name_same_no_child_first ? _name_same_no_child_first : _items_with_same_name[0];
...
QList<boost::intrusive_ptr<i_t> > tv_t::move_children(boost::intrusive_ptr<TreeIndex> _parent_modelindex // std::function<KnowModel *()> _current_model, QModelIndex _current_index
    ,
    boost::intrusive_ptr<i_t> _blank_header, const tv_t::substitute_condition_double& _substitute_condition)
{
    ...
    assert(view);
    if (static_cast<QModelIndex>(_current_model()->index(_blank_header)).isValid() && _current_model()->index(_blank_header) != _current_model()->index(host) && _substitute_condition(_blank_header->linker(), host->linker()))
        TreeLevel::instance(_parent_modelindex, _blank_header)->merge(); // && _know_root->index(_blank_header).isValid()   // _blank_header->field("name") != clipboard_items_root
    ...
    if (_blank_header->count_direct() > 0) {
        ...
        if (static_cast<QModelIndex>(host_index).isValid()) {
            ...
            if (result.size() > 0) { // && _blank_header->field("name") != clipboard_items_root
                ...
                gl_paras->main_window()->setEnabled(true);
                // return
                auto result_check = _current_model()->item([=](boost::intrusive_ptr<const i_t> it) { return it->id() == result[0]->id(); }); // setto
...
void tv_t::paste_clipboard(boost::intrusive_ptr<TreeIndex> _sibling_tree_index)
{ // std::function<QString(const QModelIndex &, ClipboardBranch *)>  _model_paste_from_clipboard
    ...
    auto paste_children_from_clipboard
        = [](boost::intrusive_ptr<i_t> _blank_header, ClipboardBranch* _sub_branch // , int _sibling_order
        ...
        auto move_as_child_from_clipboard_object
            = [](boost::intrusive_ptr<i_t> _parent, boost::intrusive_ptr<i_t> _to_be_operated, int _pos, int _mode = add_new_record_after) -> boost::intrusive_ptr<i_t> {
            ...
            if (_parent && !(_to_be_operated->parent() == _parent && _parent->contains_direct(_to_be_operated->linker()))) {
                ...
                if (!_linker) {
                    ...
                    if (parent && parent != _parent)
                        parent->release([&](boost::intrusive_ptr<const Linker> il) { return il->host()->id() == _to_be_operated->id() && il == _linker; });
        ...
        std::function<boost::intrusive_ptr<i_t>(boost::intrusive_ptr<i_t>, boost::intrusive_ptr<i_t>, ClipboardBranch*)> // , const int// , const QString &
            child_assembly_recursive
            = [&](boost::intrusive_ptr<i_t> _current_clip_parent_parent, boost::intrusive_ptr<i_t> _current_clip_parent // , const QString &_current_clip_records_parent_id
                ,
            ...
            if (contain_current_parent) {
                ...
                QList<boost::intrusive_ptr<i_t> > records = _sub_branch->records_by_parent_id(_current_clip_parent->id());
    ...
    if (static_cast<QModelIndex>(host_index).isValid()) {
        ...
        boost::fusion::at_key<id_type>(data) = get_unical_id();
        boost::fusion::at_key<name_type>(data) = clipboard_items_root;
        boost::fusion::at_key<ctime_type>(data) = ctime_;
        boost::fusion::at_key<dir_type>(data) = boost::fusion::at_key<id_type>(data);
        boost::fusion::at_key<file_type>(data) = "text.html";

        boost::intrusive_ptr<i_t> blank_header(i_t::dangle_instance(data)); // new TreeItem(nullptr, data)
        ...
        blank_header->traverse_direct([&](boost::intrusive_ptr<Linker> il_in_clipboard) {
            ...
            if (static_cast<QModelIndex>(index).isValid()) {
                ...
                if (static_cast<QModelIndex>(index).parent().isValid()) {
                    ...
                    if ((static_cast<tkm_t*>(_current_model())->child([&](boost::intrusive_ptr<const i_t> t) { return t->id() == parent_item->id(); })) // if(it_on_tree->up_linker()->host_parent()->field("name") == clipboard_items_root)it_on_tree->parent(parent_item);
        ...
        if (_result) {
            ...
            auto pasted_branch_item = _current_model()->item([=](boost::intrusive_ptr<const i_t> it) { return it->id() == pasted_branch_id; });
...
boost::intrusive_ptr<i_t> tv_t::move(boost::intrusive_ptr<TreeIndex> _treeindex // std::function<KnowModel *()> _current_model, QModelIndex _current_index
    ,
    boost::intrusive_ptr<i_t> _source_item, const tv_t::substitute_condition& _substitute_condition, bool save_immediately)
{
    //
    auto paste_sibling_impl = [&](boost::intrusive_ptr<TreeIndex> _modelindex, boost::intrusive_ptr<i_t> _source_item, const tv_t::substitute_condition& substitute_condition_) -> boost::intrusive_ptr<i_t> { // ->boost::intrusive_ptr<TreeItem>
        boost::intrusive_ptr<i_t> result(nullptr);
        ...
        if (current_parent && current_parent->id() != "") {
            ...
            gl_paras->main_window()->setDisabled(true);
            // Получение уникального идентификатора
            if (((QString)_source_item->id()).length() == 0)
                _source_item->field<id_type>(_source_item->field<dir_type>().length() > 0 ? _source_item->field<dir_type>() : get_unical_id());
            // QModelIndex setto;
            ...
            if (_alternative_items.size() > 0) { // && same->is_empty()
                ...
                while (it.hasNext()) {
                    ...
                    assert(result);
                    assert(current_model()->item([=](boost::intrusive_ptr<const i_t> t) { return t->id() == result->id(); }));
                }
                // boost::intrusive_ptr<TreeIndex> tree_index = [&] {boost::intrusive_ptr<TreeIndex> tree_index; try{tree_index = new TreeIndex(current_model, result); } catch(std::exception &e) {throw e; } return tree_index; } ();
    ...
    auto paste_sibling = [&](boost::intrusive_ptr<TreeIndex> _modelindex, boost::intrusive_ptr<i_t> _source_item, const tv_t::substitute_condition& substitute_condition_) -> boost::intrusive_ptr<i_t> {
        auto host = _modelindex->host(); // _current_model()->item(_current_index);
    ...
    if (host && _source_item) { // current_index.isValid()
        ...
        if (!host->contains_direct(_source_item->linker())) { // current_item->contains_direct(_source_item->linker()) ||
            if (host->count_direct() > 0) {
                ...
                gl_paras->main_window()->setDisabled(true);
                if (((QString)_source_item->id()).length() == 0)
                    _source_item->field<id_type>(_source_item->field<dir_type>().length() > 0 ? _source_item->field<dir_type>() : get_unical_id()); // Получение уникального идентификатора
                // QString id = get_unical_id();
                ...
                gl_paras->main_window()->setEnabled(true);
                // return
                result = current_model()->item([=](boost::intrusive_ptr<const i_t> t) { return t->id() == result->id(); });
...
QList<boost::intrusive_ptr<i_t> > tv_t::delete_permanent(const std::function<tkm_t*()>& _current_model, QList<boost::intrusive_ptr<i_t> > _items, delete_strategy _delete_strategy, const QString& _mode, const bool _cut_branch_confirm)
{
    ...
    if (_items.size() > 0) {
        ...
        std::function<bool(QList<boost::intrusive_ptr<i_t> >)>
            has_encrypt
        ...
        std::sort(_items.begin(), _items.end(), [&](boost::intrusive_ptr<i_t> t0, boost::intrusive_ptr<i_t> t1) { return static_cast<QModelIndex>(_current_model()->index(t0)).row() < static_cast<QModelIndex>(_current_model()->index(t1)).row(); });
            ...
            for (auto it : _items)
                ...
                if (it->parent() == _item_common_parent) { // if(index.isValid() && index.parent() == _index_common_parent) {
                    if (it->id() == _current_model()->session_id())
                        _current_model()->session_id(TreeIndex::require_treeindex(_current_model, _item_common_parent)); ////        keep = source; remove = target;
            ...
            if (!((gl_paras->crypt_key().size() == 0) && has_encrypt(items_candidate))) {
                ...
                if (enable_delete) {
                    ...
                    for (auto it : items_candidate) { // for(int i = 0; i < _result.size(); ++i) {
                        if (it->id() == static_cast<tkm_t*>(_current_model())->session_id())
                            static_cast<tkm_t*>(_current_model())->session_id(TreeIndex::require_treeindex(_current_model, it->parent()));
...
void tv_t::cut(bool _cut_branch_confirm)
{
    ...
    if (index_list.size() > 0) { // copy_result
        delete_permanent([&]() { return source_model(); }, [&]() {QList<boost::intrusive_ptr<i_t> > r; for(auto ix : index_list) r << source_model()->child(ix); return r; }(), &tkm_t::delete_permanent_recursive, "cut", _cut_branch_confirm);
...
{ // const
    ...
    if (_selectitems.size() >= 1) {
        ...
        if (indexs_valid)
            _index_current = static_cast<tkm_t*>(_source_model())->index(items[0]); // _selectitems[0];
    }
...
void tv_t::modify(void)
{
    ...
    if (_current_index.isValid()) {
        ...
        if (item->field<crypt_type>() == "1" && gl_paras->crypt_key().length() == 0)
            return;
...
boost::intrusive_ptr<i_t> tv_t::merge(boost::intrusive_ptr<TreeLevel> _tree_merge)
{
    ...
    if (_to_be_operated) {
        ...
        if (result) {
            if (result != _to_be_operated) { // && target_on_tree
                ...
                if (_to_be_operated->id() == static_cast<tkm_t*>(_current_model())->session_id())
                        ...
                        if (v && r_ctrl) {
                            ...
                            if (rv)
                                if (rv->current_item() == _to_be_operated)
                                    _ro_be_operated_is_current_of_rocord_view = true;
                        }
                    }
                }
                gl_paras->main_window()->setDisabled(true);
...
std::pair<boost::intrusive_ptr<i_t>, boost::intrusive_ptr<i_t> > tv_t::cursor_focus(boost::intrusive_ptr<i_t> to_be_contained)
{ // _new_session_root_item
    ...
    //
    std::pair<boost::intrusive_ptr<i_t>, boost::intrusive_ptr<i_t> > result{ current_root_, current_item_ };
    //
    auto cursor_follow_up_impl
        = [&](void) -> std::pair<boost::intrusive_ptr<i_t>, boost::intrusive_ptr<i_t> > { // std::pair<boost::intrusive_ptr<TreeItem>, boost::intrusive_ptr<TreeItem> > tv_t::cursor_follow_up(void){
        // boost::intrusive_ptr<TreeItem>		current_root_(nullptr);
        boost::intrusive_ptr<i_t> current(nullptr);
        // auto _tree_screen = static_cast<TreeScreen *>(parent());
        std::function<bool(boost::intrusive_ptr<const i_t>)>
        ...
        auto root_up_input = [&](boost::intrusive_ptr<i_t>& root_item) {
            ...
            if (root_item && !_check_crypt_conflict(root_item)) { // && _root_item->parent() != _know_model_board->root_item()
                ...
                if (0 == root_item->count_direct())
                    tree_empty_controll(); // boost::intrusive_ptr<TreeIndex> tree_absolute_index = [&] {boost::intrusive_ptr<TreeIndex> tree_index; try{tree_index = new TreeIndex(current_model, _root_up); } catch(std::exception &e) {throw e; } return tree_index; } ();
                ...
                if (!TreeIndex::is_ancestor_of(current_item_->parent(), root_item))
                    current_item_ = current_item_->parent(); // if(! current_item_->parent()->path_list().contains(root_item->field<id_type>()))
        ...
        } else if (current_root_->count_direct() == current_root_->count_children_all() && current_root_)
            current_root_ = root_up_input(current_root_);
...
boost::intrusive_ptr<i_t> tv_t::intercept(boost::intrusive_ptr<i_t> result)
{ // boost::intrusive_ptr<TreeIndex> modelindex){   // QString id
