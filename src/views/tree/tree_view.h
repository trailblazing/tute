...
public:
    typedef std::function<bool(boost::intrusive_ptr<const i_t>, const QUrl&)> equal_url_t;
    ...
    typedef std::function<boost::intrusive_ptr<i_t>(boost::intrusive_ptr<TreeIndex>, boost::intrusive_ptr<i_t>, const substitute_condition&)> paste_strategy;
    typedef std::function<boost::intrusive_ptr<i_t>(boost::intrusive_ptr<TreeIndex>, QString, QString)> add_new;
    typedef std::function<boost::intrusive_ptr<i_t>(boost::intrusive_ptr<TreeIndex>, QString, const add_new&)> add_new_delegate;
    ...
    typedef boost::intrusive_ptr<i_t> (tkm_t::*delete_strategy)(boost::intrusive_ptr<Linker>);
...
signals:
    ...
    boost::intrusive_ptr<i_t> new_child(boost::intrusive_ptr<TreeIndex> _modelindex, QString _name, const add_new& _branch_add_new_impl);

    boost::intrusive_ptr<i_t> new_item(boost::intrusive_ptr<TreeIndex> _modelindex, const add_new_delegate& _branch_add_new, const add_new& _branch_add_new_impl);

    QList<boost::intrusive_ptr<i_t> > move_children(boost::intrusive_ptr<TreeIndex> _parent_modelindex, boost::intrusive_ptr<i_t> _blank_header, const substitute_condition_double& _substitute_condition);

    void paste_clipboard(boost::intrusive_ptr<TreeIndex> _sibling_modelindex);

    boost::intrusive_ptr<i_t> move(boost::intrusive_ptr<TreeIndex> _treeindex, boost::intrusive_ptr<i_t> _source_item, const substitute_condition& _substitute_condition, bool save_immediately = false);
    ...
    QList<boost::intrusive_ptr<i_t> > delete_permanent(const std::function<tkm_t*()>& _current_model, QList<boost::intrusive_ptr<i_t> > _items, delete_strategy _delete_strategy, const QString& _mode = "cut", const bool _cut_branch_confirm = false);

    void cut(bool _cut_branch_confirm = false);
    QModelIndexList copy(void); // const;
    void modify(void);
    boost::intrusive_ptr<i_t> merge(boost::intrusive_ptr<TreeLevel> _tree_merge); // boost::intrusive_ptr<TreeIndex> tree_index, boost::intrusive_ptr<TreeItem> source);
    ...
    std::pair<boost::intrusive_ptr<i_t>, boost::intrusive_ptr<i_t> > cursor_focus(boost::intrusive_ptr<i_t> to_be_contained = nullptr);
