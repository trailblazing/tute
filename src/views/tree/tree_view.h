...
class tv_t : public QTreeView {
	...
	public:
	typedef std::function<bool(boost::intrusive_ptr<const i_t>, const QUrl &)>
	    equal_url_t;
	typedef std::function<bool(boost::intrusive_ptr<const i_t>, boost::intrusive_ptr<const i_t>)>
	    equal_t;
	typedef std::function<bool(boost::intrusive_ptr<const Linker>)>
	    substitute_condition;  // , boost::intrusive_ptr<const Linker>
	typedef std::function<bool(boost::intrusive_ptr<const Linker>,
	                           boost::intrusive_ptr<const Linker>)>
	    substitute_condition_double;  //
	typedef std::function<boost::intrusive_ptr<i_t>(
	    boost::intrusive_ptr<TreeIndex>, boost::intrusive_ptr<i_t>,
	    const substitute_condition &)>
	    paste_strategy;
	typedef std::function<boost::intrusive_ptr<i_t>(
	    boost::intrusive_ptr<TreeIndex>, QString, QString)>
	    add_new;
	typedef std::function<boost::intrusive_ptr<i_t>(
	    boost::intrusive_ptr<TreeIndex>, QString, const add_new &)>
	    add_new_delegate;
	typedef std::function<QModelIndex(tv_t *const, const QModelIndex &)>
	    select_strategy;
	typedef std::function<QModelIndex(tv_t *const, const QModelIndex &, const int)>
	    current_strategy;
	typedef boost::intrusive_ptr<i_t> (tkm_t::*delete_strategy)(
...
private:
	...
	explicit tv_t(QString name, ts_t *tree_screen);  // QWidget *_parent = 0
	...
	QModelIndex select_as_current(
	    boost::intrusive_ptr<TreeIndex> _tree_index,
	    select_strategy _select_strategy =
	        [](tv_t *const v, const QModelIndex &_i) -> QModelIndex {
		    assert(v);
		    assert(v->selectionModel());
		    assert(_i.isValid());
		    v->selectionModel()->select(_i, current_tree_selection_mode);
		    return _i;
		},
	    current_strategy _current_strategy =
	        [](tv_t *const v, const QModelIndex &_i,
	           const int dummy = 0) -> QModelIndex {
		    (void)(dummy);
		    assert(v);
		    assert(v->selectionModel());
		    assert(_i.isValid());
		    v->selectionModel()->setCurrentIndex(_i, current_tree_current_index_mode);
		    return _i;
		});

	// QModelIndex select_as_current_item(boost::intrusive_ptr<TreeItem> _item
	// , select_strategy _select_strategy = [](KnowView *const v, const
	// QModelIndex &_i) ->QModelIndex{assert(v); assert(v->selectionModel());
	// assert(_i.isValid()); v->selectionModel()->select(_i,
	// current_tree_selection_mode); return _i;}
	// , current_strategy _current_strategy = [](KnowView *const v, const
	// QModelIndex &_i, const int dummy = 0) ->QModelIndex{(void)(dummy);
	// assert(v); assert(v->selectionModel()); assert(_i.isValid());
	// v->selectionModel()->setCurrentIndex(_i, current_tree_current_index_mode);
	// return _i;}
	// );
	...
	boost::intrusive_ptr<i_t> session_root_auto();
	signals:
	...
	boost::intrusive_ptr<i_t> tree_empty_controll(void);
	void tree_crypt_control(void);

	boost::intrusive_ptr<i_t>
	new_child(boost::intrusive_ptr<TreeIndex> _modelindex, QString _name, const add_new &_branch_add_new_impl);

	boost::intrusive_ptr<i_t>
	new_item(boost::intrusive_ptr<TreeIndex> _modelindex, const add_new_delegate &_branch_add_new, const add_new &_branch_add_new_impl);

	QList<boost::intrusive_ptr<i_t>>
	move_children(boost::intrusive_ptr<TreeIndex> _parent_modelindex, boost::intrusive_ptr<i_t> _blank_header, const substitute_condition_double &_substitute_condition);

	void paste_clipboard(boost::intrusive_ptr<TreeIndex> _sibling_modelindex);

	boost::intrusive_ptr<i_t>
	move(boost::intrusive_ptr<TreeIndex> _treeindex, boost::intrusive_ptr<i_t> _source_item, const substitute_condition &_substitute_condition, bool save_immediately = false);
	// boost::intrusive_ptr<TreeItem>
	// paste_sibling(boost::intrusive_ptr<TreeIndex> _treeindex,
	// boost::intrusive_ptr<TreeItem> _source_item, const substitute_condition
	// &_substitute_condition);

	QList<boost::intrusive_ptr<i_t>> delete_permanent(
	    const std::function<tkm_t *()> &_current_model,
	    QList<boost::intrusive_ptr<i_t>> _items, delete_strategy _delete_strategy,
	    const QString &_mode = "cut", const bool _cut_branch_confirm = false);

	void cut(bool _cut_branch_confirm = false);
	QModelIndexList copy(void);  // const;
	void modify(void);
	boost::intrusive_ptr<i_t>
	merge(boost::intrusive_ptr<TreeLevel> _tree_merge);  // boost::intrusive_ptr<TreeIndex> tree_index,
...
#endif
	web::WebView *
	index_invoke(boost::intrusive_ptr<TreeIndex> _tree_index);  // web::WebView *view, const index_tree &_index
	std::pair<boost::intrusive_ptr<i_t>, boost::intrusive_ptr<i_t>>
	cursor_focus(boost::intrusive_ptr<i_t> to_be_contained = nullptr);
	// std::pair<boost::intrusive_ptr<TreeItem>, boost::intrusive_ptr<TreeItem> >
	// cursor_follow_up(void);
	// boost::intrusive_ptr<TreeItem>	cursor_follow_up(void);
	boost::intrusive_ptr<i_t> intercept(boost::intrusive_ptr<i_t> result);
	...
	bool is_dragable(X *event) {
		...
		if (source_object->objectName() ==
		    record_view_multi_instance_name)  // "recordTableView"
			return true;
		else
			return false;
	}
...
// void	onNewGroupModelItemsInserted(const QModelIndex &index, int start,
// int end);
...
// virtual void	drawBranches(QPainter *painter, const QRect &rect, const
// QModelIndex &index) const;

// private:
// void requestFileTransferChannels(const Tp::AccountPtr &account, const
// Tp::ContactPtr &contact, const QStringList &filenames, const QDateTime
// &userActionTime);
