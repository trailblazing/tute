...
ExecuteCommand::ExecuteCommand(QWidget *parent)
	...
	QString os = "unix";
#if defined(Q_WS_WIN) || defined(Q_OS_WIN32) || defined(Q_OS_WINCE) || \
    defined(Q_OS_MSDOS) || defined(Q_OS_CYGWIN)
	os = "windows";
#endif
	...
	QString localeCodepage =
	    localeCodec->name();  // Возможные варианты "windows-1251" для Windows
	// Для Windows с русской кодировкой исправляется кодировка локали, так как в
	// консли локаль CP866
...
void ExecuteCommand::run(void) {
	...
	printOutput(
	    process,
	    console);  // Считываются остатки из стандартного вывода, если они есть
	...
	if ((isError == false && process->exitCode() == 0) || isManualClose)
		...
		// Окно с консолью было просто видно (чтобы показывать вывод), а теперь
		// запускается как модальное, чтобы началось ожидание нажатия кнопки Close
		console->exec();
	}
	delete process;
...
void ExecuteCommand::printOutput(QProcess *process, ConsoleEmulator *console) {
	...
	if (readBytes >= 1) {
		...
		// QString output=QString::fromLocal8Bit(buf); // Ранее было fromAscii,
		// потом fromLatin1
...
void ExecuteCommand::errorHanler(QProcess::ProcessError error) {
	qDebug() << "ExecuteCommand::errorHanler(): Detect error! Error code: "
	         << error;
