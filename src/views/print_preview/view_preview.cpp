...
void PreviewView::paint_page(QPainter *painter, int page) {
	...
	col = col.light();
	painter->drawLine(
	    QLineF(pgSize.width(), 1, pgSize.width(), pgSize.height() - 1));

	col = col.light();
	painter->drawLine(
	    QLineF(pgSize.width(), 2, pgSize.width(), pgSize.height() - 2));
...
void PreviewView::resizeEvent(QResizeEvent *) {
	...
	QSize docSize;
	docSize.setWidth(
	    qRound(_doc->pageSize().width() * _scale + 2 * _inter_page_spacing));
