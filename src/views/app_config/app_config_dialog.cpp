...
AppConfigDialog::AppConfigDialog(
    QString first_page_name)  // rctrl_t *record_controller_,
    : QWidget() {
	...
	config_dialog->set_window_title(
	    tr(std::string(program_title_string + " settings").c_str()));

	pageMain = config_dialog->add_widget(new AppConfigPageMain(this), tr("Main"));
	pageCrypt =
	    config_dialog->add_widget(new AppConfigPageCrypt(this), tr("Crypt"));
	pageSynchro =
	    config_dialog->add_widget(new AppConfigPageSynchro(this), tr("Synchro"));
	pageRecordTable =
	    config_dialog->add_widget(new AppConfigPageTable(this), tr("Note area"));
