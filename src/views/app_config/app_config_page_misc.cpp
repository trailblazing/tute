...
AppConfigPageMisc::AppConfigPageMisc(QWidget *parent)
    : ConfigPage(parent) {
	...
	// Настройка запуска в свернутом окне
	runInMinimizedWindow = new QCheckBox(this);
	runInMinimizedWindow->setText(
	    tr(std::string("Run " + program_title_string + " in a minimized window")
	           .c_str()));
	...
	rememberAtHistoryNavigationCheckBox->setText(
	    tr("Remember cursor position at history navigation"));
	rememberAtHistoryNavigationCheckBox->setChecked(
	    appconfig->remember_cursor_at_history_navigation());

	rememberAtOrdinarySelectionCheckBox = new QCheckBox(this);
	rememberAtOrdinarySelectionCheckBox->setText(
	    tr("Try remember cursor position at ordinary selection"));
	rememberAtOrdinarySelectionCheckBox->setChecked(
	    appconfig->remember_cursor_at_ordinary_selection());
...
int AppConfigPageMisc::apply_changes(void) {
	...
	// Сохраняется настройка режима запуска - обычный или свернутый
	...
	if (appconfig->remember_cursor_at_history_navigation() !=
	    rememberAtHistoryNavigationCheckBox->isChecked())
		appconfig->remember_cursor_at_history_navigation(
		    rememberAtHistoryNavigationCheckBox->isChecked());
	...
	if (appconfig->remember_cursor_at_ordinary_selection() !=
	    rememberAtOrdinarySelectionCheckBox->isChecked())
		appconfig->remember_cursor_at_ordinary_selection(
		    rememberAtOrdinarySelectionCheckBox->isChecked());
	return 0;
}
