...
AppConfigPageSynchro::AppConfigPageSynchro(QWidget *parent)
	...
	synchroOnStartup->setText(
	    tr(std::string("Synchronize at " + program_title_string + " startup")
	           .c_str()));
	synchroOnStartup->setChecked(appconfig->synchro_on_startup());

	synchroOnExit = new QCheckBox(this);
	synchroOnExit->setText(
	    tr(std::string("Synchronize when exit from " + program_title_string)
	           .c_str()));
	...
	commandAboutText->setText(
	    tr("Use <b>%a</b> macro for get database directory path"));
