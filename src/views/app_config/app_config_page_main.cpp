...
int AppConfigPageMain::apply_changes(void)
{
    ...
    if (globalparameters.root_path() != _rootdir_input->text())
        write_root();
    auto write_data = [&]() {
        ...
        if (dir.isReadable() == false) {
            QMessageBox::warning(this, tr("Warning"), tr("The data directory does not exists or unavailable for reading."), QMessageBox::Ok);
        } // else
        if (dir.exists() == false) {
    ...
    auto write_trash = [&]() {
        ...
        // Проверяется, допустимо ли имя директории
        if (dir.isReadable() == false) {
            QMessageBox::warning(this, tr("Warning"), tr("The trash directory does not exists or unavailable for reading."), QMessageBox::Ok);
        } // else
        if (dir.exists() == false)
            DiskHelper::create_directory(root_path_, dir.dirName());
        if (dir.exists() && dir.isReadable()) {
    ...
    if (appconfig.trash_max_file_count() != _trashmaxfilecount_input->text().toInt())
    ...
    if (appconfig.enable_custom_datetime_format() != _enable_custom_datetime_format->isChecked())
        appconfig.enable_custom_datetime_format(_enable_custom_datetime_format->isChecked());
