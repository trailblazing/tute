...
void AppConfigPageMain::setup_ui(void)
{
    ...
    _application_current_path_label->setText(
        tr("Application current path: \"%1\".").arg(QDir::currentPath()));
    ...
    // _application_mode_option->setCurrentIndex(_application_mode_option->findText(globalparameters.app_mode(),
    ...
    _disable_custom_datetime_format =
        new QRadioButton(tr("Show date and time by OS locale settings"), this);
    _enable_custom_datetime_format =
        new QRadioButton(tr("Custom date and time show format"), this);
    custom_datetime_format = new QLineEdit(this);
    _datetime_format_help_button = new FlatToolButton("", this);
    QCommonStyle styleHelp;
    _datetime_format_help_button->setIcon(
        styleHelp.standardIcon(QStyle::SP_MessageBoxQuestion));
...
void AppConfigPageMain::open_rootdir_select_dialog(void)
{
    _original_root_state =
        std::tuple<const bool, const QString>(true, _rootdir_input->text());
    ...
    _rootdir_input->setText(get_root_path(_rootdir_input->text()));
    {
        ...
        //		auto root_path_ =
        //rootdirSelectDialog.directory().absolutePath();
        //		_rootdir_input->setText(root_path_);
    }
    // _datadir_input->setText(root_path_ + "/" +
    // QDir(_datadir_input->text()).dirName());
    // _trashdir_input->setText(root_path_ + "/" +
    // QDir(_trashdir_input->text()).dirName());
    ...
    // , QMessageBox::Ok);
    // }	// else
    // if(! dir.exists()) DiskHelper::create_directory(QDir::rootPath(),
    // root_path_);
    ...
    // _application_current_path_label->setText(tr("Application current path:
...
int AppConfigPageMain::apply_changes(void)
{
    ...
    // if((globalparameters.application_mode() !=
    // _application_mode_option->currentText()) || (_rootdir_input->text() !=
    // globalparameters.root_path())){
    // const auto	is_standard	= (_application_mode_option->currentText() ==
    // standardItem);
    ...
    auto write_root = [&]() {
        ...
        // else
        if (!dir.exists())
            DiskHelper::create_directory(QDir::rootPath(), root_path_);
        // else{
        //// Новое имя запоминается в конфиг
...
>>>>>>> tuple
    ...
    if (gl_paras->root_path() != _rootdir_input->text())
        write_root();
    auto write_data = [&]() {
    ...
    if (appconfig->data_dir() != _datadir_input->text())
        write_data();
    auto write_trash = [&]() {
    ...
    if ((int)appconfig->trash_size() != (int)_trashsize_input->text().toInt())
        appconfig->trash_size(_trashsize_input->text().toInt());
    // Если было изменено максимально возможное количество файлов в корзине
    if (appconfig->trash_max_file_count() !=
        _trashmaxfilecount_input->text().toInt())
        appconfig->trash_max_file_count(_trashmaxfilecount_input->text().toInt());
    // Если было изменено использование пользовательского формата даты и времени
    if (appconfig->enable_custom_datetime_format() !=
        _enable_custom_datetime_format->isChecked())
        appconfig->enable_custom_datetime_format(
            _enable_custom_datetime_format->isChecked());
    // Если было изменен пользовательский формат даты и времени
    if (appconfig->custom_datetime_format() != custom_datetime_format->text())
        appconfig->custom_datetime_format(custom_datetime_format->text());
    ...
    if (_original_root_state != result) {
        // if(std::get<1>(result) != is_standard)
        // _application_mode_option->setCurrentText(is_standard ?  standardItem :
        // portableItem);
        ...
        QMessageBox message;
        message.setText(
            "The changes of application mode will take effect after "
            "restart the application."); // You have to restart Mytetra
...
QString AppConfigPageMain::get_root_path(const QString& initia_path)
{
    ...
    select_root.setWindowTitle(tr("Specific the applications\'s data center path")); //"Select root
                                                                                     //directory"
