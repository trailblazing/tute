...
QString AppConfigPageCrypt::getRetrieveStatusText(void){
	...
	if(appconfig.password_save_flag() && appconfig.password_middle_hash().length() > 0) status = status + tr("Password is saved locally. ");
	return status;
}
...
int AppConfigPageCrypt::apply_changes(void){
	...
	if(howPassRequestRadio1->isChecked() && appconfig.howpassrequest() == "atStartProgram") appconfig.howpassrequest("atClickOnCryptBranch");
	if(howPassRequestRadio2->isChecked() && appconfig.howpassrequest() == "atClickOnCryptBranch") appconfig.howpassrequest("atStartProgram");
