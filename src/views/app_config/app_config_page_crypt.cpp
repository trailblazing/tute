...
void AppConfigPageCrypt::setup_ui(void) {
	...
	howPassRequestRadio1 = new QRadioButton(
	    tr("Ask the password when you click on an encrypted item"));
	howPassRequestRadio2 = new QRadioButton(
	    tr(std::string("Ask the password at " + program_title_string + " startup")
	           .c_str()));
	...
	passwordSaveAnnotation =
	    new QLabel(tr("Password will be saved at first next entered.<br>Stored "
	                  "password will be cleared if uncheck this checkbox."));
	...
	autoClosePasswordEnable =
	    new QCheckBox(tr("Enable auto closing password window, sec"), this);
	...
	// азрешать ли расшифровывать зашифрованные файлы аттачей в каталог корзины
	// при предпросмотре
	decryptFileToTrashDirectoryBox = new QGroupBox(this);
	decryptFileToTrashDirectoryBox->setTitle(
	    tr("File decrypt method for attach preview"));

	decryptFileToTrashDirectoryEnable = new QCheckBox(
	    tr("Enable temporary decrypt attach file to trash directory"));

	// Устанавливается галка согласно настройке из файла конфигурации
	decryptFileToTrashDirectoryEnable->setChecked(
	    appconfig->enable_decrypt_file_to_trash_directory());

	// Виджеты вставляются в группировщик
	QHBoxLayout *decryptFileToTrashDirectoryLayout = new QHBoxLayout;
	decryptFileToTrashDirectoryLayout->addWidget(
	    decryptFileToTrashDirectoryEnable);
...
QString AppConfigPageCrypt::getRetrieveAnnotationText(void) {
	...
	if (databaseconfig->get_crypt_mode() == 0)
		return tr(
		    "A password will be used to encrypt the item that you selected. "
		    "Use \"Encrypt item\" or \"Decrypt item\" in context menu.");
	else
		return tr(
		    "If you change your password all encrypted item will be "
...
int AppConfigPageCrypt::apply_changes(void) {
	...
	if (howPassRequestRadio2->isChecked() &&
	    appconfig->howpassrequest() == "atClickOnCryptBranch")
		appconfig->howpassrequest("atStartProgram");
	if (autoClosePasswordEnable->isChecked() !=
	    appconfig->auto_close_password_enable())
	...
	if (decryptFileToTrashDirectoryEnable->isChecked() !=
	    appconfig->enable_decrypt_file_to_trash_directory())
		appconfig->enable_decrypt_file_to_trash_directory(
		    decryptFileToTrashDirectoryEnable->isChecked());
	return 0;
}
