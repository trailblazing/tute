...
int AppConfigPageTable::apply_changes(void)
{
    ...
    while (i.hasNext()) {
        ...
        if (!show_fields.contains(i.key()) && i.value()->isChecked())
            add_fields_list << i.key();
        // Если поле удалилось
        if (show_fields.contains(i.key()) && !i.value()->isChecked())
            remove_fields_list << i.key();
    }
    ...
    if (new_show_fields.size() > show_fields.size()) {
        ...
        for (int n = 0; n < (new_show_fields.size() - show_fields.size()); n++)
            new_fields_width << QString::number((int)insert_field_width);
