...
const char *custom_hidabletabwidget_style = "QTabWidget::pane {"
...
std::tuple<int, QList<int> > HidableTab::on_splitter_moved(int record_pos, int index){
	...
	if(win_width == 0 || win_width <= bar_width || _main_window->geometry().width() < _main_window->sizeHint().width()) win_width = _main_window->sizeHint().width();
//	if(win_width == 0 || win_width <= bar_width) win_width = std::numeric_limits<int>::max();
	...
	if(record_pos <= bar_width){
		...
		right->setMaximumWidth(std::numeric_limits<int>::max());//win_width=>this value won't work!
		right->adjustSize();
...
void HidableTab::collapse_when_true(bool checked){
	...
	auto sizes = _splitter->sizes();
	auto win_width = _main_window->width();
	if(win_width == 0 || win_width <= bar_width) win_width = _main_window->sizeHint().width();
	if(win_width == 0 || win_width <= bar_width) win_width = std::numeric_limits<int>::max();
	auto win_height = _main_window->height();
	if(win_height == 0 || win_height <= bar_height) win_height = _main_window->sizeHint().width();
	if(win_height == 0 || win_height <= bar_height) win_height = std::numeric_limits<int>::max();
