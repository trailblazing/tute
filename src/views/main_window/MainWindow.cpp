...
MainWindow::MainWindow(GlobalParameters    &_globalparameters
    ...
    , _vtab_record([&] {auto vr = new HidableTabWidget(this); _globalparameters.vtab_record(vr); return vr; } ())
    , _vtab_tree([&] {auto vt = new HidableTabWidget(this); _globalparameters.vtab_tree(vt); return vt; } ())
    , _h_right_splitter([&] {auto _h_r_s = new QSplitter(Qt::Horizontal); _h_r_s->setSizes(_appconfig.h_right_splitter_sizelist()); _globalparameters.h_right_splitter(_h_r_s); return _h_r_s; } ())
    , _h_left_splitter([&] {if(_globalparameters.target_os() == "android")
...
//    }
    ...
    _globalparameters.h_right_splitter(_h_right_splitter);
...
void MainWindow::on_click_help_about_mytetra(void)
{
    ...
    infoDevicePixelRatio = "Device pixel ratio: " + (QString::number(QtSingleApplication::instance()->devicePixelRatio(), 'f', 8)) + "<br/>";
    infoPhysicalDpi = "Physical DPI (from screen): " + (QString::number(QApplication::screens().at(0)->physicalDotsPerInch(), 'f', 8)) + "<br/>";
#endif

    infoPhysicalDpiX = "Physical DPI X (from desktop): " + (QString::number(QtSingleApplication::instance()->desktop()->physicalDpiX(), 'f', 8)) + "<br/>";
    infoPhysicalDpiY = "Physical DPI Y (from desktop): " + (QString::number(QtSingleApplication::instance()->desktop()->physicalDpiY(), 'f', 8)) + "<br/>";
