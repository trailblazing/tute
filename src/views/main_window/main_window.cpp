...
void wn_t::on_click_help_about_this(void){
	...
	msgBox->about(this
		     , QString(program_title) + " v." + version
		     , info);
}
...
void wn_t::synchronization(void){
	...
	if(command.trimmed().length() == 0){
		QMessageBox::warning(this
				    , tr(std::string(std::string(program_title) + ": can't synchronization").c_str())
...
//    _editor_screen->save_textarea();
	...
	walkhistory.add<WALK_HISTORY_GO_PREVIOUS>(id
						 , _editentry->cursor_position() // _editor_screen->cursor_position()
