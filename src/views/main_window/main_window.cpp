...
wn_t::wn_t(gl_para& globalparameters, AppConfig& appconfig, DataBaseConfig& databaseconfig, browser::Profile* profile, QString style_source)
    : QMainWindow()
    , _globalparameters(globalparameters)
    , _appconfig(appconfig)
    , _databaseconfig(databaseconfig)
    , _central_widget(new QWidget(this))
    , _style([&]() -> QString {this->_globalparameters.style_source(style_source);return style_source; }())
...
void wn_t::setup_signals(void)
{
    ...
    connect(_h_tree_splitter, &QSplitter::splitterMoved, [&](int _tree_pos, int index) {
        ...
        //	    if(0 != pos){
        //	    auto tree_siezes = _h_tree_splitter->sizes();
        if (tree_siezes != _appconfig.h_tree_splitter_sizelist())
            _appconfig.h_tree_splitter_sizelist(tree_siezes);
        //	    }
    });
...
void wn_t::save_tree_position(void)
{
    ...
    if (current_item)
        appconfig.tree_position(_current_source_model()->root_item()->id(), current_item->path_list()); // _tree_screen->know_model_board()->root_item()->id()
