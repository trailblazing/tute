...
void wn_t::set_tree_position(QString current_root_id, QStringList current_item_absolute_path)
{
    ...
    if (source_model()->root_item()->id() != current_root_id) {
        ...
        auto it = know_model_board()->item([&](boost::intrusive_ptr<const i_t> it) { return it->id() == current_root_id; });
    ...
    if (it) {
        ...
        if (it != know_model_board()->root_item()) {
            ...
            boost::intrusive_ptr<TreeIndex> tree_index = TreeIndex::require_treeindex([&] { return _tree_view->source_model(); }, it);
            ...
            if (it->count_direct() == 0)
                ...
                r = it->child_direct(0);
            boost::intrusive_ptr<TreeIndex> tree_index = TreeIndex::require_treeindex([&] { return _tree_view->source_model(); }, r);
...
bool wn_t::is_tree_position_crypt()
{
    ...
    if (_tree_screen->view()->source_model()->root_item()->id() != id) {
        ...
        auto it = know_model_board()->item([&](boost::intrusive_ptr<const i_t> it) { return it->id() == id; });
...
web::Browser* wn_t::browser<boost::intrusive_ptr<i_t> >(const boost::intrusive_ptr<i_t>& it, bool force)
{
    ...
    if (!bro) {
        if (_item_tags_text_list.size() > 0)
            bro = browser(_item_tags_text_list[0]);
        else
            bro = browser(item->field<name_type>());
    }
    return bro;
}
...
web::Browser* wn_t::browser<QUrl>(const QUrl& url_, bool force)
{
    ...
    if (url_.isEmpty() && !url_.isValid())
        url_str = web::Browser::_defaulthome;
    auto it = TreeIndex::require_item(QUrl(QString(url_str.c_str())), std::bind(&tv_t::move, _tree_screen->view(), std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it) -> bool { return url_equal(it->field<home_type>().toStdString(), url_str) || url_equal(it->field<url_type>().toStdString(), url_str); });
...
void wn_t::synchronize_title(const QString& title_)
{
    if (title_ != "") {
        if (!this->windowTitle().contains(title_))
            this->setWindowTitle(tr(QString(QString("%1 - ") + program_title).toStdString().c_str()).arg(title_));
    } else
