...
wn_t::wn_t(
    ...
    , _profile(profile)
    , _style_source([&]() -> QString {gl_paras->main_window(this); gl_paras->style_source(style_source); return style_source; }())
    , _v_right_splitter([&]() -> QSplitter* {auto vrs = new QSplitter(Qt::Vertical); vrs->setSizes(appconfig->v_right_splitter_sizelist()); gl_paras->v_right_splitter(vrs); return vrs; }())
    , _v_find_splitter([&]() -> QSplitter* {auto vfs = new QSplitter(Qt::Vertical); vfs->setSizes(appconfig->findsplitter_sizelist()); gl_paras->find_splitter(vfs); return vfs; }())
    ...
    , _tree_screen([&]() -> ts_t* {auto ts = new ts_t(tree_screen_singleton_name, _filemenu, _toolsmenu, this); gl_paras->tree_screen(ts); return ts; }()) // _vtabwidget
    , _find_screen([&]() -> FindScreen* {auto fs = new FindScreen(find_screen_singleton_name, _tree_screen, this); gl_paras->find_screen(fs); return fs; }())
    , _browser_dock([&]() -> browser::BrowserDock* {auto bd = new browser::BrowserDock(browser_dock_singleton_name, _tree_screen, _find_screen, this, style_source, profile, Qt::Widget); gl_paras->browser_dock(bd); return bd; }()) // Qt::MaximizeUsingFullscreenGeometryHint
    , _vtab_record([&] {auto vr = new HidableTabWidget(_tree_screen, _find_screen, _browser_dock, this, profile, style_source, _h_record_splitter, std::make_shared<QSettings>(gl_paras->root_path() + "/" + gl_paras->target_os() + "/" + gl_para::_conf_filename, QSettings::IniFormat), "General", "h_record_splitter_sizelist", "collapsed", this); gl_paras->vtab_record(vr); return vr; }())
    , _editor_dock([&] {auto ed = new EditorDock("", _tree_screen, _find_screen, _vtab_record, _browser_dock, this, style_source, profile, Qt::Widget); gl_paras->editor_dock(ed); return ed; }())
    ...
    , _enable_real_close(false)
{
    ...
    //	setup_icon_actions();
    //	create_tray_icon();
    //	_tray_icon = [&] {auto ti = new SysTrayIcon(_tree_screen, _browser_dock, _vtab_record, _find_screen, _editor_dock, this, _profile, _style_source); gl_paras->tray_icon(ti); return ti;} ();
...
void wn_t::setup_signals(void)
{
    ...
    // if(sizes != appconfig->h_record_splitter_sizelist()	// && h_record_sizes[0] != bar_width
    // ){
...
void wn_t::restore_find_in_base_visible(void)
{
    ...
    FindScreen* find_screen = gl_paras->find_screen(); // find_object<FindScreen>(find_screen_singleton_name);
    if (n)
        find_screen->show();
    else
        find_screen->hide();
...
//	_tray_icon = new SysTrayIcon(_tree_screen, _browser_dock, _vtab_record, _find_screen, _editor_dock, this, _profile, _style_source);
...
//						 , _editor_dock->blog_editor()->cursor_position() // _editor_screen->cursor_position()
//						 , _editor_dock->blog_editor()->scrollbar_position() // _editor_screen->scrollbar_position()
//						 );
//	walkhistory.set_drop(true);
