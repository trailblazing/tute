...
RecordPrint::RecordPrint(rs_t *record_screen)
    : QDialog(record_screen) {
	setup_ui();
	setup_signals();
	assembly();
}
...
void RecordPrint::setup_ui() {
	...
	if (this->parent()->isWidgetType()) {
		...
		qDebug() << "Parent window geometry " << geom.x() << geom.y()
...
void RecordPrint::generateHtmlTableFromModel(void) {
	QString html =
	    "<table border='1' style='border-style:solid; margin-top:0; "
	...
	for (int nColumn = 0; nColumn < model->columnCount(); nColumn++) {
		QVariant cellTextVariant =
		    model->headerData(nColumn, Qt::Horizontal, Qt::DisplayRole);
	...
	for (int nRow = 0; nRow < model->rowCount(); nRow++) {
		...
		for (int nColumn = 0; nColumn < model->columnCount(); nColumn++)
			if (model->index(nRow, nColumn).isValid()) {
				QVariant cellTextVariant =
				    model->index(nRow, nColumn).data(Qt::DisplayRole);
				QString cellText = cellTextVariant.toString();
...
// Слот в котором происходит вызов служебного окна предпросмотра тачатаемой
// таблицы
...
void RecordPrint::save(void) {
	...
	if (fileName != "") {
		...
		if (!file.open(QIODevice::WriteOnly)) {
			...
			stream << textArea->document()->toHtml("UTF-8");  // Команда stream <<
			                                                  // textArea->toHtml() не
			                                                  // подходит, так как не
			                                                  // выствляет в заголовках
			                                                  // charset
			stream.flush();
			file.close();
		}
