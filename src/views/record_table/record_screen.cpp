...
rs_t::rs_t(ts_t* _tree_screen, FindScreen* _find_screen, Editentry* _editentry, browser::Entrance* _entrance, //	  , browser::Browser    *_browser    //		  , wn_t *main_window
    HidableTabWidget* _vtab_record, const QString& _style_source, browser::Profile* _profile)
    : QWidget(_vtab_record) // , _browser(_browser)
...
//      , _save_in_new_branch(new QAction(tr("Save in new branch manually"), this))
    ...
    , _browser(new browser::Browser(_tree_screen, _find_screen, _editentry, this, _entrance, _style_source, _profile, Qt::MaximizeUsingFullscreenGeometryHint)) // , _vtab_tree
...
void rs_t::setup_actions(void)
{
    ...
    if (_tree_screen->_actionlist[action_hide_tree_screen]->text() == tr("Show record screen"))
        emit globalparameters.h_record_splitter()->splitterMoved(globalparameters.h_record_splitter()->sizes()[0], 1);
...
// Включаются те действия которые разрешены
    ...
    if ((has_selection && 1 == selected_rows) // (item_selection_model->hasSelection() && (item_selection_model->selectedRows()).size() == 1)
        ...
        if (mime_data != nullptr)
            if (mime_data->hasFormat("hapnote/records"))
