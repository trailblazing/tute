...
// &web::TabWidget::paste
    ...
    connect(_action_move_top, &QAction::triggered, [&]() mutable {
        ...
        if (_rctrl) {
            ...
            auto pos_source_ = _rctrl->index<pos_source>(pos_target);
            ...
            RecordIndex::instance([&] { return _rctrl->source_model(); }, _rctrl->source_model()->item(pos_source_))->select_as_current(); //_rctrl->select_as_current(pos_target);
    ...
    connect(_action_move_up, &QAction::triggered, [&]() mutable {
        ...
        if (_rctrl) {
            ...
            // Установка засветки на перемещенную запись
            RecordIndex::instance([&] { return _rctrl->source_model(); }, _rctrl->source_model()->item(pos_source_))->select_as_current(); //_rctrl->select_as_current(target); //pos_proxy(static_cast<int>(pos_proxy_) - 1)
...
// tabmanager inaccessible
            ...
            if (!has_selection //&& _tab_widget->count() > 0
                ...
                if (cur_it) {
                    auto cur_web_it = _rctrl->tab_widget()->current_item();
                    if (cur_it != cur_web_it) {
                        RecordIndex::instance([&] { return _rctrl->source_model(); }, cur_web_it)->select_as_current(); // _rctrl->select_as_current(_rctrl->index<pos_proxy>(cur_it));
                    }
                    has_selection = true;
                }
