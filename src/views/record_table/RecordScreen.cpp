...
RecordScreen::RecordScreen(TreeScreen           *_tree_screen
    ...
    : QWidget(_browser)	// _main_window->vtab_record()
      ...
      , _tree_hide(new QAction(_main_window->h_left_splitter()->sizes()[0] == 0 ? tr("Show tree view") : tr("Hide tree view"), this))
...
void RecordScreen::save_in_new_branch(bool checked){
    ...
    if(_index.isValid()){
        if(_tree_screen && _entrance){
            ...
            _tree_screen->view()->view_paste_children_from_children(TreeIndex::instance(_tree_source_model, _tree_screen->view()->current_item()->parent(), _tree_screen->view()->current_item()), _blank_header, [&](boost::intrusive_ptr<const Linker> target, boost::intrusive_ptr<const Linker> source) -> bool {
...
void RecordScreen::setup_actions(void){
    ...
    QList<int> sizes = _main_window->h_left_splitter()->sizes();
    ...
    connect(_main_window->h_left_splitter(), &QSplitter::splitterMoved, [&, _hide_tree_text, _hide_tree_icon](int pos = 0, int index = 0) mutable {
            (void)pos;
            (void)index;
...
// _tree_hide->setText(_hide_tree_text);
    ...
    emit _main_window->h_right_splitter()->splitterMoved(_main_window->h_right_splitter()->sizes()[0], 1);
// emit _tree_hide->triggered();
    ...
    connect(_save_in_new_branch, &QAction::triggered, this
           , [&](bool checked = false) mutable {																				// &RecordScreen::save_in_new_branch
            Q_UNUSED(checked)
...
// }
    ...
    connect(_edit_field, &QAction::triggered, _record_controller, &RecordController::on_edit_fieldcontext);										// _tabmanager
    ...
    connect(_close, &QAction::triggered, _record_controller, &RecordController::close_context);											// _tabmanager
    ...
    connect(_delete, &QAction::triggered, _tree_screen, [&](bool checked = false) mutable -> void {
            ...
            _tree_view->view_delete_permanent(_current_model
    ...
    connect(_cut, &QAction::triggered, _record_controller, &RecordController::cut);										// &browser::TabWidget::cut
    ...
    connect(_copy, &QAction::triggered, _record_controller, &RecordController::copy);											// &browser::TabWidget::copy
    ...
    connect(_paste, &QAction::triggered, _record_controller, &RecordController::paste);											// _tabmanager
    ...
    connect(_editor, &QAction::triggered, _main_window, &MainWindow::editor_switch);
    ...
    connect(_settings, &QAction::triggered, _record_controller, &RecordController::settings);											// &browser::TabWidget::settings
...
// setStyleSheet("border : 0px;");
    ...
    insert_action_as_button<QToolButton>(_extra_toolsline, _tree_screen->_actionlist[action_main_menu]);
        // _treepathlabel = new QLabel(this);
