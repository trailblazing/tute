...
////	}else QStyledItemDelegate::paint(painter, option, index);
...
#endif
    } else if (it && header_title == pin_field_description && it->field<pin_type>() != char_from_check_state[Qt::Unchecked]) {
#ifdef USE_STAR_RATING
    ...
    // optionV4.state |= QStyle::State_Enabled;
    // initStyleOption(const_cast<QStyleOptionViewItem *>(&option), index);	// initStyleOption(&optionV4, index);
...
// qDebug() << "QStyledItemDelegate::paint(painter, option, index) : " << e.what();
// }
////	drawFocus(painter, option, displayRect);
        ...
        ////	opt.text = "<b>" + it->field<name_type>() + "</b>";	// trUtf8("Button text");
        ////	_view->style()->drawControl(QStyle::CE_PushButton, &opt, painter, 0);		//	opt.paint(painter, option.rect, option.palette, QStyleOptionButton::ReadOnly);
        // std::shared_ptr<QStyleOptionViewItem> optionV4 = std::make_shared<QStyleOptionViewItem>();	// = option;
        // optionV4->state |= QStyle::State_Enabled;
        ...
        // Highlighting text if item is selected
        if (option.state & QStyle::State_Selected)
            ctx.palette.setColor(QPalette::Text, option.palette.color(QPalette::Active, QPalette::HighlightedText)); // if(optionV4->state & QStyle::State_Selected)ctx.palette.setColor(QPalette::Text, optionV4->palette.color(QPalette::Active, QPalette::HighlightedText));
        if (0 == it->count_direct() && !(option.state & QStyle::State_Selected) && (it != _view->current_item()))
            ctx.palette.setColor(QPalette::Text, option.palette.color(QPalette::Inactive, QPalette::Shadow)); // if(0 == it->count_direct() && ! (optionV4->state & QStyle::State_Selected))ctx.palette.setColor(QPalette::Text, optionV4->palette.color(QPalette::Inactive, QPalette::Shadow));
        QRect textRect = // style->
...
QSize ViewDelegation::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    doc.setTextWidth(option.rect.width()); // doc.setTextWidth(optionV4.rect.width());
...
bool ViewDelegation::editorEvent(QEvent* event, QAbstractItemModel* model, const QStyleOptionViewItem& option, const QModelIndex& index)
{
    ...
    if (event->type() == QEvent::MouseButtonRelease) {
        if (it && header_title == rating_field_description) {
            ...
            if (it->field<pin_type>() != char_from_check_state[Qt::Unchecked])
                it->field<pin_type>(char_from_check_state[Qt::Unchecked]);
            else
                it->field<pin_type>(char_from_check_state[Qt::Checked]);
        }
    }
#else
    // style()->drawControl(...)
...
boost::intrusive_ptr<i_t> rv_t::current_item() const
{
    ...
    if (it) {
        ...
        ////	auto	source_index	= _record_controller->source_model()->current_index();
...
bool rv_t::is_selected_set_to_top(void) { return selection_first<pos_proxy>() == pos_proxy(0); }

bool rv_t::is_selected_set_to_bottom(void) { return selection_first<pos_proxy>() == pos_proxy(model()->rowCount() - 1); }
...
void rv_t::start_drag()
{
    ...
    if (currentIndex().isValid()) {
        ...
        if (result == 0) {
            ...
            gl_paras->tree_screen()->view()->source_model()->setData(QModelIndex(), QVariant(false), Qt::UserRole);
        }
    }
}
    ...
    //// Q_UNUSED(e)
    {
        ...
        for (int i = 0; i < show_fields.size(); i++) {
            if (required_width >= real_capacity) { // if((columnWidth(0) + columnWidth(1)) >= real_width){
                if (is_field_type_column(boost::mpl::c_str<pin_type>::value, i))
                    setColumnWidth(i, _pin_width);
                else if (is_field_type_column(boost::mpl::c_str<rating_type>::value, i))
                    setColumnWidth(i, rating_width());
                else if (!is_field_type_column(boost::mpl::c_str<name_type>::value, i))
...
template <>
boost::intrusive_ptr<i_t> rv_t::selection_first<boost::intrusive_ptr<i_t> >() const
{
