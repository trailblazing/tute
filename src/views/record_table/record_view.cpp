...
bool ViewDelegation::editorEvent(QEvent* event, QAbstractItemModel* model, const QStyleOptionViewItem& option, const QModelIndex& index)
{
    ...
    if (event->type() == QEvent::MouseButtonRelease) {
        if (it && header_title == rating_field_description) {
            ...
            if (it->field<pin_type>() != _string_from_check_state[Qt::Unchecked])
                it->field<pin_type>(_string_from_check_state[Qt::Unchecked]);
            else
                it->field<pin_type>(_string_from_check_state[Qt::Checked]);
        }
    }
#else
    // style()->drawControl(...)
...
void rv_t::edit_field_context(void)
{
    ...
    if (!currentIndex().isValid())
        _record_controller->select_as_current(_record_controller->index<pos_proxy>(c_i));
    ...
    //	// Q_UNUSED(e)
    {
        ...
        for (int i = 0; i < show_fields.size(); i++) {
            if (required_width >= real_capacity) { // if((columnWidth(0) + columnWidth(1)) >= real_width){
                if (_is_field_type_column(boost::mpl::c_str<pin_type>::value, i))
                    setColumnWidth(i, _pin_width);
                else if (_is_field_type_column(boost::mpl::c_str<rating_type>::value, i))
                    setColumnWidth(i, rating_width());
                else if (!_is_field_type_column(boost::mpl::c_str<name_type>::value, i))
