		...
		if(_binder->host()){
			...
			if(title != ""){
				...
				if(is_current){
					...
					if(!_mainwindow->windowTitle().contains(title)) _mainwindow->setWindowTitle(QString(program_title) + " : " + title); // table->setWorkPos(pos);
			...
			if(url_str != ""){
				...
				data_changed = true;
				if(is_current) _editing_window->url(url_str);
			}
...
// }
	...
	void WebView::current_view_global_consistency(){
		...
		if(!_mainwindow->windowTitle().contains(_page->title())) _mainwindow->setWindowTitle(QString(program_title) + " : " + _page->title()); // table->setWorkPos(pos);
