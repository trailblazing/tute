...
WebPage::WebPage(Profile* profile, boost::intrusive_ptr<TreeItem> item, ts_t* tree_screen, EditingWindow* editentry, BrowserDock* entrance, Browser* browser, TabWidget* tabmanager, rctrl_t* record_controller, WebView* parent)
    : QWebEnginePage(profile, parent)
    , _profile(profile)
    , _tree_screen(tree_screen)
    , _editing_window(editentry)
    , _entrance(entrance)
    , _browser(browser)
    ...
    if (_binder->host()) {
        ...
        if (title != "") {
            ...
            if (is_current) {
                ...
                if (!_mainwindow->windowTitle().contains(title))
                    _mainwindow->setWindowTitle(QString(program_title) + " : " + title); // table->setWorkPos(pos);
        ...
        if (url_str != "") {
            ...
            if (_host->field<home_type>() == Browser::_defaulthome && url_str != Browser::_defaulthome)
                _host->field<home_type>(url_str);
            data_changed = true;
            if (is_current)
        ...
        if (_editing_window->item() != _binder->host() && is_current)
...
// } catch(std::exception &) {}
    ...
    if (_editing_window->item() != current_item)
        _editing_window->bind(current_item);
...
// table->work_pos(pos);
    ...
    if (current_item->field<id_type>().length() == 0)
        current_item->field<id_type>(current_item->field<dir_type>().length() > 0 ? current_item->field<dir_type>() : get_unical_id()); // "id",	// || current_item->field("url") == Browser::_defaulthome
    ...
    if (current_item->field<crypt_type>() == "1") // table->field(pos, boost::mpl::c_str < crypt_type > ::value)
        if (full_dir.length() == 0 || current_file.length() == 0)
            _editing_window->dir_file_empty_reaction(EditorWrap::DIRFILEEMPTY_REACTION_SUPPRESS_ERROR);
    ...
    // Если нет приаттаченных файлов
        ...
        _editing_window->to_attach()->setIcon(_editing_window->icon_attach_exists()); // Есть приаттаченные файлы
...
boost::intrusive_ptr<TreeItem> WebPage::bind(boost::intrusive_ptr<TreeItem> host_)
...
QWebEnginePage* WebPage::createWindow(QWebEnginePage::WebWindowType type)
{
    ...
    if (!target_url.isValid())
        target_url = Browser::_defaulthome;
    // assert(url != "");
    ...
    if (type == QWebEnginePage::WebBrowserWindow) {
        ...
        Browser* _browser = gl_paras->main_window()->new_browser(); // QtSingleApplication::instance()->newMainWindow();
        ...
        page = _browser->bind(RecordIndex::instance([&] { return _browser->record_screen()->record_controller()->source_model(); }, it))->activate(std::bind(&wn_t::find, gl_paras->main_window(), std::placeholders::_1))->page();
...
#endif
    {
        ...
        if (view) {
            ...
            auto it = this_treeindex->bind(target_url, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4) // std::placeholders::_1
                ,
                [&](boost::intrusive_ptr<const TreeItem> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), target_url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), target_url.toString().toStdString()); });
            page = it ? it->activate(std::bind(&wn_t::find, gl_paras->main_window(), find_binder) // std::placeholders::_1
                              )
                            ->page()
                      : nullptr;
            assert(page || _hovered_url == Browser::_defaulthome || _hovered_url == "");
...
void WebPage::authenticationRequired(const QUrl& requestUrl, QAuthenticator* auth)
{
    ...
    Browser* browser = gl_paras->main_window()->activated_browser(); // QtSingleApplication::instance()->mainWindow();
...
void WebView::onLoadFinished(bool success)
{
    ...
    if (success) {
        ...
        assert(target_);
        if (target_)
            items_ = tree_view_->source_model()->children([&](boost::intrusive_ptr<const TreeItem> it_) { return url_equal(it_->field<url_type>().toStdString(), target_->field<url_type>().toStdString()) || it_->id() == target_->id() || it_ == target_; });
...
WebView::WebView(boost::intrusive_ptr<TreeItem> host_, Profile* profile // , bool openinnewtab
    ,
    ts_t* tree_screen, EditingWindow* editentry, BrowserDock* entrance, Browser* browser, TabWidget* tabmanager, rctrl_t* table_controller)
...
void WebView::activateWindow()
{
    ...
    _home_connection
        = QObject::connect(
            _tabmanager->browser()->_historyhome, &QAction::triggered, this, [&](bool checked = true) -> void {
                Q_UNUSED(checked)
                if (_page->binder()) {
...
void WebView::current_view_global_consistency()
{
    ...
    wn_t* _mainwindow = gl_paras->main_window();
    ...
    if (_target_item_in_browser != _tree_view->current_item())
        _tree_view->select_as_current(TreeIndex::create_treeindex_from_item([&] { return _tree_view->source_model(); }, _target_item_in_browser));
    if ((_record_controller->view()->current_item() != _target_item_in_browser) || (_tabmanager->currentWebView() != this))
        _record_controller->select_as_current(_record_controller->index<pos_proxy>(_target_item_in_browser));
    if (!_mainwindow->windowTitle().contains(_page->title()))
        _mainwindow->setWindowTitle(QString(program_title) + " : " + _page->title()); // table->setWorkPos(pos);
...
{ // boost::intrusive_ptr<TreeItem> item
    ...
    if (_host) {
        ...
        if (_page->url().toString() != _host->field<url_type>())
            _page->setUrl(QUrl(_host->field<url_type>())); // _bounded_page = view->page();
        // MetaEditor *_editor_screen = globalparameters.meta_editor();    // find_object<MetaEditor>(meta_editor_singleton_name);
        assert(_page->_editing_window);
        if (_page->_editing_window->item() != _host)
