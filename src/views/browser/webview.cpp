    ...
    , _hovered_url(Browser::_defaulthome)
{
    ...
    onUrlChanged = [&](const QUrl& url) -> void {
        ...
        // assert(url != QUrl());
        if (_binder->host()) {
            if (url.toString() != "" && url != QUrl() && !url.host().isEmpty() && !url.scheme().isEmpty() && url.isValid() && url != QUrl(web::Browser::_defaulthome) && url != _loadingurl && url.toString() != _binder->host()->field<url_key>()) {
    ...
    onTitleChanged = [&](const QString& title_) {
        ...
        if (host_) {
            ...
            //				assert(_blogger);
            //				//#endif // USE_SIGNAL_CLOSE
            if ((title_ != "" && title_ != host_->field<name_key>()) // && !QUrl(title).isValid()
            ...
            if (_blogger) {
                ...
                if (changed)
                    host_->field<tags_key>(detail::from_qstring<tags_key>(tags_list));
            }
        }
    };
...
void WebPage::record_info_update(const QUrl& url, const QString& title)
{
    ...
    if (_binder->host()) {
        ...
        if (title != "") {
            // &&
            if (title != _binder->host()->field<name_key>()) { // && ! QUrl(title).isValid()
                // assert(QUrl(title).isValid());	// always true
                // assert(title.contains("://"));	// not always true
                if (detail::to_qstring(_binder->host()->field<name_key>()).length() == 0 || (detail::to_qstring(_binder->host()->field<name_key>()).length() > 0 && !title.contains("://")))
    ...
    if (_view // && _loadingurl.isValid()     // && url().isValid()    // &&	           // url().toString() != _url_str	    // && _url_str != web::Browser::_defaulthome		// url() may be	    // nothing
        ...
        if (!_view->_load_finished || force_reload) {
            ...
            if (target_) { // ->_active_request
                ...
                //                        auto target_ = _binder->host();
                auto tree_view = _tree_screen->view();
                if (target_ != tree_view->current_item())
                    tree_view->select_as_current(TreeIndex::item_require_treeindex([&] { return tree_view->source_model(); }, target_));
            }
        }
    }
...
QWebEnginePage* WebPage::createWindow(QWebEnginePage::WebWindowType type)
{
    ...
    if (type == QWebEnginePage::WebBrowserWindow) { // this means a new topic at current design, it is not good.
        ...
        auto topic_new = (to_br_url_ == detail::to_qstring(Browser::_defaulthome) || to_br_url_ == QUrl()) ? purify(new_item->field<name_key>()) : purify(url_new.fileName());
        if (topic_new == "")
            topic_new = purify(url_new.path());
        if (topic_new == "")
            topic_new = purify(url_new.topLevelDomain());
        if (topic_new == "")
            topic_new = purify(gl_para::_default_topic);
    ...
    // }
    if (!page)
        page = this; // because of _hovered_url == "javascript:void(0)" or something else//		assert(page || _hovered_url == web::Browser::_defaulthome || _hovered_url == "" || type == WebDialog);
...
#endif
{
    ...
    QObject::connect(this, &QWebEngineView::loadFinished,
        [=](bool arg) {
            ...
            _zoomFactor = qvariant_cast<qreal>(settings.value(QLatin1String("zoomFactor"), _zoomFactor));
            settings.endGroup();
            if (zoomFactor() != _zoomFactor)
                this->setZoomFactor(_zoomFactor); // zoomFactor
        });
    ...
    connect(this->_page, &WebPage::loadingUrl, this, [&](const QUrl& url) {
        ...
        if (_binder->host()) {
            if (url.toString() != "" && url != QUrl() && !url.host().isEmpty() && !url.scheme().isEmpty() && url != QUrl(web::Browser::_defaulthome)
                // && url != _loadingurl
        ...
        if (_page) {
            ...
            connect(this->_page, &WebPage::loadingUrl, this, // &WebView::urlChanged
                [&](const QUrl& url) {
                    auto _binder = this->_page->binder();
                    if (_binder->host()) {
                        if (url.toString() != "" && url != QUrl() && !url.host().isEmpty() && !url.scheme().isEmpty() && url != QUrl(web::Browser::_defaulthome) && url.toString() != _binder->host()->field<url_key>())
                            if (url.isValid() && url != this->_page->_loadingurl)
                                this->urlChanged(url);
                    }
                });
