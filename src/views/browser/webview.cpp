...
WebPage::WebPage(Profile* profile, boost::intrusive_ptr<i_t> item, ts_t* tree_screen, Blogger* editing_window, web::Docker* browser_docker_, Browser* browser, TabWidget* tabmanager, rctrl_t* record_controller, WebView* parent)
    : QWebEnginePage(profile, parent)
    , _profile(profile)
    , _tree_screen(tree_screen)
    , _blogger(editing_window)
    ...
    , _hovered_url(Browser::_defaulthome)
{
    ...
    onTitleChanged
        = [&](const QString& title_) {
              ...
              if (host_) {
                  ...
                  assert(_blogger);
                  if (title_ != ""
                      && title_ != host_->field<name_type>() // && !QUrl(title).isValid()
                      ) {
    ...
    connect(this, &WebPage::become_current, [&](boost::intrusive_ptr< ::Binder> _binder) {
        ...
        if (host_) {
            ...
            if (_main_window)
                _main_window->synchronize_title(host_->field<name_type>());
        }
    });
    connect(this, &WebPage::become_current, [&](boost::intrusive_ptr< ::Binder> _binder) {(void) _binder; this->metaeditor_sychronize(); });
}
...
void WebPage::record_view_remove(boost::intrusive_ptr<i_t> host_)
{
    if (_tabmanager && _tabmanager->count() > 0) {
        ...
        if (source_model->item([&](const id_value id) { return id == id_value(host_->field<id_type>()); }))
            _record_controller->remove(host_->id()); // && record->unique_page() == this
    }
}
    ...
    if (_binder->host()) {
        ...
        if (title != "") {
            ...
            if (title != _binder->host()->field<name_type>()) { // && ! QUrl(title).isValid()
                ...
                if (_binder->host()->field<name_type>().size() == 0 || (_binder->host()->field<name_type>().size() > 0 && !title.contains("://")))
            ...
            if (is_current) {
                ...
                auto it = this->host();
                if (it)
                    gl_paras->main_window()->synchronize_title(it->field<name_type>());
        ...
        if (url_str != "") {
            auto host_ = _binder->host();
            host_->field<url_type>(url_str); // "url",
            if (host_->field<home_type>() == web::Browser::_defaulthome && url_str != web::Browser::_defaulthome)
                host_->field<home_type>(url_str);
            data_changed = true;
#ifdef USE_EDITOR_WRAP
            if (is_current) {
        ...
        //#else
        //			if(is_current) metaeditor_sychronize();                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          // metaeditor->bind(_record);
        //#endif //USE_EDITOR_WRAP
        ...
        if (is_current) { // globalparameters.mainwindow()
            ...
            if (_record_controller->view()->current_item() != it)
                _record_controller->select_as_current(_record_controller->index<pos_proxy>(it)); // IdType(_binder->item()->field("id"))
        }
        if (is_current)
...
// table->work_pos(pos);
    ...
    if (current_item->field<id_type>().length() == 0)
        current_item->field<id_type>(current_item->field<dir_type>().length() > 0 ? current_item->field<dir_type>() : get_unical_id()); // "id",	// || current_item->field("url") == web::Browser::_defaulthome
    ...
    QString full_dir = gl_paras->root_path() + "/" + QDir(appconfig->data_dir()).dirName() + "/base/" + current_dir;
    ...
    if (current_item->field<crypt_type>() == "1") // table->field(pos, boost::mpl::c_str < crypt_type > ::value)
        if (full_dir.length() == 0 || current_file.length() == 0)
            _blogger->dir_file_empty_reaction(EditorWrap::DIRFILEEMPTY_REACTION_SUPPRESS_ERROR);
...
WebView* WebPage::activate()
{
    ...
    if (_binder->host()) { // ->_active_request
        ...
        auto it = _binder->host();
        auto tree_view = _tree_screen->view();
        if (it != tree_view->current_item())
            tree_view->select_as_current(TreeIndex::require_treeindex([&] { return tree_view->source_model(); }, it));
        // if(_url_str != web::Browser::_defaulthome){	// && _loadingurl.isValid()   // && _loadingurl == _url
        if (_view) {
...
boost::intrusive_ptr<i_t> WebPage::bind(boost::intrusive_ptr<i_t> host_)
{ // , web::WebPage *page
    boost::intrusive_ptr<i_t> result(nullptr);

    // auto binder = [](boost::shared_ptr<WebPage::RecordBinder> ar) {
    // return std::make_shared<sd::_interface<sd::meta_info<boost::shared_ptr<void>>, web::WebView *, boost::intrusive_ptr<TreeItem>   // , boost::intrusive_ptr<TreeItem>(TreeItem::*)(WebPage *)
    // >>("", &WebPage::RecordBinder::binder, ar);
    // };
    // auto activator = [](boost::shared_ptr<WebPage::RecordBinder> ar) {
    // return std::make_shared<sd::_interface<sd::meta_info<boost::shared_ptr<void>>, web::WebView *>>("", &WebPage::RecordBinder::activator, ar);
    // };
...
QWebEnginePage* WebPage::createWindow(QWebEnginePage::WebWindowType type)
{
    ...
    boost::intrusive_ptr<TreeIndex> this_treeindex = TreeIndex::require_treeindex([&] { return tree_view->source_model(); }, this->host()); // parent_parent ? TreeIndex::instance([&] {return tree_view->source_model();}, parent) : TreeIndex::instance([&] {return tree_view->source_model();}, this->host());
    ...
    auto find_target = [&]() { return gl_paras->main_window()->find(find_binder); };
    if (type == QWebEnginePage::WebBrowserWindow) { // this means a new topic at current design, it is not reasonable.
        WebView* v = find_target();
        if (v)
            v->tabmanager()->closeTab(v->tabmanager()->indexOf(v));
        Browser* _browser_new = gl_paras->main_window()->browser(target_url, true); // QtSingleApplication::instance()->newMainWindow();

        auto it = // this_index->
            TreeIndex::require_item(target_url, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal((it_->field<home_type>()).toStdString(), target_url.toString().toStdString()) || url_equal((it_->field<url_type>()).toStdString(), target_url.toString().toStdString()); }); // return it_->field<url_type>() == target_url.toString();
        // web::Browser::_defaulthome
        it->topic_append(gl_para::_default_topic);
        page = _browser_new->bind(RecordIndex::instance([&] { return _browser_new->tabmanager()->record_controller()->source_model(); }, it))->activate(std::bind(&wn_t::find, gl_paras->main_window(), std::placeholders::_1))->page();
        ...
        if (view) {
            ...
            auto it = this_treeindex->bind(target_url, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4) // std::placeholders::_1
                ,
                [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), target_url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), target_url.toString().toStdString()); });
        ...
        if (view) {
            ...
            if (_item) {
                ...
                if (static_cast<QModelIndex>(_index).isValid())
                    _item->activate(std::bind(&wn_t::find, gl_paras->main_window(), std::placeholders::_1)); // tree_view->index_invoke(TreeIndex::instance([&] {return tree_view->source_model();}, _item, _item->parent()));	// view,
...
void WebPage::authenticationRequired(const QUrl& requestUrl, QAuthenticator* auth)
{
    ...
    //	        Browser *_browser = gl_paras->main_window()->activated_browser(); // QtSingleApplication::instance()->mainWindow();
...
{
    _icon = qobject_cast<sapp_t*>(qApp)->defaultIcon(); // QIcon();
    if (_icon_reply) {
...
void WebView::onLoadFinished(bool success)
{
    ...
    if (success) {
        ...
        assert(target_);
        if (target_)
            items_ = tree_view_->source_model()->children([&](boost::intrusive_ptr<const i_t> it_) { return url_equal(it_->field<url_type>().toStdString(), target_->field<url_type>().toStdString()) || it_->id() == target_->id() || it_ == target_; });
        ...
        if (target_ && items_.size() > 1) {
            ...
            for (auto _it : others_same) {
                if (_it->binder()) {
                    if (_it->binder()->page()) {
                        if (_it->binder()->page()->view() == _tabmanager->currentWebView())
                            _record_controller->select_as_current(_record_controller->index<pos_proxy>(target_)); // if(_record_controller->source_model()->current_item() != _it) _record_controller->select_as_current(_record_controller->index<pos_proxy>(_it));
                        auto it_ = TreeLevel::instance(TreeIndex::require_treeindex([&] { return tree_view_->source_model(); }, target_), _it)->merge(); // TreeIndex::instance([&] {return v->source_model();}, ti, ti->parent()), _it);
...
WebView::WebView(boost::intrusive_ptr<i_t> host_, Profile* profile // , bool openinnewtab
    ,
    ...
    , _page([&]() -> WebPage* {_page = nullptr; auto p = new WebPage(profile, host_, tree_screen, editing_window, browser_docker_, browser, tabmanager, table_controller, this); return p; }()) // , openinnewtab
    ...
    , _icon_reply(0)
{
    ...
    connect(this, &WebView::close_requested, [&]() {
        _tabmanager->closeTab(_tabmanager->webViewIndex(this));
    });
    // &WebPage::titleChanged(const QString &title);
    // &WebPage::urlChanged(const QUrl &url);
...
void WebView::activateWindow()
{
    ...
    QObject::connect(to_home, &QAction::triggered, this, [&](bool checked = true) -> void {
        Q_UNUSED(checked)
        if (_page->binder()) {
            boost::intrusive_ptr<i_t> _item = _page->binder()->host();
            assert(_item);
...
{
    if (event)
        if (this != _tabmanager->currentWebView())
...
void WebView::openLinkInNewTab(const QUrl& _url)
{
    ...
    if (_record_controller->view()->selection_first<id_value>() != _page->host()->field<id_type>())
        _record_controller->select_as_current(_record_controller->index<pos_proxy>(_page->host())); // IdType(_page->item()->field("id"))
...
void WebView::current_view_global_consistency()
{
    ...
    if (index != _tabmanager->currentIndex())
        _tabmanager->setCurrentIndex(index); // c->index<PosSource>(c->source_model()->index(_binder->item()))
    ...
    if (it) {
        ...
        if ((_record_controller->view()->current_item() != it) || (_tabmanager->currentWebView() != this))
            _record_controller->select_as_current(_record_controller->index<pos_proxy>(it));
        if (_page->title() != it->field<name_type>())
            _page->record_info_update(_page->url(), _page->title());
        if (it)
            gl_paras->main_window()->synchronize_title(it->field<name_type>());
    }
...
WebPage::Binder::Binder(boost::intrusive_ptr<i_t> item_, WebPage* page_) // , bool make_current = true
    : // std::enable_shared_from_this<Coupler>()
      _host(item_),
      _page(page_)
{ // , _make_current(make_current)
    // _bounded_page->binder(new TreeItem::coupler_delegation(shared_from_this()));
...
{ // boost::intrusive_ptr<TreeItem> item
    ...
    if (_host) {
        if (!_page) {
            ...
            auto record_index = RecordIndex::instance([&] { return _browser->tabmanager()->record_controller()->source_model(); }, _host);
        ...
        _page->bind(_host);
        if (_page->url().toString() != _host->field<url_type>())
            _page->setUrl(QUrl(_host->field<url_type>()));
#ifdef USE_EDITOR_WRAP
...
boost::intrusive_ptr<i_t> WebPage::host() const { return _binder ? _binder->host() : nullptr; }
