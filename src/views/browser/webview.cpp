...
WebPage::WebPage(Profile* profile, boost::intrusive_ptr<TreeItem> item, ts_t* tree_screen, Editentry* editentry, Entrance* entrance, Browser* browser, TabWidget* tabmanager, rctrl_t* record_controller, WebView* parent)
...
void WebPage::record_info_update(const QUrl& url, const QString& title)
{
    ...
    if (_binder->host()) {
        ...
        if (_editentry->item() != _binder->host() && is_current)
...
void WebPage::metaeditor_sychronize()
{
    ...
    // } catch(std::exception &) {}
    // if(record_index) {
    if (_editentry->item() != current_item)
        _editentry->bind(current_item);
    ...
    _editentry->save_callback(Editor::editor_save_callback); // _editentry->editor_save_callback
    ...
    if (current_item->field<crypt_type>() == "1") // table->field(pos, boost::mpl::c_str < crypt_type > ::value)
        if (full_dir.length() == 0 || current_file.length() == 0)
            _editentry->dir_file_empty_reaction(EditorWrap::DIRFILEEMPTY_REACTION_SUPPRESS_ERROR);
    ...
    // Если нет приаттаченных файлов
        ...
        _editentry->to_attach()->setIcon(_editentry->icon_attach_exists()); // Есть приаттаченные файлы
...
WebView::WebView(boost::intrusive_ptr<TreeItem> host_, Profile* profile // , bool openinnewtab
    ,
    ts_t* tree_screen, Editentry* editentry, Entrance* entrance, Browser* browser, TabWidget* tabmanager, rctrl_t* table_controller)
...
{ // boost::intrusive_ptr<TreeItem> item
    ...
    if (_host) {
        ...
        if (_page->_editentry->item() != _host)
