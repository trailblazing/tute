...
//		if(!parent) throw std::runtime_error("! parent");
		connect(_tabbar, &TabBar::newTab, [&](){
				auto it = TreeIndex::activate([&] {return _tree_screen->view()->source_model();}, _tree_screen->view()->current_item(), QUrl(web::Browser::_defaulthome), std::bind(&tv_t::move, _tree_screen->view(), std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(), web::Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(), web::Browser::_defaulthome);});
...
// When index is -1 index chooses the current tab
	...
	void TabWidget::moveTab(int fromIndex, int toIndex){
		...
		if(!_rctrl) _rctrl = _browser->record_screen()->record_controller();
	...
	WebView *TabWidget::select_as_current(WebView *v){
		...
		if(page){
			...
			if(static_cast<QModelIndex>(tree_screen->view()->source_model()->index(it)).isValid()){
				...
				if(!_main_window->windowTitle().contains(v->page()->title())){
// auto it = this->_tabmanager->current_item();
					if(page->title() != it->field<name_type>()) page->record_info_update(page->url(), page->title());
	...
	boost::intrusive_ptr<TreeItem> TabWidget::current_item(){
				  ...
				  if(_target_in_browser){  // controller != nullptr &&
					  ...
					  if(!index_on_tree.isValid()){
						  if(!static_cast<QModelIndex>(_tree_view->know_model_board()->index(_target_in_browser)).isValid()){
							  ...
							  _target_in_browser = TreeIndex::require_item(_target_in_browser->field<url_type>(), std::bind(&tv_t::move, _tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(), _url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), _url.toString().toStdString());}); // instance([&] {return _tree_view->source_model();}, _tree_view->current_item())->
						  }
					  ...
					  if(index_on_tree.isValid()){
						  if(_target_in_browser != _tree_view->current_item()) _tree_view->select_as_current(TreeIndex::require_treeindex([&] {return _tree_view->source_model();}, _target_in_browser));
						  if(!_rctrl) _rctrl = _browser->record_screen()->record_controller();
						  if((_rctrl->view()->current_item() != _target_in_browser) || (currentWebView() != _target_in_browser->page()->view())) _rctrl->select_as_current(_rctrl->index<pos_proxy>(_target_in_browser));
					  }
					  // else{
...
// }
	...
	WebView *TabWidget::newTab(boost::intrusive_ptr<RecordIndex> record_index, bool make_current){ // boost::intrusive_ptr<TreeItem> tab_brother, boost::intrusive_ptr<TreeItem> target
		...
		boost::intrusive_ptr<TreeItem> result_item = record_index->host();
		...
		if(view == nullptr){
			...
			if(view == nullptr){
				...
				if(item_with_same_url != result_item) result_item = TreeLevel::instance(TreeIndex::require_treeindex([&](){return _tree_screen->view()->source_model();}, result_item), item_with_same_url)->merge();
			...
			if(item_with_same_id != result_item) result_item = TreeLevel::instance(TreeIndex::require_treeindex([&](){return _tree_screen->view()->source_model();}, item_with_same_id), result_item)->merge(); // TreeIndex::instance([&](){return _tree_screen->view()->source_model();}, page_item, page_item->parent()), target);
			// if(!target->binder())target->binder(std::forward<boost::intrusive_ptr<TreeItem::coupler>>(view->page()->binder()));
		...
		if(!result_item->binder()->page()) result_item->binder()->page(view->page()); // result->bind();   // this is recursively call
	...
	void TabWidget::lineEditReturnPressed(){
		if(QLineEdit *lineEdit = qobject_cast<QLineEdit *>(sender())){
			...
			if(_lineedits->currentWidget() == lineEdit){auto v = currentWebView(); if(v) v->setFocus(); }
		}
	}
...
// deprecated, profile should be assigned at the construction of BrowserPage
	...
	void TabWidget::mouseDoubleClickEvent(QMouseEvent *event){
		if(  !childAt(event->pos())
		  ...
		  && event->pos().y() < (tabBar()->y() + tabBar()->height())){
			...
			// if(modelindex) {
			TreeIndex::activate([&] {return tree_view->source_model();}, current_item, QUrl(web::Browser::_defaulthome), std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(), web::Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(), web::Browser::_defaulthome);});
			// }
		...
		if(
		  ...
		  && event->pos().y() < (tabBar()->y() + tabBar()->height())){
			...
			if(!url.isEmpty() && url.isValid() && !url.scheme().isEmpty()){
				TreeIndex::activate([&] {return tree_view->source_model();}, current_item, url, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(), url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), url.toString().toStdString());});
			}
		}
	}
	...
	void TabWidget::loadUrlInCurrentTab(const QUrl &_url){
		...
		if(webView){
			...
			if(_item->page()->url() != _url){  // _item->page_valid() &&
				...
				auto it = TreeIndex::require_item(_url, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(), _url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), _url.toString().toStdString());}); // instance([&] {return tree_view->source_model();}, current_item)->
	...
	QByteArray TabWidget::saveState() const {
		...
		if(!_rctrl) rctrl = _browser->record_screen()->record_controller();
		else rctrl = _rctrl;
		...
		for(int i = 0; i < _count; ++i){
			...
			auto it	= rctrl->index<boost::intrusive_ptr<TreeItem> >(pos_proxy(i));
			if(it){
...
//	    if(stream.atEnd()) return false;
		if(!stream.atEnd()){
			qint32 marker;
			qint32 v;
			stream >> marker; std::get<0>(result) = marker;
			stream >> v; std::get<1>(result) = v;
//			if(marker != TabWidgetMagic || v != version) return false;
			if(marker == TabWidgetMagic && v == version){
				QStringList open_tabs;
				stream >> open_tabs; std::get<2>(result) = open_tabs;
				QString current_tab_id;
				stream >> current_tab_id; std::get<3>(result) = current_tab_id;
			}
		}
		return result;
	}
	...
	bool TabWidget::restoreState(const QByteArray &state){
		...
		for(int i = 0; i < open_tabs.count(); ++i){
			...
			if(tree_index){
				// if(i != 0){
				auto ti = tree_index->bind(_url, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(), _url.toStdString()) || url_equal(it_->field<url_type>().toStdString(), _url.toStdString());}, _browser);
...
// will fail if count()==0 or it not inside current tabmanager
	...
	RecordModel *TabWidget::source_model(){if(!_rctrl) _rctrl = _browser->record_screen()->record_controller(); return _rctrl->source_model();}

	rv_t *TabWidget::view(){if(!_rctrl) _rctrl = _browser->record_screen()->record_controller(); return _rctrl->view();}
