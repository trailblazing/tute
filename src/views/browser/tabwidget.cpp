...
TabWidget::TabWidget(Blogger* blogger_, Browser* browser_, wn_t* main_window_, web::Profile* profile_)
    : QTabWidget(browser_)
    ...
    , _fullscreennotification(nullptr)
{
    ...
    gl_paras->find_screen()->toolbarsearch()->lineedits(_lineedits);
    ...
    //		auto  _tree_screen->view()->current_item() = tree_view->current_item();// maybe no current item yet!
    //		auto parent = _tree_screen->view()->current_item()->parent();
    //		if(!parent) throw std::runtime_error("! parent");
    connect(_tabbar, &TabBar::newTab, [&]() {
	auto it = TreeIndex::activate([&] { return _tree_screen->view()->source_model(); }, _tree_screen->view()->current_item(), QUrl(web::Browser::_defaulthome), std::bind(&tv_t::move, _tree_screen->view(), std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4),
	    [](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(detail::to_string(it_->field<home_type>()).toStdString(), web::Browser::_defaulthome) || url_equal(detail::to_string(it_->field<url_type>()).toStdString(), web::Browser::_defaulthome); });
        if (currentWebView() != it->page()->view())
            setCurrentIndex(webViewIndex(it->page()->view()));
    });
    ...
    connect(this, &TabWidget::requestCloseTab, this, &TabWidget::closeTab); // added by hughvonyoung@gmail.com
    // connect(_tabbar, &TabBar::cloneTabSignal, this, &TabWidget::cloneTab);// removed by hughvonyoung@gmail.com
    ...
    {
        // _tabbar->hide();// hidden from css for sometime I can't call count() function if I hide tab bar at here
        //			_tabbar->show();
    }
    ...
    connect(_closetabaction, &QAction::triggered, [&](bool) { closeTab(webViewIndex(currentWebView())); }); // , this, &TabWidget::closeTab);
...
void TabWidget::moveTab(int fromIndex, int toIndex)
{
    ...
    auto source_index = static_cast<QModelIndex>(_rctrl->index<index_source>(pos_source(fromIndex)));
    auto target_index = static_cast<QModelIndex>(_rctrl->index<index_source>(pos_source(toIndex)));
    _rctrl->source_model()->moveRow(source_index.parent(), source_index.row(), target_index.parent(), target_index.row());
    if (_previous_index == fromIndex || _previous_index == toIndex)
        _previous_index = webViewIndex(pre_view);
}
...
// if(current_item){
// assert((current_item->page_valid() && current_item->page()) || (current_item->field<url_type>() == web::Browser::_defaulthome));
    ...
    if (page) {
        ...
        if (it) {
            if (static_cast<QModelIndex>(tree_screen->view()->source_model()->index(it)).isValid()) {
                ...
                if (page->title() != it->field<name_type>())
                    page->record_info_update(page->url(), page->title());
                _main_window->synchronize_title(it->field<name_type>());
...
WebView* TabWidget::currentWebView() const
{
    ...
    if (currentIndex() == -1) {
        ...
        //									 if(!_rctrl) const_cast<TabWidget *>(this)->_rctrl = _browser->record_screen()->record_controller();
        //									 return _rctrl->source_model();
        //								 }, nullptr, current_item))->activate(std::bind(&wn_t::find, gl_paras->main_window(), std::placeholders::_1));
    } else
              ...
              if (_target_item) { // controller != nullptr &&
                  ...
                  if (!index_on_tree.isValid()) {
                      if (!static_cast<QModelIndex>(_tree_view->know_model_board()->index(_target_item)).isValid()) {
                          auto _url = QUrl(_target_item->field<url_type>()); //
                          _target_item = TreeIndex::require_item(_target_item->field<url_type>(), std::bind(&tv_t::move, _tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), _url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), _url.toString().toStdString()); }); // instance([&] {return _tree_view->source_model();}, _tree_view->current_item())->
                      }
                  ...
                  if (!_main_nwindow->windowTitle().contains(view_current->page()->title())) {
...
WebView* TabWidget::view_no_pinned()
{
    ...
    for (int i = 0; i < count(); i++) {
        ...
        if (p) {
            auto it = p->host();
            if (it && it->field<pin_type>() == char_from_check_state[Qt::Unchecked]) {
                found++;
                if (found == 1)
                    r = webView(i);
                else
...
{ // boost::intrusive_ptr<TreeItem> tab_brother, boost::intrusive_ptr<TreeItem> target
    if (!_rctrl)
        _rctrl = _browser->record_screen()->record_controller();
    if (count() > 30) {
        ...
        //
        //
        // for(int i = 0; i < count(); i ++){	// for(std::vector<web::WebView *>::size_type i = 20; i < static_cast<std::vector<web::WebView *>::size_type>(count()); i ++){
            ...
            for (int i = 0; i < count(); i++) { // for(std::vector<web::WebView *>::size_type i = 20; i < static_cast<std::vector<web::WebView *>::size_type>(count()); i ++){
                ...
                if (v) {
                    ...
                    if (p) {
                        auto h = p->host();
                        if (h
                            && h->field<pin_type>() != char_from_check_state[Qt::Checked]
                            && v != currentWebView()
                            && v != webView(0)) {
                            //
                            // if(list_set.find(h) == list_set.end()) closeTab(webViewIndex(v));
                            if (list_v_set.find(v) == list_v_set.end())
                                closeTab(webViewIndex(v));
                        }
                    }
                }
    ...
    WebView* view = find([&](boost::intrusive_ptr<const ::Binder> b) { return b->host()->id() == result_item->id(); }); // view_no_pinned();   // nullptr;   // find(record);
    if (view == nullptr) {
        view = find([&](boost::intrusive_ptr<const ::Binder> b) { return url_equal(b->host()->field<url_type>().toStdString(), result_item->field<url_type>().toStdString()); });
        if (view == nullptr) {
            // if(view == nullptr) {
            ...
            view = new WebView(result_item, _profile // use record for return
                ,
                _tree_screen, _blogger, _browser_docker, _browser, this, _rctrl);
            // } else {
            ...
            //
            QString short_title = result_item->field<name_type>().leftJustified(5, '.', true);
            ...
            QIcon icon;
            //
            int index = insertTab(_previous_index + 1, view, icon, short_title); // index = _tabbar->insertPage(pre_index, view, icon, title);  //
            ...
            setTabToolTip(index, result_item->field<name_type>());
            ...
            if (item_with_same_url != result_item)
                result_item = TreeLevel::instance(TreeIndex::require_treeindex([&]() { return _tree_screen->view()->source_model(); }, result_item), item_with_same_url)->merge();
        ...
        if (item_with_same_id != result_item)
            result_item = TreeLevel::instance(TreeIndex::require_treeindex([&]() { return _tree_screen->view()->source_model(); }, item_with_same_id), result_item)->merge(); // TreeIndex::instance([&](){return _tree_screen->view()->source_model();}, page_item, page_item->parent()), target);
        // if(!target->binder())target->binder(std::forward<boost::intrusive_ptr<TreeItem::coupler>>(view->page()->binder()));
    ...
    assert(result_item == result_item || result_item->field<url_type>() == result_item->field<url_type>());
    // assert(result->record_binder()->bounded_page());
    if (!result_item->binder()->page())
        result_item->binder()->page(view->page()); // result->bind();   // this is recursively call
    ...
    if (result_item->field<name_type>() == "") {
        // result->field<name_type>(result->field<url_type>());
        result_item->binder()->page()->record_info_update(result_item->field<url_type>(), result_item->field<url_type>());
    }
    // view_no_pinned();
...
// assert(webView(index)->page()->host()->is_registered_to_browser() || webView(index)->page()->host()->field("url") == web::Browser::_defaulthome);
...
void TabWidget::webViewTitleChanged(const QString& title)
{
    ...
    if (currentIndex() == index)
        emit setCurrentTitle(title); // _real_title// "test"
    //
    // if(_real_title != "")
    if (title != webView->page()->title())
        webView->page()->record_info_update(webView->page()->url(), title);
...
void TabWidget::mouseDoubleClickEvent(QMouseEvent* event)
{
    if (!childAt(event->pos())
        ...
        if (!parent)
            throw std::runtime_error(formatter() << std::string(typeid(decltype(&TabWidget::mouseDoubleClickEvent)).name()) << " ! parent");
        ...
        // if(modelindex) {
        TreeIndex::activate([&] { return tree_view->source_model(); }, current_item, QUrl(web::Browser::_defaulthome), std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), web::Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(), web::Browser::_defaulthome); });
        // }
...
void TabWidget::mouseReleaseEvent(QMouseEvent* event)
{
    if (
        ...
        auto parent = current_item->parent();
        if (!parent)
            throw std::runtime_error(formatter() << typeid(decltype(&TabWidget::mouseReleaseEvent)).name() << "! parent");
        if (!url.isEmpty() && url.isValid() && !url.scheme().isEmpty()) {
            TreeIndex::activate([&] { return tree_view->source_model(); }, current_item, url, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), url.toString().toStdString()); });
        }
    }
}
...
void TabWidget::loadUrlInCurrentTab(const QUrl& _url)
{
    ...
    if (webView) {
        ...
        if (_item->page()->url() != _url) { // _item->page_valid() &&
            ...
            auto parent = current_item->parent();
            if (!parent)
                throw std::runtime_error(formatter() << typeid(decltype(&TabWidget::loadUrlInCurrentTab)).name() << "! parent");
            auto it = TreeIndex::require_item(_url, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), _url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), _url.toString().toStdString()); }); // instance([&] {return tree_view->source_model();}, current_item)->
...
QByteArray TabWidget::saveState() const
{
    ...
    for (int i = 0; i < _count; ++i) {
        ...
        if (it) {
            ...
            if (url != "" && url != web::Browser::_defaulthome
                && (it->field<pin_type>() != char_from_check_state[Qt::Unchecked] || (it->page() && it->page()->view() == currentWebView())))
                tabs_url.append(url);
        }
    }
    stream << tabs_url;
    ...
    if (v) {
        ...
        if (p) {
            auto h = p->host();
            if (h)
                stream << h->field<id_type>(); // currentIndex();
        }
    }
    return data;
}
...
bool TabWidget::restoreState(const QByteArray& state)
{
    ...
    for (int i = 0; i < open_tabs.count(); ++i) {
        ...
        boost::intrusive_ptr<TreeIndex> tree_index = TreeIndex::require_treeindex([&] { return tree_view->source_model(); }, tree_view->current_item());
        if (tree_index) {
            // if(i != 0){
            auto ti = tree_index->bind(_url, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), _url.toStdString()) || url_equal(it_->field<url_type>().toStdString(), _url.toStdString()); }, _browser);
    ...
    if (ret == QMessageBox::Ok) {
        ...
        if (state == QWebEngineDownloadItem::DownloadRequested || state == QWebEngineDownloadItem::DownloadInProgress) {
            ...
            if (state == QWebEngineDownloadItem::DownloadRequested)
                sapp_t::request_download_manager()->download(this, download);
        }
        ...
        if (b) {
            ...
            if (p) {
                ...
                if (v) {
                    ...
                    if (index < 0)
                        index = 0; // count() - 1;		// if(index != - 1 && index > 0)
...
rctrl_t* TabWidget::record_controller() { return _rctrl; }
...
WebView* TabWidget::find_nopin() const
{
    ...
    for (int i = 0; i < count(); i++) {
        ...
        if (vi != nullptr) {
            if (vi->page()->host()) {
                if (vi->page()->host()->field<pin_type>() == char_from_check_state[Qt::Unchecked]) {
                    bv = vi;
                    break;
                }
            }
...
////        boost::intrusive_ptr<TreeItem> result = _tabmanager->_record_controller->source_model()->find(QUrl(web::Browser::_defaulthome));
////        WebView *view = nullptr;
...
// assert(_item_link->is_registered_to_browser() || (_item_link->field("url") == web::Browser::_defaulthome));
