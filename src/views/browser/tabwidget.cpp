                ...
                if(_target_in_browser){	// controller != nullptr &&
                    ...
                    if(! index_on_tree.isValid()){
                        if(! static_cast<QModelIndex>(_tree_view->know_model_board()->index(_target_in_browser)).isValid()){
                            ...
                            _target_in_browser = TreeIndex::treeitem_from_url(_target_in_browser->field<url_type>()
...
//		_tabbar->setTabButton(index, QTabBar::RightSide, new FlatToolButton(_tabbar));
                ...
                if(item_with_same_url != result) result = TreeLevel::instance(TreeIndex::treeindex_from_item([&](){return _tree_screen->view()->source_model();}, item_with_same_url), result)->merge();																																																																																																																																																																																																																																																																																																					// TreeIndex::instance([&](){return _tree_screen->view()->source_model();}, page_item, page_item->parent()), target);
...
//		result = page_item;	// target;
            ...
            if(item_with_same_id != result) result = TreeLevel::instance(TreeIndex::treeindex_from_item([&](){return _tree_screen->view()->source_model();}, item_with_same_id), result)->merge();																																																																																																																																																																																																																																																																																													// TreeIndex::instance([&](){return _tree_screen->view()->source_model();}, page_item, page_item->parent()), target);
            // if(!target->binder())target->binder(std::forward<boost::intrusive_ptr<TreeItem::coupler>>(view->page()->binder()));
...
// }
    ...
    void TabWidget::loadUrlInCurrentTab(const QUrl &_url){
        ...
        if(webView){
            ...
            if(_item->page()->url() != _url){		// _item->page_valid() &&
                ...
                auto	it	= TreeIndex::treeitem_from_url(_url, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(), _url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), _url.toString().toStdString());});// instance([&] {return tree_view->source_model();}, current_item)->
    ...
    bool TabWidget::restoreState(const QByteArray &state){
        ...
        for(int i = 0; i < open_tabs.count(); ++ i){
            ...
            boost::intrusive_ptr<TreeIndex> tree_index = TreeIndex::treeindex_from_item([&] {return tree_view->source_model();}, tree_view->current_item());
            if(tree_index){
                if(i != 0){
                    ...
                    if(webView(0)->page()->url() != _url){
                        ...
                        auto it =	// tree_index->
                            TreeIndex::treeitem_from_url(QUrl(_url), std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const TreeItem> it_) -> bool {return url_equal(it_->field<home_type>().toStdString(), _url.toStdString()) || url_equal(it_->field<url_type>().toStdString(), _url.toStdString());});
