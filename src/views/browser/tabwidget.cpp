    ...
    , _fullscreennotification(nullptr)
{
    ...
    connect(
        _tabbar, &TabBar::newTab, [&, tree_view, parent, current_treeitem]() {
            auto it = TreeIndex::activate([&] { return tree_view->source_model(); }, current_treeitem, QUrl(Browser::_defaulthome), std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [](boost::intrusive_ptr<const TreeItem> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(), Browser::_defaulthome); });
    ...
    connect(_newtabaction, &QAction::triggered // , this
        ...
        [&, tree_view, parent, current_treeitem](bool make_current) {
            ...
            auto it = TreeIndex::activate([&] { return tree_view->source_model(); }, current_treeitem, QUrl(Browser::_defaulthome), std::bind(&tv_t::move, _tree_screen->view(), std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [](boost::intrusive_ptr<const TreeItem> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(), Browser::_defaulthome); });
            if (currentWebView() != it->page()->view())
                setCurrentIndex(webViewIndex(it->page()->view()));
        });
...
WebView* TabWidget::select_as_current(WebView* v)
{
    ...
    if (static_cast<QModelIndex>(tree_screen->view()->source_model()->index(it)).isValid()) {
        if (tree_screen->view()->current_item() != it)
            tree_screen->view()->select_as_current(TreeIndex::create_treeindex_from_item([&] { return tree_screen->view()->source_model(); }, it));
        if (_record_controller->view()->current_item() != it)
            _record_controller->select_as_current(_record_controller->index<pos_proxy>(pos_source(index)));
    } else
        closeTab(index);
        ...
        if (_target_in_browser) { // controller != nullptr &&
            ...
            if (!index_on_tree.isValid()) {
                if (!static_cast<QModelIndex>(_tree_view->know_model_board()->index(_target_in_browser)).isValid()) {
                    ...
                    _target_in_browser = TreeIndex::create_treeitem_from_url(_target_in_browser->field<url_type>(), std::bind(&tv_t::move, _tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const TreeItem> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), _url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), _url.toString().toStdString()); }); // instance([&] {return _tree_view->source_model();}, _tree_view->current_item())->
                }
            ...
            if (index_on_tree.isValid()) {
                if (_target_in_browser != _tree_view->current_item())
                    _tree_view->select_as_current(TreeIndex::create_treeindex_from_item([&] { return _tree_view->source_model(); }, _target_in_browser));
                if ((_record_controller->view()->current_item() != _target_in_browser) || (currentWebView() != _target_in_browser->page()->view()))
                    _record_controller->select_as_current(_record_controller->index<pos_proxy>(_target_in_browser));
            }
            //                    else{
            ...
            if (!_mainwindow->windowTitle().contains(view_current->page()->title()))
                _mainwindow->setWindowTitle(QString(program_title) + " : " + view_current->page()->title()); // webView->setFocus();
            ...
            if (_editing_window->item() != _target_in_browser)
                view_current->page()->metaeditor_sychronize(); // metaeditor->bind(record);
        }
    };
    ...
    if (view == nullptr) {
        ...
        if (view == nullptr) {
            ...
            view = new WebView(result, _profile // use record for return
                ,
                _tree_screen, _editing_window, _entrance, _browser, this, _record_controller);
            // } else {
            ...
            if (item_with_same_url != result)
                result = TreeLevel::instance(TreeIndex::create_treeindex_from_item([&]() { return _tree_screen->view()->source_model(); }, result), item_with_same_url)->merge();
        ...
        if (item_with_same_id != result)
            result = TreeLevel::instance(TreeIndex::create_treeindex_from_item([&]() { return _tree_screen->view()->source_model(); }, item_with_same_id), result)->merge(); // TreeIndex::instance([&](){return _tree_screen->view()->source_model();}, page_item, page_item->parent()), target);
        // if(!target->binder())target->binder(std::forward<boost::intrusive_ptr<TreeItem::coupler>>(view->page()->binder()));
...
{ // , bool sychronize_to_record_view
    ...
    if (index_to_close >= 0 && index_to_close < count()) { // if(index < 0 || index >= count())
        ...
        if (WebView* _view_to_close = webView(index_to_close)) {
            ...
            if (!view_privious)
                view_privious = count() > 0 ? webView(-1 == _previous_index ? 0 : _previous_index) : nullptr;
            if (view_privious)
...
void TabWidget::webViewTitleChanged(const QString& title)
{
    ...
    if (currentIndex() == index)
        emit setCurrentTitle(_real_title); // "test"
    //
    //	if(_real_title != "")
...
void TabWidget::mouseDoubleClickEvent(QMouseEvent* event)
{
    if (!childAt(event->pos())
        ...
        // if(modelindex) {
        TreeIndex::activate([&] { return tree_view->source_model(); }, current_item, QUrl(Browser::_defaulthome), std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [](boost::intrusive_ptr<const TreeItem> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), Browser::_defaulthome) || url_equal(it_->field<url_type>().toStdString(), Browser::_defaulthome); });
        // }
