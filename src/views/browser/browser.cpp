...
Browser::Browser(ts_t* _tree_screen, FindScreen* _find_screen, Editentry* _editentry, rs_t* _record_screen, Entrance* _entrance, //		    , wn_t *main_window
    const QString& style_source, Profile* _profile, Qt::WindowFlags flags)
    : QMainWindow(0, flags) // ,  boost::intrusive_ref_counter<Browser, boost::thread_safe_counter>()
    , _tree_screen(_tree_screen)
    , _find_screen(_find_screen)
    , _main_window(globalparameters.main_window())
    // , _toolbarsearch(_find_screen->toolbarsearch())
    ...
    , _entrance(_entrance->prepend(this))
...
void Browser::save()
{
    ...
    std::shared_ptr<QSettings> settings = std::make_shared<QSettings>(globalparameters.root_path() + "/" + globalparameters.target_os() + "/" + globalparameters._browser_conf_filename, QSettings::IniFormat);
