...
//		this->_destroyed_request_triggered = true;
        ...
        if (_find_screen) {
            if (_find_screen->navigater() == _navigater)
                _find_screen->switch_navigater();
        }
    }
    ...
    void Browser::activateWindow()
    {
        ...
        if (gl_paras->editor_docker()->widget() != _blogger)
            gl_paras->editor_docker()->prepend(_blogger);
    ...
    bool Browser::restore_state(const QByteArray& state)
    {
        ...
        stream >> marker;
        stream >> v;
        if (marker != Browser::browser_magic || v != version)
            return false;
        ...
        if (!_tab_widget->restoreState(tabState))
            return false;
    ...
    void Browser::init_main_menu()
    {
        ...
        if (rctrl)
            rctrl->view()->assembly_context_menu(_main_window->main_menu_map());
    }
...
//this, &Browser::slotWebSearch, QKeySequence(tr("Ctrl+K", "Web Search")));
        ...
        _tree_screen->init_context_menu();
        //		_tree_screen->append_main_menu();
        if (menuBar() != _main_window->menuBar())
            setMenuBar(_main_window->menuBar());
...
#define QT_NO_PRINTPREVIEWDIALOG
    ...
    void Browser::slotLoadProgress(int progress)
    {
        ...
        if (_chasewidget) {
            ...
            if (progress < 100 && progress > 0) {
                ...
                if (_reloadicon.isNull())
                    _reloadicon = QIcon(":/resource/pic/mobile_reload.svg");
                //				try {
    ...
    void Browser::on_topic_changed(const QString& original_topic_, const QString& topic_, bool append_mode)
    {
            ...
            this->init_configuration( // std::make_unique<QSettings>(_current_topic_folder_name + "/" + gl_para::_browser_conf_filename, QSettings::IniFormat)
                [&]() -> std::unique_ptr<QSettings> {
                    if (!QDir(dest_topic_folder).exists())
                        if (!QDir::root().mkpath(dest_topic_folder))
                            critical_error("Can not create directory: \"" + dest_topic_folder + "\"");
                    if (!QFile(_current_topic_config_name).exists()) {
        ...
        if (this->windowTitle() == new_topic)
            this->setWindowTitle(new_topic); //.leftJustified(8, '.', true)
        this->save();
    }
    //	void Browser::test()
    //	{
