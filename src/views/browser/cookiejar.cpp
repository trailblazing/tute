...
void CookieJar::loadSettings() {
	...
	QByteArray value = settings
	                       .value(QLatin1String("acceptCookies"), QLatin1String("AcceptOnlyFromSitesNavigatedTo"))
	                       .toByteArray();
	...
	value = settings
	            .value(QLatin1String("keepCookiesUntil"), QLatin1String("KeepUntilExpire"))
	            .toByteArray();
...
void CookieJar::save() {
	...
	settings.setValue(
	    ...
	    QLatin1String(acceptPolicyEnum.valueToKey(_accept_cookies)));
...
bool CookieJar::setCookiesFromUrl(const QList<QNetworkCookie> &cookieList, const QUrl &url) {
	...
	if ((acceptInitially && !eBlock) ||
		...
		foreach (QNetworkCookie cookie, cookieList) {
			...
			if (_keep_cookies == KeepUntilTimeLimit && !cookie.isSessionCookie() &&
...
int CookieModel::rowCount(const QModelIndex &parent) const {
	return (parent.isValid() || !m_cookieJar) ? 0
	                                          : m_cookieJar->allCookies().count();
}
...
int CookieExceptionsModel::rowCount(const QModelIndex &parent) const {
	return (parent.isValid() || !m_cookieJar) ? 0 : m_allowedCookies.count() +
	        m_blockedCookies.count() +
