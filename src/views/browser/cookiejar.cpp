...
QDataStream &operator <<(QDataStream &stream, const QList<QNetworkCookie> &list){
	...
	for(int i = 0; i < list.size(); ++i) stream << list.at(i).toRawForm();
	return stream;
}
...
QDataStream &operator >>(QDataStream &stream, QList<QNetworkCookie> &list){
	...
	quint32 version;
	stream >> version;
	if(version != JAR_VERSION) return stream;
	quint32 count;
	stream >> count;
	for(quint32 i = 0; i < count; ++i){
		...
		if(newCookies.count() == 0 && value.length() != 0) qWarning() << "CookieJar: Unable to parse saved cookie:" << value;
		for(int j = 0; j < newCookies.count(); ++j) list.append(newCookies.at(j));
...
namespace web {
	...
	CookieJar::~CookieJar(){
		if(_keep_cookies == KeepUntilExit) clear();
	...
	void CookieJar::loadSettings(){
		...
		if(_keep_cookies == KeepUntilExit) setAllCookies(QList<QNetworkCookie>());
		_loaded = true;
		emit cookiesChanged();
	}
	...
	void CookieJar::save(){
		...
		for(int i = cookies.count() - 1; i >= 0; --i)
			if(cookies.at(i).isSessionCookie()) cookies.removeAt(i);
	...
	void CookieJar::purgeOldCookies(){
		...
		for(int i = cookies.count() - 1; i >= 0; --i)
			if(!cookies.at(i).isSessionCookie() && cookies.at(i).expirationDate() < now) cookies.removeAt(i);
		  ...
		  || (!acceptInitially && (eAllow || eAllowSession))){
			...
			foreach(QNetworkCookie cookie, cookieList){
				...
				if(  _keep_cookies == KeepUntilTimeLimit
				  ...
				  && cookie.expirationDate() > soon) cookie.setExpirationDate(soon);
				lst += cookie;
				if(QNetworkCookieJar::setCookiesFromUrl(lst, url)) addedCookies = true;
				else{
	...
	CookieJar::AcceptPolicy CookieJar::acceptPolicy() const {
		if(!_loaded) (const_cast<CookieJar *>(this))->load();
		return _accept_cookies;
	}

	void CookieJar::setAcceptPolicy(AcceptPolicy policy){
		if(!_loaded) load();
		if(policy == _accept_cookies) return;
	...
	void CookieJar::setKeepPolicy(KeepPolicy policy){
		if(!_loaded) load();
		if(policy == _keep_cookies) return;
	...
	QVariant CookieModel::data(const QModelIndex &index, int role) const {
		QList<QNetworkCookie> lst;
		if(m_cookieJar) lst = m_cookieJar->allCookies();
	...
	bool CookieModel::removeRows(int row, int count, const QModelIndex &parent){
		if(parent.isValid() || !m_cookieJar) return false;
	...
	void CookiesExceptionsDialog::block(){
		if(domainLineEdit->text().isEmpty()) return;
