...
#endif
        public : PopupPage(PopupView* view_, Browser* browser_, Profile* profile_);
    Browser* browser();
...
public:
    WebPage(Profile* profile, boost::intrusive_ptr<i_t> item, ts_t* tree_screen, Blogger* editentry, web::Docker* browser_docker_, Browser* browser, TabWidget* tabmanager, rctrl_t* record_controller, WebView* parent = 0);
    ...
    boost::intrusive_ptr<i_t> host() const; // {return _record_binder->bounded_item();}
    ...
    public:
        ...
        ~Binder();
        void host(boost::intrusive_ptr<i_t> it) { _host = it; }

        void page(WebPage* p) { _page = p; }

        boost::intrusive_ptr<i_t> host() const { return _host; }
...
public:
    WebView(boost::intrusive_ptr<i_t> host_, Profile* profile // , bool openinnewtab
        ,
