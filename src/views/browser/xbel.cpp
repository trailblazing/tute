...
namespace web {
	...
	bool BookmarkNode::operator ==(const BookmarkNode &other){
		if(  url != other.url
		  ...
		  || _type != other._type
		  || _children.count() != other._children.count()) return false;
	...
	void BookmarkNode::add(BookmarkNode *child, int offset){
		...
		child->_parent = this;
		if(-1 == offset) offset = _children.size();
	...
	BookmarkNode *XbelReader::read(const QString &fileName){
		QFile file(fileName);
		if(!file.exists()) return new BookmarkNode(BookmarkNode::Root);
	...
	BookmarkNode *XbelReader::read(QIODevice *device){
		...
		if(readNextStartElement()){
			...
			if(  name() == QLatin1String("xbel")
			  && (version.isEmpty() || version == QLatin1String("1.0"))) readXBEL(root);
			else raiseError(QObject::tr("The file is not an XBEL version 1.0 file."));
		}
		return root;
	}
	...
	void XbelReader::readXBEL(BookmarkNode *parent){
		...
		while(readNextStartElement()){
			if(name() == QLatin1String("folder")) readFolder(parent);
			else if(name() == QLatin1String("bookmark")) readBookmarkNode(parent);
			else if(name() == QLatin1String("separator")) readSeparator(parent);
	...
	void XbelReader::readFolder(BookmarkNode *parent){
		...
		while(readNextStartElement()){
			if(name() == QLatin1String("title")) readTitle(folder);
			else if(name() == QLatin1String("desc")) readDescription(folder);
			else if(name() == QLatin1String("folder")) readFolder(folder);
			else if(name() == QLatin1String("bookmark")) readBookmarkNode(folder);
			else if(name() == QLatin1String("separator")) readSeparator(folder);
			else skipCurrentElement();
		}
	}
	...
	void XbelReader::readBookmarkNode(BookmarkNode *parent){
		...
		while(readNextStartElement()){
			if(name() == QLatin1String("title")) readTitle(bookmark);
			else if(name() == QLatin1String("desc")) readDescription(bookmark);
			else skipCurrentElement();
		}
		if(bookmark->title.isEmpty()) bookmark->title = QObject::tr("Unknown title");
	}
	...
	bool XbelWriter::write(const QString &fileName, const BookmarkNode *root){
		QFile file(fileName);
		if(!root || !file.open(QFile::WriteOnly)) return false;
	...
	bool XbelWriter::write(QIODevice *device, const BookmarkNode *root){
		...
		if(root->type() == BookmarkNode::Root)
			for(int i = 0; i < root->children().count(); ++i) writeItem(root->children().at(i));
		else writeItem(root);
		writeEndDocument();
		return true;
	}
	...
	void XbelWriter::writeItem(const BookmarkNode *parent){
		switch(parent->type()){
			case BookmarkNode::Folder:
				...
				for(int i = 0; i < parent->children().count(); ++i) writeItem(parent->children().at(i));
				writeEndElement();
				break;

			case BookmarkNode::Bookmark:
				writeStartElement(QLatin1String("bookmark"));
				if(!parent->url.isEmpty()) writeAttribute(QLatin1String("href"), parent->url);
				writeTextElement(QLatin1String("title"), parent->title);
				if(!parent->desc.isEmpty()) writeAttribute(QLatin1String("desc"), parent->desc);
				writeEndElement();
				break;
