...
namespace web {
	...
	QNetworkReply* NetworkAccessManager::createRequest(Operation op, const QNetworkRequest& req, QIODevice* outgoingData)
	{
		...
		// this is a temporary hack until we properly use the pipelining flags from
		// QtWebkit
		// pipeline everything! :)
	...
	void NetworkAccessManager::requestFinished(QNetworkReply* reply)
	{
		...
		if (reply->attribute(QNetworkRequest::HttpPipeliningWasUsedAttribute)
			.toBool() == true)
			requestFinishedPipelinedCount++;
		if (reply->attribute(QNetworkRequest::ConnectionEncryptedAttribute)
			.toBool() == true)
			requestFinishedSecureCount++;
		if (reply->attribute(QNetworkRequest::DownloadBufferAttribute).isValid() ==
		    true)
		...
		qDebug(
		    "STATS [%lli requests total] [%3.2f%% from cache] [%3.2f%% pipelined] "
		    "[%3.2f%% SSL/TLS] [%3.2f%% Zerocopy]",
		    requestFinishedCount, pctCached, pctPipelined, pctSecure,
		    pctDownloadBuffer);
		//
	...
	void NetworkAccessManager::sslErrors(QNetworkReply* reply, const QList<QSslError>& error)
	{
		...
		QString replyHost =
		    reply->url().host() + QString(":%1").arg(reply->url().port());
		if (!sslTrustedHostList.contains(replyHost)) {
			...
			int ret = QMessageBox::warning(brwoser_docker_, QCoreApplication::applicationName(),
