...
namespace web {
	...
	// public:
	// explicit DockWidget(QWidget *parent, DockedWindow *browser, Qt::WindowFlags
	...
	class Docker : public QDockWidget {
		...
		// std::pair<Browser *, WebView *> invoke_page(Record *const record);  //=
		// register_record(QUrl(DockedWindow::_defaulthome))
		...
		// Entrance    *_the;
		////            WebPage     *_page;
		// ActiveRecordBinder(Entrance *the): _the(the)    //              ,
		// _page(nullptr)
		// {}

		// WebView *binder(boost::intrusive_ptr<TreeItem> item,
		// boost::intrusive_ptr<TreeItem>(TreeItem::* _bind)(WebPage *))
		// {
		...
		// {assert(item);assert(item->page_valid()); return
		// item->unique_page()->active();}
		// };

		// Browser		*activated_browser();
		// std::set<Browser *> &browsers();	//        QList<DockedWindow*>
		// window_raw_list();

		////        void clean();

		////        std::pair<DockedWindow *, WebView *> active_record(Record *const
		///record);

		// boost::intrusive_ptr<TreeItem> item_bind(RecordModel::ModelIndex
		// modelindex);    // = boost::intrusive_ptr<Record>(nullptr)

		////        //        template<typename url_type>
		////        void activate(const QUrl &_find_url
		////                      , const TreeScreen::paste_strategy
		///&_view_paste_strategy
		////                      , equal_url_t _equal = [](boost::intrusive_ptr<const
		///TreeItem> it, const QUrl &_url)->bool {return it->field("url") ==
		///_url.toString();}
		////                     );
		...
		////                      , const TreeScreen::paste_strategy
		///&_view_paste_strategy
		////                      , equal_t _equal = [](boost::intrusive_ptr<const
		///TreeItem> it, boost::intrusive_ptr<const TreeItem> target)->bool {return
		///it->id() == target->id();}
		////                     );
		...
		// std::pair<Browser *, WebView *>
		// WebView *find(const std::function<bool (boost::intrusive_ptr<const
		// ::Binder>)> &_equal) const;
...
//		void ssl_errors(QNetworkReply *reply, const QList<QSslError>
//&errors);
...
// void on_splitter_moved(int pos, int index);
	...
	//
	template <>
	struct Docker::initialize_prior_to<wn_t> {
		static constexpr bool value = false;
	};
	template <>
	struct Docker::initialize_prior_to<sapp_t> {
		static constexpr bool value = false;
	};
	//	//
	...
	//		    ,
	//		    Qt::WindowFlags flags = 0)
	//		    : Docker(object_name, main_window, flags)
	//		{
	//		}
	...
	extern template Docker* Docker::prepend(Blogger* child_, Qt::WindowFlags flags = Qt::Widget);
	extern template Docker* Docker::prepend(Browser* child_, Qt::WindowFlags flags = Qt::Widget);
}
