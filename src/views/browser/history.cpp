...
void HistoryManager::setHistory(const QList<HistoryItem> &history, bool loadedAndSorted) {
	...
	if (!loadedAndSorted)
		qSort(_history.begin(), _history.end());
	checkForExpired();
	if (loadedAndSorted)
...
void HistoryManager::checkForExpired() {
	if (_historylimit < 0 || _history.isEmpty())
		return;
	...
	if (nextTimeout > 0)
		_expiredtimer.start(nextTimeout * 1000);
}
...
void HistoryManager::updateHistoryItem(const QUrl &url, const QString &title) {
	for (int i = 0; i < _history.count(); ++i) {
		if (url == _history.at(i)._url) {
			...
			_savetimer->changeOccurred();
			if (_lastsavedurl.isEmpty())
				_lastsavedurl = _history.at(i)._url;
			emit entryUpdated(i);
			break;
		}
	}
...
void HistoryManager::load() {
	...
	while (!historyFile.atEnd()) {
		...
		stream >> item._title;
		if (!item._datetime.isValid())
			continue;
		if (item == lastInsertedItem) {
			if (lastInsertedItem._title.isEmpty() && !list.isEmpty())
				list[0]._title = item._title;
			continue;
		}
...
QVariant HistoryModel::data(const QModelIndex &index, int role) const {
	...
	switch (role) {
		...
		case Qt::DecorationRole:
			if (index.column() == 0)
				return sapp_t::instance()->icon(item._url);
	}
	return QVariant();
}
...
QModelIndex HistoryMenuModel::mapToSource(const QModelIndex &proxyIndex) const {
	...
	if (proxyIndex.internalId() == quintptr(-1)) {
		...
		if (proxyIndex.row() < bumpedItems)
			return _treemodel->index(proxyIndex.row(), proxyIndex.column(), _treemodel->index(0, 0));
		if (bumpedItems <= MOVEDROWS &&
		    bumpedItems == sourceModel()->rowCount(_treemodel->index(0, 0)))
			--bumpedItems;
		return _treemodel->index(proxyIndex.row() - bumpedItems, proxyIndex.column());
	}
	QModelIndex historyIndex = _treemodel->sourceModel()->index(
...
QModelIndex HistoryMenuModel::index(int row, int column, const QModelIndex &parent) const {
	...
	int bumpedItems = 0;
	if (treeIndexParent == _treemodel->index(0, 0))
		bumpedItems = bumpedRows();
	QModelIndex treeIndex =
	    _treemodel->index(row + bumpedItems, column, treeIndexParent);
...
QModelIndex HistoryMenuModel::parent(const QModelIndex &index) const {
	...
	QModelIndex historyIndex =
	    _treemodel->sourceModel()->index(index.internalId(), 0);
	QModelIndex treeIndex = _treemodel->mapFromSource(historyIndex);
...
bool HistoryMenu::prePopulated() {
	if (!_history) {
		...
		_historymenumodel =
		    new HistoryMenuModel(_history->historyTreeModel(), this);
		setModel(_historymenumodel);
	}
	// initial actions
	for (int i = 0; i < _initialactions.count(); ++i)
		addAction(_initialactions.at(i));
	if (!_initialactions.isEmpty())
		addSeparator();
...
HistoryFilterModel::mapFromSource(const QModelIndex &sourceIndex) const {
	...
	// because it can't take: qBinaryFind(_sourcerow.end(), _sourcerow.begin(),
	// v);
	// so if this is a performance bottlneck then convert to binary search, until
	// then
...
/*
            Removing a continuous block of rows will remove filtered rows too as
   this is
            the users intention.
 */
...
QVariant HistoryCompletionModel::data(const QModelIndex &index, int role) const {
	if (sourceModel() && (role == Qt::EditRole || role == Qt::DisplayRole) &&
	    index.isValid()) {
...
QModelIndex
HistoryCompletionModel::mapFromSource(const QModelIndex &sourceIndex) const {
...
QModelIndex
HistoryCompletionModel::mapToSource(const QModelIndex &proxyIndex) const {
	if (!sourceModel())
		return QModelIndex();
...
void HistoryCompletionModel::setSourceModel(
...
QVariant HistoryTreeModel::data(const QModelIndex &index, int role) const {
	...
	if (role == HistoryModel::DateRole && index.column() == 0 &&
	    index.internalId() == 0) {
		int offset = sourceDateRow(index.row());
		QModelIndex idx = sourceModel()->index(offset, 0);
...
int HistoryTreeModel::rowCount(const QModelIndex &parent) const {
	...
	// row count OF dates
	if (!parent.isValid()) {
		if (!_sourcerowcache.isEmpty())
			return _sourcerowcache.count();
		QDate currentDate;
		int rows = 0;
...
void HistoryTreeModel::sourceRowsRemoved(const QModelIndex &parent, int start, int end) {
	...
	for (int i = end; i >= start;) {
		...
		for (int j = row; j < _sourcerowcache.count(); ++j)
			--_sourcerowcache[j];
		endRemoveRows();
	}
}
...
bool HistoryItem::operator==(const HistoryItem &other) const {
	return other._title == _title && other._url == _url &&
