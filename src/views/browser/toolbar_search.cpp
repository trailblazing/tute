        ...
        if (0 >= result_item->count_direct()) {
            ...
            if (tree_index) {
                ...
                if (!url.isEmpty()
                    ...
                    auto ti = tree_index->bind(url, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), url.toString().toStdString()); }, browser);
                    if (ti)
                    ...
                    auto ti = tree_index->bind(search_engine, std::bind(&tv_t::move, tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), search_engine.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), search_engine.toString().toStdString()); }, browser);
                    if (ti)
                        ti->activate(std::bind(&wn_t::find, gl_paras->main_window(), std::placeholders::_1));
                }
            }
