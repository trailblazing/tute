...
// entrance.ini
class BrowserDock : public QDockWidget {
#if QT_VERSION == 0x050600
    W_OBJECT(Entrance)
#else
    Q_OBJECT
...
public:
    BrowserDock(QString object_name, ts_t* _tree_screen, FindScreen* _find_screen
        //		    , Editentry *_editentry	// , RecordController *_record_controller
        ,
        wn_t* _main_window
        //			   , std::shared_ptr<AppConfig> appconfig_
        ,
        const QString& style_source, browser::Profile* _profile, Qt::WindowFlags flags
        = 0);
    ~BrowserDock();
