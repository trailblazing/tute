...
QMap<QString, Qt::CheckState> _state_check_from_string {
	{QChar(0x221A) // for √ // QChQChar(0x221A)//for✅// ar(0x2705)	// QChar(Qt::UNICODE_ACCEL + 2713) // QChar(Qt::ALT + 251)  //  "v"    //QChar(0xfb)
	 ,
...
bool InfoFieldEnter::checkFieldName(QString name){
	if(name == "pin" || name == "name" || name == "author" || name == "home" || name == "url" || name == "tags") return true;
	else return false;
}
...
QString InfoFieldEnter::getField(QString name){
	...
	}else critical_error("Can not get field " + name + " in InfoFieldEnter method get_field");
	return QString();
}
...
void InfoFieldEnter::setField(QString name, QString value){
	...
	}else critical_error("Can not set field " + name + " in InfoFieldEnter method set_field");
}
