...
{ // , _hidetitlebar(new QWidget(this, Qt::FramelessWindowHint | Qt::CustomizeWindowHint))
    ...
    // Editor::update_indentline_geometry();
    connect(_item_pin, &QCheckBox::clicked, [&](bool checked) { pin(checked ? char_from_check_state[Qt::CheckState::Checked] : char_from_check_state[Qt::CheckState::Unchecked]); });
}
...
boost::intrusive_ptr<i_t> EditorWrap::item() { return _item; }
...
void EditorWrap::pin(const QString& pin_)
{
    ...
    if (_item && (check_state_from_char[_item->field<pin_type>()] != _item_pin->checkState()))
        set_pin(pin_ != "");
}
