...
EditorWrap::EditorWrap(FindScreen *find_screen, Blogger *editing_win, std::shared_ptr<QSettings> topic_editor_config_, QStringList hide_editor_tools_, QStackedWidget *main_stack_, QString object_name)
    ...
    , _blog(editing_win) {  // , _hidetitlebar(new QWidget(this,
	...
	connect(_item_pin, &QCheckBox::clicked, [&](bool checked) {
		pin(checked ? char_from_check_state[Qt::CheckState::Checked]
		            : char_from_check_state[Qt::CheckState::Unchecked]);
	});
}
			...
			if (source_model && -1 != pos_source_) {
				...
				if (record_view) {
					_record_controller->select_as_current(
					    _record_controller->index<pos_proxy>(pos_source_));
