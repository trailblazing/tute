...
class EditorWrap : public Editor {
	...
	void bind(boost::intrusive_ptr<i_t> item_to_be_bound);  // {_record = r;}

	boost::intrusive_ptr<i_t> item();  // {return _record;}
	...
	// save_callback() const {return _editor->save_callback();}
	// void save_callback(const std::function<void (QObject *editor, QString
	// saveString)> &func){_editor->save_callback(func);}

	// std::function<void (QObject *editor, QString &String)> load_callback()
	// const {return _editor->load_callback();}
	// void load_callback(const std::function<void (QObject *editor, QString
	// &String)> &func){_editor->load_callback(func);}
	// static void editor_load_callback(QObject *editor, QString
	// &loadText){Editor::editor_load_callback(editor, loadText);}
	// static void editor_save_callback(QObject *editor, QString
	// saveText){Editor::editor_save_callback(editor, saveText);}
	//// Абсолютный или относительный путь (т.е. директория),
	//// куда будет сохраняться текст. Без завершающего слеша
	// bool work_directory(QString dir_name){return
	// _editor->work_directory(dir_name);}
	...
	// void dir_file_empty_reaction(int
	// mode){_editor->dir_file_empty_reaction(mode);}
	// int dir_file_empty_reaction(void){return
	// _editor->dir_file_empty_reaction();}
	...
	// void misc_field(QString name, QString value){_editor->misc_field(name,
	// value);}
	...
	// QTextDocument * textarea_document(void){return
	// _editor->textarea_document();}
	// void textarea_editable(bool
	// editable){_editor->textarea_editable(editable);}
	...
	// QIcon icon_attach_not_exists() const {return
	// _editor->_icon_attach_not_exists;}

	// QVBoxLayout *textformat_buttons_layout() const {return
	// _editor->_textformat_buttons_layout;}
	...
	QLabel *_label_pin;
	// QLabel		*_tree_path_content;            // Надпись Path (только для
	...
	// Группировалка виджетов всех слоев (слоя редактирования и слоя прикрепляемых
	// файлов)
