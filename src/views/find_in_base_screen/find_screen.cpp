...
FindScreen::FindScreen(QString object_name, ts_t* tree_screen_, QWidget* parent) // boost::intrusive_ptr<TreeItem> _selected_branch_root
    : QWidget(parent) //
      ...
      // for(int i = 0; i < _tree_screen->know_branch()->root_item()->current_count(); i++) {
...
boost::intrusive_ptr<i_t> FindScreen::find_clicked(void)
{
...
boost::intrusive_ptr<i_t> FindScreen::find_start(void)
{
    ...
    auto _current_model = [&]() { return _tree_screen->view()->source_model(); }; // static_cast<KnowModel *>(_tree_screen->tree_view()->model());
    ...
    boost::fusion::at_key<name_type>(data) = ctime;
    boost::fusion::at_key<ctime_type>(data) = ctime;
    ...
    boost::intrusive_ptr<i_t> _start_item(nullptr); // = boost::intrusive_ptr<TreeItem>(new TreeItem(QMap<QString, QString>(), nullptr));
    ...
    // _result_item->child_transfer(0, tabm->webView(i)->page()->record_binder()->bounded_item());
    // }
    // }
    ...
    //// _tree_screen->insert_branch_process(_tree_screen->last_index(), "buffer", true);
    ...
    auto browsers_search_prepare = [&](boost::intrusive_ptr<i_t>& _start_item, int& _candidate_records) { // , boost::intrusive_ptr<TreeItem>    &_result_item
        // to be done
        // QMap<QString, QString> data;
        ...
        _start_item = i_t::dangle_instance(); // boost::intrusive_ptr<TreeItem>(new TreeItem(nullptr, QMap<QString, QString>()));
    ...
    auto current_model_search_prepare = [&](boost::intrusive_ptr<i_t>& _start_item, int& _candidate_records) { // , boost::intrusive_ptr<TreeItem>   &_result_item
    ...
    auto global_search_prepare = [&](boost::intrusive_ptr<i_t>& _start_item, int& _candidate_records) { // , boost::intrusive_ptr<TreeItem>   &_result_item	// , std::shared_ptr<RecordTable>     &resultset_data
    ...
    auto final_search = [&](boost::intrusive_ptr<i_t>& final_result // QList<boost::intrusive_ptr<Linker> >    &_result_list
        ,
    ...
    std::sort(std::begin(pre), std::end(pre), [&](boost::intrusive_ptr<i_t> t1, boost::intrusive_ptr<i_t> t2) { return t1->rating() > t2->rating(); });
    full_field_map data_;

    // QDateTime	ctime_dt	= QDateTime::currentDateTime();
    QString ctime_ = get_qtime();
    boost::fusion::at_key<id_type>(data_) = get_unical_id();
    boost::fusion::at_key<name_type>(data_) = ctime_;
    boost::fusion::at_key<ctime_type>(data_) = ctime_;
    boost::fusion::at_key<dir_type>(data_) = boost::fusion::at_key<id_type>(data_);
    boost::fusion::at_key<file_type>(data_) = "text.html";
    boost::intrusive_ptr<i_t> _final_result = i_t::dangle_instance(data_);
    // if(pre.size() > 0)_final_result << pre[0];
    for (std::vector<boost::intrusive_ptr<i_t> >::size_type i = 0; i < pre.size(); i++) {
        if (i < 10)
            _final_result << pre[i];
        else
            break;
                    ...
                    sapp_t::instance()->processEvents();
                        ...
                        if (candidate)
                            if (candidate->count_direct() > 0)
                                find_recursive(final_result, _session_root_item, candidate);
#ifdef SHOW_PROCESS_DIALOG
                    } else {
                        // if(_progress->wasCanceled()) {
                        _cancel_flag = 1;
