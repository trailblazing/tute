...
AttachTableView::AttachTableView(QWidget *parent)
	...
	this->setSelectionBehavior(
	    QAbstractItemView::SelectRows);  // Выделяется вся строка
...
void AttachTableView::assemblyContextMenu(void) {
	...
	// find_object() не работает, потому что при инициализации еще не вызван метод
	// assembly() у attachTableScreen, и этому объекту не задан родитель layout
	// AttachTableScreen
	// *screenPointer=find_object<AttachTableScreen>("attachTableScreen");
	AttachTableScreen *screenPointer =
	    qobject_cast<AttachTableScreen *>(controller->parent());
...
void AttachTableView::tapAndHoldGestureTriggered(QTapAndHoldGesture *gesture) {
	...
	if (gesture->state() == Qt::GestureFinished)
		if (gl_paras->target_os() == "android")
			emit tapAndHoldGestureFinished(
