...
void AppConfig::init(void)
{
    ...
    if (!QFile(_config_file_full_name).exists()) {
        ...
        if (!QFile::copy(QString(":/resource/standardconfig/") + globalparameters.target_os() + "/" + globalparameters.config_filename(), _config_file_full_name))
            critical_error("void AppConfig::init(void) can not copy " + globalparameters.config_filename());
    }
    // throw std::runtime_error("Can not copy " + name.toStdString());
    if ((QFile::ReadUser | QFile::WriteUser) != (QFile::permissions(_config_file_full_name) & (QFile::ReadUser | QFile::WriteUser)))
...
QString AppConfig::get_parameter(QString name) const
{
    ...
    if (_app_conf->contains(name))
        ...
        if (!QFile::copy(QString(":/resource/standardconfig/") + globalparameters.target_os() + "/" + globalparameters.config_filename(), _config_file_full_name))
            critical_error("Can not copy " + globalparameters.config_filename());
        if ((QFile::ReadUser | QFile::WriteUser) != (QFile::permissions(_config_file_full_name) & (QFile::ReadUser | QFile::WriteUser)))
            QFile::setPermissions(_config_file_full_name, QFile::ReadUser | QFile::WriteUser);
...
{ // QStringList AppConfig::parameter_table_1(bool withEndSignature) const
    // Таблица параметров
...
{ // QStringList AppConfig:: parameter_table_2(bool withEndSignature) const {
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_3(bool withEndSignature) const {
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_4(bool withEndSignature) const {
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
    ...
    table << "interfacelanguage"
          ...
          << "en";
    table << "programm"
          << "QString" << globalparameters.application_name();
    if (withEndSignature)
        table << "0"
              << "0"
              << "0";
...
{ // QStringList AppConfig:: parameter_table_5(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_6(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_7(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_8(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_9(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_10(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_11(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_12(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_13(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
    ...
    // Новые параметры
    table << "rememberCursorAtHistoryNavigation"
          << "bool"
          << "true";
    table << "rememberCursorAtOrdinarySelection"
          << "bool"
          << "true";
    if (withEndSignature)
        table << "0"
...
{ // QStringList AppConfig:: parameter_table_14(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_15(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_16(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_17(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_18(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_19(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_20(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_21(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_22(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_23(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_24(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_25(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_26(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
...
{ // QStringList AppConfig:: parameter_table_27(bool withEndSignature){
    // Таблица параметров
    // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
    QStringList table;
