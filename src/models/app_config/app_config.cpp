...
AppConfig::AppConfig(std::shared_ptr<gl_para> gl_paras_, QObject *pobj){
	...
	if(!QFile(_config_file_full_name).exists()){
		...
		if(!QFile::copy(QString(":/resource/standardconfig/") + gl_paras_->target_os() + "/" + gl_para::_conf_filename, _config_file_full_name)) critical_error(QString("void AppConfig::init(void) can not copy \"") + gl_para::_conf_filename + "\"");
	}
...
QString AppConfig::get_parameter(QString name) const {
	...
	if(_app_conf->contains(name)) result = _app_conf->value(name).toString();
		...
		if(!QFile::copy(QString(":/resource/standardconfig/") + gl_paras->target_os() + "/" + gl_para::_conf_filename, _config_file_full_name)) critical_error(QString("Can not copy \"") + gl_para::_conf_filename + "\"");
		if((QFile::ReadUser | QFile::WriteUser) != (QFile::permissions(_config_file_full_name) & (QFile::ReadUser | QFile::WriteUser))) QFile::setPermissions(_config_file_full_name, QFile::ReadUser | QFile::WriteUser);
		delete _this->_app_conf;
		_this->_app_conf = new QSettings(_config_file_full_name, QSettings::IniFormat);
...
bool AppConfig::find_in_base_expand(void) const {
	return _app_conf->value(FindScreen::_find_in_base_expand).toBool(); // "findInBaseExpand"
}

void AppConfig::find_in_base_expand(bool state){
	_app_conf->setValue(FindScreen::_find_in_base_expand, state); // "findInBaseExpand"
}
...
// азрешать ли для просмотра расшифровывать зашифрованные файлы в директорию корзины
...
void AppConfig::update_version_process(void){
	QString configFileName = gl_paras->root_path() + "/" + gl_paras->target_os() + "/" + gl_para::_conf_filename;
...
template <>
QStringList AppConfig::parameter_table<0>(bool withEndSignature){ // QStringList AppConfig::parameter_table_1(bool withEndSignature) const
	                                                          // Таблица параметров
...
template <>
QStringList AppConfig::parameter_table<1>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_2(bool withEndSignature) const {
	                                                          // Таблица параметров
	                                                          // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<2>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_3(bool withEndSignature) const {
	                                                          // Таблица параметров
	                                                          // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<3>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_4(bool withEndSignature) const {
	                                                          // Таблица параметров
	                                                          // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<4>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_5(bool withEndSignature){
	                                                          // Таблица параметров
	                                                          // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<5>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_6(bool withEndSignature){
	                                                          // Таблица параметров
	                                                          // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<6>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_7(bool withEndSignature){
	                                                          // Таблица параметров
	                                                          // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<7>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_8(bool withEndSignature){
	                                                          // Таблица параметров
	                                                          // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<8>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_9(bool withEndSignature){
	                                                          // Таблица параметров
	                                                          // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<9>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_10(bool withEndSignature){
	                                                          // Таблица параметров
	                                                          // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<10>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_11(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<11>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_12(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<12>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_13(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<13>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_14(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
	...
	table	<< "findScreenTreeSearchArea"
	        << "int"
	        << "0"; // Область поиска. Искать во всем дереве - 0, искать в текущей ветке - 1
	if(withEndSignature)
		table	<< "0"
		        << "0"
		        << "0";
...
template <>
QStringList AppConfig::parameter_table<14>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_15(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
	...
	if(gl_paras->target_os() == "android")
		table	<< "uglyQssReplaceHeightForTableView"
		        << "int"
			<< "35";                                                                                                                               // Так как не все параметры можно стилизовать через QSS, здесь задается высота ячейки таблицы
	else
		table	<< "uglyQssReplaceHeightForTableView"
		        << "int"
		        << "0";
...
template <>
QStringList AppConfig::parameter_table<15>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_16(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<16>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_17(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<17>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_18(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
	...
	if(gl_paras->target_os() == "android")
		...
		table	<< "interfaceMode"
		        << "QString"
			<< "desktop";                                                                                                                                                   // На десктопе должен быть интерфейс адоптированный для работы на рабочем столе
	if(withEndSignature)
		table	<< "0"
		        << "0"
		        << "0";
...
template <>
QStringList AppConfig::parameter_table<18>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_19(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<19>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_20(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<20>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_21(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<21>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_22(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<22>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_23(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<23>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_24(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<24>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_25(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<25>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_26(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
...
template <>
QStringList AppConfig::parameter_table<26>(bool withEndSignature){ // QStringList AppConfig:: parameter_table_27(bool withEndSignature){
	                                                           // Таблица параметров
	                                                           // Имя, Тип, Значение на случай когда в конфиге параметра прочему-то нет
	QStringList table;
