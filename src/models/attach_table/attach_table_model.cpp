...
QVariant AttachTableModel::data(const QModelIndex &index, int role) const {
	...
	if(_table == nullptr)return QVariant(0);
	else return QVariant::fromValue(_table);
    }
...
bool AttachTableModel::setData(const QModelIndex &index, const QVariant &value, int role){
    ...
    if(! index.isValid() && role != ATTACHTABLE_ROLE_TABLE_DATA)return false;
	...
	if(_table != nullptr)qDebug() << "Set new data to AttachTableModel with rows: " << _table->size();
	else qDebug() << "Set new AttachTableModel to NULL data";
	endResetModel();
