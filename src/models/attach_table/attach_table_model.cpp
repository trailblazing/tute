...
AttachTableModel::~AttachTableModel() {
	// delete table; // Закомментировано - удалять здесь table нельзя, так как
	// table существует в рамках коллекции таблиц конечных записей
}
...
QVariant AttachTableModel::data(const QModelIndex &index, int role) const {
	...
	if (role == Qt::DecorationRole &&
	    index.column() == ATTACHTABLE_COLUMN_FILENAME) {
		int row = index.row();
		QString id = _table->id_by_row(row);
...
bool AttachTableModel::setData(const QModelIndex &index, const QVariant &value, int role) {
	...
	if (role == ATTACHTABLE_COMMAND_END_REMOVE_ROW ||
	    role == ATTACHTABLE_COMMAND_END_REMOVE_ROWS) {
		endRemoveRows();
	...
	if (role == Qt::EditRole) {
		...
		emit dataChanged(index,
		                 index);  // Посылается сигнал что данные были изменены
