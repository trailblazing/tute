...
QString Attach::getField(QString name) const {
    ...
    else return CryptService::decryptString(globalparameters.crypt_key(), _fields[name]);// Поле расшифровывается
}
...
void Attach::setField(QString name, QString value){
    ...
    if(fieldCryptedList().contains(name))
		if(_fields.contains(boost::mpl::c_str < crypt_type >::value))
			if(_fields[boost::mpl::c_str < crypt_type > ::value] == "1")
				if(value.length() > 0){
				    if(globalparameters.crypt_key().length() > 0) isCrypt = true;
    ...
    if(isCrypt == true) value = CryptService::encryptString(globalparameters.crypt_key(), value);
...
void Attach::encrypt(unsigned int area){
	...
	// Шифруется файл
    if(area & areaFile)
		if(getField("type") == "file") CryptService::encryptFile(globalparameters.crypt_key(), getFullInnerFileName());
	// Шифруется содержимое файла в памяти, если таковое есть
    if(area & areaMemory)
		if(_lite_flag == false && _file_content.length() > 0) _file_content = CryptService::encryptByteArray(globalparameters.crypt_key(), _file_content);
	...
	if(getField(fieldName).length() > 0) setFieldSource(fieldName, CryptService::encryptString(globalparameters.crypt_key(), getField(fieldName)));
    }
...
void Attach::decrypt(unsigned int area){
    ...
    if(area & areaFile)
		if(getField("type") == "file") CryptService::decryptFile(globalparameters.crypt_key(), getFullInnerFileName());
	// Расшифровывается содержимое файла в памяти, если таковое есть
    if(area & areaMemory)
		if(_lite_flag == false && _file_content.length() > 0) _file_content = CryptService::decryptByteArray(globalparameters.crypt_key(), _file_content);
	...
	if(getField(fieldName).length() > 0) setFieldSource(fieldName, CryptService::decryptString(globalparameters.crypt_key(), _fields[fieldName]));
    }
