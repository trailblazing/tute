...
// в настоящий момент есть два типа прикрепления - просто файл "file" или линк
// на файл "link"
...
void Attach::init(AttachTableData *iParentTable) {
	_lite_flag =
	    true;  // By default, a light object    // По-умолчанию легкий объект
...
QStringList Attach::fieldAvailableList(void) const {
	return QStringList() << "id"
	                     << "fileName"
	                     << "link"
	                     << "type" << boost::mpl::c_str<crypt_key>::value;
}
...
void Attach::dom_to_data(QDomElement iDomElement) {
	...
	foreach (QString fieldName,
	         fieldsName)  // Перебираются имена полей (XML-тегов)
		_fields[fieldName] = iDomElement.attribute(
		    fieldName);  // Напрямую устанавливаются значения из XML файла
}
...
void Attach::switchToFat() {
	...
	if (_lite_flag != true)
		critical_error(
		    "Unavailable switching attach object to fat state. Attach Id: " +
		    getField("id") + " File name: " + getField("fileName"));
	_lite_flag = false;
}
...
QString Attach::getField(QString name) const {
	...
	if (fieldCryptedList().contains(name))
		if (_fields.contains(boost::mpl::c_str<crypt_key>::value))
			if (_fields[boost::mpl::c_str<crypt_key>::value] == "1")
	...
	if (isCrypt == false)
		...
		return CryptService::decryptString(gl_paras->crypt_key(),
		                                   _fields[name]);  // Поле расшифровывается
}

// Установка значения поля
// Метод принимает незашифрованные данные, и шфирует их если запись является
// зашифрованой
void Attach::setField(QString name, QString value) {
	...
	if (name == "fileName")
		if (getField("type") == "link")  // Если устанавливается имя файла для линка
			if (getField("fileName").length() > 0 &&
			    value.length() > 0) {  // Если имя уже было задано (при создании
				                       // аттача), и новое имя не пустое
				// Имя файла для линка менять нельзя
				show_message_box(
				    QObject::tr("Can't modify file name for link type attach."));
				return;
			}
			...
			if (_fields[boost::mpl::c_str<crypt_key>::value] == "1")
				if (value.length() > 0) {
					if (gl_paras->crypt_key().length() > 0)
						...
						critical_error(
						    "In Attach::setField() can not set data to crypt field " +
						    name + ". Password not setted");
				}
...
void Attach::popFatDataFromDisk() {
	...
	_file_content.append(
	    (DiskHelper::get_files_from_directory(innerDirName, innerFileName))
	        .value(innerFileName));
}

// fullFileName - имя файла с полным путем, который нужно скопировать в каталог
// записи
bool Attach::copyFileToBase(QString iFileName) {
	...
	if (!file.exists()) {
		show_message_box(
		    QObject::tr("Can't open file %1. File not exists.").arg(iFileName));
		return false;
	}
	...
	if (result == false)
		show_message_box(QObject::tr("Can't copy file %1. May be directory %2 not "
		                             "writable, or target file %3 already exists.")
		                     .arg(iFileName)
		                     .arg(getFullInnerDirName())
		                     .arg(getFullInnerFileName()));
	return result;
}
...
void Attach::removeFile() {
	...
	if (file.exists() == false) {
		show_message_box(
		    QObject::tr("Can't delete file %1 on disk. File not exists.")
		        .arg(getFullInnerFileName()));
		return;
	}
...
// Внутрисистемное имя складывается из идентификатора аттача и расширения,
// взятого от имени файла
QString Attach::getInnerFileName() const {
	...
	if (getField("type") ==
	    "link")  // Для линка просто возвращается имя файла, куда указывает линк
...
void Attach::encrypt(unsigned int area) {
	...
	if (getField(boost::mpl::c_str<crypt_key>::value) == "1")
		critical_error(
		    "Attach::encrypt() : Cant encrypt already encrypted attach.");
	// Шифруется файл
	if (area & areaFile)
		if (getField("type") == "file")
	...
	if (area & areaMemory)
		if (_lite_flag == false && _file_content.length() > 0)
			_file_content =
			    CryptService::encryptByteArray(gl_paras->crypt_key(), _file_content);
	// Шифруются поля, которые подлежат шифрованию
	foreach (QString fieldName, fieldCryptedList()) {
		// У аттача с типом file не должно быть обращений к полю link (оно не должно
		// использоваться)
	...
	setField(boost::mpl::c_str<crypt_key>::value, "1");
}

// асшифровка аттача на диске и в памяти
void Attach::decrypt(unsigned int area) {
	// Если аттач не зашифрован, и происходит расшифровка, значит есть какая-то
	// ошибка в логике выше
	if (getField(boost::mpl::c_str<crypt_key>::value) != "1")
	...
	if (area & areaMemory)
		if (_lite_flag == false && _file_content.length() > 0)
			_file_content =
			    CryptService::decryptByteArray(gl_paras->crypt_key(), _file_content);
	...
	foreach (QString fieldName, fieldCryptedList()) {
		...
		if (getField("type") == "file" && fieldName == "link")
			continue;
		// Если поле с указанным именем существует и содержит данные, оно
		// расшифровывается из исходных зашифрованных данных
		if (getField(fieldName).length() > 0)
	...
	setField(boost::mpl::c_str<crypt_key>::value,
	         "");  // Отсутсвие значения предпочтительней, так как тогда в
	               // XML-данные не будет попадать атрибут crypt="0"
}
