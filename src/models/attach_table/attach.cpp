...
void Attach::encrypt(unsigned int area)
{
    ...
    if(area & areaFile)
        if(getField("type") == "file")
            CryptService::encryptFile(globalparameters.crypt_key(), getFullInnerFileName());
    ...
    foreach(QString fieldName, fieldCryptedList()) {
        ...
        if(getField(fieldName).length() > 0)
            setFieldSource(fieldName, CryptService::encryptString(globalparameters.crypt_key(), getField(fieldName)));
    }
...
void Attach::decrypt(unsigned int area)
{
    ...
    if(area & areaFile)
        if(getField("type") == "file")
            CryptService::decryptFile(globalparameters.crypt_key(), getFullInnerFileName());
