...
std::shared_ptr<XmlTree> tkm_t::init_from_xml(QString file_name){
...
bool tkm_t::format_check(QDomElement elementFormat){
	...
	baseSubVersion > CURRENT_FORMAT_SUBVERSION)return false;
...
boost::intrusive_ptr<TreeItem> tkm_t::move_child(boost::intrusive_ptr<TreeIndex> modelindex	// boost::intrusive_ptr<TreeItem> _parent
					       , boost::intrusive_ptr<TreeItem> source_item		//    , int _pos
					       , int mode){		//    , QString id, QString name
    boost::intrusive_ptr<TreeItem> host = modelindex->host();
    assert(host);
...
//        }
	if(static_cast<QModelIndex>(_index_parent).isValid() || host == _root_item){
	    ...
	    result = source_item->parent(host, pos, mode)->host();				// add_new_branch(parent, id, name);  // parent->count_direct()
	    ...
	    result = source_item->parent(host, pos, mode)->host();				// 1-3
	    assert(result->linker()->integrity_external(result, host));
	}
...
//            if(tree_index) {
		...
		assert(tree_view);
		result = merge(TreeIndex::merge_instance(TreeIndex::instance([&] {return this;}, linker_first->host(), linker_first->host()->parent()), result), std::bind(&tv_t::delete_permanent, tree_view, [&] {return this;}, QList<boost::intrusive_ptr<TreeItem> >() << result, &tkm_t::delete_permanent, "cut", false));
	    }
...
boost::intrusive_ptr<TreeItem> tkm_t::merge(boost::intrusive_ptr<TreeMerge> _tree_merge
