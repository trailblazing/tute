...
struct TreeLevel : public boost::intrusive_ref_counter<TreeLevel, boost::thread_safe_counter> {
    ...
    static boost::intrusive_ptr<TreeLevel> instance(boost::intrusive_ptr<TreeIndex> _tree_index, boost::intrusive_ptr<i_t> _to_be_operated);
...
public:
    ...
    static boost::intrusive_ptr<TreeIndex> require_treeindex(const std::function<tkm_t*()>& current_model_, boost::intrusive_ptr<i_t> host_);
    static boost::intrusive_ptr<i_t> require_item(const QUrl& find_url_, const insert_strategy& tree_view_insert_strategy_, equal_url equal_);

    boost::intrusive_ptr<i_t> bind(const QUrl& _find_url // boost::intrusive_ptr<TreeItem>   tab_brother,
        ,
        const insert_strategy& _view_insert_strategy, equal_url _equal, web::Browser* _recommend_browser = nullptr);

    static boost::intrusive_ptr<i_t> activate(const std::function<tkm_t*()>& current_model_, boost::intrusive_ptr<i_t> host_, const QUrl& find_url_, const insert_strategy& view_insert_strategy_, equal_url equal_) noexcept;
