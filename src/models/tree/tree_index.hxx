...
struct TreeLevel : public boost::intrusive_ref_counter<TreeLevel, boost::thread_safe_counter> {
	...
	private:
		TreeLevel(boost::intrusive_ptr<TreeIndex> tree_index, boost::intrusive_ptr<TreeItem> to_be_merged, tv_t *tree_view);
...
//	static boost::intrusive_ptr<TreeIndex> instance(const std::function<km_t *()> &current_model, boost::intrusive_ptr<TreeItem> host_parent, int sibling_order = 0);
		static boost::intrusive_ptr<TreeIndex> require_treeindex(const std::function<tkm_t *()> &current_model_, boost::intrusive_ptr<TreeItem> host_);
		static boost::intrusive_ptr<TreeItem> require_item(const QUrl &find_url_
		...
		boost::intrusive_ptr<TreeItem> bind(const QUrl &_find_url // boost::intrusive_ptr<TreeItem>   tab_brother,
						   ...
						   , equal_url _equal, web::Browser *_recommend_browser = nullptr);
