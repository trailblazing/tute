...
{ // , _root_item(nullptr)
    ...
    boost::fusion::at_key<id_type>(data) = global_root_id;
    boost::fusion::at_key<name_type>(data) = "";
    // Создание корневого Item объекта
    if (_root_item)
        _root_item.reset();
    _root_item = i_t::dangle_instance(data); // boost::intrusive_ptr<TreeItem>(new TreeItem(_root_item_parent, root_data));		// ?
...
// void KnowModel::dom_to_records(QDomElement _record_dom_element, boost::intrusive_ptr<TreeItem> _parent_item)
// {
...
// _parent_item->dom_to_direct(_record_dom_element);    // take ground from the nearest level children
//// up line duplicated with below!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
void tkm_t::save()
{
    ...
    if (content.hasChildNodes()) {
        ...
        if (doc.hasChildNodes()) {
            auto write_to
                = [&](const QString& file_name_, bool backup_ = true) {
...
// Добавление новой подветки к Item элементу
boost::intrusive_ptr<i_t> tkm_t::new_child(boost::intrusive_ptr<TreeIndex> _modelindex, QString id, QString name)
{
    boost::intrusive_ptr<i_t> parent = _modelindex->host();
    int pos = _modelindex->sibling_order();
    auto index_parent = index(parent);
    beginInsertRows(
        index_parent, pos // parent->count_direct()
        ,
        pos // parent->count_direct()
        );
    ...
    boost::fusion::at_key<name_type>(data) = name;
    boost::fusion::at_key<url_type>(data) = web::Browser::_defaulthome;
    // Подузел прикрепляется к указанному элементу
    // в конец списка подчиненных элементов
    boost::intrusive_ptr<i_t> current = new i_t(parent, data, QDomElement(), pos); // parent->child_add_new(pos, id, name);
...
// if(!item([ = ](boost::intrusive_ptr<const TreeItem> it) {return it->id() == current_parent->id();}) && current_parent->id() != "") {
// intercept_self(current_parent);
// }
// return (index(current_parent).isValid())
// && (item([ = ](boost::intrusive_ptr<const TreeItem> it) {return it->id() == current_parent->id();}));
// }();
...
// assert(item([ = ](boost::intrusive_ptr<const TreeItem> it) {return it->id() == current_parent->id();}));    // what if parent_id is _know_model_board but current model is not?
// boost::intrusive_ptr<TreeItem> _result(nullptr);
...
////                        , _index  // parent->count_direct()
////                       );
// _result = model_move_as_child_impl(current_parent, _source_item, _index.row());
////        endInsertRows();
// }

// assert(item([ = ](boost::intrusive_ptr<const TreeItem> it) {return it->id() == _result->id();}));
// return _result;
// }

//// Добавление новой подветки к указанной ветке
// boost::intrusive_ptr<TreeItem> KnowModel::model_move_as_child(const QModelIndex &_index, boost::intrusive_ptr<TreeItem> _source_item)
// {
//// Получение ссылки на Item элемент по QModelIndex
// boost::intrusive_ptr<TreeItem> current = item(_index);
// boost::intrusive_ptr<TreeItem> _result(nullptr);
...
////                    , _index.row()  // parent->count_direct()
////                   );
// _result = model_move_as_child_impl(current, _source_item, 0);  // _index.row()

////    endInsertRows();
// return _result;
// }

// boost::intrusive_ptr<TreeItem> KnowModel::clipboard_move_as_child_impl(boost::intrusive_ptr<TreeItem> _parent, boost::intrusive_ptr<TreeItem> _source_item, int _pos, int _mode)
// {
...
boost::intrusive_ptr<i_t> tkm_t::move(boost::intrusive_ptr<TreeLevel> _tree_level, int mode)
{ // , int _pos
        ...
        if (_parent) {
            ...
            if (!_linker) {
                ...
                if (old_parent && old_parent != _parent)
                    old_parent->release([&](boost::intrusive_ptr<const Linker> il) { return il->host()->id() == _to_be_operated->id() && il == _linker; });
    ...
    if (!(operate_parent == to_be_host && to_be_host->contains_direct(_to_be_operated->linker()))) {
        ...
        if (static_cast<QModelIndex>(_index_new_parent).isValid() || to_be_host == _root_item) {
            ...
            endInsertRows();
            if (_to_be_operated && child([&](boost::intrusive_ptr<const i_t> it) -> bool { return it->id() == _to_be_operated->id(); })) {
            ...
            _to_be_operated = move_impl(pos, mode); // should not use?	// source_item->parent(host, pos, mode)->host();				// 1-3
...
// Add a new highlight to the Item element  // Добавление новой подветки к Item элементу
boost::intrusive_ptr<TreeItem> KnowModel::lock_child_add(boost::intrusive_ptr<Record> record, boost::intrusive_ptr<TreeItem> parent)
{ // , QString id, QString name
...
// QString KnowModel::model_paste_sibling_from_clipboard(const QModelIndex &_index, ClipboardBranch *subbranch)
// {
...
// pasted_branch_id = paste_children_from_clipboard(parent, (ClipboardBranch *)subbranch, current->sibling_order());
////        endInsertRows();
// }
...
// QString KnowModel::model_paste_child_from_clipboard(const QModelIndex &_index, ClipboardBranch *subbranch)
// {
// QString pasted_branch_id;
...
// pasted_branch_id = paste_children_from_clipboard(parent, (ClipboardBranch *)subbranch, parent->count_direct());
////    endInsertRows();
...
// boost::intrusive_ptr<TreeItem> KnowModel::paste_children_from_clipboard(boost::intrusive_ptr<TreeItem> _target_item, ClipboardBranch *_sub_branch, int _sibling_order)
// {
...
// auto clipboard_move_as_child_impl = [](boost::intrusive_ptr<TreeItem> _parent, boost::intrusive_ptr<TreeItem> _source_item, int _pos, int _mode = ADD_NEW_RECORD_AFTER)->boost::intrusive_ptr<TreeItem> {
...
// std::function<QString(boost::intrusive_ptr<TreeItem>, const QString &, ClipboardBranch *, const int)>
...
// QList< boost::intrusive_ptr<TreeItem> > records = _sub_branch->records_by_parent_id(_clip_root_items_parent_id);
...
//// _new_item
// auto record_shadow = clipboard_move_as_child_impl(_clip_parent, record, _sibling_order + i); // _clip_parent->count_direct()
...
QModelIndex tkm_t::move_up_dn(const QModelIndex& _index, int (i_t::*_move)() // int direction
    )
{
...
// {
// boost::intrusive_ptr<TreeItem> _result(nullptr);
// boost::intrusive_ptr<TreeItem> parent_item = target->parent();  // item(parent);
...
boost::intrusive_ptr<i_t> tkm_t::delete_permanent(boost::intrusive_ptr<Linker> delete_linker)
{
    boost::intrusive_ptr<i_t> deleted_item;
    auto host_parent = delete_linker->host_parent();
    auto host = delete_linker->host();

    auto model_delete_permantent_impl = [&]( // boost::intrusive_ptr<TreeItem> parent_item,
        ...
        // boost::intrusive_ptr<TreeItem> host = delete_target_linker->host();  // item(parent);
    ...
    if (host && static_cast<QModelIndex>(index(host)).isValid()) {
        if (host && host->count_direct() > 0) {
            ...
            // try {tree_index = new TreeIndex([&] {return view->source_model(); }, host_parent, delete_linker->sibling_order()); } catch(std::exception &e) {}

            // assert(tree_index);

            // if(tree_index) {
            auto result = view->move(TreeIndex::require_treeindex([&] { return view->source_model(); }, host_parent), linker_first->host(), [&](boost::intrusive_ptr<const Linker> il) -> bool { return il->host()->id() == linker_first->host()->id(); });
            ...
            if (result != linker_first->host()) {
                ...
                assert(tree_view);
                result = merge(TreeLevel::instance(TreeIndex::require_treeindex([&] { return this; }, result), linker_first->host()), std::bind(&tv_t::delete_permanent, tree_view, [&] { return this; }, QList<boost::intrusive_ptr<i_t> >() << linker_first->host(), &tkm_t::delete_permanent, "cut", false));
            }
            ...
            for (auto& il : host->child_linkers()) {
                view->move(TreeIndex::require_treeindex([&] { return view->source_model(); }, result), il->host(), [&](boost::intrusive_ptr<const Linker> link) -> bool { return il->host()->id() == link->host()->id(); });
            }
            // }
            // }
        }
        ...
        if (host_parent && host_parent->count_direct() == 0 && root_item()->count_direct() > 0) { // (index.row() > 0) == index.isValid()
            tv_t* tree_view = static_cast<tv_t*>(static_cast<QObject*>(this)->parent());
            while ((deleted_item = host_parent->delete_permanent_recursive([&](boost::intrusive_ptr<const i_t> it) { return it->is_empty(); }))) {
...
boost::intrusive_ptr<i_t> tkm_t::delete_permanent_recursive(boost::intrusive_ptr<Linker> delete_linker)
{
    ...
    std::function<boost::intrusive_ptr<i_t>(boost::intrusive_ptr<Linker>)> model_delete_permantent_impl = [&]( // boost::intrusive_ptr<TreeItem> parent_item,
        boost::intrusive_ptr<Linker> delete_target_linker // , int position, int rows, const QModelIndex &parent
    ...
    if (host && static_cast<QModelIndex>(index(host)).isValid()) {
        ...
        // qDebug() << "Label 2, branch id:" << host->field("id") << "name:" << host->field("name");
        ...
        // qDebug() << "Delete subbranch, id:" << item(sub_branches_path.at(i))->field("id") << "name:" << item(sub_branches_path.at(i))->field("name");
        ...
        // qDebug() << "Delete rootbranch, id:" << item(_delete_path)->field("id") << "name:" << item(_delete_path)->field("name");
        ...
        deleted_root_item = model_delete_permantent_impl(delete_linker); // if(removeRows(_index_delete.row(), 1, _index_delete.parent()))
        ...
        if (parent && parent->count_direct() == 0 && root_item()->count_direct() > 0) { // (index.row() > 0) == index.isValid()
            ...
            tv_t* tree_view = static_cast<tv_t*>(static_cast<QObject*>(this)->parent());
            while ((deleted_root_item = parent->delete_permanent_recursive([&](boost::intrusive_ptr<const i_t> it) { return it->is_empty(); }))) {
...
boost::intrusive_ptr<i_t> tkm_t::merge(boost::intrusive_ptr<TreeLevel> _tree_merge, const view_delete_permantent_strategy& _view_delete_permantent)
{
    ...
    ////    auto result = tree_index->host();	// item([=](boost::intrusive_ptr<const TreeItem> t){return t->id() == tree_index->host()->id();});
    // if(source->is_ancestor_of([&](boost::intrusive_ptr<const TreeItem> it){return it == result;})){
    // view->paste_sibling(TreeIndex::instance(_current_model, source, source->parent()), result, [&](boost::intrusive_ptr<const Linker> il){return il->host()->id() == result->id();});
    // }
    if (result && result != source) {
        ...
        beginInsertColumns(_index_target // .parent()
            ,
        ...
        result = result->merge(source); // not a pure insertion, move removerows to below: _view_delete_permantent
        ...
        // assert(item([ = ] (boost::intrusive_ptr<const TreeItem> t) {return t->id() == result->id();}));
            ...
            if (static_cast<QModelIndex>(index_).isValid()) { // source->parent()->field("name") != clipboard_items_root
                if (source->count_direct() == 0) {
                    ...
                    if (source->linker()->host() && source->linker()->host_parent()) {
                        QList<boost::intrusive_ptr<i_t> > delete_list = _view_delete_permantent([&]() -> tkm_t* { return this; }, QList<boost::intrusive_ptr<i_t> >() << source); // , _delete_strategy, "cut", false
...
////        qDebug() << "Label 2, branch id:" << _item_delete->field("id") << "name:" << _item_delete->field("name");
...
////        // Получение путей ко всем подветкам
////        QList<QStringList> sub_branches_path = _item_delete->path_children_all();
...
////        //        _source_model->beginRemoveRows(_index_to_be_delete.parent(), _index_to_be_delete.row(), _index_to_be_delete.row());
////        //        if(
////        _item_deleted = model_child_remove(_item_delete); // , _index_delete.row(), 1, _index_delete.parent());
////        //        )
////        //        {
////        //            _item_deleted = _item_delete;
////        //        }
...
////        // try to rewrite, but below is just memory operation which is not we want, but but, the original code is also a memory ooperation too? why?
////        if(_parent && _parent->count_direct() == 0 && root_item()->count_direct() > 0) { // (index.row() > 0) == index.isValid()
...
void tkm_t::re_encrypt(QString previousPassword, QString currentPassword)
{
    ...
    foreach (QStringList currPath, subbranchespath) {
        ...
        if (currBranch->field<crypt_type>() == "1" && currBranchParent->field<crypt_type>() != "1") {
            Password pwd;
...
QList<boost::intrusive_ptr<i_t> > tkm_t::children(const std::function<bool(boost::intrusive_ptr<const i_t>)>& _equal)
{
    std::function<QList<boost::intrusive_ptr<i_t> >(boost::intrusive_ptr<i_t>, int)> // , std::function<bool(boost::intrusive_ptr<TreeItem>)> _equal
        item_by_equal_recurse = [&](boost::intrusive_ptr<i_t> it_, int mode) -> QList<boost::intrusive_ptr<i_t> > { // boost::intrusive_ptr<TreeItem>(*item_by_name_recurse)(boost::intrusive_ptr<TreeItem> item, QString name, int mode);
        // , std::function<bool(boost::intrusive_ptr<TreeItem>)> _equal
        static QList<boost::intrusive_ptr<i_t> > _items_list; // static boost::intrusive_ptr<TreeItem> _index_list;
        if (mode == 0) {
...
QModelIndexList tkm_t::indexes(const std::function<bool(boost::intrusive_ptr<const i_t>)>& _equal)
{
    std::function<QModelIndexList(boost::intrusive_ptr<i_t>, int)> // , std::function<bool(boost::intrusive_ptr<TreeItem>)> _equal
        item_by_equal_recurse = [&](boost::intrusive_ptr<const i_t> it_, int mode) { // boost::intrusive_ptr<TreeItem>(*item_by_name_recurse)(boost::intrusive_ptr<TreeItem> item, QString name, int mode);
            // , std::function<bool(boost::intrusive_ptr<TreeItem>)> _equal

            static QModelIndexList _index_list; // static boost::intrusive_ptr<TreeItem> _index_list;
            if (mode == 0) {
...
boost::intrusive_ptr<i_t> tkm_t::child(const std::function<bool(boost::intrusive_ptr<const i_t>)>& _equal) const // const QUrl &_find_url, equal_type _equal
{
...
boost::intrusive_ptr<i_t> tkm_t::child(QStringList path) const { return tm_t::item(path); }

boost::intrusive_ptr<i_t> tkm_t::child(const QModelIndex& _index) const { return tm_t::item(_index); }
...
{
    std::function<bool(boost::intrusive_ptr<i_t>, int)> is_contains_crypt_branches_recurse =
        [&](
...
boost::intrusive_ptr<i_t> tkm_t::intercept(boost::intrusive_ptr<i_t> _item)
{ // TreeIndex modelindex    //
    ...
    assert(_item == absolute_source_model()->root_item() || absolute_source_model()->item([&](boost::intrusive_ptr<const i_t> it) { return it->id() == _item->id(); }));
    boost::intrusive_ptr<i_t> result(nullptr);
    // QMap<QString, QString> root_data;
...
boost::intrusive_ptr<i_t> tkm_t::synchronize(boost::intrusive_ptr<i_t> source)
{
    ...
    result = child([=](boost::intrusive_ptr<const i_t> it) { return it->id() == source->id(); });
    ...
    result->dom_to_records(source->dom_from_treeitem()); // std::make_shared<QDomDocument>()
    assert(result);
    assert(!result->is_lite());
    assert(!result->attach_table()->is_lite());
