...
class tkm_t : public tm_t {
	...
	QList<boost::intrusive_ptr<TreeItem> >	children(const std::function<bool (boost::intrusive_ptr<const TreeItem>)> &_equal);
	...
	boost::intrusive_ptr<TreeItem>	child(QStringList path) const;
	boost::intrusive_ptr<TreeItem>	child(const QModelIndex &_index) const;
	...
	boost::intrusive_ptr<TreeItem> child(const std::function<bool (boost::intrusive_ptr<const TreeItem>)> &_equal) const;		// const QUrl &_find_url, equal_type _equal
	...
	boost::intrusive_ptr<TreeItem> merge(boost::intrusive_ptr<TreeMerge> _tree_merge	// boost::intrusive_ptr<TreeItem> target
					    , const view_delete_permantent_strategy &_view_delete_permantent);

	boost::intrusive_ptr<TreeItem> move_child(boost::intrusive_ptr<TreeIndex> modelindex, boost::intrusive_ptr<TreeItem> source_item, int mode = add_new_record_after);
	...
	QModelIndex move_up_dn(const QModelIndex &_index, int (TreeItem::*_move)());	// int direction

	boost::intrusive_ptr<TreeItem> delete_permanent(boost::intrusive_ptr<Linker> delete_linker);

	//    boost::intrusive_ptr<TreeItem> model_child_remove(boost::intrusive_ptr<TreeItem> target);
	boost::intrusive_ptr<TreeItem> delete_permanent_recursive(boost::intrusive_ptr<Linker> delete_linker);
	...
	boost::intrusive_ptr<TreeItem> new_child(boost::intrusive_ptr<TreeIndex> _modelindex, QString id, QString name);
