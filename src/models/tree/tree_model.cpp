...
tm_t::tm_t(boost::intrusive_ptr<TreeItem> _root_item, tv_t* parent)
    : QAbstractItemModel(parent)
...
QVariant tm_t::data(const QModelIndex& _index, int role) const
{
    ...
    if (role == Qt::ForegroundRole) {
        ...
        if (it->id() == _session_id && _index != globalparameters.tree_screen()->view()->current_index())
            return QColor(Qt::red);
        else if (it->id() == _session_id && _index == globalparameters.tree_screen()->view()->current_index())
...
QModelIndex tm_t::index(int row, int column, const QModelIndex& current_index) const
{
    ...
    if (!current_index.isValid() || (0 <= current_index.column())) { // if(!current_index.isValid() || (0 == current_index.column())) {
        ...
        if (row >= 0 && row < current_item->count_direct()) {
            ...
            if (child_item)
                result = createIndex(row, column, static_cast<void*>(child_item.get()));
            // else {
            // return QModelIndex();
            // }
        }
            ...
            if (0 != mode) {
                ...
                for (int i = 0; i < it->count_direct(); i++) { // _index.row()    //
                    ...
                    if (child == _item) { // if(_item.get() == static_cast<TreeItem *>(_index_child.internalPointer())){
                        ...
                        if (find_index.isValid())
                            if (_item.get() == static_cast<TreeItem*>(find_index.internalPointer()))
...
boost::intrusive_ptr<TreeItem> tm_t::item(const QModelIndex& _index) const // ???
{
    ...
    if (_index.isValid()) {
        ...
        // ;
        if (_item)
            _result = _item; // qDebug() << "Get tree item " << item->data("name").toString();
