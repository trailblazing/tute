...
int tm_t::columnCount(const QModelIndex& parent) const
{
    ...
    int currnet_index_path_size = parent.isValid() ? static_cast<i_t*>(parent.internalPointer())->path_list().size() : root_path_size;
...
QVariant tm_t::data(const QModelIndex& _index, int role) const
{
	...
	if (it->field<crypt_type>() == crypt_value(true)) {
...
QModelIndex tm_t::index(int row, int column, const QModelIndex& current_index) const
{
    ...
    if (!current_index.isValid() || (0 <= current_index.column())) { // if(!current_index.isValid() || (0 == current_index.column())) {
        boost::intrusive_ptr<i_t> current_item = current_index.isValid() ? this->item(current_index) : _root_item; // || 0 < current_index.column()
            ...
            if (0 != mode) {
                ...
                for (int i = 0; i < it->count_direct(); i++) { // _index.row()    //
                    ...
                    if (child == _item) { // if(_item.get() == static_cast<TreeItem *>(_index_child.internalPointer())){
                        ...
                        if (find_index.isValid())
                            if (_item.get() == static_cast<i_t*>(find_index.internalPointer()))
...
boost::intrusive_ptr<i_t> tm_t::item(const QModelIndex& _index) const // ???
{
    ...
    if (_index.isValid()) {
        boost::intrusive_ptr<i_t> _item = boost::intrusive_ptr<i_t>(static_cast<i_t*>(_index.internalPointer())); // item(static_cast<TreeItem *>(_index.internalPointer())->path_absolute());
...
boost::intrusive_ptr<i_t> tm_t::item(const std::function<bool(boost::intrusive_ptr<const i_t>)>& _equal) const
{
    std::function<boost::intrusive_ptr<i_t>(boost::intrusive_ptr<i_t>, const std::function<bool(boost::intrusive_ptr<const i_t>)>&, int)>
...
boost::intrusive_ptr<i_t> tm_t::root_item() const { return _root_item; }
...
bool tm_t::setData(const QModelIndex& _index, const QVariant& value, int role)
{
	...
	it->field<name_type>(name_value(value.toString())); // QString("name"),
...
bool tm_t::setHeaderData(int section, Qt::Orientation orientation, const QVariant& value, int role)
{
    if (role != Qt::EditRole || orientation != Qt::Horizontal)
        return false;
    Q_UNUSED(section);
    _root_item->field<name_type>(name_value(value.toString())); // QString("name"),
    return true;
}
