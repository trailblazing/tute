...
tm_t::tm_t(boost::intrusive_ptr<TreeItem> root_item, tv_t *parent)
	: QAbstractItemModel(parent)
...
QVariant tm_t::data(const QModelIndex &_index, int role) const {
	...
	if(role == Qt::ForegroundRole){
		...
		if(it->id() == _session_id && _index != gl_paras->tree_screen()->view()->current_index()) return QColor(Qt::red);
		else if(it->id() == _session_id && _index == gl_paras->tree_screen()->view()->current_index()) return QColor(Qt::cyan);
	...
	if(role == Qt::DisplayRole || role == Qt::EditRole){
		...
		return QVariant(it->field<dynamic_name_type>()); // "dynamicname"  // Запрашивается строка имени с количеством элементов
	}
	...
	if(role == Qt::DecorationRole){
		...
		if(it->field<crypt_type>() == "1"){
			...
			if(gl_paras->crypt_key().length() == 0) return QIcon(":/resource/pic/branch_closed.svg");
...
index_tree tm_t::index(const std::function<bool (boost::intrusive_ptr<const Linker>)> &_equal) const {
	// QModelIndex result;

	std::function<QModelIndex(QModelIndex, std::function<bool (boost::intrusive_ptr<const Linker>)>, const int)>
	index_recursive = [&](QModelIndex _index, std::function<bool (boost::intrusive_ptr<const Linker>)> _equal, const int mode){
				  static QModelIndex find_index;
				  if(0 != mode){
					  auto it = _index.isValid() ? this->item(_index) : _root_item; // how about _index itself? as a child of root, it will ever checked
					  for(int i = 0; i < it->count_direct(); i++){
						  QModelIndex _index_child = createIndex(i, 0, static_cast<void *>(it->child_direct(i).get())); // index(i, 0, _index);		//
...
index_tree tm_t::index(boost::intrusive_ptr<const TreeItem> _item) const {
	index_tree result;
	// assert(!result.isValid());

	std::function<QModelIndex(boost::intrusive_ptr<const TreeItem>, boost::intrusive_ptr<const TreeItem>, const int)> // QModelIndex, int
	index_recursive = [&](boost::intrusive_ptr<const TreeItem> _parent // QModelIndex _index
	                     ,
				  ...
				  // }
				  if(0 != mode){
					  auto it = _parent ? _parent : _root_item; // _index.isValid() ? this->item(_index) : _root_item;	// TreeItem *index_item = static_cast<TreeItem *>(_index.internalPointer());    // same
					  // assert(_index.row() == it->current_count());  // wrong
					  for(int i = 0; i < it->count_direct(); i++){  // _index.row()    //
						  auto child = it->child_direct(i);
						  auto _index_child = createIndex(i, 0, static_cast<void *>(child.get())); // index(i, 0, _index);
						  //// index_recursive(_idx, item, 1);
						  // assert(static_cast<TreeItem *>(_index_child.internalPointer()) == it->child_direct(i).get());
						  if(child == _item){  // if(_item.get() == static_cast<TreeItem *>(_index_child.internalPointer())){
							  ...
							  if(find_index.isValid())
								  if(_item.get() == static_cast<TreeItem *>(find_index.internalPointer())) break; // same as find_index.isValid()
						  }
					  }
				  }else find_index = QModelIndex();
				  // for(int i = 0; i < _index.row(); i++) {
	...
	// auto index_no_recursive = [&](QModelIndex _index, boost::intrusive_ptr<const TreeItem> _item){
	// QModelIndex find_index;
...
boost::intrusive_ptr<TreeItem> tm_t::item(const QModelIndex &_index) const // ???
{
	boost::intrusive_ptr<TreeItem> _result = (nullptr); // _root_item; //
	if(_index.isValid()){
...
boost::intrusive_ptr<TreeItem> tm_t::item(QStringList path) const {
...
boost::intrusive_ptr<TreeItem> tm_t::item(const std::function<bool (boost::intrusive_ptr<const TreeItem>)> &_equal) const {
	std::function<boost::intrusive_ptr<TreeItem>(boost::intrusive_ptr<TreeItem>, const std::function<bool (boost::intrusive_ptr<const TreeItem>)> &, int)>
	item_recurse // boost::intrusive_ptr<TreeItem>(*item_by_name_recurse)(boost::intrusive_ptr<TreeItem> item, QString name, int mode);
	        = [&](boost::intrusive_ptr<TreeItem> _it, const std::function<bool (boost::intrusive_ptr<const TreeItem>)> &_equal, int mode){
...
int tm_t::rowCount(const QModelIndex &_index) const {
	...
	}else count = _root_item->count_direct();
	return // item(_index)->count_direct();    //
	       count;
}

void tm_t::session_id(boost::intrusive_ptr<TreeIndex> modelindex // const QString &id
                     ){
...
bool tm_t::setData(const QModelIndex &_index, const QVariant &value, int role){
	...
	if(role == Qt::EditRole){
		...
		it->field<name_type>(value.toString()); // QString("name"),
