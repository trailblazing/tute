...
// }
boost::intrusive_ptr<TreeIndex> TreeIndex::require_treeindex(const std::function<tkm_t *()> &current_model_, boost::intrusive_ptr<TreeItem> host_){
...
boost::intrusive_ptr<TreeItem> TreeIndex::require_item(const QUrl &find_url_, const insert_strategy &tree_view_insert_strategy_, equal_url equal_){
	...
	if(find_url_ != QUrl()){
		...
		auto _tree_view_insert_strategy_boot
		        = [&](boost::intrusive_ptr<TreeItem> _result // boost::intrusive_ptr<TreeIndex>_treeindex,
		             , const substitute_condition &_substitute_condition
		             , const insert_strategy &_tree_view_insert_strategy_impl
		             , const bool _item_is_brand_new, const QUrl &_find_url
		             , const std::function<bool (boost::intrusive_ptr<TreeItem>)> &_check_url) -> boost::intrusive_ptr<TreeItem> { // KnowView::
				  ...
				  if(_item_is_brand_new){  // || ! _current_model()->item([&](boost::intrusive_ptr<const TreeItem> it){return it->id() == _result->id();})
					  ...
					  if(_result->field<url_type>() == web::Browser::_defaulthome && _find_url.toString() != web::Browser::_defaulthome) _result->field<url_type>(_find_url.toString()); // item->field("url")
		...
		if(in_tree && in_tree != _tree_view->know_model_board()->root_item()){
			if(!in_browser){
				...
				if(in_browser != in_tree) result_ = TreeLevel::instance(TreeIndex::require_treeindex(_current_model, in_tree), in_browser)->merge();
			...
			if(!in_browser){
				...
				assert(  gl_paras->main_window()->find([&](boost::intrusive_ptr<const ::Binder> b) -> bool {return url_equal((b->host()->field<home_type>()).toStdString(), _result->field<home_type>().toStdString());})
				      || _result->field<url_type>() == web::Browser::_defaulthome);
				if(_result->field<url_type>() == web::Browser::_defaulthome && find_url_.toString() != web::Browser::_defaulthome) _result->field<url_type>(find_url_.toString());
				assert(equal_(_result));
			}
		}
		assert(_result != _tree_view->know_model_board()->root_item());
...
boost::intrusive_ptr<TreeItem> TreeIndex::bind(const QUrl &_find_url, const insert_strategy &_view_insert_strategy, equal_url _equal, web::Browser *_recommend_browser){
...
boost::intrusive_ptr<TreeItem> TreeIndex::activate(const std::function<tkm_t *()> &current_model_, boost::intrusive_ptr<TreeItem> host_, const QUrl &find_url_, const insert_strategy &view_insert_strategy_, equal_url equal_) noexcept {
	...
	if(result) result->activate(std::bind(&wn_t::find, gl_paras->main_window(), std::placeholders::_1));
	return result;
}

TreeLevel::TreeLevel(boost::intrusive_ptr<TreeIndex> tree_index, boost::intrusive_ptr<TreeItem> to_be_merged, tv_t *tree_view)
