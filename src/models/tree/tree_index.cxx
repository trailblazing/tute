...
// }
boost::intrusive_ptr<TreeIndex> TreeIndex::require_treeindex(const std::function<tkm_t*()>& current_model_, boost::intrusive_ptr<i_t> host_)
{
        ...
        if (!host_parent)
            ...
            if (host_parent == absolute_root)
                ...
                result = (static_cast<QModelIndex>(current_model_()->index(host_parent)).isValid())
                    && (current_model_()->item([=](boost::intrusive_ptr<const i_t> it) { return it->id() == host_parent->id(); }));
            }
    ...
    if (!static_cast<QModelIndex>(host_index_).isValid()) {
        ...
        if (target_url == QUrl() || !target_url.isValid()) {
            ...
            host_->field<home_type>(web::Browser::_defaulthome);
            host_->field<url_type>(web::Browser::_defaulthome);
        }
        // result = new TreeIndex(current_model_, host_, host_index_, sibling_order_);
        host_ = TreeIndex::require_item(target_url, std::bind(&tv_t::move, tree_view_, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4) // std::placeholders::_1
            ,
            [&](boost::intrusive_ptr<const i_t> it_) -> bool { return url_equal(it_->field<home_type>().toStdString(), target_url.toString().toStdString()) || url_equal(it_->field<url_type>().toStdString(), target_url.toString().toStdString()); }); // instance(current_model_, tree_view_->current_item())->
    ...
    if (static_cast<QModelIndex>(host_index_).isValid()) {
        ...
        if (host_parent_) {
            ...
            if (!host_parent_->contains_direct(host_)) {
                tree_view_->move(TreeIndex::require_treeindex(current_model_, host_parent_), host_, [&](boost::intrusive_ptr<const Linker> il) -> bool { return url_equal(host_->field<url_type>().toStdString(), il->host()->field<url_type>().toStdString()) && il->host()->id() == host_->id() && il == host_->linker() && il->host_parent() == host_->parent(); }, true);
...
boost::intrusive_ptr<i_t> TreeIndex::host() const { return _host; } // {return _host_parent ? _host_parent->child_direct(_sibling_order) : nullptr;}
...
boost::intrusive_ptr<i_t> TreeIndex::require_item(const QUrl& find_url_, const insert_strategy& tree_view_insert_strategy_, equal_url equal_)
{
    ...
    if (find_url_ != QUrl()) {
        ...
        auto _treeindex = TreeIndex::require_treeindex(_current_model, _current_item);
        auto _tree_view_insert_strategy_boot
            = [&](boost::intrusive_ptr<i_t> _result // boost::intrusive_ptr<TreeIndex>_treeindex,
                ,
                const substitute_condition& _substitute_condition, const insert_strategy& _tree_view_insert_strategy_impl, const bool _item_is_brand_new, const QUrl& _find_url, const std::function<bool(boost::intrusive_ptr<i_t>)>& _check_url) -> boost::intrusive_ptr<i_t> { // KnowView::
            ...
            if (_item_is_brand_new) { // || ! _current_model()->item([&](boost::intrusive_ptr<const TreeItem> it){return it->id() == _result->id();})
                ...
                assert(_result);
                assert(_check_url(_result) || _result->field<url_type>() == web::Browser::_defaulthome || _find_url.toString() == web::Browser::_defaulthome);
                if (_result->field<url_type>() == web::Browser::_defaulthome && _find_url.toString() != web::Browser::_defaulthome)
                ...
                if (TreeIndex::is_ancestor_of(_result, _current_item)) {
                    auto host_new = TreeLevel::instance(TreeIndex::require_treeindex(_current_model, _result->parent()), _current_item)->move(); // static_cast<tkm_t *>(current_model())
        ...
        boost::intrusive_ptr<i_t> in_browser(nullptr);
        if (browser_view) {
        ...
        if (in_tree && in_tree != _tree_view->know_model_board()->root_item()) {
            if (!in_browser) {
                ...
                assert(equal_(in_browser)); // assert(_result->url<url_type>() == url_type()(_find_url));
                assert(equal_(in_tree)); // assert(_source_item->url<url_type>() == url_type()(_find_url));
                boost::intrusive_ptr<i_t> result_(in_browser);
                if (in_browser != in_tree)
                    result_ = TreeLevel::instance(TreeIndex::require_treeindex(_current_model, in_tree), in_browser)->merge();
                ...
                assert(gl_paras->main_window()->find([&](boost::intrusive_ptr<const ::Binder> b) -> bool { return url_equal((b->host()->field<home_type>()).toStdString(), _result->field<home_type>().toStdString()) || url_equal((b->host()->field<url_type>()).toStdString(), _result->field<url_type>().toStdString()); })
                    || _result->field<url_type>() == web::Browser::_defaulthome);
                assert(equal_(_result));
            }
		...
		boost::fusion::at_key<pin_type>(data) = pin_value(bool_from_check_state[Qt::Unchecked]);
                boost::fusion::at_key<name_type>(data) = "";
                boost::fusion::at_key<author_type>(data) = "";
                boost::fusion::at_key<home_type>(data) = find_url_.toString();
                boost::fusion::at_key<url_type>(data) = find_url_.toString();
		boost::fusion::at_key<tags_type>(data) = tags_value(QStringList());
                boost::fusion::at_key<dir_type>(data) = boost::fusion::at_key<id_type>(data);
                boost::fusion::at_key<file_type>(data) = "text.html";

                boost::intrusive_ptr<i_t> new_item = i_t::dangle_instance(data); // boost::intrusive_ptr<TreeItem>(new TreeItem(nullptr, data));
                ...
                assert(gl_paras->main_window()->find([&](boost::intrusive_ptr<const ::Binder> b) -> bool { return url_equal((b->host()->field<home_type>()).toStdString(), _result->field<home_type>().toStdString()); })
                    || _result->field<url_type>() == web::Browser::_defaulthome);
                if (_result->field<url_type>() == web::Browser::_defaulthome && find_url_.toString() != web::Browser::_defaulthome)
        ...
        assert(equal_(_result) || _result->field<url_type>() == web::Browser::_defaulthome || find_url_.toString() == web::Browser::_defaulthome);
        if (_result->field<url_type>() == web::Browser::_defaulthome && find_url_.toString() != web::Browser::_defaulthome)
            _result->field<url_type>(find_url_.toString());
...
boost::intrusive_ptr<i_t> TreeIndex::bind(const QUrl& _find_url, const insert_strategy& _view_insert_strategy, equal_url _equal, web::Browser* _recommend_browser)
{
    ...
    auto browser_ = _recommend_browser ? _recommend_browser : view ? view->page()->browser() : gl_paras->main_window()->browser(_find_url);
    _current_item = browser_->tabmanager()->current_item(); // _tree_view->current_item();

    auto target_ = TreeIndex::require_item(_find_url, _view_insert_strategy, _equal);
    if (target_) {
...
boost::intrusive_ptr<i_t> TreeIndex::activate(const std::function<tkm_t*()>& current_model_, boost::intrusive_ptr<i_t> host_, const QUrl& find_url_, const insert_strategy& view_insert_strategy_, equal_url equal_) noexcept
{
    boost::intrusive_ptr<i_t> result(nullptr);
    result = TreeIndex::require_treeindex(current_model_, host_)->bind(find_url_, view_insert_strategy_, equal_);
    if (result)
...
boost::intrusive_ptr<i_t> TreeLevel::to_be_operated() { return _to_be_operated; }

boost::intrusive_ptr<i_t> TreeLevel::merge() { return (_to_be_operated == _tree_index->host()) ? _to_be_operated : _tree_view->merge(this); }
...
boost::intrusive_ptr<TreeLevel> TreeLevel::instance(boost::intrusive_ptr<TreeIndex> _tree_index, boost::intrusive_ptr<i_t> _to_be_operated)
{
    auto current_model_ = _tree_index->current_model();
    auto tree_view_ = static_cast<tv_t*>(static_cast<QObject*>(current_model_())->parent());
    auto host = _tree_index->host();
    if (TreeIndex::is_ancestor_of(_to_be_operated, host)) {
        auto host_new = TreeLevel::instance(TreeIndex::require_treeindex(current_model_, _to_be_operated->parent()), host)->move(); // static_cast<tkm_t *>(current_model())
...
boost::intrusive_ptr<const i_t> TreeIndex::is_ancestor_of(boost::intrusive_ptr<const i_t> target, boost::intrusive_ptr<const i_t> reference)
