...
public:
    typedef std::function<QList<boost::intrusive_ptr<i_t> >(const std::function<tkm_t*()>&, const QList<boost::intrusive_ptr<i_t> >&)> view_delete_permantent_strategy;

    tkm_t(tv_t* parent = 0); // KnowTreeModel(const QStringList &headers, QDomDocument domModel, QObject *parent = 0);
    tkm_t(boost::intrusive_ptr<i_t> root_item, tv_t* parent = 0);
    ~tkm_t();
    ...
    QModelIndexList indexes(const std::function<bool(boost::intrusive_ptr<const i_t>)>& _equal);
    QList<boost::intrusive_ptr<i_t> > children(const std::function<bool(boost::intrusive_ptr<const i_t>)>& _equal);
    ...
    boost::intrusive_ptr<i_t> child(const std::function<bool(boost::intrusive_ptr<const i_t>)>& _equal) const; // const QUrl &_find_url, equal_type _equal
...
// bool is_id_exists(QString findId) const;
    ...
    boost::intrusive_ptr<i_t> merge(boost::intrusive_ptr<TreeLevel> _tree_merge, const view_delete_permantent_strategy& _view_delete_permantent); // boost::intrusive_ptr<TreeItem> target
...
private:
    ...
    QModelIndex move_up_dn(const QModelIndex& _index, int (i_t::*_move)()); // int direction

    boost::intrusive_ptr<i_t> delete_permanent(boost::intrusive_ptr<Linker> delete_linker);

    // boost::intrusive_ptr<TreeItem> model_child_remove(boost::intrusive_ptr<TreeItem> target);
    boost::intrusive_ptr<i_t> delete_permanent_recursive(boost::intrusive_ptr<Linker> delete_linker);
    ...
    boost::intrusive_ptr<i_t> new_child(boost::intrusive_ptr<TreeIndex> _modelindex, QString id, QString name);
    ...
    boost::intrusive_ptr<i_t> intercept(boost::intrusive_ptr<i_t> _item); // boost::intrusive_ptr<TreeItem> _item
