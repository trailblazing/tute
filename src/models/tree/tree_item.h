...
public:
    ...
    TreeItem(boost::intrusive_ptr<TreeItem> _host_parent // = nullptr
        ,
        QMap<QString, QString> _field_data = QMap<QString, QString>(), const QDomElement& _dom_element = QDomElement(), int pos = 0, int mode = add_new_record_before);
    ...
    QString field() const { return Record::field<field_type>(); } // , typename field_type_switch = typename switch_type<field_type>::type

    // , field_type_switch   // field_content;

    template <typename field_type>
    void field(const QString& value) { Record::field<field_type>(value); } // , typename field_type_switch = typename switch_type<field_type>::type
