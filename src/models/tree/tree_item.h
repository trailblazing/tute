...
public:
    ...
    i_t(boost::intrusive_ptr<i_t> host_parent // = nullptr
        //	    , QMap<QString, QString> field_data = QMap<QString, QString>()
        ,
        full_field_map field_data_static_map_ = full_field_map(), const QDomElement& _dom_element = QDomElement(), int pos = 0, int mode = add_new_record_before);
    ...
    static boost::intrusive_ptr<i_t> dangle_instance(const full_field_map& _field_data = full_field_map(), const QDomElement& _dom_element = QDomElement());
    ...
    typename boost::remove_reference<typename boost::fusion::result_of::at_key<full_field_map, field_key_type>::type>::type field() const;
    // {return Record::field<field_type>();} // , typename field_type_switch = typename switch_type<field_type>::type

    // , field_type_switch   // field_content;

    template <typename field_key_type>
    void field(const typename boost::remove_reference<typename boost::fusion::result_of::at_key<full_field_map, field_key_type>::type>::type& value);
    //{Record::field<field_type>(value);} // , typename field_type_switch = typename switch_type<field_type>::type
    ...
    boost::intrusive_ptr<i_t> contains_direct(boost::intrusive_ptr<const i_t>&& _item) const;
    boost::intrusive_ptr<i_t> contains_direct(boost::intrusive_ptr<const Linker>&& _item_linker) const;
    ...
    void binder_reset();
    boost::intrusive_ptr<i_t> delete_permanent_recursive(boost::intrusive_ptr<Linker> _to_be_removed_linker, std::function<bool(boost::intrusive_ptr<const i_t>)> condition);
    ...
    int move_up(void);
    int move_dn(void);
    void move_up(int pos) { vector_t::move_up(pos); }

    void move_dn(int pos) { vector_t::move_dn(pos); }
...
// void record_table(QDomElement i_dom_element);
    ...
    boost::intrusive_ptr<i_t> delete_permanent(const std::function<bool(boost::intrusive_ptr<const Linker>)>& _equal);
    boost::intrusive_ptr<i_t> delete_permanent_recursive(std::function<bool(boost::intrusive_ptr<const i_t>)> condition);
    ...
    boost::intrusive_ptr<const i_t> is_ancestor_of(boost::intrusive_ptr<const i_t> reference);
    int distance(boost::intrusive_ptr<const i_t> reference);

    full_field_map& field_data_static_map();
public slots:
...
private:
    ...
    i_t(full_field_map field_data_static_map_, const QDomElement& _dom_element = QDomElement());
};
