...
Binder::status_type Binder::state_impl()
{
    ...
    std::get<6>(status) = [&] (boost::intrusive_ptr<const TreeItem> host_) {
            return host_ == host();
        };                                                                                                                                                                                                                   //
...
void Binder::page(browser::WebPage *page) {
    return _page_linker_set(std::forward<browser::WebPage *>(page));
}

boost::intrusive_ptr<TreeItem> Binder::host() const {
    return _item_linker();
}
...
browser::WebView *Binder::bind() {
    return _bind_helper();
}

browser::WebView *Binder::activate() {
