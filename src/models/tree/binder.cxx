...
Binder::Binder(
    item_interface_set _item_linker_set, page_interface_set _page_linker_set, item_interface _item_linker, page_interface _page_linker, bind_interface _bind_helper, activate_interface _activate_helper, binder_type_interface _binder_type)
...
Binder::status_type Binder::state_impl()
{
    ...
    std::get<2>(status) = [&]() { return _host_linker() ? _host_linker()->binder() : nullptr; }; //
    ...
    std::get<3>(status) = [&]() { return _page_linker() ? _page_linker()->binder() : nullptr; }; //
    ...
    // auto ib = [&]() {return item_link()->binder() == this;};    // 4
    // auto pb = [&]() {return page_link()->binder() == this;};    // 5
    std::get<4>(status) = [&]() { return _host_linker() ? _host_linker()->binder() ? _host_linker()->binder().get() == this : false : false; }; //
    // std::get<4>(*_state) = std::make_shared<binder_self_reference>([&]() {return item_link()->binder() == this;});
    std::get<5>(status) = [&]() { return _page_linker() ? _page_linker()->binder() ? _page_linker()->binder().get() == this : false : false; }; //
    ...
    std::get<6>(status) = [&](boost::intrusive_ptr<const i_t> host_) { return host_ == _host_linker(); }; //
...
void Binder::host(boost::intrusive_ptr<i_t>&& it) { _item_linker_set(std::forward<boost::intrusive_ptr<i_t> >(it)); }

void Binder::page(web::WebPage* page) { return _page_linker_set(std::forward<web::WebPage*>(page)); }

boost::intrusive_ptr<i_t> Binder::host() const { return _host_linker(); }
