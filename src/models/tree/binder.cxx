...
Binder::Binder(
    item_interface_set _item_linker_set
              ...
              , binder_type_interface _binder_type) : _item_linker_set(_item_linker_set), _page_linker_set(_page_linker_set), _item_linker(_item_linker), _page_linker(_page_linker), _bind_helper(_bind_helper), _activate_helper(_activate_helper), _binder_type(_binder_type), _status(state_impl()){
...
Binder::status_type Binder::state_impl(){
    ...
    std::get<2>(status) = [&](){return host() ? host()->binder() : nullptr;};																							//
    ...
    std::get<3>(status) = [&](){return page() ? page()->binder() : nullptr;};																										//
        ...
        //    auto ib = [&]() {return item_link()->binder() == this;};    // 4
        //    auto pb = [&]() {return page_link()->binder() == this;};    // 5
    std::get<4>(status) = [&](){return host() ? host()->binder() ? host()->binder().get() == this : false : false;};																										//
        //    std::get<4>(*_state) = std::make_shared<binder_self_reference>([&]() {return item_link()->binder() == this;});
    std::get<5>(status) = [&](){return page() ? page()->binder() ? page()->binder().get() == this : false : false;};																										//
    ...
    std::get<6>(status) = [&](boost::intrusive_ptr<const TreeItem> host_){return host_ == host();};																													//
...
boost::intrusive_ptr<TreeItem> Binder::host() const {return _item_linker();}

browser::WebPage *Binder::page() const {return _page_linker();}
