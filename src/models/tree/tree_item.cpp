...
TreeItem::TreeItem(boost::intrusive_ptr<TreeItem> _host_parent, QMap<QString, QString> _field_data, const QDomElement& _dom_element, int pos, int mode)
    : Record(_field_data)
    ...
    , _linker(new Linker(_host_parent, this))
{ // , pos, mode
...
TreeItem::TreeItem(QMap<QString, QString> _field_data, const QDomElement& _dom_element)
    : Record(_field_data)
...
int TreeItem::field_count() const { return _field_data.count(); }
...
template <>
QString TreeItem::field<dynamic_name_type>() const
{
...
void TreeItem::traverse_direct(const std::function<void(boost::intrusive_ptr<Linker>)>& operation) { return ItemsFlat::traverse(operation); }
...
void TreeItem::clear() { ItemsFlat::clear(); }
...
const boost::intrusive_ptr<Binder>&& TreeItem::binder() const { return std::forward<const boost::intrusive_ptr<Binder> >(_binder); }
...
boost::intrusive_ptr<const TreeItem> TreeItem::is_ancestor_of(boost::intrusive_ptr<const TreeItem> reference)
{ // const std::function<tkm_t *()> &current_model_, const std::function<bool (boost::intrusive_ptr<const TreeItem>)> &_equal
    ...
    if (reference)
        if (reference->path_list().contains(this->field<id_type>()) && reference != this)
            result = reference;
    return result;
}

int TreeItem::distance(boost::intrusive_ptr<const TreeItem> reference)
{ // const std::function<tkm_t *()> &current_model_, const std::function<bool (boost::intrusive_ptr<const TreeItem>)> &_equal
    //    boost::intrusive_ptr<const TreeItem>	result(nullptr);
    int distance_ = -1;
    if (reference) {
