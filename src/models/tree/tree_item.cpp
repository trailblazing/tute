...
i_t::i_t(boost::intrusive_ptr<i_t> host_parent, full_field_map field_data_static_map_, const QDomElement& _dom_element, int pos, int mode)
    ...
    , _linker([&] {auto l = new Linker(host_parent, this); return l; }())
{ // , pos, mode
...
i_t::i_t(full_field_map field_data_static_map_, const QDomElement& _dom_element)
    : Record(field_data_static_map_)
...
boost::intrusive_ptr<i_t> i_t::dangle_instance(const full_field_map& _field_data, const QDomElement& _dom_element)
{
    return new i_t(_field_data, _dom_element);
}
...
int i_t::sibling_order(const std::function<bool(boost::intrusive_ptr<const Linker>)>& _equal) const
{
...
int i_t::field_count() const { return _field_data.count(); }
...
//QString TreeItem::field() const {return Record::field<field_type>();} // , typename field_type_switch = typename switch_type<field_type>::type
...
template <>
typename boost::remove_reference<typename boost::fusion::result_of::at_key<full_field_map, dynamic_name_type>::type>::type i_t::field<dynamic_name_type>() const
{
...
QMap<QString, QString> i_t::fields_all()
{
...
QMap<QString, QString> i_t::fields_all_direct()
{
    return _field_data;
}
...
boost::intrusive_ptr<i_t> i_t::contains_direct(const std::function<bool(boost::intrusive_ptr<const Linker>)>&& _equal) const
{
    return vector_t::contains_direct(std::forward<const std::function<bool(boost::intrusive_ptr<const Linker>)>&&>(_equal));
}

boost::intrusive_ptr<i_t> i_t::contains_direct(boost::intrusive_ptr<const i_t>&& _item) const
{
    return vector_t::contains_direct(std::forward<boost::intrusive_ptr<const i_t> >(_item)); // result;
...
boost::intrusive_ptr<i_t> i_t::contains_direct(boost::intrusive_ptr<const Linker>&& _item_linker) const
{
    return vector_t::contains_direct(std::forward<boost::intrusive_ptr<const Linker> >(_item_linker)); //
...
boost::intrusive_ptr<i_t> i_t::delete_permanent(const std::function<bool(boost::intrusive_ptr<const Linker>)>& _equal)
{
    return vector_t::delete_permanent(_equal);
}

boost::intrusive_ptr<i_t> i_t::delete_permanent_recursive(std::function<bool(boost::intrusive_ptr<const i_t>)> condition)
{
    boost::intrusive_ptr<i_t> result(nullptr);
...
boost::intrusive_ptr<i_t> i_t::delete_permanent_recursive(boost::intrusive_ptr<Linker> _to_be_removed_linker, std::function<bool(boost::intrusive_ptr<const i_t>)> condition)
{
    ...
    // return result;
    return vector_t::delete_permanent_recursive(_to_be_removed_linker, condition);
}
...
void i_t::traverse_direct(const std::function<void(boost::intrusive_ptr<Linker>)>& operation) { return vector_t::traverse(operation); }

boost::intrusive_ptr<Linker> i_t::dangle()
{
    auto _parent = _linker->host_parent();
    if (_parent)
        _parent->release([&](boost::intrusive_ptr<const Linker> il) { return il->host()->id() == this->id() && il == _linker; }); // delete_permanent([&](boost::intrusive_ptr<const Linker> il){return il->host()->id() == this->id() && il == _linker;});
...
QList<boost::intrusive_ptr<i_t> > i_t::children_insert_new(int position, int count, int columns)
{
    Q_UNUSED(columns);
    // bool result = false;
...
boost::intrusive_ptr<i_t> i_t::merge(boost::intrusive_ptr<i_t> cut)
{
    ...
    if (!_linker) {
        if (cut->linker()) {
            _linker = [&] {auto l = new Linker(cut->linker()->host_parent(), boost::intrusive_ptr<i_t>(this)); return l; }(); // cut->linker()
...
QStringList i_t::path_list(QString field_name) const
{
    // auto path_absolute_as_field = [&](QString field_name)->QStringList { // const
    QStringList path;
    boost::intrusive_ptr<const i_t> current_item(this); // = boost::intrusive_ptr<TreeItem>(const_cast<TreeItem *>(this))   // shared_from_this()
...
QString i_t::path_string(QString field_name, QString delimeter) const
{
...
QList<QStringList> i_t::path_children_all(QString field_name) const
{
    ...
    // Возвращает массив указанных полей всех подветок, которые содержит ветка
    // Внутренняя рекурсивная функция
    std::function<QList<QStringList>(boost::intrusive_ptr<const i_t>, QString, int)> path_children_all_as_field_impl
        = [&](boost::intrusive_ptr<const i_t> item, QString field_name, int mode) -> QList<QStringList> { // const
...
void i_t::to_decrypt(void)
{
    ...
    if (_field_data[boost::mpl::c_str<crypt_type>::value].length() == 0 || _field_data[boost::mpl::c_str<crypt_type>::value] == "0")
        return;
...
bool i_t::is_empty() const
{
    return static_cast<const Record*>(this)->is_empty()
        && static_cast<const vector_t*>(this)->is_empty();
}

boost::intrusive_ptr<Binder> i_t::binder() { return _binder; }

const boost::intrusive_ptr<Binder>&& i_t::binder() const { return std::forward<const boost::intrusive_ptr<Binder> >(_binder); }
...
boost::intrusive_ptr<Linker> i_t::linker() { return _linker; }

const boost::intrusive_ptr<Linker>&& i_t::linker() const { return std::forward<const boost::intrusive_ptr<Linker> >(_linker); }
...
void i_t::binder_reset()
{
    if (_binder) { // _page != nullptr
        ...
        // auto _page_binder = _binder->integrity_external(this, _binder->page()) ? _binder->page()->binder() : nullptr;
...
boost::intrusive_ptr<const i_t> i_t::is_ancestor_of(boost::intrusive_ptr<const i_t> reference)
{ // const std::function<tkm_t *()> &current_model_, const std::function<bool (boost::intrusive_ptr<const TreeItem>)> &_equal
    ...
    if (reference)
        if (reference->path_list().contains(field<id_type>()) && reference != this)
            result = reference;
    return result;
}

int i_t::distance(boost::intrusive_ptr<const i_t> reference)
{ // const std::function<tkm_t *()> &current_model_, const std::function<bool (boost::intrusive_ptr<const TreeItem>)> &_equal
    // boost::intrusive_ptr<const TreeItem>	result(nullptr);
    int distance_ = -1;
    if (reference) {
        if (reference != this) {
            if (reference->path_list().contains(field<id_type>())) {
                // result	= reference;
                distance_ = reference->path_list().indexOf(field<id_type>());
...
{
    auto tags_ = field<tags_type>();
    if (tags_.size() > 0)
        while (tags_.at(0) == ',' || tags_.at(0) == ';')
            tags_.remove(0, 1);
    auto tag_list = tags_.split(QRegExp("[,:]+"), QString::SkipEmptyParts);
    //	auto new_topic = new_topic;//_toolbarsearch->text();
    if (tag_list.size() > 0) {
        if (!tag_list.contains(new_topic))
            tags_ += "," + new_topic;
    } else // (tag_list.size() == 0)
        tags_ += new_topic;
    field<tags_type>(tags_);
}

full_field_map& i_t::field_data_static_map() { return _field_data_map; }
