...
boost::intrusive_ptr<TreeItem> TreeItem::merge(boost::intrusive_ptr<TreeItem> cut){
	...
	if(!cut->is_lite() && !is_lite()){
		...
		if(pic.size() > 0){
			for(auto i : pic.keys())
				if(this_pic[i].isEmpty()) this_pic[i] = pic[i]; // auto new_pic = this_pic.merge(pic);
			picture_files(this_pic);
		}
	}
	if(!_linker){
		if(cut->linker()){
			_linker = [&] {auto l = new Linker(cut->linker()->host_parent(), boost::intrusive_ptr<TreeItem>(this)); return l;} (); // cut->linker()
...
void TreeItem::to_decrypt(void){
	...
	if(_field_data[boost::mpl::c_str < crypt_type > ::value].length() == 0 || _field_data[boost::mpl::c_str < crypt_type > ::value] == "0") return;
...
boost::intrusive_ptr<const TreeItem> TreeItem::is_ancestor_of(boost::intrusive_ptr<const TreeItem> reference){ // const std::function<tkm_t *()> &current_model_, const std::function<bool (boost::intrusive_ptr<const TreeItem>)> &_equal
	boost::intrusive_ptr<const TreeItem> result(nullptr);
	if(reference)
		if(reference->path_list().contains(this->field<id_type>()) && reference != this) result = reference;
	return result;
}
