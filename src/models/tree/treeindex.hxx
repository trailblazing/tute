...
struct TreeMerge : public boost::intrusive_ref_counter<TreeMerge, boost::thread_safe_counter> {
    boost::intrusive_ptr<TreeIndex> tree_index();
    boost::intrusive_ptr<TreeItem> to_be_merged();
    private:
	TreeMerge(boost::intrusive_ptr<TreeIndex> _tree_index, boost::intrusive_ptr<TreeItem> _to_be_merged);
	boost::intrusive_ptr<TreeIndex> _tree_index;
	boost::intrusive_ptr<TreeItem> _to_be_merged;
...
//	static boost::intrusive_ptr<TreeIndex> instance(const std::function<km_t *()> &current_model, boost::intrusive_ptr<TreeItem> host_parent, int sibling_order = 0);
	...
	//                                             , equal_t _equal = [](boost::intrusive_ptr<const TreeItem> it, boost::intrusive_ptr<const TreeItem> target)->bool {return it->id() == target->id();}
	//                                            );
	static boost::intrusive_ptr<TreeMerge> merge_instance(boost::intrusive_ptr<TreeIndex> _tree_index, boost::intrusive_ptr<TreeItem> _to_be_merge);
    protected:
