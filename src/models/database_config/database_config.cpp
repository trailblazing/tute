...
DataBaseConfig::DataBaseConfig(std::shared_ptr<gl_para> gl_paras_, std::shared_ptr<AppConfig> appconfig_, QObject *pobj) {
	...
	// QString
	// configFileName=globalParameters.getWorkDirectory()+"/"+appconfig->datadir()+"/database.ini";
	QString configFileName = gl_paras_->root_path() + "/" +
	    QDir(appconfig_->data_dir()).dirName() + "/" +
	    gl_para::_data_conf_filename;
...
// void DataBaseConfig::init(const gl_para &gl_paras_, const AppConfig
// &appconfig_)
// {}
