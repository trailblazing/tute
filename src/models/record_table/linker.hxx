...
public:
    ...
    typedef std::tuple<item_exist, item_exist, linker_exist, linker_exist, linker_self_reference, linker_self_reference, item_consistency, item_consistency> status_type;

    Linker(boost::intrusive_ptr<i_t> host_parent_item, boost::intrusive_ptr<i_t> host_item); // , const int pos = 0, const int mode = add_new_record_after
    ...
    boost::intrusive_ptr<Linker> parent(boost::intrusive_ptr<i_t> _parent, const int pos = 0, const int mode = add_new_record_after); //
    ...
    static boost::intrusive_ptr<Linker> instance(boost::intrusive_ptr<i_t> _host, boost::intrusive_ptr<i_t> _parent, int pos = 0, int mode = add_new_record_before);
...
private:
    ...
    std::function<boost::intrusive_ptr<Linker>(boost::intrusive_ptr<i_t>, int, int)> integrity_fix_up; //        bool integrity();
