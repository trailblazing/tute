...
struct check_id {
    check_id(full_field_map&) {}
};

template <>
struct check_id<id_type> {
    check_id(full_field_map& full_field_map_)
    {
        auto dir = QString(boost::fusion::at_key<dir_type>(full_field_map_));
        if (id_value("") == boost::fusion::at_key<id_type>(full_field_map_))
            boost::fusion::at_key<id_type>(full_field_map_) = dir.length() > 0 ? dir : get_unical_id();
    }
};
}
...
//		const QString &field_name = boost::mpl::c_str<index_type>::value;//index_type is mpl_::void_?// boost::mpl::c_str<typename boost::mpl::at<natural_field_set, index_type>::type>::value;

//		auto internal_field = [&](QString name){if(_this->_field_data.contains(name)) return _this->_field_data[name]; else return QString(); };
//		auto internal_field_write = [&](QString key, QString value){_this->_field_data.insert(key, value);};// = available_field_list.at(j);
#ifndef USE_CXX_14
        typedef typename sd::static_if<sd::equal<index_type, id_type>::value, detail::check_id<id_type>, detail::check_id<index_type> >::type check;
        check c(_this->_field_data_map);
#endif //USE_CXX_14
        //		std::enable_if<sd::STATIC_SAME<index_type, id_type>::value, check_id>::type();
        detail::static_if<sd::equal<index_type, id_type>::value>([&]() {
            auto dir = QString(boost::fusion::at_key<dir_type>(_this->_field_data_map));
            if (id_value("") == boost::fusion::at_key<id_type>(_this->_field_data_map))
                boost::fusion::at_key<id_type>(_this->_field_data_map) = dir.length() > 0 ? dir : get_unical_id();
        })();

        // Устанавливается значение поля как атрибут DOM-узла
        //		if(_this->_field_data.contains(field_name))
        elem.setAttribute(boost::mpl::c_str<index_type>::value, boost::fusion::at_key<index_type>(_this->_field_data_map)); //internal_field(field_name)
        return 0;
    }
};
...
////			  auto dst = FixedParameters::_record_natural_field_static;
//			  // Перебираются допустимые имена полей, доступных для сохранения
////			  QStringList available_field_list;//= FixedParameters::_record_natural_field_static;
////			  natural_field_type::item_ iti;
////			  boost::mpl::begin<natural_field_type> te;
//			  // int available_field_list_size = available_field_list.size();
////			  void (Record::*func)(QString, QString) = &Record::natural_field_source;
...
//				  >(element_fullfill(elem, this))(FixedParameters::_record_natural_field_tuple);//_record_natural_field_static
...
//		  })
//	  , write_crypt_field(
//		  [&](const QString &_name, const QString &value) -> void { // template <typename field_type>void Record::field_write<field_type, crypt_field_type>::operator()(const QString &value){// template<typename field_type> void Record::field(const QString &value){
...
//					  if(value.length() > 0){
//						  if(gl_paras->crypt_key().length() > 0) is_crypt = true;
...
//			  if(is_crypt == true) _value = CryptService::encryptString(gl_paras->crypt_key(), value);
...
Record::Record(full_field_map field_data_static_map_)
    ...
    , _field_data_map(field_data_static_map_)
    , _text("")
#ifdef USE_STAR_RATING
    , _star_rating(new StarRating(1, 5))
#endif
    ...
    , dom_from_record_impl(
          [&](std::shared_ptr<QDomDocument> doc) -> QDomElement {
              ...
              Apply<boost::mpl::size<natural_field_set>::type::value, element_fullfill //std::bind(func, this)//<boost::mpl::at<natural_field_type, boost::mpl::_1> >
                  >(element_fullfill(elem, this))(std::forward<natural_field_tuple const>(FixedParameters::_record_natural_field_tuple));
              // К элементу записи прикрепляется элемент таблицы приаттаченных файлов, если таковые есть
...
void Record::dom_to_record(const QDomElement& _dom_element)
{
    ...
    for (i = 0; i < attList.count(); i++) {
        ...
        _field_data[name] = value; //this->natural_field_source(name, value);
    ...
    if (_lite_flag && natural_field_source<id_type>() != global_root_id)
        to_fat();
}
...
{
    return field<crypt_type>() == crypt_value(true); // "1";
}
...
{
    QString id, name;
    if (_field_data.contains(boost::mpl::c_str<id_type>::value))
        id = _field_data[boost::mpl::c_str<id_type>::value];
    if (_field_data.contains(boost::mpl::c_str<name_type>::value))
        name = _field_data[boost::mpl::c_str<name_type>::value];
...
struct full_fill_natural_list {
    const Record* const _this;
    QMap<QString, QString>& _result_field_list;
    bool _is_crypt;

    full_fill_natural_list(const Record* const this_, QMap<QString, QString>& resultFieldList, bool isCrypt)
        : _this(this_)
        , _result_field_list(resultFieldList)
        , _is_crypt(isCrypt)
    {
    }
    template <typename field_type>
    void operator()(field_type field_type_value) const
    {
        (void)field_type_value;
        QString currName = boost::mpl::c_str<field_type>::value; //fieldNames.at(i);
        // Если поле с таким именем существует
        if (_this->_field_data.contains(currName)) {
            QString result = "";
            if (_is_crypt == false)
                result = _this->_field_data[currName]; // Напрямую значение поля
            else {
                ...
                if (fixedparameters._record_field_crypted.contains(currName) == false)
                    ...
                    result = CryptService::decryptString(gl_paras->crypt_key(), _this->_field_data[currName]); // асшифровывается значение поля
            }
            _result_field_list[currName] = result;
        }
    }
};
...
QMap<QString, QString> Record::natural_field_list() const
{
    ...
    Apply<boost::mpl::size<natural_field_set>::type::value, full_fill_natural_list>(full_fill_natural_list(this, resultFieldList, isCrypt))(std::forward<natural_field_tuple const>(FixedParameters::_record_natural_field_tuple));

    //	for(int i = 0; i < boost::mpl::size<natural_field_tuple>::type::value; ++i){//for(int i = 0; i < fieldNames.size(); ++i){
    ...
    //				else if(gl_paras->crypt_key().length() > 0 && fixedparameters._record_field_crypted.contains(currName)) result = CryptService::decryptString(gl_paras->crypt_key(), _field_data[currName]); // асшифровывается значение поля
    //			}
...
void Record::text_to_direct(QString _text_source)
{
    ...
    if (_field_data.value(boost::mpl::c_str<crypt_type>::value).length() == 0 || _field_data.value(boost::mpl::c_str<crypt_type>::value) == "0") {
        ...
        critical_error("Record::text_to_direct() : Unavailable crypt field value \"" + _field_data.value(boost::mpl::c_str<crypt_type>::value) + "\"");
}
    ...
    foreach (QString field_name, fixedparameters._record_field_crypted) {
        ...
        if (_field_data.contains(field_name))
            if (_field_data[field_name].length() > 0)
                _field_data[field_name] = _field_data.value(field_name); // Устанавливаются значения, при установке произойдет шифрация
    }
}
...
{
    if (_field_data.contains(boost::mpl::c_str<dir_type>::value) == false)
        const_cast<Record*>(this)->_field_data[boost::mpl::c_str<dir_type>::value] = _field_data.value(boost::mpl::c_str<id_type>::value); // critical_error("Record::short_dir() : Not present dir field");
...
//}
//template <typename field_type> // , typename field_type_switch = typename switch_type<field_type>::type
//void Record::field(const QString &value){
...
//QString Record::natural_field_source() const {//QString name
//	// Если имя поля недопустимо
//#ifdef TEST
...
//template<typename concrete>
//void Record::natural_field_source(const QString &value){//QString name,, concrete cc
////		(void) cc;
//	// Если имя поля недопустимо
//#ifdef TEST
...
const typename boost::fusion::result_of::at_key<crypt_field_map const, typename boost::mpl::at<crypt_field_set, field_key_type>::type>::type Record::read_crypt_field() const
{
    typedef typename boost::fusion::result_of::at_key<crypt_field_map const, typename boost::mpl::at<crypt_field_set, field_key_type>::type>::type return_type;
    //    QString name = boost::mpl::c_str<typename boost::mpl::at<append_to_crypt_set, field_type>::type>::value;
    return_type result = boost::fusion::at_key<field_key_type>(_field_data_map);

    //    QString crypt_field_name = boost::mpl::c_str<crypt_type>::value;
    detail::static_if<boost::fusion::result_of::has_key<crypt_field_map const, crypt_type> >([&]() {
	if (true == boost::fusion::at_key<crypt_type>(_field_data_map)) {
	    auto value = detail::to_string(boost::fusion::at_key<field_key_type>(_field_data_map));
	    if (value.length() > 0 && gl_paras->crypt_key().length() > 0) {
		result = detail::from_string<return_type>(CryptService::decryptString(gl_paras->crypt_key(), value));
	    }
	}
    })();
    //    if (_field_data.contains(crypt_field_name)) { // boost::mpl::c_str < crypt_type > ::value
...
template <typename field_key_type>
void Record::write_crypt_field(const typename boost::remove_reference<typename boost::fusion::result_of::at_key<crypt_field_map const, typename boost::mpl::at<crypt_field_set, field_key_type>::type>::type>::type& value)
{
    //    const QString& _name;
    // template <typename field_type>void Record::field_write<field_type, crypt_field_type>::operator()(const QString &value){// template<typename field_type> void Record::field(const QString &value){
    QString _name = boost::mpl::c_str<typename boost::mpl::at<crypt_field_set, field_key_type>::type>::value;
    //// Если имя поля недопустимо (установить значение можно только для натурального поля)
    // if(fixedparameters.is_record_field_natural(_name) == false)critical_error("In Record::field() unavailable field name " + _name + " try set to " + value);
    bool is_crypt = false;
...
//	    auto value = detail::to_string(boost::fusion::at_key<field_key_type>(_field_data_map));
		...
		if (gl_paras->crypt_key().length() > 0) {
		    is_crypt = true;
		    boost::fusion::at_key<field_key_type>(_field_data_map) = CryptService::encryptString(gl_paras->crypt_key(), value);
		    ;
		} else
