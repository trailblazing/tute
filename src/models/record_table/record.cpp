...
Record::Record(QMap<QString, QString> field_data_)
...
void Record::dom_to_record(const QDomElement &_dom_element){
	...
	if(_lite_flag && natural_field_source<id_type>() != global_root_id) to_fat();
}
...
struct full_fill_natural_list {
	...
	full_fill_natural_list(const Record *const this_, QMap<QString, QString> &resultFieldList, bool isCrypt) : _this(this_), _result_field_list(resultFieldList), _is_crypt(isCrypt){}
	...
	void operator ()(field_type field_type_value) const {
		...
		if(_this->_field_data.contains(currName)){
			QString result = "";
			if(_is_crypt == false) result = _this->_field_data[currName]; // Напрямую значение поля
			else{
				...
				if(fixedparameters._record_field_crypted.contains(currName) == false) result = _this->_field_data[currName]; // Напрямую значение поля
				else if(gl_paras->crypt_key().length() > 0 && fixedparameters._record_field_crypted.contains(currName)) result = CryptService::decryptString(gl_paras->crypt_key(), _this->_field_data[currName]); // асшифровывается значение поля
			}
...
void Record::to_decrypt_fields(void){
	...
	foreach(QString field_name, fixedparameters._record_field_crypted){
		...
		// Поле расшифровывается
		if(_field_data.contains(field_name))
			if(_field_data[field_name].length() > 0) _field_data[field_name] = _field_data.value(field_name); //natural_field_source(field_name, _field_data[field_name]); // field(field_name)
	}
...
void Record::push_lite_attributes(){
	...
	if(_field_data.value(boost::mpl::c_str<crypt_type>::value) == "1" && gl_paras->crypt_key().length() == 0) critical_error("Record::push_lite_attributes() : Try save text for crypt record while password not setted.");
...
void Record::push_fat_attributes(){
	...
	// Если запись зашифрована, но ключ не установлен (т.е. человек не вводил пароль)
	// то зашифровать текст невозможно
	if(_field_data.value(boost::mpl::c_str<crypt_type>::value) == "1" && gl_paras->crypt_key().length() == 0) critical_error("Record::push_fat_attributes() : Try save text for crypt record while password not setted.");
	// Заполняются имена директории и полей
	// Директория при проверке создается если ее небыло
	QString dir_name;
	QString file_name;
...
void Record::check_and_fill_file_dir(QString &idir_name, QString &i_file_name){
	...
	if(!recordDir.exists()){
		...
		if(!directory.exists()) QDir(gl_paras->root_path() + "/" + QDir(appconfig->data_dir()).dirName()).mkdir("base");
