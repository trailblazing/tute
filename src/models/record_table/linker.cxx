                      ...
                      if (!integrity_external(this->_host, _parent_target)) {
                          ...
                          if (0 == found) { // !item_direct(_source_item)
                              ...
                              if (0 == _host->field<id_type>().length()) { // || dataModel->isRecordIdExists(record->getField("id"))  // ? Is this a correct policy? can we only create a pure view?
                                  ...
                                  if (_host->field<dir_type>() == "")
                                      _host->field<dir_type>(id); // get_unical_id()
                                  if (_host->field<file_type>() == "")
                                      _host->field<file_type>("text.html");
                                  // Unique ID of XML records             // Уникальный идентификатор XML записи
                                  // QString id = get_unical_id();
                                  _host->field<id_type>(id);
                              } else {
                                  if (_host->field<dir_type>() == "")
                                      _host->field<dir_type>(_host->field<id_type>()); // get_unical_id()
                                  if (_host->field<file_type>() == "")
                                      _host->field<file_type>("text.html");
                              }
                          ...
                          if (_host_parent->_field_data[boost::mpl::c_str<crypt_type>::value] == "1") { // new_host_parent &&
                              if (_host->_field_data[boost::mpl::c_str<crypt_type>::value] != "1")
                                  _host->to_encrypt();
                          } else if (_host->_field_data[boost::mpl::c_str<crypt_type>::value] == "1")
                              _host->to_decrypt();
                          //		      }
...
boost::intrusive_ptr<Linker> Linker::instance(boost::intrusive_ptr<TreeItem> _host, boost::intrusive_ptr<TreeItem> _parent, int pos, int mode)
    ...
    if (_parent) {
        if (!_linker) {
            ...
            if (_linker->host() != _host)
                _linker->host(std::forward<boost::intrusive_ptr<TreeItem> >(_host)); // std::move(boost::intrusive_ptr<TreeItem>(this))
