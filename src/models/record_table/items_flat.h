...
//	ItemsFlat(const bool is_crypt = false);
    ...
    QList<boost::intrusive_ptr<i_t> > children_direct(const std::function<bool(boost::intrusive_ptr<const Linker>)>& _substitute_condition) const;
    ...
    boost::intrusive_ptr<i_t> child_direct(const std::function<bool(boost::intrusive_ptr<const Linker>)>& _substitute_condition) const;
    ...
    boost::intrusive_ptr<i_t> delete_permanent_recursive(boost::intrusive_ptr<Linker> remove_linker, std::function<bool(boost::intrusive_ptr<const i_t>)> condition);
...
private:
    ...
    friend boost::intrusive_ptr<i_t>& operator<<(boost::intrusive_ptr<i_t>& host, boost::intrusive_ptr<i_t> _item); // {return *it_left << it_right;}
};
