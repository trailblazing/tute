...
vector_t::vector_t(
    ...
    const bool is_crypt)                                                               // , boost::intrusive_ptr<TreeItem> _parent_item
    : dom_from_itemsflat_impl([&](std::shared_ptr<QDomDocument> doc) -> QDomElement {  // Преобразование
	                                                                                   // таблицы конечных
	                                                                                   // записей в Dom
	                                                                                   // документ
	    ...
	    for (int i = 0; i < _child_linkers.size(); i++) {
		    if (_child_linkers.at(i)->host() != this) {  //(static_cast<ItemsFlat
			                                             //*>(_child_linkers.at(i)->host().get()) != this)
			    item_flat_dom.appendChild(
			        _child_linkers.at(i)->host()->dom_from_treeitem_impl(
			            doc));  // boost::static_pointer_cast<Record>(_child_items.at(i))->dom_from_record(doc)
			                    // К элементу recordtabledata прикрепляются конечные записи
		    }
	    }
	    // qDebug() << "In export_modeldata_to_dom() recordtabledata " <<
	    // doc.toString();
...
//			  // Если у ветки нет таблицы конечных записей, возвращается
//пустой документ
//			  QDomElement item_flat_dom;
//			  if(_child_linkers.size() != 0){  //
//if(_child_linkers.size() == 0)return QDomElement();
//				  item_flat_dom =
//doc->createElement("recordtable");
//				  // Пробегаются все записи в таблице
//				  for(int i = 0; i < _child_linkers.size();
//i++){
//					  if(_child_linkers.at(i)->host() != this){  //
//history error
//						  item_flat_dom.appendChild(_child_linkers.at(i)->host()->dom_from_treeitem_impl(doc));
////
//boost::static_pointer_cast<Record>(_child_items.at(i))->dom_from_record(doc)
//						  // К элементу recordtabledata
//прикрепляются конечные записи
//					  }
//				  }
//			  }
//			  // qDebug() << "In export_modeldata_to_dom()
//recordtabledata " << doc.toString();
...
//	// && (parent_item->_field_data[crypt_key] == crypt_value)) ? crypt_1 :
//crypt_0;
...
//	// {
//	// if(_parent_item)
//	// _field_data[boost::mpl::c_str < crypt_type > ::value] =
//(_parent_item->_field_data.contains(boost::mpl::c_str < crypt_type > ::value)
//&& _parent_item->_field_data[boost::mpl::c_str < crypt_type > ::value] == "1")
//? "1" : "0";
...
ItemsFlat::ItemsFlat(const ItemsFlat& obj)
	...
	foreach (auto i, obj._child_items) {
		_child_items.push_back(
		    i);  // boost::intrusive_ptr<TreeItem>(new TreeItem(*i))
	}
}
...
//// Имя поля - все возможные имена полей, кроме text (такого поля теперь вообще
///нет, текст запрашивается как отдельное свойство)
...
QList<boost::intrusive_ptr<i_t> > vector_t::children_direct(
    const std::function<bool(boost::intrusive_ptr<const Linker>)>& _substitute_condition) const {
...
vector_t::children_direct(const QString& name) const {
	QList<boost::intrusive_ptr<i_t> > results;
	for (auto il : _child_linkers)  // for(int i = 0; i < count_direct(); i++) {
		if (il->host()->field<name_key>() == name)
			results.push_back(il->host());  // return i;
	return results;                         // -1;
}
...
boost::intrusive_ptr<i_t> vector_t::child_direct(const QUrl& url) const {
	boost::intrusive_ptr<i_t> result(nullptr);
	for (int i = 0; i < count_direct(); i++) {
		if (url_equal(detail::to_string(child_direct(i)->field<url_key>()), url.toString().toStdString())) {
...
boost::intrusive_ptr<i_t> vector_t::child_direct(
    const std::function<bool(boost::intrusive_ptr<const Linker>)>& _substitute_condition) const {
	boost::intrusive_ptr<i_t> result(nullptr);
	for (auto il : _child_linkers) {
		if (_substitute_condition(
		        il)) {  // if(i->field("id") == item->field("id")) {
...
// ItemsFlat::item_direct(boost::intrusive_ptr<TreeItem> item)const
// {
// boost::intrusive_ptr<TreeItem> result;
...
// QString i;
// i.setNum(pos);
// critical_error("In RecordTableData::setField() unavailable record index " + i
// + " in table while field " + name + " try set to " + value);
// }
...
// Если возникнет проблема, что файла с текстом записи нет, будет создан пустой
// файл
...
// Внимание! Метод не содержит работы с данными записи. Подумать, где его
// разместить
...
void vector_t::editor_load_callback(QObject* editor, QString& loadText) {
	...
	if (currEditor->misc_field<crypt_key>() == crypt_value(true)) {
	...
	if (workWithCrypt == false)
		...
		loadText = CryptService::decryptStringFromByteArray(
...
void vector_t::editor_save_callback(QObject* editor, QString saveText) {
	...
	if (workWithCrypt == false) {
		...
		if (!wfile.open(QIODevice::WriteOnly | QIODevice::Text))
			critical_error(
			    "ItemsFlat::editor_save_callback() : Can\'t open text file " + fileName + " for write.");
		...
		if (!wfile.open(QIODevice::WriteOnly))
			critical_error(
			    "ItemsFlat::editor_save_callback() : Can\'t open binary file " + fileName + " for write.");
...
// Возвращается запись с "сырыми" данными. Если запись была зашифрована, метод
// вернет зашифрованные данные
// Get a copy of the full image recording
// Returns the record with "raw" data. If the record was encrypted, the method
// returns the encrypted data
boost::intrusive_ptr<i_t> vector_t::item_fat(int pos) {
	// Копия записи из дерева
	boost::intrusive_ptr<i_t> result = child_direct(
	    pos);  // boost::intrusive_ptr<Record> resultRecord = getRecordLite(pos);
	// original
	...
	QString directory = gl_paras->root_path() + "/" + QDir(appconfig->data_dir()).dirName() + "/base/" + result->field<dir_key>();
	result->picture_files(
...
// {
//// Store a reference to the thread that owns the table  // Запоминается ссылка
///на ветку, которой принадлежит данная таблица
// _tree_item = item;
// init(item->export_to_dom());
// }

//// Initialize the data table based on the transmitted item DOM- //
...
// QDomElement				record_dom_data =
// doc->createElement("recordtable");
//// Пробегаются все записи в таблице
// for(int i = 0; i < _child_linkers.size(); i ++){
// if(  _child_linkers.at(i)->host()->page_valid()
// // unique_page()
// && static_cast<ItemsFlat *const>(_child_linkers.at(i)->host().get()) !=
// this)record_dom_data.appendChild(_child_linkers.at(i)->host()->dom_from_treeitem());
// // К элементу recordtabledata прикрепляются конечные записи
// }
//// qDebug() << "In export_modeldata_to_dom() recordtabledata " <<
///doc.toString();
...
void vector_t::dom_to_itemsflat(const QDomElement& dom_model) {  // , boost::intrusive_ptr<TreeItem> _parent_item
	...
	while (!dom_record.isNull()) {
		...
		// QString			url		=
		// attribute_map.namedItem("url").nodeValue();
		// QString			name		=
		// attribute_map.namedItem("name").nodeValue();
		if (id == "")
			id = attribute_map.namedItem("dir").nodeValue() != "" ? attribute_map.namedItem("dir").nodeValue() : detail::to_qstring(get_unical_id());
		assert(id != "");
		...
		// KnowModel::dom_to_records::assembly_record_and_table_to_parent(dom_model,
		// _parent_item);
		...
		if (index != -1)
			...
			child_item = new i_t(dynamic_cast<i_t* const>(this), data, QDomElement(), this->count_direct());  // static_cast<TreeItem
															  // *const>(this)->child_add_new(this->count_direct(),
															  // id, "");
		...
		// boost::intrusive_ptr<TreeItem> child_item =
		// boost::intrusive_ptr<TreeItem>(new TreeItem(_parent_item));

		child_item->dom_to_records(dom_record);
		// current_item->is_registered_to_record_controller_and_tabmanager(true);

		// Текущая запись добавляется в таблицу конечных записей (и располагается по
		// определенному адресу в памяти)
		// The current record is added to the final table of records (and located at
		// a certain address in memory)
		...
		// запись
		// Write initialized data. It should initsializirovatsya after placement in
		// the list tableData,
		// Order in subordinate objects have registered a valid pointer to this
		// entry

		// static_cast<Record
		// *>(_child_items.last().get())->record_from_dom(current_record_dom);
...
QDomElement vector_t::dom_from_itemsflat() const {
	//// Преобразование таблицы конечных записей в Dom документ
	// auto dom_from_itemsflat = [&](std::shared_ptr<QDomDocument> doc)
	// ->QDomElement const {
	//// Если у ветки нет таблицы конечных записей, возвращается пустой документ
	// if(_child_linkers.size() == 0)
	// return QDomElement();
	...
	// item_flat_dom.appendChild(_child_linkers.at(i)->host()->dom_from_treeitem());
	// //
	// boost::static_pointer_cast<Record>(_child_items.at(i))->dom_from_record(doc)
	//// К элементу recordtabledata прикрепляются конечные записи
	// }
...
//// Преобразование таблицы конечных записей в Dom документ
// QDomElement ItemsFlat::dom_from_itemsflat(std::shared_ptr<QDomDocument> doc)
// const
// {
//// Если у ветки нет таблицы конечных записей, возвращается пустой документ
// if(_child_linkers.size() == 0)
...
// item_flat_dom.appendChild(_child_linkers.at(i)->host()->dom_from_treeitem(doc));
// //
// boost::static_pointer_cast<Record>(_child_items.at(i))->dom_from_record(doc)
//// К элементу recordtabledata прикрепляются конечные записи
// }
...
// record_dom_data.appendChild(_tabledata.at(i)->export_to_dom(doc));     // К
// элементу recordtabledata прикрепляются конечные записи
...
// ) {
// record_dom_data.appendChild(_child_linkers.at(i)->host()->dom_from_treeitem());
// // К элементу recordtabledata прикрепляются конечные записи
// }
// }
...
struct operation {
	...
	input_type& _input;
	operation(full_fields_map* const field_data_map_, input_type& input)
	    : _field_data_map(field_data_map_)
	    , _input(input) {
	}
	operation(boost::intrusive_ptr<i_t> it, input_type& input)
	    : _field_data_map(&(it->fields_data_map()))
	    , _input(input) {
	}
	template <typename key_base_t, typename pair_t>
	struct is_same_key
	    : public std::is_same<key_base_t, typename pair_t::first_type> {
	};
	...
	////		boost::fusion::at_key<key_type_>(_it) =
	///boost::fusion::at_key<key_type_>(_input);
	//		boost::fusion::filter_if<func>(_it) =
	//boost::fusion::filter_if<func>(_input);
	//	}
	...
	operator()(const boost::fusion::pair<key_type_, value_type_>& pair) const {
		...
		//		boost::fusion::filter_if<func>(_it) =
		//boost::fusion::filter_if<func>(_input);
		//		boost::result_of::value_at_key<key_type_>(_it) =
		//boost::result_of::value_at_key<key_type_>(_input);
		//		std::cout << "type: " << typeid(key_type_).name() << ", value: " <<
		//pair.second << std::endl;
	}

	//	template<typename key_type_, typename value_type_>
	//	inline void operator ()(const boost::fusion::pair<key_type_,
	//value_type_> &pair){
	//		(void) pair;
	//		typedef is_same_key<key_type_, boost::mpl::_1>	expr;
	//		typedef typename boost::mpl::lambda<expr>::type	func;
	//		boost::fusion::at_key<key_type_>(_it) =
	//pair.second;//boost::fusion::at_key<key_type_>(_input);
	////		boost::fusion::filter_if<func>(_it) =
	///boost::fusion::filter_if<func>(_input);
	////		boost::result_of::value_at_key<key_type_>(_it) =
	///boost::result_of::value_at_key<key_type_>(_input);
...
// not a good design, suppose to delete // Замена в указанной записи переданных
// полей на новые значения
template <typename input_type>
void vector_t::fields(
    int pos, const input_type& input) {  //, QMap<QString, QString> edit_fields
...
//// todo: добавить удаление приаттаченных файлов и очистку таблицы
///приаттаченных файлов
// bool ItemsFlat::remove(int i){
// bool result = false;
// qDebug() << "Try delete record num " << i << " table count " <<
// _child_items.size();
...
// for(int i = 0; i < count_direct(); i ++){
// if(field(i, "id") == id){
// result = remove(i);	// Так как id уникальный, удаляться будет только одна
// запись
// break;
// }
// }
...
boost::intrusive_ptr<i_t> vector_t::contains_direct(
    const std::function<bool(boost::intrusive_ptr<const Linker>)>&& _equal)
    const {
	boost::intrusive_ptr<i_t> result(nullptr);
	for (auto il : _child_linkers) {
		if (il && _equal(il)) {  // &&
			...
			// && static_cast<ItemsFlat *>(_item_linker->host_parent().get()) == this
...
vector_t::contains_direct(boost::intrusive_ptr<const i_t>&& _item) const {
	boost::intrusive_ptr<i_t> result(nullptr);
	if (_item) {
		for (auto it : _child_linkers) {
			if (it && it->host() == _item  // && _item->up_linker()->integrity_external(_item,
			                               // static_cast<const TreeItem *>(this))   // recursive call
			    && it == _item->linker() && static_cast<vector_t*>(_item->_linker->host_parent().get()) == this) {
...
boost::intrusive_ptr<i_t> vector_t::contains_direct(
    boost::intrusive_ptr<const Linker>&& _item_linker) const {
	boost::intrusive_ptr<i_t> result(nullptr);
	if (_item_linker) {
		for (auto it : _child_linkers) {
			if (it && it == _item_linker                                              // &&
			                                                                          // _item_linker->integrity_external(_item_linker->host(),
			                                                                          // static_cast<const TreeItem *>(this))  // recursive
			                                                                          // call
...
boost::intrusive_ptr<i_t> vector_t::delete_permanent(
    const std::function<bool(boost::intrusive_ptr<const Linker>)>& _equal) {
	...
	for (auto& il : _child_linkers) {  // while(it.hasNext()) {
		...
		auto it = il->host();
		if (_equal(il)) {
			// Удаление директории и файлов внутри, с сохранением в резервной
			// директории
			QString dir_for_delete = gl_paras->root_path() + "/" + QDir(appconfig->data_dir()).dirName() + "/base/" + it->field<dir_key>();  // field(i, "dir")
			...
			////		it->binder()->page()->on_close_requested();
			///// it->binder()->break_page();  // it->page_break();
			// }
			web::WebView* web_view = nullptr;
			if ((web_view = gl_paras->main_window()->find(
			         [&](boost::intrusive_ptr<const ::Binder> b) {
				         return url_equal(url_value(detail::to_qstring(b->host()->field<home_key>())), it->field<url_key>()) && b == it->binder() && b->host()->id() == it->id();
				     }))) {
				if (it->binder()) {
...
void vector_t::remove_duplicated() {
	for (auto _item :
	     _child_linkers) {  // QList<boost::intrusive_ptr<TreeItem>>::iterator
		int found = 0;
...
QList<boost::intrusive_ptr<i_t> >
vector_t::delete_permanent_recursive(int position, int count) {
	// bool result = false;
	...
	if (position >= 0 && position + count <= _child_linkers.size()) {
		...
		for (auto il : _child_linkers) {
			...
			if (del_count >= position && del_count < position + count) {
				_child_linkers.removeOne(il);
				result << it;
				delete_permanent_recursive(
				    il, [&](boost::intrusive_ptr<const i_t>) { return true; });
			}
			// il.reset();
	...
	////            _child_linkers.removeOne(il);   //removeAt(position);    //
...
boost::intrusive_ptr<i_t> vector_t::delete_permanent_recursive(
	...
	// if(_to_be_removed->parent_item() !=
	// this)_to_be_removed->self_remove_from_parent();
	if (remove_linker->host_parent().get() == dynamic_cast<i_t* const>(this) && condition(dynamic_cast<i_t* const>(this))) {
		for (auto il : _child_linkers) {  // for(int row = 0; row <
			// _child_items.size(); ++row) {
			// auto it = _child_items.at(row);

			auto it = il->host();
			if (it->id() == remove_linker->host()->id()) {  // _child_items.removeAt(position);    //
				                                            // _child_items.takeAt(position).reset(); // delete
				                                            // _child_items.takeAt(position);
				...
				web::WebView* web_view = nullptr;
				if ((web_view = gl_paras->main_window()->find([&](boost::intrusive_ptr<const ::Binder> b) {
					    return url_equal(url_value(detail::to_qstring(b->host()->field<home_key>())), it->field<url_key>()) && b == it->binder() && b->host()->id() == it->id();
					}))) {
					if (it->binder()) {
						assert(web_view->page() == it->page());
				...
				////                    result =
				///_to_be_removed_linker->host()->merge(it)->up_linker();
				////                } else {
	...
	// qDebug() << "Try delete record num " << _child_items.indexOf(item) << "
	// table count " << _child_items.size();
...
// {
// int tableSize = count_direct(); // Запоминается размер таблицы, так как он
// при удалении меняется

// for(int i = 0; i < tableSize; i++)
// remove(0);   // Deleted very first record many times   // Удаляется самая
// первая запись много раз
// }
...
void vector_t::clear(void) {
	...
	////        //        _child_items.removeAt(i);    //
	///_child_items.takeAt(i).reset(); // delete _child_items.takeAt(i);
	////    }
...
void vector_t::to_encrypt(void) {
	...
	for (int i = 0; i < count_direct(); i++) {
		...
		if (_child_linkers[i] && _child_linkers[i]->host()->field<crypt_key>() == crypt_value(true))
			continue;
		// Шифрация записи
		_child_linkers[i]->host()->to_encrypt_and_save_lite();  // В таблице конечных
		                                                        // записей хранятся
		                                                        // легкие записи
	}
	this->_is_crypt = true;
...
void vector_t::to_decrypt(void) {
	...
	for (int i = 0; i < count_direct(); i++) {
		...
		if (_child_linkers[i] && (_child_linkers[i]->host()->field<crypt_key>() != crypt_value(true)))  //|| _child_linkers[i]->host()->field<crypt_key>() == "0"
			continue;
		// асшифровка записи
		_child_linkers[i]->host()->to_decrypt_and_save_lite();  // В таблице конечных
		                                                        // записей хранятся
		                                                        // легкие записи
	}
...
void vector_t::traverse(
    const std::function<void(boost::intrusive_ptr<Linker>)>& operation) {
	for (auto il : vector_t::_child_linkers)
		operation(il);
}

void vector_t::release(
    const std::function<bool(boost::intrusive_ptr<const Linker>)>& _equal) {
	// QList<boost::intrusive_ptr<Linker> >::Iterator pos =
	// _child_linkers.begin();
