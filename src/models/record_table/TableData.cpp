...
int TableData::insert_new_record(
    int pos
    , std::shared_ptr<Record> record
    , int mode
)
{
    ...
    if(_treeitem != nullptr) qDebug() << "TableData::insert_new_record() : Insert new record to branch " << _treeitem->all_fields();

    // The method must take a full-fledged object record    // Мотод должен принять полновесный объект записи
    if(record->isLite() == true)
        criticalError("TableData::insert_new_record() can't insert lite record");
