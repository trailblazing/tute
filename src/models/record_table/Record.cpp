...
QString Record::getNaturalField(QString name) const
{
    ...
    if(fieldList.contains(name)) {
        ...
        if(isCrypt == false)
            ...
            result = CryptService::decryptString(globalparameters.crypt_key(), fieldList[name]);
        }
    }
        ...
        if(fieldList.contains(currName)) {
            ...
            if(isCrypt == false)
                ...
                if(fixedparameters.recordFieldCryptedList.contains(currName) == false)
                    ...
                    result = CryptService::decryptString(globalparameters.crypt_key(), fieldList[currName]); // Расшифровывается значение поля
            }
...
QString Record::getTextFromFat() const
{
    ...
    if(fieldList.value("crypt").length() == 0 || fieldList.value("crypt") == "0")
        ...
        return CryptService::decryptStringFromByteArray(globalparameters.crypt_key(), text);
    else
...
void Record::setTextToFat(QString iText)
{
    ...
    if(fieldList.value("crypt").length() == 0 || fieldList.value("crypt") == "0")
        ...
        text = CryptService::encryptStringToByteArray(globalparameters.crypt_key(), iText);
    else
...
void Record::switchToEncryptAndSaveFat(void)
{
    ...
    text = CryptService::encryptByteArray(globalparameters.crypt_key(), text);
...
void Record::switchToDecryptAndSaveFat(void)
{
    ...
    text = CryptService::decryptByteArray(globalparameters.crypt_key(), text);
