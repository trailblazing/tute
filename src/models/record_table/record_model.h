...
public:
    ...
    boost::intrusive_ptr<i_t> item(const std::function<bool(const id_value)> _equal) const;
...
private:
    ...
    // Добавление записей
    web::WebView* insert_new_item(boost::intrusive_ptr<i_t> _target_item // , int mode = add_new_record_after
        );
