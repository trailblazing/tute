    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole || role == SORT_ROLE || role == Qt::UserRole) {
        ...
        if (index.column() < show_fields.size()) {
            ...
            if (role == Qt::DisplayRole && field_name == boost::mpl::c_str<ctime_type>::value) {
                ...
                if (field_name == boost::mpl::c_str<rating_type>::value)
                    return field.toULongLong();
                return field;
            } else
                return field;
        }
...
{ // , int mode
    ...
    auto insert_new_tab = [&](pos_source& returned_position, boost::intrusive_ptr<i_t> _item) { // , const pos_source source_insert_pos
    ...
    if (_target_item) {
        ...
        if (_target_item->binder()) {
            ...
            auto _target_item = // this_index->
                TreeIndex::require_item(target_url, std::bind(&tv_t::move, _tree_view, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), [&](boost::intrusive_ptr<const i_t> it_) -> bool {
...
boost::intrusive_ptr<i_t> RecordModel::item(const std::function<bool(const id_value)> _equal) const
{
    boost::intrusive_ptr<i_t> result = nullptr;
    for (int _index = 0; _index < count(); _index++) {
        auto it = _record_controller->tabmanager()->webView((int)_index)->page()->binder()->host(); // item(pos_source(i));
        if (_equal(it->id())) {
            result = it;
            break;
        }
...
// boost::intrusive_ptr<TreeItem> RecordModel::item(const QUrl &_url) const {	// lead implicit construct
// boost::intrusive_ptr<TreeItem> result(nullptr);
// for(int i = 0; i < count(); i ++){
