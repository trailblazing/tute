...
boost::intrusive_ptr<TreeItem> ItemsFlat::item_direct(const QUrl& url) const
{
    ...
    for (int i = 0; i < count_direct(); i++) {
        std::string compare = url_difference((item_direct(i)->field("url")).toStdString(), url.toString().toStdString());
...
boost::intrusive_ptr<Linker> ItemsFlat::remove(const std::function<bool(boost::intrusive_ptr<Linker>)>& _equal)
{
    boost::intrusive_ptr<Linker> result;
    ...
    for (auto il : _child_linkers) {     // while(it.hasNext()) {
        ...
        if (_equal(il)) {
            ...
            if (globalparameters.entrance()->find([&] (boost::intrusive_ptr<const ::Binder> b){
                return b->host()->field("url") == it->field("url") && b == it->binder() && b->host()->id() == it->id();
            })) {
                if(it->binder())
    ...
    if (result) {
        result->host(std::move(boost::intrusive_ptr<TreeItem>(nullptr)));         // il.reset();
        result->host_parent(std::move(boost::intrusive_ptr<TreeItem>(nullptr)));
    }
    result.reset();
    return result;
}
        ...
        for (auto il : _child_linkers) {         // QList<boost::intrusive_ptr<TreeItem>>::iterator
            if (il->host().get() == _item->host().get()) {
                ...
                if (1 == found) {
                    ...
                    il->host()->delete_permanent_recursive();
...
QList<boost::intrusive_ptr<Linker> > ItemsFlat::delete_permanent_recursive(int position, int count)
{
    //    bool result = false;
    //    QList<boost::intrusive_ptr<Linker>> candidates;
    QList<boost::intrusive_ptr<Linker> > result;
...
boost::intrusive_ptr<Linker> ItemsFlat::delete_permanent_recursive(boost::intrusive_ptr<Linker> _to_be_removed_linker)
{
    boost::intrusive_ptr<Linker> result(nullptr);     // bool result = false;
