...
boost::intrusive_ptr<Linker> ItemsFlat::remove(const std::function<bool(boost::intrusive_ptr<Linker>)> &_equal)
{
    ...
    for(auto &_il : _child_linkers) { // while(it.hasNext()) {
        // auto _it = it.next();
        auto it = _il->host();
...
QList<boost::intrusive_ptr<Linker>> ItemsFlat::child_linkers() {return _child_linkers;}
